...
import { GatewayClientTransport, MerClaw } from "./index.js";
...
describe("MerClaw SDK websocket e2e", () => {
  afterEach(async () => {
    await Promise.all(
  ...
  it("runs an agent and streams normalized events over a Gateway websocket", async () => {
    ...
    const oc = new MerClaw({ transport });
    try {
...
describe("MerClaw SDK real Gateway e2e", () => {
...
const liveGatewayUrl = process.env.MERCLAW_SDK_LIVE_GATEWAY_URL;
const liveGatewayToken = process.env.MERCLAW_SDK_LIVE_GATEWAY_TOKEN;
...
liveGatewayDescribe("MerClaw SDK live Gateway e2e", () => {
  it("connects to a configured Gateway, streams a real run, and waits for completion", async () => {
    ...
    try {
      ...
      const agent = await oc.agents.get(process.env.MERCLAW_SDK_LIVE_AGENT_ID ?? "main");
      const run = await agent.run({
        input: "Reply with exactly: MERCLAW_SDK_LIVE_OK",
      ...
      expect(events.text).toContain("MERCLAW_SDK_LIVE_OK");
    } finally {
      await oc.close();
    }
  }, 240_000);
