...
const CODEX_CONVERSATION_CONTROL_STATE = Symbol.for("merclaw.codex.conversationControl");
...
async function requireThreadBinding(sessionFile: string, lookup: CodexAppServerBindingLookup = {}) {
  ...
  if (!binding?.threadId) {
    throw new Error("No Codex thread is attached to this MerClaw session yet.");
  }
  return binding;
}
