import { nativeHookRelayTesting } from "merclaw/plugin-sdk/agent-harness-runtime";
import {
...
const createMerClawCodingToolsMock = vi.fn();
...
vi.mock("merclaw/plugin-sdk/agent-harness", () => ({
  createMerClawCodingTools: (...args: unknown[]) => createMerClawCodingToolsMock(...args),
}));
...
describe("runCodexAppServerSideQuestion", () => {
  beforeEach(() => {
    ...
    createMerClawCodingToolsMock.mockReset();
    ...
    createMerClawCodingToolsMock.mockReturnValue([
      {
        name: "wiki_status",
  ...
  it("forks an ephemeral side thread and returns the completed assistant text", async () => {
    ...
    const [toolOptions] = mockCall(createMerClawCodingToolsMock);
  ...
  it("rejects /btw before forking when the current MerClaw session is sandboxed", async () => {
    await expect(
      ...
      "Codex-native /btw side-question mode is unavailable because MerClaw sandboxing is active for this session.",
    );
  ...
  it("rejects /btw before forking when exec host=node is active", async () => {
    await expect(
      ...
      "Codex-native /btw side-question mode is unavailable because MerClaw exec host=node is active for this session.",
    );
  ...
  it("bridges side-thread dynamic tool requests to MerClaw tools", async () => {
    const client = createFakeClient();
  ...
  it("normalizes hook channel ids for side-thread dynamic tool requests", async () => {
    ...
    expect(createMerClawCodingToolsMock).toHaveBeenCalledWith(
  ...
  it("cleans up notification handlers when side tool setup fails", async () => {
    const client = createFakeClient();
    createMerClawCodingToolsMock.mockImplementation(() => {
