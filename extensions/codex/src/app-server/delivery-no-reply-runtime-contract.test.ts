...
import { isSilentReplyPayloadText } from "merclaw/plugin-sdk/reply-chunking";
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-codex-delivery-contract-"));
