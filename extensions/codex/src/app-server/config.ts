...
} from "merclaw/plugin-sdk/exec-approvals-runtime";
import { resolvePositiveTimerTimeoutMs } from "merclaw/plugin-sdk/number-runtime";
import { normalizeAgentId } from "merclaw/plugin-sdk/routing";
import { normalizeTrimmedStringList } from "merclaw/plugin-sdk/string-coerce-runtime";
import { detectWindowsSpawnCommandInlineArgs } from "merclaw/plugin-sdk/windows-spawn";
import { z } from "zod";
import type { CodexSandboxPolicy, CodexServiceTier } from "./protocol.js";

const START_OPTIONS_KEY_SECRET_SYMBOL = Symbol.for("merclaw.codexAppServerStartOptionsKeySecret");
...
type MerClawExecMode = "deny" | "allowlist" | "ask" | "auto" | "full";
type MerClawExecSecurity = "deny" | "allowlist" | "full";
type MerClawExecAsk = "off" | "on-miss" | "always";
type MerClawExecApprovalFloorsForCodexAppServer = {
  security?: MerClawExecSecurity;
  ask?: MerClawExecAsk;
};
export type MerClawExecPolicyForCodexAppServer = {
  mode?: MerClawExecMode;
  security: MerClawExecSecurity;
  ask: MerClawExecAsk;
  touched: boolean;
};
type MerClawExecPolicy = MerClawExecPolicyForCodexAppServer;
...
export function resolveCodexAppServerRuntimeOptions(
  ...
  const envCommand = readNonEmptyString(env.MERCLAW_CODEX_APP_SERVER_BIN);
  ...
  const args = resolveArgs(config.args, env.MERCLAW_CODEX_APP_SERVER_ARGS);
  const headers = normalizeHeaders(config.headers);
  const clearEnv = normalizeStringList(config.clearEnv);
  const authToken = readNonEmptyString(config.authToken);
  const url = readNonEmptyString(config.url);
  const execMode = resolveEffectiveMerClawExecModeForCodexAppServer({
    execMode: params.execMode,
    execPolicy: params.execPolicy,
  });
  assertCodexAppServerAllowedForMerClawExecMode(execMode);
  const explicitPolicyMode =
    resolvePolicyMode(config.mode) ?? resolvePolicyMode(env.MERCLAW_CODEX_APP_SERVER_MODE);
  const configuredSandbox =
    resolveSandbox(config.sandbox) ?? resolveSandbox(env.MERCLAW_CODEX_APP_SERVER_SANDBOX);
  const explicitApprovalsReviewer = resolveApprovalsReviewer(config.approvalsReviewer);
  const normalizedPolicyMode = resolveCodexPolicyModeForMerClawExecMode(execMode);
        ...
        sandbox: preserveExplicitAutoSandbox
          ...
          : forceDangerFullAccessSandbox
            ? selectForcedDangerFullAccessSandbox({
                configuredSandbox,
                defaultPolicy,
                merClawSandboxActive: Boolean(params.merClawSandboxActive),
              })
  ...
  const envApprovalPolicy = resolveApprovalPolicy(env.MERCLAW_CODEX_APP_SERVER_APPROVAL_POLICY);
...
export function resolveCodexComputerUseConfig(
  ...
  const marketplaceSource =
    ...
    readNonEmptyString(env.MERCLAW_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE);
  const marketplacePath =
    readNonEmptyString(params.overrides?.marketplacePath) ??
    readNonEmptyString(config.marketplacePath) ??
    readNonEmptyString(env.MERCLAW_CODEX_COMPUTER_USE_MARKETPLACE_PATH);
  const marketplaceName =
    readNonEmptyString(params.overrides?.marketplaceName) ??
    readNonEmptyString(config.marketplaceName) ??
    readNonEmptyString(env.MERCLAW_CODEX_COMPUTER_USE_MARKETPLACE_NAME);
  const autoInstall =
    params.overrides?.autoInstall ??
    config.autoInstall ??
    readBooleanEnv(env.MERCLAW_CODEX_COMPUTER_USE_AUTO_INSTALL) ??
    false;
  const marketplaceDiscoveryTimeoutMs = normalizePositiveNumber(
    params.overrides?.marketplaceDiscoveryTimeoutMs ??
      ...
      readNumberEnv(env.MERCLAW_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS),
    DEFAULT_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS,
  );
  const enabled =
    params.overrides?.enabled ??
    config.enabled ??
    readBooleanEnv(env.MERCLAW_CODEX_COMPUTER_USE) ??
  ...
  return {
    ...
    pluginName:
      ...
      readNonEmptyString(env.MERCLAW_CODEX_COMPUTER_USE_PLUGIN_NAME) ??
    ...
    mcpServerName:
      ...
      readNonEmptyString(env.MERCLAW_CODEX_COMPUTER_USE_MCP_SERVER_NAME) ??
...
function resolveDefaultCodexAppServerPolicy(params: {
  ...
  execModeRequiringPromptingApprovals?: Extract<MerClawExecMode, "auto" | "ask">;
  env?: NodeJS.ProcessEnv;
  requirementsToml?: string | null;
  requirementsPath?: string;
  readRequirementsFile?: (path: string) => string | undefined;
...
function selectGuardianApprovalsReviewer(
  ...
  execModeRequiringAutoReviewer?: Extract<MerClawExecMode, "auto">,
...
function selectForcedDangerFullAccessSandbox(params: {
  ...
  if (params.defaultPolicy?.dangerFullAccessAllowed === false) {
    if (params.merClawSandboxActive) {
...
export function resolveMerClawExecModeFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): MerClawExecMode | undefined {
  const policy = resolveMerClawExecPolicyFromConfig(params);
  return policy.touched ? policy.mode : undefined;
}

function resolveMerClawExecPolicyFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): MerClawExecPolicy {
  const root = readRecord(params.config);
  const globalExec = readRecord(readRecord(root?.tools)?.exec);
  const globalPolicy = applyMerClawExecPolicyLayer(createDefaultMerClawExecPolicy(), globalExec);
  ...
  return applyMerClawExecPolicyLayer(globalPolicy, agentExec);
}

export function resolveMerClawExecModeForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  const policy = resolveMerClawExecPolicyForCodexAppServer(params);
  return policy.touched ? policy.mode : undefined;
}

export function resolveMerClawExecPolicyForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  const basePolicy = resolveMerClawExecPolicyFromConfig({
    config: params.config,
    agentId: params.agentId,
  });
  const overridePolicy = applyMerClawExecPolicyLayer(basePolicy, params.execOverrides);
  const approvalFloors = resolveMerClawExecApprovalFloorsForCodexAppServer({
    approvals: params.approvals,
    agentId: params.agentId,
    policy: overridePolicy,
  });
  return applyMerClawExecApprovalFloors(overridePolicy, approvalFloors);
}

function resolveEffectiveMerClawExecModeForCodexAppServer(params: {
  execMode?: MerClawExecMode;
  execPolicy?: MerClawExecPolicyForCodexAppServer;
}): MerClawExecMode | undefined {
...
function resolveCodexPolicyModeForMerClawExecMode(
  mode: MerClawExecMode | undefined,
...
function assertCodexAppServerAllowedForMerClawExecMode(mode: MerClawExecMode | undefined): void {
...
function createDefaultMerClawExecPolicy(): MerClawExecPolicy {
  return {
    security: "full",
    ask: "off",
    touched: false,
...
function applyMerClawExecPolicyLayer(
  base: MerClawExecPolicy,
  ...
  if (mode !== undefined) {
    return {
      ...resolveMerClawExecPolicyForMode(mode),
      touched: true,
    };
  }
  ...
  return {
    mode: resolveMerClawExecModeFromPolicy({ security: nextSecurity, ask: nextAsk }),
...
function resolveMerClawExecApprovalFloorsForCodexAppServer(params: {
  approvals?: ExecApprovalsFile;
  agentId?: string;
  policy: MerClawExecPolicy;
}): MerClawExecApprovalFloorsForCodexAppServer | undefined {
  if (!params.approvals) {
    return undefined;
  }
...
function applyMerClawExecApprovalFloors(
  base: MerClawExecPolicy,
  approvalFloors?: MerClawExecApprovalFloorsForCodexAppServer,
): MerClawExecPolicy {
  if (!approvalFloors) {
    return base;
  }
  const nextSecurity = approvalFloors.security
    ? minMerClawExecSecurity(base.security, approvalFloors.security)
    : base.security;
  const nextAsk = approvalFloors.ask ? maxMerClawExecAsk(base.ask, approvalFloors.ask) : base.ask;
...
function resolveMerClawExecPolicyForMode(
  mode: MerClawExecMode,
): Omit<MerClawExecPolicy, "touched"> {
  switch (mode) {
    case "deny":
...
function resolveMerClawExecModeFromPolicy(params: {
  security: MerClawExecSecurity;
  ask: MerClawExecAsk;
}): MerClawExecMode {
...
function minMerClawExecSecurity(
  left: MerClawExecSecurity,
  right: MerClawExecSecurity,
): MerClawExecSecurity {
  const order: Record<MerClawExecSecurity, number> = { deny: 0, allowlist: 1, full: 2 };
  return order[left] <= order[right] ? left : right;
}

function maxMerClawExecAsk(left: MerClawExecAsk, right: MerClawExecAsk): MerClawExecAsk {
  const order: Record<MerClawExecAsk, number> = { off: 0, "on-miss": 1, always: 2 };
  return order[left] >= order[right] ? left : right;
}

function readExecMode(value: unknown): MerClawExecMode | undefined {
  return value === "deny" ||
    value === "allowlist" ||
    value === "ask" ||
    value === "auto" ||
...
function readExecSecurity(value: unknown): MerClawExecSecurity | undefined {
  return value === "deny" || value === "allowlist" || value === "full" ? value : undefined;
}

function readExecAsk(value: unknown): MerClawExecAsk | undefined {
