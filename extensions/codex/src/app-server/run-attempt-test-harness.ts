...
import { resetDiagnosticEventsForTest } from "merclaw/plugin-sdk/diagnostic-runtime";
import { clearInternalHooks, resetGlobalHookRunner } from "merclaw/plugin-sdk/hook-runtime";
import { clearPluginCommands } from "merclaw/plugin-sdk/plugin-runtime";
...
export function setupRunAttemptTestHooks(): void {
  beforeEach(async () => {
    ...
    vi.stubEnv("MERCLAW_TRAJECTORY", "0");
    vi.stubEnv("CODEX_API_KEY", "");
    vi.stubEnv("OPENAI_API_KEY", "");
    tempDir = await fs.mkdtemp(path.join(resolvePreferredMerClawTmpDir(), "merclaw-codex-run-"));
  });
  ...
  afterEach(async () => {
    ...
    testing.resetMerClawCodingToolsFactoryForTests();
