...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/merclaw-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("merclaw/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("merclaw/plugin-sdk/security-runtime")>(
    "merclaw/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("merclaw/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("merclaw/plugin-sdk/system-event-runtime")>(
    "merclaw/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("merclaw/plugin-sdk/reply-runtime"),
      import("merclaw/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
