...
import { runPluginCommandWithTimeout } from "merclaw/plugin-sdk/run-command";
import type { RuntimeEnv } from "merclaw/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "merclaw/plugin-sdk/setup-tools";
...
export async function installSignalCliFromRelease(
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredMerClawTmpDir(), "merclaw-signal-"));
