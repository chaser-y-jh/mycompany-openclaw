...
import { resolvePositiveTimeoutSeconds } from "merclaw/plugin-sdk/provider-web-search";
import {
...
export function resolveTavilySearchConfig(cfg?: MerClawConfig): TavilySearchConfig {
...
export function resolveTavilyApiKey(cfg?: MerClawConfig): string | undefined {
...
export function resolveTavilyBaseUrl(cfg?: MerClawConfig): string {
  const search = resolveTavilySearchConfig(cfg);
  const configured =
