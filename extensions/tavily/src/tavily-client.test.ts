...
vi.mock("merclaw/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: merclaw", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "merclaw" });
  });
  ...
  it("runTavilyExtract sends X-Client-Source: merclaw", async () => {
