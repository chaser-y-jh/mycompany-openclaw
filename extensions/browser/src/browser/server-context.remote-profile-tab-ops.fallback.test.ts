...
describe("browser remote profile fallback and attachOnly behavior", () => {
  it("uses profile-level attachOnly when global attachOnly is false", async () => {
    const state = deps.makeState("merclaw");
    ...
    const launchMock = vi.mocked(deps.chromeModule.launchMerClawChrome);
  ...
  it("does not enforce managed tab cap for remote merclaw profiles", async () => {
  ...
  it("uses remote-class tab-open timeouts for attachOnly loopback CDP profiles", async () => {
    ...
    const opened = await ctx.forProfile("merclaw").openTab("https://example.com");
  ...
  it("keeps managed loopback tab opens on local CDP defaults", async () => {
    ...
    await ctx.forProfile("merclaw").openTab("http://127.0.0.1:3000");
