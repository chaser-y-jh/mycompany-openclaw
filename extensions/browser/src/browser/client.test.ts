...
describe("browser client", () => {
  ...
  it("uses the expected endpoints + methods for common calls", async () => {
    ...
    expect(doctorResult.profile).toBe("merclaw");
    ...
    expect(deepDoctorResult.profile).toBe("merclaw");
    ...
    expect(urls.some((url) => url.endsWith("/doctor?profile=merclaw&deep=true"))).toBe(true);
    ...
    const deepDoctor = calls.find((c) => c.url.endsWith("/doctor?profile=merclaw&deep=true"));
