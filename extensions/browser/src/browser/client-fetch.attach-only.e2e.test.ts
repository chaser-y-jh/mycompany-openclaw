...
import { clearRuntimeConfigSnapshot } from "merclaw/plugin-sdk/runtime-config-snapshot";
...
describe("browser client fetch attachOnly diagnostics", () => {
  ...
  it("does not suggest gateway restart when an attachOnly CDP endpoint hangs", async () => {
    tempHome = await createTempHomeEnv("merclaw-browser-client-fetch-live-");
    ...
    const configPath = path.join(tempHome.home, ".merclaw", "merclaw.json");
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
    ...
    try {
      ...
      expect(message).toContain("browser profile is external to MerClaw");
      expect(message).toContain("Restarting the MerClaw gateway will not launch it");
      expect(message).not.toContain("Restart the MerClaw gateway");
