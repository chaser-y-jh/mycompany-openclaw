...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        merclaw: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.MERCLAW_GATEWAY_PORT;
    process.env.MERCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.MERCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.MERCLAW_GATEWAY_PASSWORD;
