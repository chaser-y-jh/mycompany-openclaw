...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<MerClawConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: MerClawConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): MerClawConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: MerClawConfig) {
