...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".merclaw-profile-decorated");
}
 ...
 */
export function decorateMerClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_MERCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MERCLAW_BROWSER_COLOR).toUpperCase();
