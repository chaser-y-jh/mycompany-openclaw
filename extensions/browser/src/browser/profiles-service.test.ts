...
import { resolveMerClawUserDataDir } from "./chrome.js";
import type { BrowserRouteContext, BrowserServerState } from "./server-context.js";
import { movePathToTrash } from "./trash.js";

const configMocks = vi.hoisted(() => ({
  getRuntimeConfig: vi.fn<() => MerClawConfig>(),
  writeConfigFile: vi.fn<(cfg: MerClawConfig) => Promise<void>>(async (_cfg) => {}),
  mutateConfigFile: vi.fn(
    async (params: {
      mutate: (draft: MerClawConfig, context: { snapshot: { path: string } }) => unknown;
    }) => {
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    replaceConfigFile: vi.fn(async ({ nextConfig }: { nextConfig: MerClawConfig }) => {
...
vi.mock("./chrome.js", () => ({
  resolveMerClawUserDataDir: vi.fn(() => "/tmp/merclaw-test/merclaw/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "merclaw-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveMerClawUserDataDir).mockReturnValue(userDataDir);
