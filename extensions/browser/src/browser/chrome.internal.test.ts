...
vi.mock("../infra/tmp-merclaw-dir.js", () => ({
  resolvePreferredMerClawTmpDir: () => "/tmp/merclaw-browser-test",
}));
...
import {
  buildMerClawChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchMerClawChrome,
  resolveMerClawUserDataDir,
  stopMerClawChrome,
} from "./chrome.js";
 ...
 * not exercise: launchMerClawChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopMerClawChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildMerClawChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveMerClawUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveMerClawUserDataDir();
      expect(dir.endsWith(path.join("merclaw", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveMerClawUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildMerClawChromeLaunchArgs branches", () => {
    ...
    it("toggles headless args", () => {
      const args = buildMerClawChromeLaunchArgs({
    ...
    it("lets a request headless override beat env and profile headed settings", () => {
      const args = buildMerClawChromeLaunchArgs({
        ...
        env: { MERCLAW_BROWSER_HEADLESS: "0" },
      });
      expect(args).toContain("--headless=new");
      expect(args).toContain("--disable-gpu");
    });
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchMerClawChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchMerClawChrome(resolved, profile);
  ...
  describe("launchMerClawChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "merclaw-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchMerClawChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchMerClawChrome(makeResolved(), profile);
          expect(running.pid).toBe(4242);
    ...
    it("accepts a ready CDP diagnostic after the launch HTTP probe expires", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          const port = new URL(baseUrl).port;
          const profile = makeProfile(Number(port));
          const running = await launchMerClawChrome(
    ...
    it("clears stale singleton locks and retries once after profile-in-use launch failure", async () => {
      const configPath = path.join(tmpDir, "merclaw.json");
      await fsp.writeFile(
        configPath,
        JSON.stringify({
          logging: {
      ...
      vi.stubEnv("MERCLAW_CONFIG_PATH", configPath);
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const userDataDir = resolveMerClawUserDataDir(profile.name);
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchMerClawChrome(resolved, profile)).rejects.toThrow(
  ...
  describe("stopMerClawChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopMerClawChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopMerClawChrome>[0],
        1,
      );
  ...
  describe("launchMerClawChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "merclaw-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveMerClawUserDataDir(profileName));
        ...
        const staged = resolveMerClawUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_MERCLAW_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      const configDir = await fsp.mkdtemp(path.join(os.tmpdir(), "merclaw-redact-off-"));
      const configPath = path.join(configDir, "merclaw.json");
      ...
      let message = "";
      try {
        await launchMerClawChrome(resolved, profile);
      } catch (err) {
    ...
    it("logs a warning when decorateMerClawProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateMerClawProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateMerClawProfile }, "decorateMerClawProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".merclaw-profile-decorated") || s.endsWith("Preferences")) {
  ...
  describe("launchMerClawChrome managed-proxy CDP bypass", () => {
    ...
    it("surfaces loopbackMode=block as BrowserProfileUnavailableError without spawning Chrome", async () => {
      ...
      await expect(launchMerClawChrome(makeResolved(), profile)).rejects.toBeInstanceOf(
    ...
    it("does not register a bypass for a remote attachOnly CDP URL (loopback gate)", async () => {
      stubExecutableAndPrefsExist();
      // For this test we want launchMerClawChrome to reject before any
      ...
      await expect(launchMerClawChrome(makeResolved(), remoteProfile)).rejects.toThrow(
        /is remote; cannot launch local Chrome/,
      );
