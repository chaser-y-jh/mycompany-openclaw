...
const { stopMerClawChromeMock } = vi.hoisted(() => ({
  stopMerClawChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopMerClawChrome: stopMerClawChromeMock,
}));
...
beforeEach(() => {
  ...
  stopMerClawChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["merclaw", "user"]);
    ...
    expect(stopMap.merclaw).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopMerClawChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("merclaw browser stop failed: Error: oops");
  });
});
