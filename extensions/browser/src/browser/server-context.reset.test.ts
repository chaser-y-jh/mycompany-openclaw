...
function localMerClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "merclaw",
...
function createLocalMerClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localMerClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveMerClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-reset-"));
    const profileDir = path.join(tempRoot, "merclaw");
    ...
    const ops = createLocalMerClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveMerClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "merclaw");
    fs.mkdirSync(profileDir, { recursive: true });
