...
const EAGER_BROWSER_CONTROL_SERVICE_ENV = "MERCLAW_EAGER_BROWSER_CONTROL_SERVER";
...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage MerClaw's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
};
...
function createLazyBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via MerClaw's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated MerClaw-managed browser (`merclaw`).",
...
function createBrowserToolOptions(ctx: MerClawPluginToolContext): {
  sandboxBridgeUrl?: string;
  allowHostControl?: boolean;
  agentSessionKey?: string;
  agentDir?: string;
...
export const browserPluginNodeHostCommands: MerClawPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: MerClawPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): MerClawPluginService {
  let service: MerClawPluginService | null = null;
...
export function registerBrowserPlugin(api: MerClawPluginApi) {
  api.registerTool(((ctx: MerClawPluginToolContext) =>
    createLazyBrowserTool(createBrowserToolOptions(ctx))) as MerClawPluginToolFactory);
  api.registerCli(
    async ({ program }) => {
