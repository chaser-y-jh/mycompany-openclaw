...
function resolveSessionHeaders(params: {
  ...
  return {
    ...
    "x-merclaw-session-id": sessionId,
  };
}
...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-merclaw-turn-id": turnId,
      "x-merclaw-turn-attempt": attempt,
    },
    metadata: {
      merclaw_session_id: sessionHeaders["x-merclaw-session-id"] ?? "",
      merclaw_turn_id: turnId,
      merclaw_turn_attempt: attempt,
      merclaw_transport: ctx.transport,
    },
  };
}
