import type { ProviderRuntimeModel } from "merclaw/plugin-sdk/plugin-entry";
...
describe("openai transport policy", () => {
  ...
  it("builds native turn state for direct OpenAI routes", () => {
    ...
    expect(state?.headers?.["x-merclaw-session-id"]).toBe("session-123");
    expect(state?.headers?.["x-merclaw-turn-id"]).toBe("turn-123");
    expect(state?.headers?.["x-merclaw-turn-attempt"]).toBe("2");
    expect(state?.metadata?.merclaw_session_id).toBe("session-123");
    expect(state?.metadata?.merclaw_turn_id).toBe("turn-123");
    expect(state?.metadata?.merclaw_turn_attempt).toBe("2");
    expect(state?.metadata?.merclaw_transport).toBe("websocket");
  });
  ...
  it("returns websocket session headers and cooldown for native routes", () => {
    ...
    expect(policy?.headers?.["x-merclaw-session-id"]).toBe("session-123");
