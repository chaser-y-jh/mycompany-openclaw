...
import type { ResolvedTtsConfig } from "merclaw/plugin-sdk/agent-runtime";
import { AuthStorage, ModelRegistry } from "merclaw/plugin-sdk/agent-sessions";
import type { MerClawConfig } from "merclaw/plugin-sdk/config-contracts";
import type { Api, Model } from "merclaw/plugin-sdk/llm";
import { encodePngRgba, fillPixel } from "merclaw/plugin-sdk/media-runtime";
import {
  registerProviderPlugin,
  requireRegisteredProvider,
} from "merclaw/plugin-sdk/plugin-test-runtime";
import { runRealtimeSttLiveTest } from "merclaw/plugin-sdk/provider-test-contracts";
...
const LIVE_MODEL_ID = process.env.MERCLAW_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.5";
const LIVE_IMAGE_MODEL = process.env.MERCLAW_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.MERCLAW_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-5.4-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.MERCLAW_LIVE_TEST === "1";
...
function createLiveConfig(): MerClawConfig {
...
describeLive("openai plugin live", () => {
  ...
  it("lists voices and synthesizes audio through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "MerClaw integration test OK.",
      cfg,
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const cfg = createLiveConfig();
    const ttsConfig = createLiveTtsConfig();
    const phrase = "Testing MerClaw OpenAI realtime transcription integration test OK.";
