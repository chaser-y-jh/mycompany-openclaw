import type { LookupFn } from "merclaw/plugin-sdk/ssrf-runtime";
...
vi.mock("merclaw/plugin-sdk/provider-web-search", async (importOriginal) => {
  const actual = await importOriginal<typeof import("merclaw/plugin-sdk/provider-web-search")>();
...
describe("searxng client", () => {
  ...
  it("preserves a configured base-path prefix when building the search URL", () => {
    expect(
      ...
      "https://search.example.com/searxng/search?q=merclaw&format=json&categories=general%2Cnews&language=en",
    );
  });
