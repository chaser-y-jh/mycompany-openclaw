...
`merclaw.invoke --each`) without adding new MerClaw code per workflow.
...
This extension depends on MerClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** MerClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.merclaw/extensions` as a standalone plugin directory.
