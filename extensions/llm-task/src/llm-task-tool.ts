...
import { buildModelAliasIndex, resolveModelRefFromString } from "merclaw/plugin-sdk/agent-runtime";
import {
...
import { resolvePreferredMerClawTmpDir, withTempWorkspace } from "../api.js";
import type { MerClawPluginApi } from "../api.js";
...
type ThinkingPolicy = ReturnType<MerClawPluginApi["runtime"]["agent"]["resolveThinkingPolicy"]>;

export const llmTaskToolDefinition = {
  name: "llm-task",
  label: "LLM Task",
  description:
    "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via merclaw.invoke.",
...
function supportsThinkingPolicyLevel(
  ...
  level: ReturnType<MerClawPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]>,
): boolean {
  return Boolean(level) && policy.levels.some((entry) => entry.id === level);
}

export function createLlmTaskTool(api: MerClawPluginApi) {
  return {
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      let thinkLevel: ReturnType<MerClawPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]> =
        undefined;
      if (thinkingRaw) {
      ...
      return await withTempWorkspace(
        { rootDir: resolvePreferredMerClawTmpDir(), prefix: "merclaw-llm-task-" },
        async ({ dir: tmpDir }) => {
