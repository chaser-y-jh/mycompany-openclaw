...
const SUPPORTED_DURATIONS_HINT = Symbol.for("merclaw.videoGeneration.supportedDurations");
...
vi.mock("merclaw/plugin-sdk/provider-http", async () => {
  const actual = await vi.importActual<typeof import("merclaw/plugin-sdk/provider-http")>(
    "merclaw/plugin-sdk/provider-http",
  );
  return {
    ...actual,
