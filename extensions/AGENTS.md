...
- Extension production code should import from `merclaw/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `merclaw.plugin.json` and the package
  `merclaw` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `merclaw/plugin-sdk/*`.
