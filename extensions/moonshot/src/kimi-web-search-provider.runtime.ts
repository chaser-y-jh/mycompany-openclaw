...
function resolveKimiBaseUrl(kimi?: KimiConfig, merClawConfig?: MerClawConfig): string {
  ...
  const moonshotBaseUrl = merClawConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: MerClawConfig; searchConfig?: SearchConfigRecord },
  ...
  if (!result.grounded) {
    return {
      ...
      provider: "kimi",
      model,
      docs: "https://docs.merclaw.ai/tools/kimi-search",
