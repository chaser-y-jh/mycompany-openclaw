...
} from "merclaw/plugin-sdk/error-runtime";
import { resolveGlobalMap } from "merclaw/plugin-sdk/global-singleton";
import * as memoryCoreHostRuntimeCoreModule from "merclaw/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "merclaw/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "merclaw/plugin-sdk/session-store-runtime";
...
const DREAMS_FILE_LOCKS_KEY = Symbol.for("merclaw.memoryCore.dreamingNarrative.fileLocks");
const NARRATIVE_SESSION_LOCKS_KEY = Symbol.for(
  "merclaw.memoryCore.dreamingNarrative.sessionLocks",
);
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("merclaw:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("merclaw-dreaming-backfill-");
    ...
    expect(content.match(/merclaw:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("merclaw-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- merclaw:dreaming:diary:start -->");
    expect(content).toContain("<!-- merclaw:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- merclaw:dreaming:diary:start -->");
    const end = content.indexOf("<!-- merclaw:dreaming:diary:end -->");
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("merclaw-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("merclaw-dreaming-narrative-");
    const stateDir = await createTempWorkspace("merclaw-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("merclaw-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("merclaw-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDirs = await Promise.all(
      Array.from({ length: 5 }, () => createTempWorkspace("merclaw-dreaming-detach-")),
    );
    const logger = createMockLogger();
  ...
  it("serializes detached narratives that reuse a workspace and phase session", async () => {
    ...
    const workspaceDir = await createTempWorkspace("merclaw-dreaming-detach-");
    const logger = createMockLogger();
