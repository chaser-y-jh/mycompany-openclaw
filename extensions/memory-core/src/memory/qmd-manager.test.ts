...
import { withMockedWindowsPlatform } from "merclaw/plugin-sdk/test-node-mocks";
...
const MCPORTER_STATE_KEY = Symbol.for("merclaw.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("merclaw.qmdEmbedQueueTail");
...
vi.mock("merclaw/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("merclaw/plugin-sdk/memory-core-host-engine-foundation")
  >("merclaw/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("merclaw/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("merclaw/plugin-sdk/file-lock")>(
    "merclaw/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { MerClawConfig } from "merclaw/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
