...
import type { AgentHarnessAttemptParams } from "merclaw/plugin-sdk/agent-harness-runtime";
...
vi.mock("merclaw/plugin-sdk/agent-harness", async (importOriginal) => {
  const actual = await importOriginal<typeof import("merclaw/plugin-sdk/agent-harness")>();

  return {
    ...actual,
    createMerClawCodingTools: vi.fn(() => [
      {
...
const LIVE = isLiveTestEnabled(["MERCLAW_COPILOT_AGENT_LIVE_TEST"]);
const TOKEN =
  process.env.MERCLAW_COPILOT_AGENT_LIVE_TOKEN ||
...
describeLive("copilot agent runtime live smoke", () => {
  it("runs one turn on gpt-4.1 with one custom tool", async () => {
    ...
    const copilotHome = await mkdtemp(join(tmpdir(), "merclaw-copilot-live-"));
