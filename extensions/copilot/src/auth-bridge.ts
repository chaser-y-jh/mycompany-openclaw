 ...
 *     use cases.
 *   - Falls back to MERCLAW_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *     (`<merClawHome>/.merclaw/agents/<agentId>/copilot`, or
 *     `<agentDir>/copilot` when an agent directory is supplied) that
 *     respects `MERCLAW_HOME` for the home directory root.
 ...
 *   4. MERCLAW_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  // "copilot" subdir so it cannot collide with MerClaw's own files
  ...
  const merClawHome = readString(args.env.MERCLAW_HOME);
  const rootHome = merClawHome ? resolve(merClawHome) : safeHomeDir(args.homeDir);
  // Per-agent isolation per proposal section 3.6:
  //   <merClawHome>/.merclaw/agents/<agentId>/copilot
  return resolve(join(rootHome, ".merclaw", "agents", args.agentId, "copilot"));
}
...
function readEnvTokenFallback(
  ...
  // MERCLAW_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "MERCLAW_GITHUB_TOKEN", value: readString(env.MERCLAW_GITHUB_TOKEN) },
