...
type CreateMerClawCodingTools =
  (typeof import("merclaw/plugin-sdk/agent-harness"))["createMerClawCodingTools"];
type MerClawCodingToolsOptions = NonNullable<Parameters<CreateMerClawCodingTools>[0]>;
 ...
 * authoritative `createMerClawCodingTools({...})` call shape).
 *
 * Declared as `Partial<EmbeddedRunAttemptParams>` (imported from the
 * `merclaw/plugin-sdk/agent-harness-runtime` boundary, *not* from
...
export interface CopilotToolBridgeInput {
   ...
   * model, and run-trace fields to `createMerClawCodingTools` so the
  ...
  createMerClawCodingTools?: (opts: unknown) => AnyAgentTool[] | Promise<AnyAgentTool[]>;
  beforeExecute?: (ctx: {
    toolName: string;
    toolCallId: string;
    args: unknown;
...
export async function createCopilotToolBridge(
  ...
  const createMerClawCodingTools =
    input.createMerClawCodingTools ??
    (await import("merclaw/plugin-sdk/agent-harness")).createMerClawCodingTools;

  const toolOptions = buildMerClawCodingToolsOptions(input, effectiveToolPlan);

  let sourceTools: unknown;
  try {
    sourceTools = await createMerClawCodingTools(toolOptions);
  } catch (error: unknown) {
    throw createError(
      `[copilot-tool-bridge] createMerClawCodingTools failed: ${toError(error).message}`,
      error,
    );
  }
  ...
  if (!Array.isArray(sourceTools)) {
    throw new Error(
      "[copilot-tool-bridge] createMerClawCodingTools must return an array of tools",
    );
  }
  ...
  return {
    sdkTools: filteredTools.map((sourceTool) =>
      convertMerClawToolToSdkTool(sourceTool, {
...
/**
 * Builds the full `createMerClawCodingTools` options bag mirroring the
 * PI in-tree call at `src/agents/pi-embedded-runner/run/attempt.ts:1029-1117`.
 *
 * Why PI parity matters: bridged MerClaw tools register with the SDK
 * as `overridesBuiltInTool: true, skipPermission: true` (see
 * `convertMerClawToolToSdkTool` below). That means the wrapped-tool
 * enforcement layer
...
function buildMerClawCodingToolsOptions(
  ...
  const modelCompat =
    ...
    typeof model.compat === "object"
      ? (model.compat as MerClawCodingToolsOptions["modelCompat"])
      : undefined;
...
export function convertMerClawToolToSdkTool(
  sourceTool: AnyAgentTool,
  ctx: {
    abortSignal?: AbortSignal;
  ...
  return {
    ...
    // MerClaw owns its bridged tools by design (the harness docs:
    // "MerClaw still owns ... MerClaw dynamic tools (bridged)"). The bundled
    // Copilot CLI ships built-in tools whose names (edit, read, write, bash,
    // ...) collide with MerClaw's coding-tool set. Mark every bridged tool as
    ...
    // same name." MerClaw's tool layer is the source of truth for these
    // names within a copilot attempt.
    overridesBuiltInTool: true,
    parameters: sourceTool.parameters as Record<string, unknown> | undefined,
    // Bridged MerClaw tools enforce their own permission/policy decisions
    ...
    // `kind: "custom-tool"` would either short-circuit MerClaw's richer
    // enforcement (if we allow-all) or block every call (if we
    // reject-all) — neither matches PI parity. The in-tree codex harness
    // takes the same approach: bridged MerClaw tools are wrapped with
