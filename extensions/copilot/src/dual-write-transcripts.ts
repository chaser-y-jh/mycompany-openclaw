 ...
 * MerClaw audit transcript that sits next to (but is distinct from) the
 * SDK's own session storage.
 *
 * The MerClaw shell (src/agents/command/attempt-execution.ts) already
 ...
 * assistant + toolResult) into the MerClaw transcript via the same
 ...
 * the MerClaw audit trail just misses the intermediate messages for
 * this turn.
 */
