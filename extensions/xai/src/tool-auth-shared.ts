...
function readLegacyGrokFallbackAuth(cfg?: MerClawConfig): XaiFallbackAuth | undefined {
...
function readLegacyGrokApiKeyResult(cfg?: MerClawConfig): ConfiguredRuntimeApiKeyResolution {
  const search = cfg?.tools?.web?.search;
  if (!search || typeof search !== "object") {
    return { status: "missing" };
  }
...
export function resolveFallbackXaiAuth(cfg?: MerClawConfig): XaiFallbackAuth | undefined {
...
export function resolveFallbackXaiApiKey(cfg?: MerClawConfig): string | undefined {
