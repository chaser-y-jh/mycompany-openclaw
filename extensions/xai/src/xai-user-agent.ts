// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatMerClawUserAgent`.

import { MERCLAW_VERSION as PACKAGE_VERSION } from "merclaw/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "merclaw";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.MERCLAW_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.MERCLAW_SERVICE_VERSION) ??
...
// an empty record so the merclaw identity is not forwarded to the proxy.
