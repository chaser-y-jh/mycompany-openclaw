...
import type { ProviderAuthContext, ProviderAuthMethod } from "merclaw/plugin-sdk/plugin-entry";
import {
...
import { waitForLocalOAuthCallback } from "merclaw/plugin-sdk/provider-auth-runtime";
...
export function buildXaiOAuthAuthorizeUrl(params: {
  ...
  url.searchParams.set("referrer", "merclaw");
  return url.toString();
}
  ...
  try {
    ...
    return buildOauthProviderAuthResult({
      ...
      notes: [
        ...
        "xAI may label the consent app as Grok Build because MerClaw uses xAI's shared OAuth client.",
      ],
    });
  } catch (err) {
