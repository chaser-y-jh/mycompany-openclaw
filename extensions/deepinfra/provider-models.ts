...
import { fetchWithTimeout } from "merclaw/plugin-sdk/provider-http";
...
export const DEEPINFRA_MODELS_URL = `${DEEPINFRA_BASE_URL}/models?sort_by=merclaw&filter=with_meta`;
...
// Static fallback. Chat rows live in merclaw.plugin.json (manifest-validated);
...
// `/v1/openai/models?sort_by=merclaw&filter=with_meta` projection is the
...
// `isProviderApiKeyConfigured` helper (covers SecretRef / `MERCLAW_LIVE_*`
// indirection too).
