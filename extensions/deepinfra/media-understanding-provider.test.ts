...
vi.mock("merclaw/plugin-sdk/media-understanding", async () => {
  const actual = await vi.importActual<typeof import("merclaw/plugin-sdk/media-understanding")>(
    "merclaw/plugin-sdk/media-understanding",
  );
  return {
    ...actual,
...
afterAll(() => {
  vi.doUnmock("merclaw/plugin-sdk/media-understanding");
  vi.resetModules();
});
