...
metadata: { "merclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == MerClaw `sessions_spawn`
- **File I/O** == MerClaw `read`/`write`
- **Remote fetch** == MerClaw `web_fetch` (or `exec` with curl when POST is required)
