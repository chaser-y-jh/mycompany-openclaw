...
vi.mock("merclaw/plugin-sdk/acp-runtime-backend", () => ({
...
const previousSkipRuntime = process.env.MERCLAW_SKIP_ACPX_RUNTIME;

function restoreEnv(): void {
  if (previousSkipRuntime === undefined) {
    delete process.env.MERCLAW_SKIP_ACPX_RUNTIME;
  } else {
    process.env.MERCLAW_SKIP_ACPX_RUNTIME = previousSkipRuntime;
  }
}

function createServiceContext() {
  return {
    workspaceDir: "/tmp/merclaw-acpx-register-test",
    stateDir: "/tmp/merclaw-acpx-register-test/state",
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
