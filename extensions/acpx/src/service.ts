...
const ENABLE_STARTUP_PROBE_ENV = "MERCLAW_ACPX_RUNTIME_STARTUP_PROBE";
const SKIP_RUNTIME_PROBE_ENV = "MERCLAW_SKIP_ACPX_RUNTIME_PROBE";
  ...
  async function resolveRuntime(): Promise<AcpxRuntimeLike> {
    ...
    runtimePromise ??= loadRuntimeModule().then((module) => {
      runtime = new module.AcpxRuntime({
        ...
        merclawGatewayInstanceId: params.gatewayInstanceId,
        merclawProcessLeaseStore: params.processLeaseStore,
        merclawWrapperRoot: params.wrapperRoot,
...
function resolveAllowedAgentsProbeAgent(ctx: MerClawPluginServiceContext): string | undefined {
...
async function measureAcpxStartup<T>(
  ctx: MerClawPluginServiceContext,
  name: string,
  run: () => T | Promise<T>,
): Promise<T> {
...
async function reapOpenAcpxProcessLeases(params: {
  ...
  for (const lease of leases) {
    if (lease.rootPid <= 0) {
      ...
      if (!result) {
        result = await reapStaleMerClawOwnedAcpxOrphans({
...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: MerClawPluginServiceContext): Promise<void> {
      ...
      if (startupReap.terminatedPids.length > 0) {
        ctx.logger.info(
          `reaped ${startupReap.terminatedPids.length} stale MerClaw-owned ACPX process${startupReap.terminatedPids.length === 1 ? "" : "es"}`,
        );
      }
    ...
    async stop(_ctx: MerClawPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
