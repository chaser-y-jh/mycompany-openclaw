...
The ACPX extension is a thin MerClaw wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `merclaw/acpx`, not in this extension.
...
Use this flow when MerClaw needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `merclaw/acpx` repo first.
2. In MerClaw, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild MerClaw and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for MerClaw ACP validation.
...
If a change feels like shared ACP runtime behavior instead of MerClaw-specific glue, move it to `merclaw/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
