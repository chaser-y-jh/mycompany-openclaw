...
import { createSendCfgThreadingRuntime } from "merclaw/plugin-sdk/channel-test-helpers";
...
vi.mock("merclaw/plugin-sdk/plugin-config-runtime", async () => {
  const original = (await vi.importActual("merclaw/plugin-sdk/plugin-config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("merclaw/plugin-sdk/text-chunking", async () => {
  const original = (await vi.importActual("merclaw/plugin-sdk/text-chunking")) as Record<
    string,
    unknown
  >;
  return {
...
afterAll(() => {
  ...
  vi.doUnmock("merclaw/plugin-sdk/plugin-config-runtime");
  vi.doUnmock("merclaw/plugin-sdk/text-chunking");
  vi.resetModules();
});
