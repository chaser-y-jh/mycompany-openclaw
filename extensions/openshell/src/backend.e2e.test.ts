...
import { createSandboxTestContext } from "merclaw/plugin-sdk/test-fixtures";
import {
...
const MERCLAW_OPENSHELL_E2E = process.env.MERCLAW_E2E_OPENSHELL === "1";
const MERCLAW_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const MERCLAW_OPENSHELL_COMMAND =
  process.env.MERCLAW_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "merclaw-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && MERCLAW_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: MERCLAW_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(MERCLAW_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(MERCLAW_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-openshell-e2e-"));
      ...
      const gatewayName = `merclaw-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `merclaw-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/merclaw-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: MERCLAW_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("merclaw-openshell-e2e");
