...
function resolveQianfanPreset(cfg: MerClawConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: MerClawConfig) => {
...
export function applyQianfanProviderConfig(cfg: MerClawConfig): MerClawConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: MerClawConfig): MerClawConfig {
