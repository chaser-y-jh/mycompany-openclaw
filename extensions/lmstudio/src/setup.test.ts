import { CUSTOM_LOCAL_AUTH_MARKER } from "merclaw/plugin-sdk/provider-auth";
...
import { resolveAgentModelPrimaryValue } from "merclaw/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "merclaw/plugin-sdk/provider-setup";
import type { WizardPrompter } from "merclaw/plugin-sdk/setup";
...
vi.mock("merclaw/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("merclaw/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("merclaw/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("merclaw/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("merclaw/plugin-sdk/provider-auth");
  vi.doUnmock("merclaw/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
function buildConfig(): MerClawConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
describe("lmstudio setup", () => {
  ...
  it("non-interactive setup fails when requested model is missing", async () => {
    ...
    const dockerSetup = ["1", "true", "yes", "on"].includes(
      process.env.MERCLAW_DOCKER_SETUP?.trim().toLowerCase() ?? "",
    );
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("MERCLAW_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([
