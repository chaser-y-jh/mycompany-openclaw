...
import type { waitForTransportReady } from "merclaw/plugin-sdk/transport-ready-runtime";
...
vi.mock("merclaw/plugin-sdk/transport-ready-runtime", () => ({
  waitForTransportReady: waitForTransportReadyMock,
}));

vi.mock("merclaw/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("merclaw/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
vi.mock("merclaw/plugin-sdk/channel-inbound", async (importOriginal) => {
  const actual = await importOriginal<typeof import("merclaw/plugin-sdk/channel-inbound")>();
  return {
    ...actual,
...
vi.mock("merclaw/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("merclaw/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
describe("iMessage monitor last-route updates", () => {
  ...
  async function createMessagesDbWithMaxRowid(maxRowid: number, dbPath?: string): Promise<string> {
    ...
    if (!resolvedDbPath) {
      const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-imsg-watch-watermark-"));
  ...
  it("uses the default local chat.db path for the startup watermark", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-imsg-default-home-"));
  ...
  it("subscribes without a startup watermark when the configured dbPath is not readable", async () => {
    const dbPath = path.join(os.tmpdir(), `merclaw-missing-chat-${Date.now()}.db`);
    const client = {
      request: vi.fn(async () => ({ subscription: 1 })),
      waitForClose: vi.fn(async () => {}),
      stop: vi.fn(async () => {}),
  ...
  it("advances the catchup cursor after startup catchup succeeds and a live row is handled", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-imsg-live-cursor-"));
    tempDirs.push(stateDir);
  ...
  it("flushes live cursor advancement for rows handled while startup catchup is running", async () => {
    ...
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-imsg-live-during-catchup-"));
    tempDirs.push(stateDir);
    vi.stubEnv("MERCLAW_STATE_DIR", stateDir);
  ...
  it("repairs anchorless group watch payloads before routing or cursor updates", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-imsg-anchor-repair-"));
    tempDirs.push(stateDir);
    vi.stubEnv("MERCLAW_STATE_DIR", stateDir);
        ...
        onNotification?.({
          ...
          params: {
            message: {
              ...
              sender: "+15550001111",
              is_from_me: false,
              text: "@merclaw check this https://example.com",
              is_group: false,
              chat_guid: "",
              chat_identifier: "",
              chat_name: "",
  ...
  it("does not advance the live cursor after partial startup catchup", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-05-22T15:31:00.000Z"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-imsg-partial-cursor-"));
    tempDirs.push(stateDir);
    vi.stubEnv("MERCLAW_STATE_DIR", stateDir);
  ...
  it("advances a coalesced live bucket to the highest source row", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-imsg-coalesced-cursor-"));
    tempDirs.push(stateDir);
    vi.stubEnv("MERCLAW_STATE_DIR", stateDir);
