...
import type { PluginLogger } from "merclaw/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "merclaw/plugin-sdk/runtime-env";
...
// See https://github.com/merclaw/merclaw/issues/72481
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.MERCLAW_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "MerClaw"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "MerClaw";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(MerClaw\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.MERCLAW_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "merclaw";
    ...
    const recreateAdvertiser = async (reason: string, opts?: { stuckState?: boolean }) => {
      ...
      recreatePromise = (async () => {
        ...
        if (tooManyConsecutive || tooManyStuckStates || tooManyInWindow) {
          ...
          logger.warn(
            `bonjour: disabling advertiser after ${detail} (${reason}); set discovery.mdns.mode="off" or MERCLAW_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );
