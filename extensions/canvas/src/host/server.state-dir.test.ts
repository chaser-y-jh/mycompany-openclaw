...
import { withStateDirEnv } from "merclaw/plugin-sdk/test-env";
...
describe("canvas host state dir defaults", () => {
  ...
  it("uses MERCLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("merclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("MerClaw Canvas");
      } finally {
        await handler.close();
      }
    });
