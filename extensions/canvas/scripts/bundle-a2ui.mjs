...
async function main() {
  ...
  if (!hasA2uiPackage || !hasAppDir) {
    ...
    if (process.env.MERCLAW_SPARSE_PROFILE || process.env.MERCLAW_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI package missing; skipping bundle because MERCLAW_A2UI_SKIP_MISSING=1 or MERCLAW_SPARSE_PROFILE is set.",
      );
      return;
    }
