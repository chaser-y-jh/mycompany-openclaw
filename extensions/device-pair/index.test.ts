...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredMerClawTmpDir: vi.fn(() => path.join(os.tmpdir(), "merclaw-device-pair-tests")),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredMerClawTmpDir: pluginApiMocks.resolvePreferredMerClawTmpDir,
...
function createApi(params?: {
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as MerClawPluginApi["runtime"],
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredMerClawTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredMerClawTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the MerClaw iOS app:");
    ...
    expect(text).not.toContain("![MerClaw pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the MerClaw iOS app:");
...
describe("device-pair /pair default setup code", () => {
  ...
  it("allows mdns cleartext setup urls", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://merclaw.local:18789",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(requireText(result)).toContain("Gateway: ws://merclaw.local:18789");
  });
