...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): MerClawConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as MerClawConfig;
  ...
  function expectLastSlackSend(content: string, cfg: MerClawConfig, threadTs?: string) {
  ...
  async function resolveReadToken(cfg: MerClawConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: MerClawConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
  ...
  it("returns non-image downloadFile results as file metadata instead of image content", async () => {
    downloadSlackFile.mockResolvedValueOnce({
      path: "/tmp/merclaw-media/report.pdf",
    ...
    expect(String(firstContent.text)).toContain("/tmp/merclaw-media/report.pdf");
    ...
    expect(details.media).toEqual({
      mediaUrl: "/tmp/merclaw-media/report.pdf",
      outbound: false,
      contentType: "application/pdf",
    });
  });
