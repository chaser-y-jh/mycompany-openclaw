...
import { expectPairingReplyText } from "merclaw/plugin-sdk/channel-test-helpers";
import { HISTORY_CONTEXT_MARKER } from "merclaw/plugin-sdk/reply-history";
import { resetInboundDedupe } from "merclaw/plugin-sdk/reply-runtime";
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bmerclaw\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("merclaw: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("merclaw: hello <@U2>");
  });
