import { hasConfiguredSecretInput } from "merclaw/plugin-sdk/secret-input";
import {
...
function enableSlackAccount(cfg: MerClawConfig, accountId: string): MerClawConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: MerClawConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: MerClawConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: MerClawConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: MerClawConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: MerClawConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: MerClawConfig; accountId: string }) =>
    ...
    disable: (cfg: MerClawConfig) => setSetupChannelEnabled(cfg, channel, false),
