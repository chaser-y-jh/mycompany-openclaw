...
import { patchChannelConfigForAccount } from "merclaw/plugin-sdk/setup-runtime";
import { formatDocsLink } from "merclaw/plugin-sdk/setup-tools";
import type { ResolvedSlackAccount } from "./accounts.js";
import type { MerClawConfig } from "./channel-api.js";

export const SLACK_CHANNEL = "slack" as const;

export function buildSlackManifest(botName = "MerClaw") {
  const safeName = botName.trim() || "MerClaw";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for MerClaw`,
    },
    features: {
      bot_user: {
      ...
      assistant_view: {
        assistant_description: `${safeName} connects Slack assistant threads to MerClaw agents.`,
        suggested_prompts: [
          {
