import { asOptionalRecord as asRecord } from "merclaw/plugin-sdk/string-coerce-runtime";
...
const MERCLAW_SLACK_CLIENT_PING_TIMEOUT_MS = 15_000;
const MERCLAW_SLACK_SOCKET_START_FAILED_EVENT = "unable_to_socket_mode_start";
const MERCLAW_SLACK_NATIVE_RECONNECT_OBSERVER_KEY = "__merclawNativeReconnectFailureObserver";
...
function installSlackNativeReconnectFailureObserver(receiver: unknown) {
  ...
  if (Reflect.get(client, MERCLAW_SLACK_NATIVE_RECONNECT_OBSERVER_KEY)) {
    return;
  }
  ...
  Reflect.set(client, MERCLAW_SLACK_NATIVE_RECONNECT_OBSERVER_KEY, true);
  Reflect.set(
    client,
    ...
    function patchedDelayReconnectAttempt(this: object, callback: unknown) {
      ...
      const delayMs =
        (Number.isFinite(pingTimeoutMs) && pingTimeoutMs >= 0
          ? pingTimeoutMs
          : MERCLAW_SLACK_CLIENT_PING_TIMEOUT_MS) * nextFailureCount;
        ...
        setTimeout(() => {
          ...
          Promise.resolve(callback.call(this)).then(resolve, (error: unknown) => {
            if (callback === Reflect.get(this, "start")) {
              emit.call(this, MERCLAW_SLACK_SOCKET_START_FAILED_EVENT, error);
              resolve(undefined);
              return;
            }
            reject(error);
...
export function shouldSkipMerClawSlackSelfEvent(args: SlackSelfFilterArgs): boolean {
...
export function createSlackBoltApp(params: {
  ...
  const socketModeReceiverOptions: SlackSocketModeReceiverOptions = {
    ...
    clientPingTimeout:
      params.socketMode?.clientPingTimeout ?? MERCLAW_SLACK_CLIENT_PING_TIMEOUT_MS,
  ...
  const app = new params.interop.App({
    ...
    // consumes that promise, tripping MerClaw's fatal unhandled-rejection path.
    tokenVerificationEnabled: false,
  });
  app.use(async (args) => {
    if (shouldSkipMerClawSlackSelfEvent(args)) {
      return;
    }
    await args.next();
  });
