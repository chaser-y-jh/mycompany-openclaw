...
export function buildComfyConfig(config: Record<string, unknown>): MerClawConfig {
  return {
    plugins: {
      entries: {
        comfy: { config },
...
export function buildLegacyComfyConfig(config: Record<string, unknown>): MerClawConfig {
  return {
    models: {
      providers: {
        comfy: config,
