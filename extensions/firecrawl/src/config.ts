...
import { resolvePositiveTimeoutSeconds } from "merclaw/plugin-sdk/provider-web-fetch";
import { resolveSecretInputString, normalizeSecretInput } from "merclaw/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<MerClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: MerClawConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: MerClawConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: MerClawConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: MerClawConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: MerClawConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: MerClawConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: MerClawConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: MerClawConfig, override?: number): number {
