...
export function createToolFactoryHarness(cfg: MerClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<MerClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
