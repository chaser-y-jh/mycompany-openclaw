...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__merclaw_channel_id";
...
type InteractionCallbackConfig = Pick<MerClawConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "merclaw-mattermost-interactions").update(botToken).digest("hex");
}
