...
import { registerApiProvider, streamSimple } from "merclaw/plugin-sdk/llm";
...
import { streamWithPayloadPatch } from "merclaw/plugin-sdk/provider-stream-shared";
 ...
 * IDs that may not contain the model name). When MerClaw's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if MerClaw identifies this as an Anthropic model via the ARN heuristic.
 ...
 * same matcher MerClaw uses for direct model IDs.
 */
 ...
 * the MerClaw config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: MerClawPluginApi): void {
