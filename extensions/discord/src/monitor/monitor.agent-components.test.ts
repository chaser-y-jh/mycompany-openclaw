...
import { peekSystemEvents, resetSystemEventsForTest } from "merclaw/plugin-sdk/test-fixtures";
...
describe("agent components", () => {
  ...
  const createCfg = (): MerClawConfig => ({}) as MerClawConfig;
  ...
  it("sends pairing reply when DM sender is not allowlisted", async () => {
    ...
    expect(pairingText).toContain(`merclaw pairing approve discord ${code}`);
