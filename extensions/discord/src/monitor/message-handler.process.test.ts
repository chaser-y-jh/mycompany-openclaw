...
import { DEFAULT_EMOJIS, DEFAULT_TIMING } from "merclaw/plugin-sdk/channel-feedback";
import {
...
import { setReplyPayloadMetadata } from "merclaw/plugin-sdk/reply-payload-testing";
import * as runtimeEnvModule from "merclaw/plugin-sdk/runtime-env";
...
function createNonTerminalToolWarningPayload(): ReplyPayload {
  return setReplyPayloadMetadata(
    {
      text: "⚠️ 🛠️ `run merclaw definitely-not-a-real-subcommand (agent)` failed",
      isError: true,
    },
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveAndPersistSessionFile: vi.fn<(params?: unknown) => Promise<{ sessionFile: string }>>(
    async () => ({ sessionFile: "/tmp/merclaw-discord-process-test-session.jsonl" }),
  ),
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/merclaw-discord-process-test-sessions.json",
  ),
}));
...
vi.mock("merclaw/plugin-sdk/conversation-runtime", () => ({
...
vi.mock("merclaw/plugin-sdk/session-store-runtime", () => ({
...
beforeEach(() => {
  ...
  resolveAndPersistSessionFile.mockResolvedValue({
    sessionFile: "/tmp/merclaw-discord-process-test-session.jsonl",
  });
  resolveSessionStoreEntry.mockImplementation((params) => ({
    existing: params.sessionKey ? params.store[params.sessionKey] : undefined,
  }));
  resolveStorePath.mockReturnValue("/tmp/merclaw-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createAutomaticSourceDeliveryContext({
      cfg: {
        ...
        session: { store: "/tmp/merclaw-discord-process-test-sessions.json" },
      },
    });
  ...
  it("pins Discord text DM main-route updates to the single configured DM owner", async () => {
    const ctx = await createBaseContext({
      ...
      cfg: {
        messages: { ackReaction: "👀" },
        session: {
          store: "/tmp/merclaw-discord-process-test-sessions.json",
          dmScope: "main",
        },
      },
