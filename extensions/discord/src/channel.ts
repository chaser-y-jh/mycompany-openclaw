...
import { sleepWithAbort } from "merclaw/plugin-sdk/runtime-env";
import {
...
import { resolveTargetsWithOptionalToken } from "merclaw/plugin-sdk/target-resolver-runtime";
import {
...
function resolveDiscordStartupDelayMs(cfg: MerClawConfig, accountId: string): number {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). MerClaw adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
