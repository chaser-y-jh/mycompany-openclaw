...
import { resolveInboundRouteEnvelopeBuilderWithRuntime } from "merclaw/plugin-sdk/inbound-envelope";
import {
...
export async function handleNextcloudTalkInbound(params: {
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as MerClawConfig,
    surface: CHANNEL_ID,
  });
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as MerClawConfig);
  ...
  const { groupPolicy, providerMissingFallbackApplied } =
    resolveAllowlistProviderRuntimeGroupPolicy({
      ...
      defaultGroupPolicy: resolveDefaultGroupPolicy(config as MerClawConfig),
    });
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as MerClawConfig);
