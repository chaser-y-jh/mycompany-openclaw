...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<MerClawConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<MerClawConfig["channels"]>["matrix"]>>,
): MerClawConfig {
  return {
    channels: {
      matrix: {
