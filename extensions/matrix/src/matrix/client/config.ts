...
import { resolveOptionalIntegerOption } from "merclaw/plugin-sdk/number-runtime";
import { requireRuntimeConfig } from "merclaw/plugin-sdk/plugin-config-runtime";
import { retryAsync } from "merclaw/plugin-sdk/retry-runtime";
import {
  coerceSecretRef,
  normalizeResolvedSecretInputString,
} from "merclaw/plugin-sdk/secret-input-runtime";
import type { PinnedDispatcherPolicy } from "merclaw/plugin-sdk/ssrf-dispatcher";
import {
...
export async function resolveMatrixAuth(params?: {
  ...
  const login = (await retryMatrixAuthRequest("matrix auth login", async () => {
    return (await loginClient.doRequest("POST", "/_matrix/client/v3/login", undefined, {
      ...
      initial_device_display_name: resolved.deviceName ?? "MerClaw Gateway",
    })) as {
      access_token?: string;
      user_id?: string;
      device_id?: string;
