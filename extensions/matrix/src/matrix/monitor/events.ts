...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'merclaw matrix verify status --verbose --account ${accountId}' and 'merclaw matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another MerClaw runtime is using the same account. " +
    `Check 'merclaw matrix verify status --verbose --account ${accountId}' and 'merclaw matrix devices list --account ${accountId}'.`
  );
}
