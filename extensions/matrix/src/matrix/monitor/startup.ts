...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleMerClawDevices.length > 0) {
      params.logger.warn(
        `matrix: stale MerClaw devices detected for ${params.auth.userId}: ${deviceHealth.staleMerClawDevices.map((device) => device.deviceId).join(", ")}. Run 'merclaw matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
