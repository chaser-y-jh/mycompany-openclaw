...
function resolveDefaultAgentId(cfg: MerClawConfig): string {
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  const chosen = (agents.find((agent) => agent?.default) ?? agents[0])?.id;
  return normalizeAgentId(chosen);
}

function listConfiguredAccountIds(cfg: MerClawConfig): string[] {
...
function listBoundAccountIds(cfg: MerClawConfig, channelId: string): string[] {
  const ids = new Set<string>();
...
function resolveDefaultAgentBoundAccountId(cfg: MerClawConfig, channelId: string): string | null {
...
function hasImplicitDefaultTelegramAccount(cfg: MerClawConfig): boolean {
...
export function listTelegramAccountIds(cfg: MerClawConfig): string[] {
...
export function resolveDefaultTelegramAccountSelection(cfg: MerClawConfig): {
  accountId: string;
  accountIds: string[];
...
export function resolveDefaultTelegramAccountId(cfg: MerClawConfig): string {
