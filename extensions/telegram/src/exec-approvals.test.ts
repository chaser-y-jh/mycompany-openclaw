...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<MerClawConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<MerClawConfig["channels"]>["telegram"]>>,
): MerClawConfig {
  return {
    channels: {
      telegram: {
