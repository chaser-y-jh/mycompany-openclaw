...
export function shouldShowTelegramDmAccessWarning(cfg: MerClawConfig, accountId: string): boolean {
...
export function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    "  " + formatCliCommand(`merclaw config set ${configBase}.dmPolicy "allowlist"`),
    "  " + formatCliCommand(`merclaw config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`),
