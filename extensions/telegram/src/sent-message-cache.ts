...
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("merclaw.telegramSentMessagesState");
const TELEGRAM_SENT_MESSAGES_STORE_FOR_TEST_KEY = Symbol.for(
  "merclaw.telegramSentMessagesStoreForTest",
);
...
function resolveSentMessageStorePath(cfg?: Pick<MerClawConfig, "session">): string {
  return `${resolveStorePath(cfg?.session?.store)}.telegram-sent-messages.json`;
}

function resolveSentMessageScopeKey(cfg?: Pick<MerClawConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<MerClawConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<MerClawConfig, "session">): SentMessageStore {
...
export function recordSentMessage(
  ...
  messageId: number,
  cfg?: Pick<MerClawConfig, "session">,
): void {
