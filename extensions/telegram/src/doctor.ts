...
function formatTelegramAccountConfigPath(cfg: MerClawConfig, accountId: string): string {
  const telegram = asObjectRecord((cfg.channels as Record<string, unknown> | undefined)?.telegram);
  const accounts = asObjectRecord(telegram?.accounts);
...
export function maybeRepairTelegramApiRoots(cfg: MerClawConfig): {
...
async function repairTelegramConfig(params: { cfg: MerClawConfig }): Promise<{
  config: MerClawConfig;
  changes: string[];
}> {
...
export async function maybeRepairTelegramAllowFromUsernames(cfg: MerClawConfig): Promise<{
  config: MerClawConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("merclaw/plugin-sdk/runtime");
