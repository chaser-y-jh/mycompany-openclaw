...
} from "merclaw/plugin-sdk/fetch-runtime";
import {
...
import { resolveRequestUrl } from "merclaw/plugin-sdk/request-url";
...
// fix for the leak observed in merclaw#68128 is the transport lifecycle that
...
function resolveMerClawProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.MERCLAW_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveMerClawProxyUrlForTelegram() : undefined;
