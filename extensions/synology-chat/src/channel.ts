/**
 * Synology Chat Channel Plugin for MerClaw.
 *
 * Implements the ChannelPlugin interface following the LINE pattern.
 */

import { DEFAULT_ACCOUNT_ID } from "merclaw/plugin-sdk/account-id";
import type { MerClawConfig } from "merclaw/plugin-sdk/account-resolution";
import {
...
import { createEmptyChannelDirectoryAdapter } from "merclaw/plugin-sdk/directory-runtime";
import {
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "merclaw pairing approve synology-chat <code>",
...
> & {
  pairing: {
    ...
    notifyApproval: (params: { cfg: MerClawConfig; id: string }) => Promise<void>;
  };
  security: {
    resolveDmPolicy: (params: { cfg: MerClawConfig; account: ResolvedSynologyChatAccount }) => {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      meta: {
        ...
        blurb: "Connect your Synology NAS Chat to MerClaw",
        order: 90,
      },
      capabilities: {
