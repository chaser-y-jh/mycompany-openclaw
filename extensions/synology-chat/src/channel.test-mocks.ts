...
vi.mock("merclaw/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("merclaw/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("merclaw/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("merclaw/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("merclaw/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("merclaw/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/merclaw/synology-chat-sessions.json"),
