import { formatCliCommand, parseDurationMs } from "merclaw/plugin-sdk/cli-runtime";
import { resolveExpiresAtMsFromDurationMs } from "merclaw/plugin-sdk/number-runtime";
import type {
  MerClawPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type MerClawConfig as ProviderAuthConfig,
...
import { fetchClaudeUsage } from "merclaw/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("merclaw models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("merclaw models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("merclaw doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: MerClawPluginApi): void {
