...
function usesClaudeCliModelSelection(config: MerClawConfig): boolean {
...
function usesSelectedClaudeCliAuthProfile(config: MerClawConfig): boolean {
  const profiles = config.auth?.profiles ?? {};
...
function collectClaudeCliRuntimeRefsFromConfig(config: MerClawConfig): string[] {
  const refs = new Set<string>(
