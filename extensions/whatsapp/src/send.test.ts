...
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS } from "merclaw/plugin-sdk/media-runtime";
...
let resetLogger: typeof import("merclaw/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("merclaw/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: MerClawConfig = {
  channels: { whatsapp: {} },
};
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("merclaw/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `merclaw-outbound-${crypto.randomUUID()}.log`);
