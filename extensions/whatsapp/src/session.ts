...
import { VERSION } from "merclaw/plugin-sdk/cli-runtime";
import {
...
import { getChildLogger, toPinoLikeLogger } from "merclaw/plugin-sdk/runtime-env";
    ...
    try {
      ...
      if (connection === "close") {
        ...
        if (status === LOGGED_OUT_STATUS) {
          console.error(
            danger(
              `WhatsApp session logged out. Run: ${formatCliCommand("merclaw channels login")}`,
            ),
          );
        }
      }
