...
import { sleep } from "merclaw/plugin-sdk/text-utility-runtime";
...
vi.mock("merclaw/plugin-sdk/text-utility-runtime", async () => {
  const actual = await vi.importActual<typeof import("merclaw/plugin-sdk/text-utility-runtime")>(
    "merclaw/plugin-sdk/text-utility-runtime",
  );
  return {
    ...actual,
    sleep: vi.fn(async () => {}),
