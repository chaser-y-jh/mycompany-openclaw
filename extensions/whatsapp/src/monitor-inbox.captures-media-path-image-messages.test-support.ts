...
vi.mock("merclaw/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("merclaw/plugin-sdk/logging-core")>(
    "merclaw/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("MerClaw WhatsApp listener close");
