...
import type { DmPolicy, GroupPolicy, ReplyToMode } from "merclaw/plugin-sdk/config-contracts";
import { resolveOAuthDir } from "merclaw/plugin-sdk/state-paths";
...
export function listWhatsAppAuthDirs(cfg: MerClawConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: MerClawConfig): boolean {
...
export function resolveWhatsAppAuthDir(params: { cfg: MerClawConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: MerClawConfig): ResolvedWhatsAppAccount[] {
