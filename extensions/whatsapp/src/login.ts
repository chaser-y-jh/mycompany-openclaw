...
import { logInfo } from "merclaw/plugin-sdk/logging-core";
import { getRuntimeConfig } from "merclaw/plugin-sdk/runtime-config-snapshot";
import { danger, success } from "merclaw/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "merclaw/plugin-sdk/runtime-env";
...
export async function loginWeb(
  ...
  try {
    ...
    if (result.outcome === "logged-out") {
      runtime.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("merclaw channels login")} and scan the QR again.`,
        ),
      );
