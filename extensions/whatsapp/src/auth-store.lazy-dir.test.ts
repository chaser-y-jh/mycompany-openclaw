...
describe("WhatsApp auth dir profile resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["MERCLAW_STATE_DIR", "MERCLAW_OAUTH_DIR"]);
    tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-wa-profile-"));
  ...
  it("resolves the default web auth dir from MERCLAW_STATE_DIR at call time", async () => {
