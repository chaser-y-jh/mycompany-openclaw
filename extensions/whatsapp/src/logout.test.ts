...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "merclaw-test-web-logout-"));
    previousOAuthDir = process.env.MERCLAW_OAUTH_DIR;
    process.env.MERCLAW_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  it("does not delete through symlinked auth dirs inside the MerClaw auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the MerClaw auth root", async () => {
