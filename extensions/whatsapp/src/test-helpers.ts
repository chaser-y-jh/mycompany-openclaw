...
import { formatEnvelopeTimestamp } from "merclaw/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("merclaw:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("merclaw:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[merclaw]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/merclaw-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("merclaw:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("merclaw:lastSocket")];
  if (typeof getter === "function") {
