/**
 * Smoke tests for the `merclaw path` CLI handlers.
 *
...
describe("merclaw path CLI", () => {
  ...
  describe("set", () => {
    ...
    it("CLI-S08 sets slash-deep JSONC paths and parsed JSON values", async () => {
      const filePath = join(workspaceDir, "merclaw.json");
      writeFileSync(
        filePath,
      ...
      await pathSetCommand(
        "oc://merclaw.json/gateway/auth/token",
      ...
      await pathSetCommand(
        "oc://merclaw.json/agents/list/0/tools/exec/security",
        "allowlist",
        { cwd: workspaceDir, json: true },
        rt2,
      );
