...
vi.mock("merclaw/plugin-sdk/gateway-runtime", () => ({
  ...
  it("calls the in-process gateway cli helper without mutating process.env", async () => {
    ...
    try {
      ...
      if (originalHome === undefined) {
        ...
        process.env.MERCLAW_HOME = originalHome;
      }
    }

    expect(process.env.MERCLAW_HOME).toBe(originalHome);
  });
