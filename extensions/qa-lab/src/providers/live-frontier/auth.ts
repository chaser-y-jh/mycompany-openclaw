...
import { normalizeStringEntries, uniqueStrings } from "merclaw/plugin-sdk/string-coerce-runtime";
import { resolveQaAgentAuthDir, writeQaAuthProfiles } from "../shared/auth-store.js";

export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "MERCLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "MERCLAW_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "MERCLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["MERCLAW_LIVE_ANTHROPIC_KEY"],
  gemini: ["MERCLAW_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
    "MERCLAW_LIVE_CODEX_API_KEY",
...
function qaLiveOpenAiUsesCodexByDefault(cfg: MerClawConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: MerClawConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: MerClawConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.MERCLAW_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "merclaw") {
    return false;
  }
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or MERCLAW_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
      "Host MerClaw OAuth refresh profiles are not copied into QA temp stores.",
    ].join(" "),
  );
}
