...
export type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { merclaw: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { merclaw: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      merclaw: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.merclaw.push(parity.cells.merclaw);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# MerClaw JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
