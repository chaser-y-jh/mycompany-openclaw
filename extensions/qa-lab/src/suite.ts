...
import { disposeRegisteredAgentHarnesses } from "merclaw/plugin-sdk/agent-harness";
...
function shouldLogQaSuiteProgress(env: NodeJS.ProcessEnv = process.env) {
  const override = parseQaSuiteBooleanEnv(env.MERCLAW_QA_SUITE_PROGRESS);
...
function resolveQaSuiteTransportReadyTimeoutMs(
  ...
  const raw = env.MERCLAW_QA_TRANSPORT_READY_TIMEOUT_MS;
  if (!raw) {
    return 120_000;
  }
...
async function runScenario(name: string, steps: QaSuiteStep[]): Promise<QaSuiteScenarioResult> {
  ...
  for (const step of steps) {
    try {
      if (process.env.MERCLAW_QA_DEBUG === "1") {
...
function buildRuntimeParityScenarioResult(params: {
  ...
  const merclawCell = params.result.cells.merclaw;
  return {
    ...
    steps: [
      {
        name: merclawCell.runtime,
        status:
          merclawCell.runtimeErrorClass || merclawCell.transportErrorClass ? "fail" : "pass",
        details: formatRuntimeParityCellDetails(merclawCell),
      },
      {
...
function buildQaRuntimeEnvPatch(params: {
  ...
  if (params.forcedRuntime) {
    patch.MERCLAW_BUILD_PRIVATE_QA = "1";
    patch.MERCLAW_QA_FORCE_RUNTIME = params.forcedRuntime;
  }
  if (params.forcedRuntime !== "codex" || params.providerMode !== "mock-openai") {
  ...
  patch.MERCLAW_CODEX_APP_SERVER_ARGS = `app-server -c openai_base_url=${mockBaseUrl}/v1 --listen stdio://`;
...
function shouldCaptureGatewayHeapCheckpoints(env: NodeJS.ProcessEnv = process.env) {
  return parseQaSuiteBooleanEnv(env.MERCLAW_QA_GATEWAY_HEAP_CHECKPOINTS) === true;
}
...
async function writeQaSuiteArtifacts(params: {
  ...
  const report = renderQaMarkdownReport({
    title: "MerClaw QA Scenario Suite",
    startedAt: params.startedAt,
...
export async function runQaSuite(params?: QaSuiteRunParams): Promise<QaSuiteResult> {
  ...
  const enabledPluginIds = [
    ...new Set([
      ...
      ...(params?.forcedRuntime && params.forcedRuntime !== "merclaw"
  ...
  const gateway = await startQaGatewayChild({
    ...
    mutateConfig: gatewayConfigPatch
      ? (cfg) => applyQaMergePatch(cfg, gatewayConfigPatch) as MerClawConfig
      : undefined,
  ...
  try {
    ...
    const keepTemp = process.env.MERCLAW_QA_KEEP_TEMP === "1" || false;
    await gateway.stop({
      keepTemp,
