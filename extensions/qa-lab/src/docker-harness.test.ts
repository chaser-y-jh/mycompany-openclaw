...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    for (const expectedFile of [
      ...
      path.join(outputDir, "state", "merclaw.json"),
    ...
    expect(compose).toContain("image: merclaw:qa-local-prebaked");
    ...
    expect(compose).toContain(":/opt/merclaw-qa-lab-ui:ro");
    expect(compose).toContain("      - sh");
    expect(compose).toContain("      - -lc");
    expect(compose).toContain(
      '        - fetch("http://127.0.0.1:18789/healthz").then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))',
    );
    expect(compose).toContain("--control-ui-proxy-target http://merclaw-qa-gateway:18789/");
    ...
    expect(compose).toContain("--ui-dist-dir /opt/merclaw-qa-lab-ui");
    expect(compose).toContain(":/opt/merclaw-repo:ro");
    expect(compose).toContain("./state:/opt/merclaw-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/merclaw-scaffold/seed-workspace/. /tmp/merclaw/workspace/",
    );
    expect(compose).toContain("MERCLAW_CONFIG_PATH: /tmp/merclaw/merclaw.json");
    expect(compose).toContain("MERCLAW_STATE_DIR: /tmp/merclaw/state");
    expect(compose).toContain('MERCLAW_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("MERCLAW_GATEWAY_TOKEN=qa-token");
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    expect(result.imageName).toBe("merclaw:qa-local-prebaked");
    expect(calls).toEqual([
      "docker build -t merclaw:qa-local-prebaked --build-arg MERCLAW_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/merclaw",
    ]);
  });
});
