...
export const CODEX_PLUGIN_LIFECYCLE_MESSAGES = Object.freeze({
  missingPlugin:
    'Codex plugin is required for Codex runtime. Run "merclaw doctor --fix" to install @merclaw/codex, then retry.',
});
...
function formatPinnedOldRemediation(pluginVersion: string, hostVersion: string) {
  return `Codex plugin version ${pluginVersion} is older than MerClaw ${hostVersion}. Run "merclaw plugins update codex" or unpin codex, then rerun "merclaw doctor --fix".`;
}

function formatPinnedNewRemediation(pluginVersion: string, hostVersion: string) {
  return `Codex plugin version ${pluginVersion} requires a newer MerClaw host than ${hostVersion}. Upgrade MerClaw or install a codex plugin version pinned to ${hostVersion}.`;
}
...
export async function seedCodexPluginAt(
  ...
  await fs.writeFile(
    path.join(targetDir, "merclaw.plugin.json"),
