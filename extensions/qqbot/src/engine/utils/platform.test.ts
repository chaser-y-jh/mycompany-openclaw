...
describe("qqbot local media path remapping", () => {
  ...
  function createMerClawTestRoot() {
    const actualHome = getHomeDir();
    const merclawDir = path.join(actualHome, ".merclaw");
    fs.mkdirSync(merclawDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(merclawDir, "qqbot-platform-test-"));
  ...
  function createQqbotMediaFile(fileName: string) {
    const { actualHome, testRootName } = createMerClawTestRoot();
  ...
  it("allows structured payload files inside sibling MerClaw media subdirectories", () => {
    ...
    // trust the shared `~/.merclaw/media` root so auto-routed sends can access
    // those files without the path-outside-storage guard firing.
    const actualHome = getHomeDir();
    const outboundDir = path.join(actualHome, ".merclaw", "media", "outbound");
...
// Regression coverage for https://github.com/merclaw/merclaw/issues/83562 —
// when HOME and MERCLAW_HOME diverge (Docker, multi-user hosts), QQ Bot media
// paths must be anchored on MERCLAW_HOME so files written under
// `$MERCLAW_HOME/.merclaw/media/qqbot/` are accepted by the outbound
// allowlist.
//
...
// home as the baseline and only varies `process.env.MERCLAW_HOME`.
describe("qqbot media path resolution honors MERCLAW_HOME (#83562)", () => {
  ...
  function makeFakeMerclawHome(): string {
  ...
  it("accepts files under $MERCLAW_HOME/.merclaw/media/qqbot when MERCLAW_HOME differs from HOME", () => {
    const fakeMerclawHome = makeFakeMerclawHome();
    vi.stubEnv("MERCLAW_HOME", fakeMerclawHome);

    const mediaFile = path.join(fakeMerclawHome, ".merclaw", "media", "qqbot", "repro.png");
    ...
    const oldHomeMediaRoot = path.join(realOsHome, ".merclaw", "media", "qqbot");
    expect(isPathInsideOrEqual(mediaFile, oldHomeMediaRoot)).toBe(false);
    fs.mkdirSync(path.dirname(mediaFile), { recursive: true });
    fs.writeFileSync(mediaFile, "image", "utf8");

    expect(getQQBotMediaPath()).toBe(path.join(fakeMerclawHome, ".merclaw", "media", "qqbot"));
    expect(resolveQQBotPayloadLocalFilePath(mediaFile)).toBe(fs.realpathSync(mediaFile));
  });

  it("expands tilde-prefixed MERCLAW_HOME against the OS home", () => {
    ...
    vi.stubEnv("MERCLAW_HOME", `~/${sub}`);

    expect(getQQBotMediaPath()).toBe(path.join(expectedHome, ".merclaw", "media", "qqbot"));

    const mediaFile = path.join(expectedHome, ".merclaw", "media", "qqbot", "tilde.png");
    fs.mkdirSync(path.dirname(mediaFile), { recursive: true });
    fs.writeFileSync(mediaFile, "image", "utf8");
  ...
  it("falls back to OS home when MERCLAW_HOME is unset (no regression)", () => {
    vi.stubEnv("MERCLAW_HOME", "");

    expect(getQQBotMediaPath()).toBe(path.join(realOsHome, ".merclaw", "media", "qqbot"));
  });

  it("treats sentinel strings 'undefined' and 'null' as unset", () => {
    for (const sentinel of ["undefined", "null"]) {
      vi.stubEnv("MERCLAW_HOME", sentinel);
      expect(getQQBotMediaPath()).toBe(path.join(realOsHome, ".merclaw", "media", "qqbot"));
    }
  });
  ...
  it("keeps persisted QQ Bot data anchored on the OS home (compatibility)", () => {
    ...
    // operator adds MERCLAW_HOME — otherwise existing deployments would lose
    // their session state. Only the media root follows MERCLAW_HOME.
    expect(getQQBotDataPath()).toBe(path.join(realOsHome, ".merclaw", "qqbot"));
  });

  it("rejects files that live under HOME tree when MERCLAW_HOME is the active root", () => {
    const fakeMerclawHome = makeFakeMerclawHome();
    vi.stubEnv("MERCLAW_HOME", fakeMerclawHome);

    // File under the HOME-side mirror — exactly the path that *worked* on
    // current main and *broke* the MERCLAW_HOME setup. After the fix the
    // active media root is MERCLAW_HOME, so a file under HOME is no longer
    ...
    const homeOnlyFile = path.join(realOsHome, ".merclaw", "media", "qqbot", stale);
  ...
  it("remaps workspace paths under either HOME or MERCLAW_HOME to the MERCLAW_HOME media root", () => {
    const fakeMerclawHome = makeFakeMerclawHome();
    vi.stubEnv("MERCLAW_HOME", fakeMerclawHome);

    const baseName = `remap-${process.pid}-${Date.now()}`;

    // Real file lives under the MERCLAW_HOME media tree.
    const mediaFile = path.join(
      fakeMerclawHome,
      ".merclaw",
      "media",
    ...
    const homeWorkspaceDir = path.join(realOsHome, ".merclaw", "workspace", "qqbot");
    ...
    // HOME-side path, so nothing to clean up there beyond the MERCLAW_HOME tree.
    expect(resolveQQBotLocalMediaPath(homeWorkspacePath)).toBe(mediaFile);

    // Same path but under MERCLAW_HOME should also remap.
    const merclawWorkspacePath = path.join(
      fakeMerclawHome,
      ".merclaw",
      "workspace",
      "qqbot",
    ...
    expect(resolveQQBotLocalMediaPath(merclawWorkspacePath)).toBe(mediaFile);
  });
});
