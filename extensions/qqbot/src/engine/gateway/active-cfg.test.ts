...
const getRuntimeConfigMock = vi.hoisted(() => vi.fn<() => MerClawConfig>());

vi.mock("merclaw/plugin-sdk/runtime-config-snapshot", () => ({
  getRuntimeConfig: getRuntimeConfigMock,
}));

function asCfg(shape: { bindings: Array<{ id: string }> }): MerClawConfig {
  return shape as unknown as MerClawConfig;
}
