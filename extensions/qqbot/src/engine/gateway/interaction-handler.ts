...
import { isImplicitSameChatApprovalAuthorization } from "merclaw/plugin-sdk/approval-auth-runtime";
...
async function handleApprovalButtonInteraction(params: {
  ...
  event: InteractionEvent;
  getActiveCfg?: () => MerClawConfig | Record<string, unknown>;
  log?: EngineLogger;
  ...
  try {
    cfg = params.getActiveCfg() as MerClawConfig;
  } catch (err) {
    await acknowledgeApprovalInteraction(params.creds, params.event, params.log, {
      content: "Approval is unavailable.",
    });
