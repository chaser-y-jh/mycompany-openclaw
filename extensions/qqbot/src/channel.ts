import { getExecApprovalReplyMetadata } from "merclaw/plugin-sdk/approval-runtime";
import {
...
import type { ChannelPlugin } from "merclaw/plugin-sdk/core";
...
import { toGatewayAccount, writeMerClawConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped merclaw.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // merclaw.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeMerClawConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe merclaw.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeMerClawConfigThroughRuntime(getQQBotRuntime(), nextCfg as MerClawConfig);
      }

      const resolved = resolveQQBotAccount((changed ? nextCfg : cfg) as MerClawConfig, accountId);
