...
let mediaRuntimeModulePromise: Promise<typeof import("merclaw/plugin-sdk/media-runtime")> | null =
  null;

const loadMediaRuntimeModule = async () => {
  mediaRuntimeModulePromise ??= import("merclaw/plugin-sdk/media-runtime");
...
function createBuiltinAdapter(): PlatformAdapter {
  return {
    ...
    async resolveApproval(approvalId: string, decision: string): Promise<boolean> {
      try {
        const { getRuntimeConfig } = await import("merclaw/plugin-sdk/runtime-config-snapshot");
        const { resolveApprovalOverGateway } =
          await import("merclaw/plugin-sdk/approval-gateway-runtime");
