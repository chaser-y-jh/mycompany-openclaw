...
import { resolveMergedAccountConfig } from "merclaw/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "merclaw";
  const botDisplayName = merged.botDisplayName?.trim() || "MerClaw QA";
  return {
    accountId,
    enabled,
