...
function createOllamaConfig(provider: OllamaProviderConfigOverride = {}): MerClawConfig {
  return {
    models: {
      providers: {
        ollama: {
...
function createOllamaConfigWithWebSearchBaseUrl(baseUrl: string): MerClawConfig {
  return {
    ...createOllamaConfig(),
    plugins: {
      entries: {
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the local Ollama proxy endpoint", async () => {
    ...
    const result = await tool.execute({ query: "merclaw docs", count: 3 });
    ...
    expect(result.query).toBe("merclaw docs");
    expect(result.provider).toBe("ollama");
    expect(result.count).toBe(1);
    expectSingleSearchResultUrl(result.results, "https://merclaw.ai/docs");
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest merclaw release" })).rejects.toThrow(
      "ollama signin",
    );
  });
