...
function usage() {
  return [
    "Usage: pnpm rtt <merclaw@spec> [--package-tgz PATH] [--provider mock-openai|live-frontier] [--credential-source env|convex] [--credential-role maintainer|ci] [--runs N] [--samples N] [--sample-timeout-ms N] [--timeout-ms N] [--harness-root PATH] [--output PATH]",
    "",
    "Examples:",
    "  pnpm rtt merclaw@main --package-tgz .artifacts/package/merclaw.tgz",
    "  pnpm rtt merclaw@beta",
    "  pnpm rtt merclaw@2026.4.30",
    "  pnpm rtt merclaw@latest --provider live-frontier",
  ].join("\n");
}
...
function parseArgs(argv: string[]) {
  ...
  return {
    spec: validateMerClawPackageSpec(spec),
    options: {
      packageTgz,
      credentialRole,
      credentialSource,
...
async function main() {
  ...
  if (spec === "merclaw@main" && !options.packageTgz) {
    throw new Error("merclaw@main requires --package-tgz.");
  }
  const version =
    spec === "merclaw@main"
