...
import { resolveNpmCommandInvocation } from "./merclaw-npm-release-check.ts";
...
function main(): void {
  ...
  if (!tarballPath) {
    throw new Error(
      "Usage: node --import tsx scripts/merclaw-npm-prepublish-verify.ts <tarball.tgz> [expected-version]",
    );
  }

  const workingDir = mkdtempSync(join(tmpdir(), "merclaw-prepublish-"));
  ...
  try {
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== resolvedExpectedVersion) {
      errors.push(
        `installed merclaw binary version mismatch: expected ${resolvedExpectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    if (errors.length === 0) {
    ...
    console.log(
      `merclaw-npm-prepublish-verify: prepared tarball install OK (${resolvedExpectedVersion}).`,
    );
  } finally {
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`merclaw-npm-prepublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
