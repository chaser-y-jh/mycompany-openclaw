...
# MerClaw Auth Widget for Termux
...
SERVER="${MERCLAW_SERVER:-l36}"

# Check auth status
termux-toast "Checking MerClaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/merclaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/merclaw/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|MERCLAW_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/merclaw/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|MERCLAW_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: MerClaw won't work until re-auth"
                ;;
        esac
        ;;
