...
const TMUX_CWD_ENV_KEY = "MERCLAW_GATEWAY_WATCH_CWD";
const TMUX_CWD_OPTION_KEY = "@merclaw.gateway_watch.cwd";
const TMUX_CHILD_ENV_KEYS = [
  "NODE_OPTIONS",
  "MERCLAW_CONFIG_PATH",
  "MERCLAW_DIAGNOSTICS",
  "MERCLAW_DIAGNOSTICS_EVENT_LOOP",
  "MERCLAW_DIAGNOSTICS_TIMELINE_PATH",
  "MERCLAW_GATEWAY_PORT",
  "MERCLAW_GATEWAY_RESTART_TRACE",
  "MERCLAW_GATEWAY_STARTUP_TRACE",
  "MERCLAW_HOME",
  "MERCLAW_PROFILE",
...
const resolveGatewayWatchBenchmarkArgs = ({ args = [], env = process.env } = {}) => {
  ...
  if (benchmarkFlagSeen) {
    ...
    nextEnv.MERCLAW_TRACE_SYNC_IO ??= "0";
    if (nextEnv.MERCLAW_TRACE_SYNC_IO === "1") {
...
export const resolveGatewayWatchTmuxSessionName = ({ args = [], env = process.env } = {}) => {
  ...
  const port = env.MERCLAW_GATEWAY_PORT || readArgValue(args, "--port");
  const parts = [
    "merclaw",
    "gateway",
    "watch",
...
export const buildGatewayWatchTmuxCommand = ({
  ...
  const childEnv = [
    "env",
    ...colorEnv.options,
    `MERCLAW_GATEWAY_WATCH_TMUX_CHILD=1`,
    `MERCLAW_GATEWAY_WATCH_SESSION=${sessionName}`,
...
const shouldAttachTmux = ({ env, stdinIsTTY, stdoutIsTTY }) => {
  const raw = String(env.MERCLAW_GATEWAY_WATCH_ATTACH ?? "").toLowerCase();
...
export const runGatewayWatchTmuxMain = (params = {}) => {
  ...
  if (TMUX_DISABLE_VALUES.has((deps.env.MERCLAW_GATEWAY_WATCH_TMUX ?? "").toLowerCase())) {
  ...
  if (deps.env.MERCLAW_GATEWAY_WATCH_TMUX_CHILD === "1") {
    return runForegroundWatcher({
      args: deps.args,
      cwd: deps.cwd,
      env: deps.env,
