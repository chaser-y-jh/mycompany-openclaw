...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const merclawMeta =
    ...
    "merclaw" in source.packageJson
      ? (source.packageJson.merclaw as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    merclawMeta &&
    typeof merclawMeta.channel === "object" &&
    merclawMeta.channel &&
    !Array.isArray(merclawMeta.channel)
      ? (merclawMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
