...
const MANUAL_SECTION_START = "<!-- merclaw-plugin-reference:manual-start -->";
const MANUAL_SECTION_END = "<!-- merclaw-plugin-reference:manual-end -->";
...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving MerClaw messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to MerClaw.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an MerClaw plugin.";
}
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.merclaw?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    return "included in MerClaw";
  }
  const install = packageJson.merclaw?.install;
  const release = packageJson.merclaw?.release;
  const clawhubSpec =
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.merclaw?.install?.clawhubSpec === "string" ||
    typeof packageJson.merclaw?.install?.npmSpec === "string";
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "merclaw.plugin.json");
