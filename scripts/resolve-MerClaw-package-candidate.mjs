...
const DEFAULT_OUTPUT_NAME = "merclaw-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "MERCLAW_TRUSTED_PACKAGE_TOKEN";
const BLOCKED_PACKAGE_HOSTNAMES = new Set([
  "localhost",
  "localhost.localdomain",
  "metadata.google.internal",
]);
export const MERCLAW_PACKAGE_SPEC_RE =
  /^merclaw@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-merclaw-package-candidate.mjs --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
export function validateMerClawPackageSpec(spec) {
  if (!MERCLAW_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be merclaw@alpha, merclaw@beta, merclaw@latest, or an exact MerClaw release version; got: ${spec}`,
    );
  }
}

export function resolveNpmPackageCandidatePackRunner(packageSpec, outputDir, params = {}) {
  validateMerClawPackageSpec(packageSpec);
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an MerClaw branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `merclaw-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => /^merclaw-.*\.tgz$/u.test(entry))
      .toSorted((a, b) => a.localeCompare(b))
      .at(-1);
  }
  if (!filename) {
    throw new Error(`npm pack produced no MerClaw tarball in ${outputDir}`);
  }
...
async function resolveCandidate(options) {
  ...
  console.error(`Checking MerClaw package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-merclaw-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
    `MerClaw package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
  ...
  if (pkg.name !== "merclaw") {
    throw new Error(`package candidate must be named "merclaw"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
