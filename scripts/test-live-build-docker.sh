...
DOCKER_BUILD_EXTENSIONS="${MERCLAW_DOCKER_BUILD_EXTENSIONS:-${MERCLAW_EXTENSIONS:-}}"
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "MERCLAW_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi

if [[ "${MERCLAW_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  echo "==> Live-test image not available; building because MERCLAW_DOCKER_BUILD_ON_MISSING/MERCLAW_TESTBOX allows fallback."
fi
