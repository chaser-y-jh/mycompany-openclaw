#!/usr/bin/env node
// Builds the MerClaw package artifact used by Docker E2E.
...
const PACKAGE_ARTIFACT_BUILD_STEPS = [
  {
    label: "Building MerClaw package artifacts",
    command: "node",
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      ...
      timeoutMs: resolveTimeoutMs(
        "MERCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
...
async function newestMerClawTarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed MerClaw tarball in ${outputDir}`);
  }
  return path.join(outputDir, packed);
}

export async function packMerClawPackageForDocker(sourceDir, outputDir, options = {}) {
  ...
  console.error("==> Packing MerClaw package");
  ...
  try {
    packOutput = await runCaptureImpl(
      ...
      sourceDir,
      {
        ...
        timeoutMs: resolveTimeoutMs(
          "MERCLAW_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
  ...
  return await newestMerClawTarball(outputDir, packOutput);
}
...
async function main() {
  ...
  console.error("==> Writing MerClaw package inventory");
  await run(
    "node",
    [
      "--import",
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "MERCLAW_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
        DEFAULT_PACKAGE_INVENTORY_TIMEOUT_MS,
      ),
    },
  );

  let tarball = await packMerClawPackageForDocker(sourceDir, outputDir);
  ...
  console.error("==> Checking MerClaw package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-merclaw-package-tarball.mjs"), tarball],
    sourceDir,
    {
      timeoutMs: resolveTimeoutMs(
        "MERCLAW_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",
