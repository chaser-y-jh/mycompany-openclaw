...
BASE_IMAGE="${BASE_IMAGE:-merclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-merclaw-sandbox-common:bookworm-slim}"
...
MERCLAW_DOCKER_BUILD_USE_BUILDX="${MERCLAW_DOCKER_BUILD_USE_BUILDX:-0}"
MERCLAW_DOCKER_BUILD_CACHE_FROM="${MERCLAW_DOCKER_BUILD_CACHE_FROM:-}"
MERCLAW_DOCKER_BUILD_CACHE_TO="${MERCLAW_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=merclaw.sandbox=1)
NOTE
