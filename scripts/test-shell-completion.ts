 ...
 * `merclaw update`. Use it to verify the completion installation flow
...
\`merclaw update\`, \`merclaw doctor\`, and \`merclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# MerClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`merclaw completion --install\` later to enable.`));
    return;
  }
