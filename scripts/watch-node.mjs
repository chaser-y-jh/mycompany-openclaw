...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.MERCLAW_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[merclaw] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[merclaw] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[merclaw] This usually means a file in node_modules is empty or truncated.");
  console.error("[merclaw] Recommended recovery:");
  console.error("[merclaw]   rm -rf node_modules");
  console.error("[merclaw]   pnpm store prune");
  console.error("[merclaw]   pnpm install");
  console.error("");
  console.error("[merclaw] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.MERCLAW_WATCH_MODE = "1";
  childEnv.MERCLAW_WATCH_SESSION = watchSession;
  ...
  childEnv.MERCLAW_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.MERCLAW_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `merclaw doctor --fix --non-interactive` once.",
        deps,
      );
