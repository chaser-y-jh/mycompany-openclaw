...
import { discoverMerClawPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']merclaw\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']merclaw\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']merclaw\/plugin-sdk\/channel-runtime["']/,
    label: "merclaw/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']merclaw\/plugin-sdk\/config-runtime["']/,
    label: "merclaw/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']merclaw\/plugin-sdk\/infra-runtime["']/,
    label: "merclaw/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic merclaw/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy merclaw/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused merclaw/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
