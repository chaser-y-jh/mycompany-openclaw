# MerClaw Installer for Windows
# Usage: powershell -c "irm https://merclaw.ai/install.ps1 | iex"
#        powershell -c "& ([scriptblock]::Create((irm https://merclaw.ai/install.ps1))) -Tag beta -NoOnboard -DryRun"
...
function Complete-Install {
    ...
    throw "MerClaw installation failed with exit code $($script:InstallExitCode)."
}

Write-Host ""
Write-Host "  MerClaw Installer" -ForegroundColor Cyan
Write-Host ""
...
if (-not $PSBoundParameters.ContainsKey("InstallMethod")) {
    if (-not [string]::IsNullOrWhiteSpace($env:MERCLAW_INSTALL_METHOD)) {
        $InstallMethod = $env:MERCLAW_INSTALL_METHOD
    }
}
if (-not $PSBoundParameters.ContainsKey("GitDir")) {
    if (-not [string]::IsNullOrWhiteSpace($env:MERCLAW_GIT_DIR)) {
        $GitDir = $env:MERCLAW_GIT_DIR
    }
}
if (-not $PSBoundParameters.ContainsKey("NoOnboard")) {
    if ($env:MERCLAW_NO_ONBOARD -eq "1") {
        $NoOnboard = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("NoGitUpdate")) {
    if ($env:MERCLAW_GIT_UPDATE -eq "0") {
        $NoGitUpdate = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("DryRun")) {
    if ($env:MERCLAW_DRY_RUN -eq "1") {
        $DryRun = $true
    }
}
...
if ([string]::IsNullOrWhiteSpace($GitDir)) {
    ...
    $GitDir = (Join-Path $userHome "merclaw")
}
...
function Get-MerClawDepsRoot {
    ...
    return (Join-Path $localAppData "MerClaw\deps")
}

function Get-PortableNodeRoot {
    return (Join-Path (Get-MerClawDepsRoot) "portable-node")
}
...
function Ensure-PortableNodeOnUserPath {
    ...
    if (Add-ToUserPath $nodeDir) {
        Write-Host "[!] Added $nodeDir to user PATH (restart terminal if node or merclaw is not found)" -ForegroundColor Yellow
    }
}
...
function Check-ExistingMerClaw {
    if (Get-MerClawCommandPath) {
        Write-Host "[*] Existing MerClaw installation detected" -ForegroundColor Yellow
        return $true
    }
    return $false
}
...
function Get-PortableGitRoot {
    return (Join-Path (Get-MerClawDepsRoot) "portable-git")
}
...
function Install-PortableGit {
    ...
    $portableParent = Split-Path -Parent $portableRoot
    $tmpZip = Join-Path $env:TEMP $download.Name
    $tmpExtract = Join-Path $env:TEMP ("merclaw-portable-git-" + [guid]::NewGuid().ToString("N"))
...
function Ensure-Git {
    ...
    Write-Host ""
    Write-Host "Error: Git is required to install MerClaw." -ForegroundColor Red
...
function Get-MerClawCommandPath {
    $merclawCmd = Get-Command merclaw.cmd -ErrorAction SilentlyContinue
    if ($merclawCmd -and $merclawCmd.Source) {
        return $merclawCmd.Source
    }

    $merclaw = Get-Command merclaw -ErrorAction SilentlyContinue
    if ($merclaw -and $merclaw.Source) {
        return $merclaw.Source
    }

    return $null
}

function Invoke-MerClawCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )

    $commandPath = Get-MerClawCommandPath
    if (-not $commandPath) {
        throw "merclaw command not found on PATH."
    }

    & $commandPath @Arguments
}

function Invoke-InteractiveMerClawCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )
...
function Ensure-MerClawOnPath {
    ...
    foreach ($npmBin in $npmBins) {
        if (-not (Test-Path (Join-Path $npmBin "merclaw.cmd"))) {
            continue
        }
    ...
    Write-Host "[!] merclaw is not on PATH yet." -ForegroundColor Yellow
...
function Resolve-NpmMerClawInstallSpec {
    param(
        [string]$PackageName,
        [string]$RequestedTag
    )
...
function Test-MerClawSourcePackageInstallSpec {
    ...
    if ($normalizedTag.StartsWith("merclaw@")) {
        $normalizedTag = $normalizedTag.Substring("merclaw@".Length)
    }

    if ($normalizedTag -eq "main") {
        return $true
    }
    if ($normalizedTag -match '^github:merclaw/merclaw($|[#/])') {
        return $true
    }
    ...
    return (
        $normalizedTag -match '^https?://github\.com/merclaw/merclaw(\.git)?($|[?#])' -or
        $normalizedTag -match '^ssh://git@github\.com[:/]merclaw/merclaw(\.git)?($|[?#])' -or
        $normalizedTag -match '^git://github\.com/merclaw/merclaw(\.git)?($|[?#])' -or
        $normalizedTag -match '^git@github\.com:merclaw/merclaw(\.git)?($|[?#])'
    )
}
...
function Install-MerClaw {
    if ([string]::IsNullOrWhiteSpace($Tag)) {
        $Tag = "latest"
    }
    if (Test-MerClawSourcePackageInstallSpec -RequestedTag $Tag) {
        Write-Host "Error: npm installs do not support MerClaw GitHub source targets like '$Tag'." -ForegroundColor Red
    ...
    # Use merclaw package for beta, merclaw for stable
    $packageName = "merclaw"
    if ($Tag -eq "beta" -or $Tag -match "^beta\.") {
        $packageName = "merclaw"
    }
    $installSpec = Resolve-NpmMerClawInstallSpec -PackageName $packageName -RequestedTag $Tag
    Write-Host "[*] Installing MerClaw ($installSpec)..." -ForegroundColor Yellow
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if ($npmOutput -match "spawn git" -or $npmOutput -match "ENOENT.*git") {
                ...
                Write-Host '  powershell -c "irm https://merclaw.ai/install.ps1 | iex"' -ForegroundColor Cyan
            }
    ...
    Write-Host "[OK] MerClaw installed" -ForegroundColor Green
    return $true
}

# Install MerClaw from GitHub
function Install-MerClawFromGit {
    param(
        [string]$RepoDir,
        [switch]$SkipUpdate
    )
    ...
    $repoUrl = "https://github.com/merclaw/merclaw.git"
    Write-Host "[*] Installing MerClaw from GitHub ($repoUrl)..." -ForegroundColor Yellow
    ...
    if (-not (Test-Path $entryPath)) {
        Write-Host "[!] MerClaw build did not produce $entryPath" -ForegroundColor Red
        return $false
    }
    ...
    $cmdPath = Join-Path $binDir "merclaw.cmd"
    ...
    Write-Host "[OK] MerClaw wrapper installed to $cmdPath" -ForegroundColor Green
...
function Run-Doctor {
    ...
    try {
        Invoke-MerClawCommand doctor --non-interactive
    } catch {
...
function Test-GatewayServiceLoaded {
    try {
        $statusJson = (Invoke-MerClawCommand daemon status --json 2>$null)
...
function Refresh-GatewayServiceIfLoaded {
    ...
    try {
        Invoke-MerClawCommand gateway install --force | Out-Null
    } catch {
    ...
    try {
        Invoke-MerClawCommand gateway restart | Out-Null
        Invoke-MerClawCommand gateway status --json | Out-Null
...
function Get-LegacyRepoDir {
    if (-not [string]::IsNullOrWhiteSpace($env:MERCLAW_GIT_DIR)) {
        return $env:MERCLAW_GIT_DIR
    }
    $userHome = [Environment]::GetFolderPath("UserProfile")
    return (Join-Path $userHome "merclaw")
}
...
function Main {
    ...
    $isUpgrade = Check-ExistingMerClaw
    ...
    if ($InstallMethod -eq "git") {
        try {
            ...
            if ($npmCommand) {
                Invoke-NpmCommand -Arguments @("uninstall", "-g", "merclaw") 2>$null | Out-Null
                Write-Host "[OK] Removed npm global install if present" -ForegroundColor Green
            }
        } catch { }
        $finalGitDir = $GitDir
        $gitInstallResults = @(Install-MerClawFromGit -RepoDir $GitDir -SkipUpdate:$NoGitUpdate)
        if (-not (Test-BooleanSuccessResult -Results $gitInstallResults)) {
            return (Fail-Install)
        }
    } else {
        $gitWrapper = Join-Path (Join-Path $env:USERPROFILE ".local\\bin") "merclaw.cmd"
        ...
        $npmInstallResults = @(Install-MerClaw)
        if (-not (Test-BooleanSuccessResult -Results $npmInstallResults)) {
            return (Fail-Install)
        }
    }

    if (-not (Ensure-MerClawOnPath)) {
        Write-Host "Install completed, but MerClaw is not on PATH yet." -ForegroundColor Yellow
        Write-Host "Open a new terminal, then run: merclaw doctor" -ForegroundColor Cyan
        return
    }
    ...
    try {
        $installedVersion = (Invoke-MerClawCommand --version 2>$null).Trim()
    } catch {
        $installedVersion = $null
    }
    if (-not $installedVersion) {
        try {
            ...
            if ($npmList -and $npmList.dependencies -and $npmList.dependencies.merclaw -and $npmList.dependencies.merclaw.version) {
                $installedVersion = $npmList.dependencies.merclaw.version
            }
        } catch {
            $installedVersion = $null
        }
    ...
    Write-Host ""
    if ($installedVersion) {
        Write-Host "MerClaw installed successfully ($installedVersion)!" -ForegroundColor Green
    } else {
        Write-Host "MerClaw installed successfully!" -ForegroundColor Green
    }
    Write-Host ""
    if ($isUpgrade) {
        $updateMessages = @(
    ...
    if ($InstallMethod -eq "git") {
        ...
        Write-Host "Wrapper: $env:USERPROFILE\\.local\\bin\\merclaw.cmd" -ForegroundColor Cyan
        Write-Host ""
    }

    if ($isUpgrade) {
        Write-Host "Upgrade complete. Run " -NoNewline
        Write-Host "merclaw doctor" -ForegroundColor Cyan -NoNewline
        Write-Host " to check for additional migrations."
    } else {
        if ($NoOnboard) {
            Write-Host "Skipping onboard (requested). Run " -NoNewline
            Write-Host "merclaw onboard" -ForegroundColor Cyan -NoNewline
            Write-Host " later."
        } else {
            Write-Host "Starting setup..." -ForegroundColor Cyan
            Write-Host ""
            Invoke-InteractiveMerClawCommand onboard
        }
    }
