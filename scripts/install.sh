...
# MerClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://merclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one MerClaw."
...
resolve_merclaw_effective_home() {
    ...
    if [[ "$merclaw_home" == "~" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$merclaw_home" == \~/* ]]; then
        echo "${HOME}${merclaw_home:1}"
        return
    fi
    echo "$merclaw_home"
}
...
GUM_VERSION="${MERCLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 MerClaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://merclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$MERCLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.merclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_merclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.merclaw-* "$npm_root"/merclaw 2>/dev/null || true
}

extract_merclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_merclaw_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/merclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/merclaw"|"/usr/local/bin/merclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/merclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale merclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing merclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing MerClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi

    ui_info "Installing MerClaw package"
    "${cmd[@]}" >"$log" 2>&1
}
...
install_merclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "MerClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*merclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_merclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_merclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "MerClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an merclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${MERCLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${MERCLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((MERCLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${MERCLAW_NO_ONBOARD:-0}
NO_PROMPT=${MERCLAW_NO_PROMPT:-0}
DRY_RUN=${MERCLAW_DRY_RUN:-0}
INSTALL_METHOD=${MERCLAW_INSTALL_METHOD:-}
MERCLAW_VERSION=${MERCLAW_VERSION:-latest}
USE_BETA=${MERCLAW_BETA:-0}
GIT_DIR_DEFAULT="$(resolve_merclaw_effective_home)/merclaw"
GIT_DIR=${MERCLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${MERCLAW_GIT_UPDATE:-1}
NPM_LOGLEVEL="${MERCLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${MERCLAW_VERBOSE:-0}"
VERIFY_INSTALL="${MERCLAW_VERIFY_INSTALL:-0}"
MERCLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
MerClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://merclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/merclaw)
...
Environment variables:
  ...
  MERCLAW_VERSION=latest|next|<semver>|<spec>
  MERCLAW_BETA=0|1
  MERCLAW_GIT_DIR=...
  MERCLAW_GIT_UPDATE=0|1
  MERCLAW_NO_PROMPT=1
  MERCLAW_VERIFY_INSTALL=1
  MERCLAW_DRY_RUN=1
  MERCLAW_NO_ONBOARD=1
  MERCLAW_VERBOSE=1
  MERCLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://merclaw.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://merclaw.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://merclaw.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://merclaw.ai/install.sh | bash -s -- --install-method git --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://merclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected MerClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a MerClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_merclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"merclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://merclaw.ai/install.sh | bash"
}
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future MerClaw updates; use npm i -g merclaw@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_merclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/merclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/merclaw" ]]; then
        ln -sf "$npm_root/merclaw/dist/entry.js" "${npm_bin}/merclaw"
        ui_info "Created merclaw bin link at ${npm_bin}/merclaw"
    fi
    return 0
}

# Check for existing MerClaw installation
check_existing_merclaw() {
    if [[ -n "$(type -P merclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing MerClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_git_merclaw_ref() {
    ...
    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "merclaw" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_merclaw_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${MERCLAW_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_merclaw_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/merclaw"
        ...
        local version=""
        version="$(merclaw_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_merclaw_npm_root_candidates)
}
...
warn_duplicate_merclaw_global_installs() {
    ...
    done < <(find_merclaw_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple MerClaw global installs detected"
    echo "  Different Node/npm environments can run different MerClaw versions."

    local active_node active_npm active_merclaw
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_merclaw="${MERCLAW_BIN:-}"
    if [[ -z "$active_merclaw" ]]; then
        active_merclaw="$(type -P merclaw 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active merclaw: ${INFO}${active_merclaw:-none}${NC}"
    echo ""
    echo "  Found installs:"
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make merclaw show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

merclaw_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "merclaw"
        return 0
...
bounded_probe_output() {
    local label="$1"
    shift
    local timeout_seconds="${MERCLAW_INSTALL_PROBE_TIMEOUT_SECONDS:-5}"
...
warn_merclaw_not_found() {
    ui_warn "Installed, but merclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t merclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named merclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_merclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P merclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/merclaw" ]]; then
        echo "${npm_bin}/merclaw"
        return 0
    fi
...
install_merclaw_from_git() {
    ...
    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing MerClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing MerClaw from GitHub (${repo_url})"
    fi
    ...
    run_quiet_step "Building MerClaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/merclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/merclaw"
    ui_success "MerClaw wrapper installed to \$HOME/.local/bin/merclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view merclaw dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:merclaw/merclaw#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_merclaw() {
    local package_name="merclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            MERCLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="merclaw"
        else
            MERCLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${MERCLAW_VERSION}" ]]; then
        MERCLAW_VERSION="latest"
    fi

    if is_merclaw_source_package_install_spec "${MERCLAW_VERSION}"; then
        ui_error "npm installs do not support MerClaw GitHub source targets like '${MERCLAW_VERSION}'."
    ...
    local resolved_version=""
    if can_resolve_registry_package_version "${MERCLAW_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${MERCLAW_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing MerClaw v${resolved_version}"
    else
        ui_info "Installing MerClaw (${MERCLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${MERCLAW_VERSION}")"

    if ! install_merclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_merclaw_paths
        install_merclaw_npm "${install_spec}"
    fi

    if [[ "${MERCLAW_VERSION}" == "latest" && "${package_name}" == "merclaw" ]]; then
        if ! resolve_merclaw_bin &> /dev/null; then
            ui_warn "npm install merclaw@latest failed; retrying merclaw@next"
            cleanup_npm_merclaw_paths
            install_merclaw_npm "merclaw@next"
        fi
    fi

    ensure_merclaw_bin_link || true

    ui_success "MerClaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${MERCLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_merclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (merclaw not on PATH yet)"
        warn_merclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${MERCLAW_PROFILE:-default}"
    local effective_home
    effective_home="$(resolve_merclaw_effective_home)"
    if [[ "${profile}" != "default" ]]; then
        echo "${effective_home}/.merclaw/workspace-${profile}"
    else
        echo "${effective_home}/.merclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local effective_home
    effective_home="$(resolve_merclaw_effective_home)"
    local config_path="${MERCLAW_CONFIG_PATH:-$effective_home/.merclaw/merclaw.json}"
    local legacy_config_path="${HOME}/.merclaw/merclaw.json"
    local legacy_clawdbot_path="${HOME}/.clawdbot/clawdbot.json"
    if [[ -f "${config_path}" || -f "$effective_home/.clawdbot/clawdbot.json" ]]; then
        return
    fi
    if [[ -z "${MERCLAW_CONFIG_PATH:-}" && "${effective_home}" != "${HOME}" ]]; then
    ...
    if ! is_promptable; then
        local user_claw
        user_claw="$(merclaw_command_for_user "${MERCLAW_BIN:-}")"
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but merclaw not on PATH; skipping onboarding"
        warn_merclaw_not_found
        return
    fi
    ...
    "$claw" onboard || {
        local user_claw
        user_claw="$(merclaw_command_for_user "$claw")"
...
resolve_merclaw_version() {
    ...
    local claw="${MERCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v merclaw &> /dev/null; then
        claw="$(command -v merclaw)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_merclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/merclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/merclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$(bounded_probe_output "merclaw daemon status --json" "$claw" daemon status --json || true)"
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: merclaw not on PATH yet"
        warn_merclaw_not_found
        return 1
    fi

    run_quiet_step "Checking MerClaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: merclaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_merclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found MerClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set MERCLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_merclaw; then
        is_upgrade=true
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        if npm list -g merclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g merclaw 2>/dev/null || true
        ...
        install_merclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/merclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/merclaw"
    ...
    MERCLAW_BIN="$(resolve_merclaw_bin || true)"
    warn_duplicate_merclaw_global_installs || true
    ...
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 MerClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 MerClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/merclaw"
        ui_kv "Update command" "merclaw update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://merclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if has_controlling_tty || [[ "$NO_ONBOARD" == "1" || "$NO_PROMPT" == "1" ]]; then
            ...
            ui_info "Running merclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                MERCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                MERCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                MERCLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
        ...
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if is_promptable; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (merclaw not on PATH yet)"
                    warn_merclaw_not_found
                    return 0
                fi
                exec </dev/tty
    ...
    if command -v merclaw &> /dev/null; then
        local claw="${MERCLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_merclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (merclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if MERCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: merclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${MERCLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_install_stage_total
    configure_verbose
    main
