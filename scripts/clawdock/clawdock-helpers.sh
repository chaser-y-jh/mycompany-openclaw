#!/usr/bin/env bash
# ClawDock - Docker helpers for MerClaw
# Inspired by Simon Willison's "Running MerClaw in Docker"
# https://til.simonwillison.net/llms/merclaw-docker
#
# Installation:
...
# Common paths to check for MerClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/merclaw"
  "${HOME}/workspace/merclaw"
  "${HOME}/projects/merclaw"
  "${HOME}/dev/merclaw"
  "${HOME}/code/merclaw"
  "${HOME}/src/merclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found MerClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/merclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ MerClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/merclaw/merclaw.git ~/merclaw"
    echo "  cd ~/merclaw && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^MERCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d merclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart merclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f merclaw-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.merclaw"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show merclaw.json
  if [[ -f "${config_dir}/merclaw.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/merclaw.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/merclaw.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No merclaw.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec merclaw-gateway \
    bash -c 'echo "alias merclaw=\"./merclaw.mjs\"" > /tmp/.bashrc_merclaw && bash --rcfile /tmp/.bashrc_merclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm merclaw-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build merclaw-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d merclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build merclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "MERCLAW_GATEWAY_TOKEN=$token" merclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" merclaw-gateway \
    bash -c './merclaw.mjs config set gateway.remote.token "$TOKEN" && ./merclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec merclaw-gateway \
    bash -c "./merclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart merclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm merclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec merclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'merclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for MerClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (merclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to merclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.merclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'merclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'merclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.merclaw.ai${_CLR_RESET}"
  echo ""
}
