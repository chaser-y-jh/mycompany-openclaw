...
const MERCLAW_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncMerClawDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.merclaw;
  if (!current || current === "workspace:*" || !MERCLAW_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.merclaw?.compat;
  const current = compat?.pluginApi;
  if (!current || !MERCLAW_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildMerClawVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.merclaw?.build;
  const current = build?.merclawVersion;
  if (!current) {
    return false;
  }
  ...
  build.merclawVersion = targetVersion;
  return true;
}
...
export function syncPluginVersions(
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncMerClawDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncMerClawDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildMerClawVersionChanged = syncBuildMerClawVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildMerClawVersionChanged;
    if (!packageChanged) {
