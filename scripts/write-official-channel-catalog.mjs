...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.merclaw) ? packageJson.merclaw : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.merclaw?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.merclaw?.channel?.id) || left.name;
    const rightId = trimString(right.merclaw?.channel?.id) || right.name;
