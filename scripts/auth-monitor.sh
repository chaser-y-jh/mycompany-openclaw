...
#
# Suggested cron: */30 * * * * /home/admin/merclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send MerClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., merclaw-alerts)
...
STATE_FILE="$HOME/.merclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via MerClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use merclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via MerClaw to $NOTIFY_PHONE..."
            merclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! MerClaw is down. Run: ssh l36 '~/merclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
