...
function usage() {
  return `Usage:
  node scripts/lib/merclaw-test-state.mjs -- create [--label <name>] [--scenario <name>] [--env-file <path>] [--json]
  node scripts/lib/merclaw-test-state.mjs shell [--label <name>] [--scenario <name>]
  node scripts/lib/merclaw-test-state.mjs shell-function
...
function scenarioConfig(scenario, options = {}) {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      skills: {
        allowBundled: ["memory", "merclaw-testing"],
        limits: {
          maxSkillsInPrompt: 8,
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.token || "merclaw-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function renderAuthProfileSecretKeyExport() {
  return [
    'MERCLAW_AUTH_PROFILE_SECRET_KEY_FILE="$MERCLAW_TEST_STATE_HOME/.merclaw-test-auth-profile-secret-key"',
    'if [ -s "$MERCLAW_AUTH_PROFILE_SECRET_KEY_FILE" ]; then',
    '  MERCLAW_AUTH_PROFILE_SECRET_KEY="$(cat "$MERCLAW_AUTH_PROFILE_SECRET_KEY_FILE")"',
    "else",
    '  MERCLAW_AUTH_PROFILE_SECRET_KEY="$(od -An -N 32 -tx1 /dev/urandom | tr -d " \\n")"',
    '  ( umask 077; printf "%s\\n" "$MERCLAW_AUTH_PROFILE_SECRET_KEY" > "$MERCLAW_AUTH_PROFILE_SECRET_KEY_FILE" )',
    "fi",
    'if [ -z "$MERCLAW_AUTH_PROFILE_SECRET_KEY" ]; then',
    '  echo "failed to generate MERCLAW_AUTH_PROFILE_SECRET_KEY" >&2',
    "  return 1 2>/dev/null || exit 1",
    "fi",
    "export MERCLAW_AUTH_PROFILE_SECRET_KEY",
  ];
}
...
function renderConfigWrite(configPathExpression, config) {
  ...
  return [
    `cat > ${configPathExpression} <<'MERCLAW_TEST_STATE_JSON'`,
    json,
    "MERCLAW_TEST_STATE_JSON",
  ].join("\n");
}
...
function buildCreatePlan(options = {}) {
  ...
  const env = {
    ...
    MERCLAW_AUTH_PROFILE_SECRET_KEY: generateAuthProfileSecretKey(),
...
export async function createState(options = {}) {
  const label = normalizeLabel(options.label);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `merclaw-${label}-`));
...
export function renderShellSnippet(options = {}) {
  ...
  const homeTemplate = `merclaw-${label}-${scenario}-home.XXXXXX`;
  const lines = [
    'MERCLAW_TEST_STATE_TMP_ROOT="${MERCLAW_TEST_STATE_TMPDIR:-${TMPDIR:-/tmp}}"',
    'MERCLAW_TEST_STATE_TMP_ROOT="${MERCLAW_TEST_STATE_TMP_ROOT%/}"',
    '[ -n "$MERCLAW_TEST_STATE_TMP_ROOT" ] || MERCLAW_TEST_STATE_TMP_ROOT="/tmp"',
    "export MERCLAW_TEST_STATE_TMP_ROOT",
    'mkdir -p "$MERCLAW_TEST_STATE_TMP_ROOT"',
    `MERCLAW_TEST_STATE_HOME="$(mktemp -d "$MERCLAW_TEST_STATE_TMP_ROOT/${homeTemplate}")"`,
    'export HOME="$MERCLAW_TEST_STATE_HOME"',
    'export USERPROFILE="$MERCLAW_TEST_STATE_HOME"',
    'export MERCLAW_HOME="$MERCLAW_TEST_STATE_HOME"',
    'export MERCLAW_STATE_DIR="$MERCLAW_TEST_STATE_HOME/.merclaw"',
    'export MERCLAW_CONFIG_PATH="$MERCLAW_STATE_DIR/merclaw.json"',
    ...renderAuthProfileSecretKeyExport(),
    'export MERCLAW_TEST_WORKSPACE_DIR="$MERCLAW_TEST_STATE_HOME/workspace"',
    'mkdir -p "$MERCLAW_STATE_DIR" "$MERCLAW_TEST_WORKSPACE_DIR"',
  ];
  for (const [key, value] of Object.entries(env)) {
    lines.push(`export ${key}=${shellQuote(value)}`);
  }
  const configWrite = renderConfigWrite('"$MERCLAW_CONFIG_PATH"', config);
  if (configWrite) {
...
export function renderShellFunction() {
  return `merclaw_test_state_create() {
  ...
  case "$scenario" in
    ...
    *)
      echo "unknown MerClaw test-state scenario: $scenario" >&2
      return 1
      ;;
  esac
  case "$raw_label" in
    /*)
      MERCLAW_TEST_STATE_HOME="$raw_label"
      mkdir -p "$MERCLAW_TEST_STATE_HOME"
      ;;
    *)
      label="$(printf "%s" "$label" | tr -cs "A-Za-z0-9_.-" "-" | sed -e "s/^-*//" -e "s/-*$//")"
      [ -n "$label" ] || label="state"
      local tmp_root="\${MERCLAW_TEST_STATE_TMPDIR:-\${TMPDIR:-/tmp}}"
      tmp_root="\${tmp_root%/}"
      [ -n "$tmp_root" ] || tmp_root="/tmp"
      mkdir -p "$tmp_root"
      MERCLAW_TEST_STATE_HOME="$(mktemp -d "$tmp_root/merclaw-$label-$scenario-home.XXXXXX")"
      ;;
  esac
  export HOME="$MERCLAW_TEST_STATE_HOME"
  export USERPROFILE="$MERCLAW_TEST_STATE_HOME"
  ...
  export MERCLAW_TEST_WORKSPACE_DIR="$MERCLAW_TEST_STATE_HOME/workspace"
  unset MERCLAW_AGENT_DIR
  unset MERCLAW_SERVICE_REPAIR_POLICY
  mkdir -p "$MERCLAW_STATE_DIR" "$MERCLAW_TEST_WORKSPACE_DIR"
  case "$scenario" in
    minimal)
      cat > "$MERCLAW_CONFIG_PATH" <<'MERCLAW_TEST_STATE_JSON'
{}
MERCLAW_TEST_STATE_JSON
      ;;
    update-stable)
...
MERCLAW_TEST_STATE_JSON
      ;;
    external-service)
      export MERCLAW_SERVICE_REPAIR_POLICY="external"
      cat > "$MERCLAW_CONFIG_PATH" <<'MERCLAW_TEST_STATE_JSON'
{}
MERCLAW_TEST_STATE_JSON
      ;;
