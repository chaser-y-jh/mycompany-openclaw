...
function isSystemThrottleDisabled(env) {
  const normalized = env.MERCLAW_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestEnv(env = process.env) {
  const normalizedLocalCheck = env.MERCLAW_LOCAL_CHECK?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.MERCLAW_VITEST_MAX_WORKERS ?? env.MERCLAW_TEST_WORKERS);
  if (override !== null) {
