...
# Builds or resolves one MerClaw npm tarball and exposes mount/build-context
...
if ! declare -F docker_e2e_docker_run_cmd >/dev/null 2>&1; then
  docker_e2e_docker_run_cmd() {
    if declare -F docker_e2e_timeout_cmd >/dev/null 2>&1; then
      docker_e2e_timeout_cmd "${DOCKER_COMMAND_TIMEOUT:-${MERCLAW_DOCKER_E2E_RUN_TIMEOUT:-3600s}}" docker "$@"
      return
    fi
    local timeout_value="${DOCKER_COMMAND_TIMEOUT:-${MERCLAW_DOCKER_E2E_RUN_TIMEOUT:-3600s}}"
    local timeout_bin=""
...
docker_e2e_prepare_package_tgz() {
  local label="$1"
  local package_tgz="${2:-${MERCLAW_CURRENT_PACKAGE_TGZ:-}}"

  if [ -n "$package_tgz" ]; then
    if [ ! -f "$package_tgz" ]; then
      echo "MerClaw package tarball does not exist: $package_tgz" >&2
      return 1
    fi
  ...
  local pack_dir
  pack_dir="$(mktemp -d "${TMPDIR:-/tmp}/merclaw-docker-e2e-pack.XXXXXX")"
  local pack_status=0
  package_tgz="$(
    node "$ROOT_DIR/scripts/package-merclaw-for-docker.mjs" \
      --output-dir "$pack_dir" \
      --output-name merclaw-current.tgz
  )" || pack_status="$?"
  ...
  touch "$pack_dir/.merclaw-docker-e2e-generated-package"
  docker_e2e_abs_path "$package_tgz"
}
...
docker_e2e_prepare_package_context() {
  local package_tgz="$1"
  local context_dir
  context_dir="$(mktemp -d "${TMPDIR:-/tmp}/merclaw-docker-e2e-package-context.XXXXXX")"
  # BuildKit named contexts must be directories, so expose the tarball as a
  # stable filename inside a tiny temporary context.
  local copy_status=0
  cp "$package_tgz" "$context_dir/merclaw-current.tgz" || copy_status="$?"
...
docker_e2e_package_mount_args() {
  local package_tgz="$1"
  local target="${2:-/tmp/merclaw-current.tgz}"
  DOCKER_E2E_PACKAGE_ARGS=(-v "$package_tgz:$target:ro" -e "MERCLAW_CURRENT_PACKAGE_TGZ=$target")
  if [ -n "${MERCLAW_E2E_NPM_INSTALL_TIMEOUT:-}" ]; then
    DOCKER_E2E_PACKAGE_ARGS+=(-e "MERCLAW_E2E_NPM_INSTALL_TIMEOUT=$MERCLAW_E2E_NPM_INSTALL_TIMEOUT")
  fi
  if [ -n "${MERCLAW_E2E_COMMAND_TIMEOUT:-}" ]; then
    DOCKER_E2E_PACKAGE_ARGS+=(-e "MERCLAW_E2E_COMMAND_TIMEOUT=$MERCLAW_E2E_COMMAND_TIMEOUT")
  fi
}
...
docker_e2e_cleanup_package_tgz() {
  ...
  [ "$(basename "$package_tgz")" = "merclaw-current.tgz" ] || return 0

  local pack_dir
  pack_dir="$(dirname "$package_tgz")"
  if [ -f "$pack_dir/.merclaw-docker-e2e-generated-package" ]; then
    rm -rf "$pack_dir"
  fi
}
...
docker_e2e_run_logged_print_with_harness() {
  ...
  run_logged_print_heartbeat \
    "$label" \
    "${MERCLAW_DOCKER_E2E_LOG_HEARTBEAT_SECONDS:-30}" \
    docker_e2e_run_with_harness \
    "$@"
}
