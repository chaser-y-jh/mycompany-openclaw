...
merclaw_live_stage_source_tree() {
  local dest_dir="${1:?destination directory required}"
  local stage_mode="${MERCLAW_LIVE_DOCKER_SOURCE_STAGE_MODE:-copy}"

  if [ "$stage_mode" = "symlink" ]; then
    echo "MERCLAW_LIVE_DOCKER_SOURCE_STAGE_MODE=symlink is disabled; using copy staging." >&2
  fi
...
merclaw_live_link_runtime_tree() {
  local dest_dir="${1:?destination directory required}"
...
merclaw_live_stage_node_modules() {
  local dest_dir="${1:?destination directory required}"
...
merclaw_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.merclaw"
  ...
  export MERCLAW_STATE_DIR="$dest_dir"
  export MERCLAW_CONFIG_PATH="$dest_dir/merclaw.json"
}

merclaw_live_prepare_staged_config() {
  if [ ! -f "${MERCLAW_CONFIG_PATH:-}" ]; then
    return 0
  fi
