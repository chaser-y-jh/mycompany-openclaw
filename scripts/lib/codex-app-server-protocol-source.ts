...
export async function resolveCodexAppServerProtocolSource(repoRoot: string): Promise<{
  ...
  throw new Error(
    [
      ...
      "Set MERCLAW_CODEX_REPO to a checkout of openai/codex, or keep a sibling `codex` checkout next to the primary MerClaw checkout.",
...
async function collectCodexRepoCandidates(repoRoot: string): Promise<string[]> {
  const candidates = [
    process.env.MERCLAW_CODEX_REPO,
