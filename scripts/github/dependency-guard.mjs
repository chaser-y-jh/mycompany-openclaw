...
export const dependencyChangeMarker = "<!-- merclaw:dependency-guard -->";
export const dependencyGraphGuardMarker = "<!-- merclaw:dependency-graph-guard -->";
...
const securityTeamSlug = process.env.MERCLAW_SECURITY_TEAM_SLUG ?? "merclaw-secops";
...
export function renderAuthorizedDependencyComment(override) {
  const lines = [
    ...
    "",
    "This PR includes dependency graph changes. A member of `@merclaw/merclaw-secops` authorized this exact head SHA with `/allow-dependencies-change`.",
    "",
...
MerClaw does not accept package lockfile changes through PRs. This PR did not change dependency graph fields in package manifests, so the workflow restored the lockfile residue from the target branch automatically.
...
export function renderBlockedDependencyComment({
  ...
  return [
    ...
    "",
    "MerClaw does not accept dependency graph changes through PRs unless security explicitly authorizes the current head SHA. Dependency updates are generated internally by maintainers so external PRs cannot change the resolved graph.",
    "",
    ...
    ...removalSteps,
    "",
    "If this PR intentionally needs a dependency graph change, ask a member of `@merclaw/merclaw-secops` to comment:",
    "",
    "```text",
...
export function githubApi(token) {
  const baseHeaders = {
    ...
    "user-agent": "merclaw-dependency-guard",
...
async function main() {
  ...
  const autoscrubToken = process.env.MERCLAW_DEPENDENCY_GUARD_AUTOSCRUB_TOKEN;
  const autoscrubApi = autoscrubToken ? githubApi(autoscrubToken) : null;
  const explicitSecurityApprovers = securityApproverSet(process.env.MERCLAW_SECURITY_APPROVERS);
  const trustedCommentAuthors = dependencyGuardCommentAuthors(
    process.env.MERCLAW_DEPENDENCY_GUARD_COMMENT_BOTS,
  );
  ...
  const mode = process.env.MERCLAW_DEPENDENCY_GUARD_MODE ?? "enforce";
