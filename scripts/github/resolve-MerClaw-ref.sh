...
REMOTE_URL="${MERCLAW_REF_REMOTE:-https://github.com/merclaw/merclaw.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-merclaw-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves MerClaw branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve MerClaw ref: ${REF}" >&2
exit 1
