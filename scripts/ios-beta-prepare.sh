...
Optional custom relay:
  MERCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com \
    scripts/ios-beta-prepare.sh --build-number 7 [--team-id TEAMID]
...
- regenerates apps/ios/MerClaw.xcodeproj via xcodegen
EOF
}
...
DEFAULT_IOS_PUSH_RELAY_BASE_URL="https://ios-push-relay.merclaw.ai"
PUSH_RELAY_BASE_URL="${MERCLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-${DEFAULT_IOS_PUSH_RELAY_BASE_URL}}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid MERCLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid MERCLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid MERCLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid MERCLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(MERCLAW_URL_SLASH)$(MERCLAW_URL_SLASH)#g'
)"
...
MERCLAW_CODE_SIGN_STYLE = Automatic
MERCLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
MERCLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
MERCLAW_APP_BUNDLE_ID = ai.merclaw.client
MERCLAW_SHARE_BUNDLE_ID = ai.merclaw.client.share
MERCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.merclaw.client.activitywidget
MERCLAW_WATCH_APP_BUNDLE_ID = ai.merclaw.client.watchkitapp
MERCLAW_WATCH_EXTENSION_BUNDLE_ID = ai.merclaw.client.watchkitapp.extension
MERCLAW_APP_PROFILE =
MERCLAW_SHARE_PROFILE =
MERCLAW_PUSH_TRANSPORT = relay
MERCLAW_PUSH_DISTRIBUTION = official
MERCLAW_URL_SLASH = /
MERCLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
MERCLAW_PUSH_APNS_ENVIRONMENT = production
EOF
