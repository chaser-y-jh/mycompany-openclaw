#!/usr/bin/env bash
# Rootless MerClaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-merclaw-podman.sh launch        # Start gateway
#   ./scripts/run-merclaw-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   merclaw --container merclaw dashboard --no-open
#   merclaw --container merclaw channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      MERCLAW_GATEWAY_TOKEN|MERCLAW_PODMAN_CONTAINER|MERCLAW_PODMAN_IMAGE|MERCLAW_IMAGE|MERCLAW_PODMAN_PULL|MERCLAW_PODMAN_RUN_TIMEOUT|MERCLAW_PODMAN_GATEWAY_HOST_PORT|MERCLAW_GATEWAY_PORT|MERCLAW_PODMAN_BRIDGE_HOST_PORT|MERCLAW_BRIDGE_PORT|MERCLAW_GATEWAY_BIND|MERCLAW_PODMAN_USERNS|MERCLAW_BIND_MOUNT_OPTIONS|MERCLAW_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-merclaw-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${MERCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.merclaw}"
ENV_FILE="${MERCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${MERCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.merclaw}"
ENV_FILE="${MERCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${MERCLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${MERCLAW_PODMAN_CONTAINER:-merclaw}"
MERCLAW_IMAGE="${MERCLAW_PODMAN_IMAGE:-${MERCLAW_IMAGE:-merclaw:local}}"
PODMAN_PULL="${MERCLAW_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${MERCLAW_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${MERCLAW_PODMAN_GATEWAY_HOST_PORT:-${MERCLAW_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${MERCLAW_PODMAN_BRIDGE_HOST_PORT:-${MERCLAW_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${MERCLAW_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$MERCLAW_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v merclaw >/dev/null 2>&1; then
    return 0
  fi
  MERCLAW_CONTAINER="" MERCLAW_CONFIG_DIR="$config_dir" \
    merclaw config get gateway.bind 2>/dev/null || true
}
...
# MERCLAW_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${MERCLAW_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'MERCLAW_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v merclaw >/dev/null 2>&1; then
    echo "Warning: merclaw not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    MERCLAW_CONTAINER="" MERCLAW_CONFIG_DIR="$config_dir" \
      merclaw config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    MERCLAW_CONTAINER="" MERCLAW_CONFIG_DIR="$config_dir" \
      merclaw config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  MERCLAW_CONTAINER="" MERCLAW_CONFIG_DIR="$config_dir" \
    merclaw config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${MERCLAW_GATEWAY_TOKEN:-}" ]]; then
  export MERCLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "MERCLAW_GATEWAY_TOKEN" "$MERCLAW_GATEWAY_TOKEN"
  echo "Generated MERCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/merclaw.json"
...
PODMAN_USERNS="${MERCLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported MERCLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (MERCLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${MERCLAW_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${MERCLAW_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$MERCLAW_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.merclaw/.npm \
    -e MERCLAW_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.merclaw:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.merclaw/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$MERCLAW_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME merclaw dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME merclaw devices approve --latest  # if pairing required"
