...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/merclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "MerClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/merclaw/scripts/mobile-reauth.sh'" --id merclaw-auth
        ;;
    *)
