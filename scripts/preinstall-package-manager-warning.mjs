...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[merclaw] warning: detected ${packageManager} for install lifecycle.`,
    "[merclaw] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[merclaw] prefer: corepack pnpm install",
  ].join("\n");
}
