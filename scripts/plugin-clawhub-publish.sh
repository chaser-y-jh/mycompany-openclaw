...
source_repo="${SOURCE_REPO:-${GITHUB_REPOSITORY:-merclaw/merclaw}}"
...
pack_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/merclaw-clawhub-pack.XXXXXX")"
cleanup() {
  rm -rf "${pack_dir}"
}
trap cleanup EXIT
...
build_package_runtime() {
  if [[ "${MERCLAW_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "0" || "${MERCLAW_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "false" ]]; then
...
for attempt in $(seq 1 "${MERCLAW_CLAWHUB_PUBLISH_ATTEMPTS:-8}"); do
  ...
  echo "ClawHub publish hit a rate limit; retrying (${attempt}/${MERCLAW_CLAWHUB_PUBLISH_ATTEMPTS:-8})." >&2
  sleep "${MERCLAW_CLAWHUB_PUBLISH_RETRY_DELAY_SECONDS:-60}"
done

echo "ClawHub publish failed after ${MERCLAW_CLAWHUB_PUBLISH_ATTEMPTS:-8} attempts." >&2
exit 1
