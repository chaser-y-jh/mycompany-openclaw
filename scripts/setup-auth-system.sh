#!/bin/bash
# Setup MerClaw Auth Management System
...
echo "=== MerClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'merclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  MerClaw message: Send warning via MerClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/merclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/merclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/merclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now merclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status merclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
