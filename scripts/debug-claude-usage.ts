...
const loadAuthProfiles = (agentId: string) => {
  const stateRoot = process.env.MERCLAW_STATE_DIR?.trim() || path.join(os.homedir(), ".merclaw");
...
const resolveFetchTimeoutMs = (raw = process.env.MERCLAW_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS) => {
  return parseStrictIntegerOption({
    fallback: DEFAULT_FETCH_TIMEOUT_MS,
    label: "MERCLAW_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
};
