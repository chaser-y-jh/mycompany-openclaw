...
function usage() {
  return "Usage: node scripts/check-merclaw-package-tarball.mjs <merclaw.tgz>";
}
...
if (!fs.existsSync(tarball)) {
  fail(`MerClaw package tarball does not exist: ${tarball}`);
}

const phaseTimingsEnabled = process.env.MERCLAW_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-merclaw-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-package-tarball-"));
try {
...
if (!entrySet.has("npm-shrinkwrap.json")) {
  ...
  try {
    ...
    if (shrinkwrap.name !== "merclaw") {
      errors.push("npm-shrinkwrap.json root name must be merclaw");
    }
    ...
    if (!rootPackage || rootPackage.name !== "merclaw") {
      errors.push("npm-shrinkwrap.json packages root must name merclaw");
    }
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`MerClaw package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`MerClaw package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("MerClaw package tarball integrity passed.");
