...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.MERCLAW_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "merclaw.plugin.json");
    ...
    if (packageJson.merclaw && "extensions" in packageJson.merclaw) {
      packageJson.merclaw = {
        ...packageJson.merclaw,
        extensions: rewritePackageExtensions(packageJson.merclaw.extensions),
        ...(typeof packageJson.merclaw.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.merclaw.setupEntry) }
          : {}),
      };
    }
