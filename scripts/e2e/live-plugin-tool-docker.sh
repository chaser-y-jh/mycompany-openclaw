...
IMAGE_NAME="$(docker_e2e_resolve_image "merclaw-live-plugin-tool-e2e" MERCLAW_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${MERCLAW_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${MERCLAW_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${MERCLAW_CURRENT_PACKAGE_TGZ:-}"
AGENT_TURN_TIMEOUT_SECONDS="${MERCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}"
PROFILE_FILE="${MERCLAW_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${MERCLAW_TESTBOX_PROFILE_FILE:-$HOME/.merclaw-testbox-live.profile}}"
run_log=""
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${MERCLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "MERCLAW_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires MERCLAW_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
MERCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e MERCLAW_LIVE_PLUGIN_TOOL_MODEL="${MERCLAW_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.5}" \
  -e "MERCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
MODEL_REF="${MERCLAW_LIVE_PLUGIN_TOOL_MODEL:?missing MERCLAW_LIVE_PLUGIN_TOOL_MODEL}"
PLUGIN_ID="e2e-slug-tool"
PLUGIN_NAME="@merclaw/e2e-slug-tool"
PLUGIN_VERSION="0.0.0-e2e.1"
TOOL_NAME="e2e_slug_probe"
SEED="MerClaw E2E Plugin Tool $(date +%s)-$RANDOM"
...
dump_debug_logs() {
  ...
  merclaw_e2e_dump_logs \
    ...
    /tmp/merclaw-plugin-install.log \
    /tmp/merclaw-plugin-enable.log \
    /tmp/merclaw-plugins-list.json \
    /tmp/merclaw-plugin-inspect.json \
...
fixture_dir="$(mktemp -d /tmp/merclaw-live-plugin-tool.XXXXXX)"
...
merclaw plugins install "npm-pack:$plugin_tgz" --force >/tmp/merclaw-plugin-install.log 2>&1
node scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
merclaw plugins enable "$PLUGIN_ID" >/tmp/merclaw-plugin-enable.log 2>&1
merclaw plugins list --json >/tmp/merclaw-plugins-list.json
merclaw plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/merclaw-plugin-inspect.json
...
merclaw agent --local \
  ...
  --thinking off \
  --timeout "${MERCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/merclaw-agent.json 2>/tmp/merclaw-agent.err
