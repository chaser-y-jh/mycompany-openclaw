...
IMAGE_NAME="$(docker_e2e_resolve_image "merclaw-release-upgrade-user-journey-e2e" MERCLAW_RELEASE_UPGRADE_USER_JOURNEY_E2E_IMAGE)"
SKIP_BUILD="${MERCLAW_RELEASE_UPGRADE_USER_JOURNEY_E2E_SKIP_BUILD:-0}"
run_log=""
cleanup() {
...
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz release-upgrade-user-journey "${MERCLAW_CURRENT_PACKAGE_TGZ:-}")"
...
MERCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 release-upgrade-user-journey empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e "MERCLAW_RELEASE_UPGRADE_BASELINE_SPEC=${MERCLAW_RELEASE_UPGRADE_BASELINE_SPEC:-merclaw@latest}" \
