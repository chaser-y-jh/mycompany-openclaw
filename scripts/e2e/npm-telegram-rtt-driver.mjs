...
const groupId = process.env.MERCLAW_QA_TELEGRAM_GROUP_ID;
const driverToken = process.env.MERCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN;
const sutToken = process.env.MERCLAW_QA_TELEGRAM_SUT_BOT_TOKEN;
const outputDir = process.env.MERCLAW_NPM_TELEGRAM_OUTPUT_DIR ?? ".artifacts/rtt/raw";
const telegramApiBaseUrl = (
  process.env.MERCLAW_QA_TELEGRAM_API_BASE_URL ?? "https://api.telegram.org"
).replace(/\/+$/u, "");
const timeoutMs = readPositiveIntEnv("MERCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", 180000);
const canaryTimeoutMs = readPositiveIntEnv("MERCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS", timeoutMs);
const warmSampleCount = readPositiveIntEnv("MERCLAW_NPM_TELEGRAM_WARM_SAMPLES", 20);
const sampleTimeoutMs = readPositiveIntEnv("MERCLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS", 30000);
const botApiTimeoutMs = readPositiveIntEnv("MERCLAW_NPM_TELEGRAM_BOT_API_TIMEOUT_MS", 30000);
const botApiBodyMaxBytes = readPositiveIntEnv(
  "MERCLAW_NPM_TELEGRAM_BOT_API_BODY_MAX_BYTES",
  1024 * 1024,
);
const maxWarmFailures = readPositiveIntEnv("MERCLAW_NPM_TELEGRAM_MAX_FAILURES", warmSampleCount);
const successMarker = process.env.MERCLAW_NPM_TELEGRAM_SUCCESS_MARKER ?? "MERCLAW_E2E_OK";
const scenarioIds = new Set(
  (process.env.MERCLAW_NPM_TELEGRAM_SCENARIOS ?? "telegram-mentioned-message-reply")
    .split(",")
...
if (!groupId || !driverToken || !sutToken) {
  throw new Error(
    "missing Telegram env: MERCLAW_QA_TELEGRAM_GROUP_ID, MERCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN, MERCLAW_QA_TELEGRAM_SUT_BOT_TOKEN",
  );
}
