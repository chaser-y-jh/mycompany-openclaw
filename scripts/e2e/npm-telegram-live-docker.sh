#!/usr/bin/env bash
# Installs an MerClaw package candidate in Docker, performs Telegram
...
IMAGE_NAME="$(docker_e2e_resolve_image "merclaw-npm-telegram-live-e2e" MERCLAW_NPM_TELEGRAM_LIVE_E2E_IMAGE)"
...
OUTPUT_DIR="${MERCLAW_NPM_TELEGRAM_OUTPUT_DIR:-.artifacts/qa-e2e/npm-telegram-live}"
...
validate_merclaw_package_spec() {
  ...
  echo "MERCLAW_NPM_TELEGRAM_PACKAGE_SPEC must be merclaw@alpha, merclaw@beta, merclaw@latest, or an exact MerClaw release version; got: $spec" >&2
  exit 1
}
...
validate_credential_preflight() {
  if [ "${MERCLAW_NPM_TELEGRAM_SKIP_CREDENTIAL_PREFLIGHT:-0}" = "1" ]; then
    return 0
  fi
  ...
  if [ "${#missing[@]}" -gt 0 ]; then
    {
      ...
      echo "Use one of:"
      echo "  direct Telegram env: MERCLAW_QA_TELEGRAM_GROUP_ID, MERCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN, MERCLAW_QA_TELEGRAM_SUT_BOT_TOKEN"
      echo "  Convex env: MERCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex plus MERCLAW_QA_CONVEX_SITE_URL and a role secret"
    } >&2
    exit 1
  fi
}
...
run_log="$(mktemp "${TMPDIR:-/tmp}/merclaw-npm-telegram-live.XXXXXX")"
...
docker_env=(
  ...
  -e MERCLAW_NPM_TELEGRAM_PACKAGE_SPEC="$PACKAGE_SPEC"
  -e MERCLAW_NPM_TELEGRAM_PACKAGE_LABEL="$PACKAGE_LABEL"
  -e MERCLAW_NPM_TELEGRAM_OUTPUT_DIR="$OUTPUT_DIR"
  -e MERCLAW_NPM_TELEGRAM_FAST="${MERCLAW_NPM_TELEGRAM_FAST:-1}"
)
...
if [ -n "$credential_source" ]; then
  docker_env+=(-e MERCLAW_QA_CREDENTIAL_SOURCE="$credential_source")
fi
if [ -n "$credential_role" ]; then
  docker_env+=(-e MERCLAW_QA_CREDENTIAL_ROLE="$credential_role")
fi
...
for key in \
  ...
  MERCLAW_QA_ALLOW_INSECURE_HTTP \
  MERCLAW_QA_REDACT_PUBLIC_METADATA \
  MERCLAW_QA_TELEGRAM_CAPTURE_CONTENT \
  MERCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS \
  MERCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS \
  MERCLAW_QA_SUITE_PROGRESS \
  MERCLAW_NPM_TELEGRAM_PROVIDER_MODE \
  MERCLAW_NPM_TELEGRAM_MODEL \
  MERCLAW_NPM_TELEGRAM_ALT_MODEL \
  MERCLAW_NPM_TELEGRAM_SCENARIOS \
  MERCLAW_NPM_TELEGRAM_SKIP_HOTPATH \
  MERCLAW_NPM_TELEGRAM_SUT_ACCOUNT \
  MERCLAW_NPM_TELEGRAM_ALLOW_FAILURES; do
...
export HOME="$(mktemp -d "/tmp/merclaw-npm-telegram-install.XXXXXX")"
...
export HOME="$(mktemp -d "/tmp/merclaw-npm-telegram-runtime.XXXXXX")"
export NPM_CONFIG_PREFIX="/npm-global"
export PATH="$NPM_CONFIG_PREFIX/bin:$PATH"
export MERCLAW_NPM_TELEGRAM_REPO_ROOT="/app"

dump_hotpath_logs() {
  local status="$1"
  echo "installed-package onboarding recovery hot path failed with exit code $status" >&2
  for file in \
    /tmp/merclaw-npm-telegram-onboard.json \
    /tmp/merclaw-npm-telegram-channel-add.log \
    /tmp/merclaw-npm-telegram-doctor-fix.log \
    /tmp/merclaw-npm-telegram-doctor-check.log; do
    if [ -f "$file" ]; then
      echo "--- $file ---" >&2
...
command -v merclaw
merclaw_e2e_run_command merclaw --version
mkdir -p /app/node_modules
merclaw_package_dir="/npm-global/lib/node_modules/merclaw"
# The mounted QA harness imports merclaw/plugin-sdk and package dependencies;
# point those imports at the installed package without copying source plugins into the test image.
rm -rf /app/node_modules/merclaw
ln -sfnT "$merclaw_package_dir" /app/node_modules/merclaw
rm -rf /app/dist
ln -sfnT "$merclaw_package_dir/dist" /app/dist
cp "$merclaw_package_dir/package.json" /app/package.json
node scripts/e2e/lib/npm-telegram-live/prepare-package.mjs \
  /app/package.json \
  /app/node_modules/merclaw/package.json
for deps_dir in "$merclaw_package_dir/node_modules" /npm-global/lib/node_modules; do
  [ -d "$deps_dir" ] || continue
...
if [ "${MERCLAW_NPM_TELEGRAM_SKIP_HOTPATH:-0}" != "1" ]; then
  echo "Running installed-package onboarding recovery hot path..."
  hotpath_openai_api_key="${OPENAI_API_KEY:-sk-merclaw-npm-telegram-hotpath}"
  OPENAI_API_KEY="$hotpath_openai_api_key" merclaw_e2e_run_command merclaw onboard \
    ...
    --skip-skills \
    --skip-health \
    --json >/tmp/merclaw-npm-telegram-onboard.json </dev/null

  merclaw_e2e_run_command merclaw channels add --channel telegram --token "123456:merclaw-npm-telegram-hotpath" >/tmp/merclaw-npm-telegram-channel-add.log 2>&1 </dev/null
  merclaw_e2e_run_command merclaw doctor --fix --non-interactive >/tmp/merclaw-npm-telegram-doctor-fix.log 2>&1 </dev/null
  merclaw_e2e_run_command merclaw doctor --non-interactive >/tmp/merclaw-npm-telegram-doctor-check.log 2>&1 </dev/null
fi

export MERCLAW_NPM_TELEGRAM_SUT_COMMAND="$(command -v merclaw)"
trap - ERR
