...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/merclaw-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/merclaw-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${MERCLAW_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${MERCLAW_SECRET_ENV_NAME:-}" ] && [ -n "${MERCLAW_SECRET_FILE:-}" ] && [ -f "$MERCLAW_SECRET_FILE" ]; then
  secret_value="$(cat "$MERCLAW_SECRET_FILE")"
  rm -f "$MERCLAW_SECRET_FILE"
  export "${MERCLAW_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "MERCLAW_SECRET_ENV_NAME=$api_key_env" \
      "MERCLAW_SECRET_FILE=$secret_path" \
      "MERCLAW_WRAPPER_FILE=$wrapper_path" \
