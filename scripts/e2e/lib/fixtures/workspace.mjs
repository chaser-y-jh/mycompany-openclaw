...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.MERCLAW_WORKSPACE_DIR || path.join(process.env.HOME, ".merclaw", "workspace");
  write(
...
function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.MERCLAW_STATE_DIR, "MERCLAW_STATE_DIR");
  const sharedWorkspace = requireArg(process.env.SHARED_WORKSPACE, "SHARED_WORKSPACE");
  const gatewayToken = process.env.MERCLAW_GATEWAY_TOKEN?.trim();
  fs.mkdirSync(sharedWorkspace, { recursive: true });
  writeJson(path.join(stateDir, "merclaw.json"), {
    agents: {
      list: [
...
function assertAgentsDeleteResult([outputPath]) {
  ...
  const remaining =
    readJson(path.join(process.env.MERCLAW_STATE_DIR, "merclaw.json"))?.agents?.list ?? [];
