...
write_update_restart_service_auth_env() {
  ...
  printf 'GATEWAY_AUTH_TOKEN_REF=%s\n' "$GATEWAY_AUTH_TOKEN_REF" >"$MERCLAW_STATE_DIR/gateway.systemd.env"
}

prepare_update_restart_probe_current_install() {
  local port="$1"
  local log_file="$2"
  local command_timeout="${MERCLAW_UPGRADE_SURVIVOR_COMMAND_TIMEOUT:-900s}"
  local start_epoch
  local ready_epoch
  ...
  env -u MERCLAW_GATEWAY_TOKEN -u MERCLAW_GATEWAY_PASSWORD merclaw gateway --port "$port" --bind loopback --allow-unconfigured >"$log_file" 2>&1 &
  gateway_pid="$!"
  printf '%s\n' "$gateway_pid" >"$MERCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE"
  merclaw_e2e_wait_gateway_ready "$gateway_pid" "$log_file" 360
  ready_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  start_seconds=$(((ready_epoch - start_epoch + 999) / 1000))
  write_update_restart_service_auth_env
  if ! merclaw_e2e_maybe_timeout "$command_timeout" env -u MERCLAW_GATEWAY_TOKEN -u MERCLAW_GATEWAY_PASSWORD merclaw gateway install --force --json >"$MERCLAW_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_JSON" 2>"$MERCLAW_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_ERR"; then
    echo "gateway service install failed" >&2
    cat "$MERCLAW_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_ERR" >&2 || true
    cat "$MERCLAW_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_JSON" >&2 || true
    return 1
  fi
}
