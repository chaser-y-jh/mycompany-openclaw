...
if [ -f dist/index.mjs ]; then
  MERCLAW_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  MERCLAW_ENTRY="dist/index.js"
else
...
lifecycle_trace_enabled() {
  case "${MERCLAW_PLUGIN_LIFECYCLE_TRACE:-}" in
    1 | true | TRUE | yes | YES)
      return 0
      ;;
    *)
...
echo "Selected ${#plugin_entries[@]} bundled plugins for shard ${MERCLAW_BUNDLED_PLUGIN_SWEEP_INDEX:-0}/${MERCLAW_BUNDLED_PLUGIN_SWEEP_TOTAL:-1}: ${selected_labels[*]}"

plugin_index=0
for plugin_entry in "${plugin_entries[@]}"; do
  IFS=$'\t' read -r plugin_id plugin_dir requires_config plugin_root <<<"$plugin_entry"
  install_log="/tmp/merclaw-install-${plugin_index}.log"
  uninstall_log="/tmp/merclaw-uninstall-${plugin_index}.log"
  plugin_started_at="$(now_ms)"
  echo "Installing bundled plugin: $plugin_id ($plugin_dir)"
  node "$MERCLAW_ENTRY" plugins install "$plugin_id" >"$install_log" 2>&1 || {
    cat "$install_log"
    exit 1
  }
  ...
  if [[ "${MERCLAW_BUNDLED_PLUGIN_RUNTIME_SMOKE:-1}" != "0" ]]; then
    ...
    if [[ "$plugin_id" == "${MERCLAW_BUNDLED_PLUGIN_TTS_LIVE_PROVIDER:-openai}" ]]; then
  ...
  node "$MERCLAW_ENTRY" plugins uninstall "$plugin_id" --force >"$uninstall_log" 2>&1 || {
    cat "$uninstall_log"
    exit 1
  }
  if lifecycle_trace_enabled; then
