...
SUCCESS_MARKER="MERCLAW_SCHEMA_E2E_OK"
RAW_SCHEMA_ERROR="400 The following tools cannot be used with reasoning.effort 'minimal': web_search."
MOCK_REQUEST_LOG="/tmp/merclaw-openai-web-search-minimal-requests.jsonl"
GATEWAY_LOG="/tmp/merclaw-openai-web-search-minimal-gateway.log"
mock_pid=""
gateway_pid=""
...
dump_debug_logs() {
  ...
  for file in \
    "$GATEWAY_LOG" \
    /tmp/merclaw-openai-web-search-minimal-mock.log \
    /tmp/merclaw-openai-web-search-minimal-client-success.log \
    /tmp/merclaw-openai-web-search-minimal-client-reject.log \
    "$MOCK_REQUEST_LOG" \
    "$MERCLAW_STATE_DIR/merclaw.json"; do
    if [ -f "$file" ]; then
      echo "--- $file ---" >&2
...
MOCK_PORT="$MOCK_PORT" \
  ...
  node scripts/e2e/lib/openai-web-search-minimal/mock-server.mjs >/tmp/merclaw-openai-web-search-minimal-mock.log 2>&1 &
mock_pid="$!"
...
PORT="$PORT" MERCLAW_GATEWAY_TOKEN="$TOKEN" node scripts/e2e/lib/openai-web-search-minimal/client.mjs success >/tmp/merclaw-openai-web-search-minimal-client-success.log 2>&1

node scripts/e2e/lib/openai-web-search-minimal/assertions.mjs assert-success-request "$MOCK_REQUEST_LOG"

PORT="$PORT" MERCLAW_GATEWAY_TOKEN="$TOKEN" node scripts/e2e/lib/openai-web-search-minimal/client.mjs reject >/tmp/merclaw-openai-web-search-minimal-client-reject.log 2>&1
