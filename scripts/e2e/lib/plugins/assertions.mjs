...
const scratchRoot = process.env.MERCLAW_PLUGINS_TMP_DIR || os.tmpdir();
...
function readClawHubPreflightLimits() {
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "MERCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES",
      1024 * 1024,
    ),
    timeoutMs: readPositiveIntEnv(
      "MERCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS",
      30_000,
    ),
  };
}
...
function getInstallRecords() {
  const indexPath = path.join(process.env.HOME, ".merclaw", "plugins", "installs.json");
  const index = fs.existsSync(indexPath) ? readJson(indexPath) : {};
  const configPath = path.join(process.env.HOME, ".merclaw", "merclaw.json");
  const config = fs.existsSync(configPath) ? readJson(configPath) : {};
  const allowLegacyCompat = process.env.MERCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function assertPluginRemoved(params) {
  ...
  const config = readMerClawConfig();
...
function recordFixturePluginTrust() {
  ...
  const ledgerPath = path.join(process.env.HOME, ".merclaw", "plugins", "installs.json");
  const ledger = fs.existsSync(ledgerPath)
    ? readJson(ledgerPath)
    : {
        version: 1,
        warning:
          "DO NOT EDIT. This file is generated by MerClaw plugin install/update/uninstall commands. Use `merclaw plugins install/update/uninstall` instead.",
        records: {},
      };
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".merclaw", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  const merclawPeerPath = path.join(installPath, "node_modules", "merclaw");
  if (!fs.existsSync(merclawPeerPath)) {
    throw new Error(`missing ClawHub merclaw peer symlink: ${merclawPeerPath}`);
  }
  if (!fs.lstatSync(merclawPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub merclaw peer is not a symlink: ${merclawPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(merclawPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub merclaw peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@merclaw/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@merclaw/demo-plugin-npm") {
...
function assertInvalidMerClawExtensionsRejected() {
  const pluginId = "demo-plugin-invalid-metadata";
  const output = fs.readFileSync(scratchFile("plugins-invalid-merclaw-extensions.log"), "utf8");
  for (const expected of ["merclaw.extensions[1]", "non-empty string"]) {
  ...
  const list = readJson(scratchFile("plugins-invalid-merclaw-extensions-list.json"));
  ...
  const managedInstallPath = path.join(process.env.HOME, ".merclaw", "extensions", pluginId);
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.MERCLAW_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.MERCLAW_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const installPath = resolveHomePath(record.installPath);
  const extensionsRoot = path.join(process.env.HOME, ".merclaw", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".merclaw", "merclaw.json");
...
const commands = {
  ...
  "invalid-merclaw-extensions": assertInvalidMerClawExtensionsRejected,
