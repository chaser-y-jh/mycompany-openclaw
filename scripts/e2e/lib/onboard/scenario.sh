...
start_gateway() {
  GATEWAY_PID="$(merclaw_e2e_start_gateway "$MERCLAW_ENTRY" 18789 /tmp/gateway-e2e.log)"
}

wait_for_gateway() {
  for _ in $(seq 1 20); do
    if merclaw_e2e_probe_tcp 127.0.0.1 18789 500 >/dev/null 2>&1; then
      return 0
    fi
...
run_wizard_cmd() {
  ...
  set_isolated_merclaw_env "$state_ref"

  input_fifo_dir="$(mktemp -d "/tmp/merclaw-onboard-${case_name}.XXXXXX")"
  input_fifo="$input_fifo_dir/stdin.fifo"
  if ! mkfifo "$input_fifo"; then
    rm -rf "$input_fifo_dir"
    return 1
  fi
  local log_path="/tmp/merclaw-onboard-${case_name}.log"
  WIZARD_LOG_PATH="$log_path"
  export WIZARD_LOG_PATH
  # Run under script to keep an interactive TTY for clack prompts.
  merclaw_e2e_run_script_with_pty "$command" "$log_path" <"$input_fifo" >/dev/null 2>&1 &
  wizard_pid=$!
  if ! exec 3>"$input_fifo"; then
    merclaw_e2e_stop_process "$wizard_pid"
    rm -rf "$input_fifo_dir"
    return 1
  fi
...
run_wizard() {
  ...
  run_wizard_cmd "$case_name" "$state_ref" "node \"$MERCLAW_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
}

assert_onboard_config() {
  local scenario="$1"
  shift
  merclaw_e2e_assert_file "$MERCLAW_CONFIG_PATH"
  node scripts/e2e/lib/onboard/assert-config.mjs "$scenario" "$MERCLAW_CONFIG_PATH" "$@"
}

set_isolated_merclaw_env() {
  local state_ref="$1"
  merclaw_test_state_create "$state_ref" empty
}
...
run_case_local_basic() {
  set_isolated_merclaw_env local-basic
  merclaw_e2e_run_logged local-basic node "$MERCLAW_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
  ...
  validate_local_basic_log /tmp/merclaw-onboard-local-basic.log

  # Assert config + workspace scaffolding.
  workspace_dir="$MERCLAW_STATE_DIR/workspace"
  sessions_dir="$MERCLAW_STATE_DIR/agents/main/sessions"

  merclaw_e2e_assert_dir "$sessions_dir"
  for file in AGENTS.md BOOTSTRAP.md IDENTITY.md SOUL.md TOOLS.md USER.md; do
    merclaw_e2e_assert_file "$workspace_dir/$file"
  done
...
run_case_remote_non_interactive() {
  set_isolated_merclaw_env remote-non-interactive
  # Smoke test non-interactive remote config write.
  merclaw_e2e_run_logged remote-non-interactive node "$MERCLAW_ENTRY" onboard --non-interactive --accept-risk \
    --mode remote \
...
run_case_reset() {
  set_isolated_merclaw_env reset-config
  node scripts/e2e/lib/onboard/write-config.mjs reset "$MERCLAW_CONFIG_PATH"

  merclaw_e2e_run_logged reset-config node "$MERCLAW_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
...
run_case_channels() {
  ...
  run_wizard_cmd channels channels "node \"$MERCLAW_ENTRY\" configure --section channels" send_channels_flow
...
run_case_skills() {
  local home_dir
  set_isolated_merclaw_env skills
  home_dir="$HOME"
  node scripts/e2e/lib/onboard/write-config.mjs skills "$MERCLAW_CONFIG_PATH"

  run_wizard_cmd skills "$home_dir" "node \"$MERCLAW_ENTRY\" configure --section skills" send_skills_flow
...
validate_local_basic_log() {
  local log_path="$1"
  merclaw_e2e_assert_log_not_contains "$log_path" "systemctl --user unavailable"
}
