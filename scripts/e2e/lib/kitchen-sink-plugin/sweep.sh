...
if [[ -z "${KITCHEN_SINK_TMP_DIR:-}" ]]; then
  KITCHEN_SINK_TMP_DIR="$(mktemp -d "/tmp/merclaw-kitchen-sink.XXXXXX")"
else
...
run_kitchen_sink_merclaw_logged() {
  local label="$1"
  shift
  run_logged_print "$label" merclaw_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$MERCLAW_ENTRY" "$@"
}

run_kitchen_sink_merclaw_capture() {
  local output_file="$1"
  shift
  merclaw_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$MERCLAW_ENTRY" "$@" >"$output_file"
}
...
start_kitchen_sink_clawhub_fixture_server() {
  ...
  local wait_attempts="${MERCLAW_CLAWHUB_FIXTURE_WAIT_ATTEMPTS:-600}"
...
run_success_scenario() {
  ...
  if [ -n "${KITCHEN_SINK_PREINSTALL_SPEC:-}" ]; then
    run_kitchen_sink_merclaw_logged "kitchen-sink-preinstall-${KITCHEN_SINK_LABEL}" plugins install "$KITCHEN_SINK_PREINSTALL_SPEC"
    assert_kitchen_sink_cutover_preinstalled
    install_args+=("--force")
  fi
  run_kitchen_sink_merclaw_logged "kitchen-sink-install-${KITCHEN_SINK_LABEL}" plugins install "${install_args[@]}"
  configure_kitchen_sink_runtime
  run_kitchen_sink_merclaw_logged "kitchen-sink-enable-${KITCHEN_SINK_LABEL}" plugins enable "$KITCHEN_SINK_ID"
  run_kitchen_sink_merclaw_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-plugins.json" plugins list --json
  run_kitchen_sink_merclaw_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-inspect.json" plugins inspect "$KITCHEN_SINK_ID" --runtime --json
  run_kitchen_sink_merclaw_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-inspect-all.json" plugins inspect --all --runtime --json
  assert_kitchen_sink_installed
  if [ "$KITCHEN_SINK_SOURCE" = "clawhub" ]; then
    run_kitchen_sink_merclaw_logged "kitchen-sink-uninstall-${KITCHEN_SINK_LABEL}" plugins uninstall "$KITCHEN_SINK_SPEC" --force
  else
    run_kitchen_sink_merclaw_logged "kitchen-sink-uninstall-${KITCHEN_SINK_LABEL}" plugins uninstall "$KITCHEN_SINK_ID" --force
  fi
  remove_kitchen_sink_channel_config
  run_kitchen_sink_merclaw_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-uninstalled.json" plugins list --json
  assert_kitchen_sink_removed
}

run_failure_scenario() {
  echo "Testing expected ${KITCHEN_SINK_LABEL} install failure from ${KITCHEN_SINK_SPEC}..."
  run_expect_failure "install-${KITCHEN_SINK_LABEL}" merclaw_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$MERCLAW_ENTRY" plugins install "$KITCHEN_SINK_SPEC"
  remove_kitchen_sink_channel_config
  run_kitchen_sink_merclaw_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-uninstalled.json" plugins list --json
  assert_kitchen_sink_removed
}
...
if [[ "$KITCHEN_SINK_SCENARIOS" == *"clawhub:"* ]]; then
  if [[ "${MERCLAW_KITCHEN_SINK_LIVE_CLAWHUB:-0}" = "1" ]]; then
    export MERCLAW_CLAWHUB_URL="${MERCLAW_CLAWHUB_URL:-${CLAWHUB_URL:-https://clawhub.ai}}"
  else
    if [[ -n "${MERCLAW_CLAWHUB_URL:-}" || -n "${CLAWHUB_URL:-}" ]]; then
      echo "Ignoring ambient ClawHub URL for fixture-mode kitchen-sink E2E; set MERCLAW_KITCHEN_SINK_LIVE_CLAWHUB=1 for live ClawHub."
    fi
...
while IFS='|' read -r label spec plugin_id source expectation surface_mode personality preinstall_spec; do
  ...
  export MERCLAW_KITCHEN_SINK_PERSONALITY="${personality:-}"
