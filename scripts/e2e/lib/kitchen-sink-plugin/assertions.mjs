...
function scanLogs() {
  ...
  const roots = [scratchRoot, path.join(process.env.HOME, ".merclaw")];
  const files = [];
  const visit = (entry) => {
    ...
    if (/\.(?:log|jsonl)$/u.test(entry) || /merclaw-kitchen-sink-/u.test(path.basename(entry))) {
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(merclawPeerPath)) {
    throw new Error(`missing kitchen-sink merclaw peer symlink: ${merclawPeerPath}`);
  }
  if (!fs.lstatSync(merclawPeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink merclaw peer is not a symlink: ${merclawPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(merclawPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected kitchen-sink merclaw peer ${linkedHostRoot} to target ${hostRoot}`);
  }
