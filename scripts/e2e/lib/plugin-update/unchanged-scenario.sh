...
merclaw_e2e_install_package /tmp/merclaw-install.log "mounted MerClaw package" /tmp/npm-prefix
...
MERCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT="$(node "$probe" legacy-compat "$package_version")"
...
node scripts/e2e/lib/plugin-update/registry-server.mjs >/tmp/merclaw-e2e-registry.log 2>&1 &
registry_pid=$!
...
if ! node "$probe" wait-registry; then
  ...
  cat /tmp/merclaw-e2e-registry.log || true
  exit 1
fi

before_config_hash=""
if [ "$MERCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT" != "1" ]; then
  before_config_hash="$(sha256sum "$MERCLAW_CONFIG_PATH" | awk '{print $1}')"
fi
plugin_update_timeout_seconds="${MERCLAW_PLUGIN_UPDATE_TIMEOUT_SECONDS:-180}"

node "$probe" snapshot > /tmp/plugin-update-before.json

set +e
merclaw_e2e_maybe_timeout "${plugin_update_timeout_seconds}s" node "$entry" plugins update @example/lossless-claw > /tmp/plugin-update-output.log 2>&1
...
if [ -n "$before_config_hash" ]; then
  after_config_hash="$(sha256sum "$MERCLAW_CONFIG_PATH" | awk '{print $1}')"
