...
BUN_BIN="${BUN_BIN:-bun}"
HOST_BUILD="${MERCLAW_BUN_GLOBAL_SMOKE_HOST_BUILD:-1}"
DIST_IMAGE="${MERCLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE:-}"
PACKAGE_TGZ="${MERCLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ:-}"
COMMAND_TIMEOUT_MS="${MERCLAW_BUN_GLOBAL_SMOKE_TIMEOUT_MS:-180000}"
...
resolve_package_tgz() {
  if [ -n "$PACKAGE_TGZ" ]; then
    if [ ! -f "$PACKAGE_TGZ" ]; then
      echo "MERCLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ does not exist: $PACKAGE_TGZ" >&2
      exit 1
    fi
  ...
  if [ -n "$DIST_IMAGE" ]; then
    ...
    echo "==> Skipping host build (MERCLAW_BUN_GLOBAL_SMOKE_HOST_BUILD=0)"
  fi

  if [ ! -d "$ROOT_DIR/dist" ]; then
    echo "dist/ is missing; run pnpm build or set MERCLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE" >&2
    exit 1
  fi
  ...
  PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/merclaw-bun-pack.XXXXXX")"
  pack_json_file="$PACK_DIR/pack.json"

  echo "==> Pack MerClaw tarball"
...
' "$pack_json_file" "$PACK_DIR"
  ...
  if [ -z "$PACKAGE_TGZ" ] || [ ! -f "$PACKAGE_TGZ" ]; then
    echo "missing packed MerClaw tarball" >&2
    exit 1
  fi
}
...
main() {
  ...
  echo "==> Bun global install packed MerClaw"
  "$bun_path" install -g "$PACKAGE_TGZ" --no-progress

  merclaw_bin="$BUN_INSTALL/bin/merclaw"
  if [ ! -x "$merclaw_bin" ]; then
    merclaw_bin="$(command -v merclaw || true)"
  fi
  if [ -z "$merclaw_bin" ] || [ ! -x "$merclaw_bin" ]; then
    echo "Bun global install did not create an executable merclaw binary" >&2
    exit 1
  fi

  echo "==> MerClaw version through Bun global install"
  run_with_timeout "$COMMAND_TIMEOUT_MS" "$merclaw_bin" --version

  echo "==> MerClaw image providers through Bun global install"
  local providers_json
  providers_json="$(run_with_timeout "$COMMAND_TIMEOUT_MS" "$merclaw_bin" infer image providers --json)"
  MERCLAW_IMAGE_PROVIDERS_JSON="$providers_json" node scripts/e2e/lib/bun-global-install/assertions.mjs assert-image-providers
}
