...
const smokeMode =
  process.env.OPENWEBUI_SMOKE_MODE ?? process.env.MERCLAW_OPENWEBUI_SMOKE_MODE ?? "chat";
...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsResult?.ok) {
    ...
    targetModel =
      modelIds.find((id) => id === "merclaw/default") ?? modelIds.find((id) => id === "merclaw");
    if (targetModel) {
      break;
    }
    lastModelsError = `missing merclaw model: ${JSON.stringify(modelIds)}`;
...
if (!targetModel) {
  throw new Error(
    `merclaw model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
if (smokeMode === "models") {
