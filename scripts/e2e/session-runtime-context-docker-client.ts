...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".merclaw", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(
    runtimeContextMessage.customType === "merclaw.runtime-context",
  ...
  assert(
    !userText.includes("MERCLAW_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-session-runtime-context-"));
  process.env.HOME = root;
  process.env.MERCLAW_STATE_DIR = path.join(root, ".merclaw");
  process.env.MERCLAW_CONFIG_PATH = path.join(process.env.MERCLAW_STATE_DIR, "merclaw.json");
  try {
    ...
    if (process.env.MERCLAW_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
