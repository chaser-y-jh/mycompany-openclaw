#!/usr/bin/env bash
# Runs Open WebUI against a Dockerized MerClaw Gateway and verifies the proxied
...
IMAGE_NAME="$(docker_e2e_resolve_image "merclaw-openwebui-e2e" MERCLAW_OPENWEBUI_E2E_IMAGE)"
...
MODEL="${MERCLAW_OPENWEBUI_MODEL:-openai/gpt-5.5}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${MERCLAW_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${MERCLAW_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${MERCLAW_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${MERCLAW_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${MERCLAW_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="merclaw-openwebui-e2e-$$"
GW_NAME="merclaw-openwebui-gateway-$$"
OW_NAME="merclaw-openwebui-$$"
PROVIDER_TIMEOUT_SECONDS="${MERCLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS:-900}"
PROBE_FETCH_TIMEOUT_MS="${MERCLAW_OPENWEBUI_FETCH_TIMEOUT_MS:-$((PROVIDER_TIMEOUT_SECONDS * 1000 + 60000))}"
DOCKER_COMMAND_TIMEOUT="${MERCLAW_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-$((PROVIDER_TIMEOUT_SECONDS + 90))s}"
DOCKER_PULL_TIMEOUT="${MERCLAW_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"
...
PROFILE_FILE="${MERCLAW_TESTBOX_PROFILE_FILE:-$HOME/.merclaw-testbox-live.profile}"
...
docker_e2e_docker_cmd run -d \
  ...
  -e "MERCLAW_OPENWEBUI_MODEL=$MODEL" \
  -e "MERCLAW_SKIP_CHANNELS=1" \
  -e "MERCLAW_SKIP_GMAIL_WATCHER=1" \
  -e "MERCLAW_SKIP_CRON=1" \
  -e "MERCLAW_SKIP_CANVAS_HOST=1" \
  -e "MERCLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=$PROVIDER_TIMEOUT_SECONDS" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/merclaw-openwebui-config.XXXXXX.json)"
    MERCLAW_CONFIG_BATCH_PATH="$batch_file" node scripts/e2e/lib/fixture.mjs openwebui-config "$openai_api_key"
    node "$entry" config set --batch-file "$batch_file" >/dev/null
    rm -f "$batch_file"
    node scripts/e2e/lib/fixture.mjs openwebui-workspace

    merclaw_e2e_exec_gateway "$entry" '"$PORT"' lan /tmp/openwebui-gateway.log
  ' >/dev/null

echo "Waiting for gateway HTTP surface..."
if ! docker_e2e_wait_container_bash "$GW_NAME" 240 1 "MERCLAW_HTTP_PROBE_BEARER='$TOKEN' node scripts/e2e/lib/openwebui/http-probe.mjs 'http://127.0.0.1:$PORT/v1/models' 200"; then
...
docker_e2e_docker_cmd run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="MerClaw E2E" \
  -e WEBUI_SECRET_KEY="merclaw-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="MerClaw E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="merclaw/default" \
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 90 5 "MERCLAW_HTTP_PROBE_BEARER='$TOKEN' MERCLAW_HTTP_PROBE_TIMEOUT_MS=8000 node scripts/e2e/lib/openwebui/http-probe.mjs 'http://$GW_NAME:$PORT/v1/models' 200"; then
...
echo "Running Open WebUI -> MerClaw smoke..."
