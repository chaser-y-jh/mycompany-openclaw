...
IMAGE_NAME="$(docker_e2e_resolve_image "merclaw-onboard-e2e" MERCLAW_ONBOARD_E2E_IMAGE)"
MERCLAW_TEST_STATE_FUNCTION_B64="$(docker_e2e_test_state_function_b64)"
MAX_MEMORY_MIB="${MERCLAW_ONBOARD_MAX_MEMORY_MIB:-2048}"
MAX_CPU_PERCENT="${MERCLAW_ONBOARD_MAX_CPU_PERCENT:-1200}"
DOCKER_RUN_TIMEOUT="${MERCLAW_ONBOARD_DOCKER_RUN_TIMEOUT:-1200s}"
COMMAND_TIMEOUT="${MERCLAW_ONBOARD_COMMAND_TIMEOUT:-${MERCLAW_E2E_COMMAND_TIMEOUT:-300s}}"
CONTAINER_NAME="merclaw-onboard-e2e-$$"
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/merclaw-onboard.XXXXXX")"
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/merclaw-onboard-stats.XXXXXX")"
...
DOCKER_COMMAND_TIMEOUT="$DOCKER_RUN_TIMEOUT" docker_e2e_docker_run_cmd run --name "$CONTAINER_NAME" "${DOCKER_E2E_HARNESS_ARGS[@]}" -t \
  ...
  -e "MERCLAW_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT" \
