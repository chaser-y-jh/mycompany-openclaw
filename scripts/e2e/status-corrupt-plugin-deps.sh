...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/merclaw-status-corrupt-plugin-deps.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
const stageDir = process.env.MERCLAW_PLUGIN_STAGE_DIR || "";
const renameResidue = path.join(stageDir, "node_modules", "ansi-escapes", ".merclaw-rename-tmp");
...
run_merclaw() {
  HOME="$HOME_DIR" \
  MERCLAW_HOME="$STATE_DIR" \
  MERCLAW_STATE_DIR="$STATE_DIR" \
  MERCLAW_CONFIG_PATH="$CONFIG_PATH" \
  MERCLAW_PLUGIN_STAGE_DIR="$STAGE_DIR" \
...
run_merclaw status --all --timeout 1 > "$BEFORE"
grep -F "e2e-corrupt-chat" "$BEFORE" >/dev/null
grep -F "plugin load failed: dependency tree corrupted; run merclaw doctor --fix" "$BEFORE" >/dev/null

run_merclaw doctor --fix --non-interactive --yes > "$DOCTOR"
if [[ -e "$STAGE_DIR" ]]; then
  echo "doctor --fix did not remove corrupt plugin stage dir: $STAGE_DIR" >&2
  exit 1
fi

run_merclaw status --all --timeout 1 > "$AFTER"
