...
export function windowsModelProviderTimeoutScript(modelId: string): string {
  ...
  return `$providerTimeoutBatchPath = Join-Path ([System.IO.Path]::GetTempPath()) 'merclaw-provider-timeout.batch.json'
@'
${batchJson}
'@ | Set-Content -Path $providerTimeoutBatchPath -Encoding UTF8
Invoke-MerClaw config set --batch-file $providerTimeoutBatchPath --strict-json
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:MERCLAW_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.merclaw\\merclaw.json' }
$agentTurnVersionText = Invoke-MerClaw --version 2>$null | Out-String
$agentTurnRuntimePolicySupported = $false
if ($agentTurnVersionText -match 'MerClaw\\s+(\\d{4})\\.(\\d{1,2})\\.(\\d{1,2})') {
...
$env:MERCLAW_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:MERCLAW_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$env:MERCLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED = if ($agentTurnRuntimePolicySupported) { '1' } else { '0' }
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'merclaw-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.MERCLAW_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.MERCLAW_PARALLELS_AGENT_CONFIG_PATCH || "{}");
const canWriteAgentRuntime = process.env.MERCLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED === "1";
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@merclaw", "codex"), { recursive: true, force: true });
...
if (selectedModelEntry && typeof selectedModelEntry === "object" && !Array.isArray(selectedModelEntry)) {
  if (canWriteAgentRuntime) {
    selectedModelEntry.agentRuntime = { id: "merclaw" };
  } else {
...
Remove-Item Env:MERCLAW_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:MERCLAW_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
Remove-Item Env:MERCLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsMerClawResolver = String.raw`function Resolve-MerClawCommand {
  if ($script:MerClawResolvedCommand) { return $script:MerClawResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\merclaw.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\merclaw.ps1'
  }
  foreach ($name in @('merclaw.cmd', 'merclaw.ps1', 'merclaw')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'merclaw.cmd'
    $shimCandidates += Join-Path $npmPrefix 'merclaw.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:MerClawResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:MerClawResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
  ...
  if ($npmPrefix) {
    $entryCandidates += Join-Path $npmPrefix 'node_modules\merclaw\merclaw.mjs'
  }
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:MerClawResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:MerClawResolvedCommand
    }
  }
  throw 'merclaw command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-MerClaw {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $MerClawArgs)
  $command = Resolve-MerClawCommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @MerClawArgs
    } else {
      & $command.Path @MerClawArgs
    }
  } finally {
