...
IMAGE_NAME="${MERCLAW_PLUGIN_BINDING_COMMAND_ESCAPE_E2E_IMAGE:-merclaw-plugin-binding-command-escape-e2e}"
CONTAINER_NAME="merclaw-plugin-binding-command-escape-e2e-$$"
DOCKER_RUN_TIMEOUT="${MERCLAW_PLUGIN_BINDING_COMMAND_ESCAPE_DOCKER_RUN_TIMEOUT:-900s}"
RUN_LOG="$(mktemp -t merclaw-plugin-binding-command-escape-log.XXXXXX)"
...
DOCKER_COMMAND_TIMEOUT="$DOCKER_RUN_TIMEOUT" docker_e2e_docker_run_cmd run --rm \
  ...
  -e MERCLAW_VITEST_FS_MODULE_CACHE_PATH=/tmp/merclaw-vitest-cache \
  "$IMAGE_NAME" \
