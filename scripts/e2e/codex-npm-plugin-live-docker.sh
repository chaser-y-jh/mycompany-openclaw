#!/usr/bin/env bash
# Installs MerClaw from a prepared package tarball, installs @merclaw/codex
...
CANDIDATE_ROOT="${MERCLAW_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"
...
IMAGE_NAME="$(docker_e2e_resolve_image "merclaw-codex-npm-plugin-live-e2e" MERCLAW_CODEX_NPM_PLUGIN_E2E_IMAGE)"
DOCKER_TARGET="${MERCLAW_CODEX_NPM_PLUGIN_DOCKER_TARGET:-bare}"
HOST_BUILD="${MERCLAW_CODEX_NPM_PLUGIN_HOST_BUILD:-1}"
PACKAGE_TGZ="${MERCLAW_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${MERCLAW_CODEX_NPM_PLUGIN_PROFILE_FILE:-${MERCLAW_TESTBOX_PROFILE_FILE:-$HOME/.merclaw-testbox-live.profile}}"
CODEX_PLUGIN_SPEC="${MERCLAW_CODEX_NPM_PLUGIN_SPEC:-}"
CODEX_PLUGIN_MOUNT=()
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${MERCLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "MERCLAW_CODEX_NPM_PLUGIN_HOST_BUILD=0 requires MERCLAW_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
prepare_codex_plugin_spec() {
  ...
  if [ -z "$CODEX_PLUGIN_SPEC" ]; then
    CODEX_PLUGIN_PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/merclaw-codex-plugin-pack.XXXXXX")"
    (
      cd "$CANDIDATE_ROOT"
    ...
    ) >/tmp/merclaw-codex-plugin-pack.log 2>&1
    pack_output=()
    ...
    if [ "${#pack_output[@]}" -ne 1 ]; then
      ...
      cat /tmp/merclaw-codex-plugin-pack.log >&2 || true
      exit 1
    fi
...
MERCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 codex-npm-plugin-live empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e MERCLAW_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS="${MERCLAW_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS:-0}" \
  -e MERCLAW_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL="${MERCLAW_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-1}" \
  -e MERCLAW_CODEX_NPM_PLUGIN_MODEL="${MERCLAW_CODEX_NPM_PLUGIN_MODEL:-codex/gpt-5.4}" \
  -e MERCLAW_CODEX_NPM_PLUGIN_SPEC="$CODEX_PLUGIN_SPEC" \
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
...
CODEX_PLUGIN_SPEC="${MERCLAW_CODEX_NPM_PLUGIN_SPEC:?missing MERCLAW_CODEX_NPM_PLUGIN_SPEC}"
MODEL_REF="${MERCLAW_CODEX_NPM_PLUGIN_MODEL:?missing MERCLAW_CODEX_NPM_PLUGIN_MODEL}"
POST_UNINSTALL_MODEL_REF="codex/${MODEL_REF#*/}"
SESSION_ID="codex-npm-plugin-live"
SUCCESS_MARKER="MERCLAW-CODEX-NPM-PLUGIN-LIVE-OK"
PLUGIN_INSTALL_FLAGS=(--force)
if [ "${MERCLAW_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-0}" = "1" ]; then
...
dump_debug_logs() {
  ...
  merclaw_e2e_dump_logs \
    ...
    /tmp/merclaw-codex-plugin-install.log \
    /tmp/merclaw-codex-plugin-enable.log \
    /tmp/merclaw-codex-plugins-list.json \
    /tmp/merclaw-codex-plugin-inspect.json \
    /tmp/merclaw-codex-preflight.log \
    /tmp/merclaw-codex-agent.json \
    /tmp/merclaw-codex-agent.err \
    /tmp/merclaw-codex-agent-turn1.json \
    /tmp/merclaw-codex-agent-turn1.err \
    /tmp/merclaw-codex-agent-turn2.json \
    /tmp/merclaw-codex-agent-turn2.err \
    /tmp/merclaw-codex-plugin-uninstall.log \
    /tmp/merclaw-codex-plugins-list-after-uninstall.json \
    /tmp/merclaw-codex-agent-after-uninstall.json \
    /tmp/merclaw-codex-agent-after-uninstall.err
}
...
merclaw plugins install "$CODEX_PLUGIN_SPEC" "${PLUGIN_INSTALL_FLAGS[@]}" >/tmp/merclaw-codex-plugin-install.log 2>&1

node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs configure "$MODEL_REF"

echo "Enabling Codex plugin..."
merclaw plugins enable codex >/tmp/merclaw-codex-plugin-enable.log 2>&1

merclaw plugins list --json >/tmp/merclaw-codex-plugins-list.json
merclaw plugins inspect codex --runtime --json >/tmp/merclaw-codex-plugin-inspect.json
...
"$CODEX_BIN" exec \
  ...
  --color never \
  --skip-git-repo-check \
  "Reply exactly: ${SUCCESS_MARKER}-PREFLIGHT" >/tmp/merclaw-codex-preflight.log 2>&1 </dev/null
...
echo "Running MerClaw local agent turns through npm-installed Codex plugin..."
run_agent_turn \
  "turn1" \
  "${SUCCESS_MARKER}-TURN-1" \
  "Reply in one short sentence. Include token ${SUCCESS_MARKER}-TURN-1 and say hello from the MerClaw Codex plugin." \
  /tmp/merclaw-codex-agent-turn1.json \
  /tmp/merclaw-codex-agent-turn1.err
run_agent_turn \
  "turn2" \
...
merclaw plugins uninstall codex --force >/tmp/merclaw-codex-plugin-uninstall.log 2>&1
merclaw plugins list --json >/tmp/merclaw-codex-plugins-list-after-uninstall.json
node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs assert-uninstalled

if merclaw agent --local \
  --agent main \
  ...
  --thinking low \
  --timeout 120 \
  --json >/tmp/merclaw-codex-agent-after-uninstall.json 2>/tmp/merclaw-codex-agent-after-uninstall.err; then
  echo "Expected MerClaw agent to fail after Codex uninstall, got status 0" >&2
  exit 1
fi
if ! grep -Fq 'Requested agent harness "codex" is not registered' /tmp/merclaw-codex-agent-after-uninstall.err &&
  ! grep -Fq 'Unknown model: codex/' /tmp/merclaw-codex-agent-after-uninstall.err; then
  echo "Unexpected post-uninstall agent error:" >&2
  cat /tmp/merclaw-codex-agent-after-uninstall.err >&2 || true
  exit 1
fi
