...
import { applyDockerOpenAiProviderConfig, type MerClawConfig } from "./docker-openai-seed.ts";
...
process.title = "merclaw-cron-mcp-cleanup-probe";
...
async function main() {
  const stateDir = process.env.MERCLAW_STATE_DIR?.trim() || path.join(os.homedir(), ".merclaw");
  const configPath =
    process.env.MERCLAW_CONFIG_PATH?.trim() || path.join(stateDir, "merclaw.json");
