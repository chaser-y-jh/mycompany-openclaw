...
const EXPECTED_REPOSITORY_URL = "https://github.com/merclaw/merclaw";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@merclaw/fs-safe";
...
const skipPackValidationEnv = "MERCLAW_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return positiveEnvInt(
    "MERCLAW_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "merclaw") {
    errors.push(`package.json name must be "merclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.merclaw !== "merclaw.mjs") {
    errors.push(
      `package.json bin.merclaw must be "merclaw.mjs"; found "${pkg.bin?.merclaw ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`merclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `merclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
