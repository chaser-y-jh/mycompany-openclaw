...
description: "Triage MerClaw security advisories, drafts, and GHSA reports with shipped-tag and trust-model proof."
---

# Security Triage

Use when reviewing MerClaw security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/merclaw/merclaw/security-advisories/<GHSA>`.
3. Inspect the exact implicated code paths.
4. Verify shipped state:
   - `git tag --sort=-creatordate | head`
   - `npm view merclaw version --userconfig "$(mktemp)"`
...
```bash
gh api /repos/merclaw/merclaw/security-advisories/<GHSA>
gh api /repos/merclaw/merclaw/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view merclaw version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo merclaw/merclaw --match title,body,comments -- "<terms>"
gh search prs --repo merclaw/merclaw --match title,body,comments -- "<terms>"
```
