---
name: crabbox
description: Use the Crabbox wrapper for MerClaw remote validation across Linux, macOS, Windows, and WSL2, including delegated Blacksmith Testbox proof. Report the actual provider and id.
---

# Crabbox

Use the Crabbox wrapper when MerClaw needs remote Linux proof for broad tests,
...
For MerClaw maintainer broad `pnpm` gates, Blacksmith Testbox through the
...
comparison. Use `--provider blacksmith-testbox` when the task needs MerClaw
...
- MerClaw scripts prefer `../crabbox/bin/crabbox` when present. The user PATH
  shim can be stale.
...
- The brokered AWS default is a Linux developer image in `eu-west-1`; the repo
  ...
  region/AZ placement, and FSR state before blaming MerClaw.
- For broad MerClaw maintainer `pnpm` gates, prefer the repo wrapper with
...
- Do not treat inherited shell env as operator intent. In particular,
  `MERCLAW_LOCAL_CHECK_MODE=throttled` from the local shell is not permission
  to move broad `pnpm check:changed`, `pnpm test:changed`, full `pnpm test`, or
  lint/typecheck fan-out onto the laptop.
- Only use `MERCLAW_LOCAL_CHECK_MODE=throttled|full` when the user explicitly
...
Use these only when the task needs an existing non-Linux host. MerClaw broad
...
Do not silently substitute AWS macOS for normal MerClaw Linux proof. Report
...
Use this for MerClaw maintainer broad/heavy `pnpm` gates when the prepared CI
...
node scripts/crabbox-wrapper.mjs run \
  --provider blacksmith-testbox \
  --cache-volume pnpm-store=merclaw-node24-pnpm-lock:/tmp/merclaw-pnpm-store \
  --timing-json \
  -- \
  corepack pnpm check:changed
```
...
install MerClaw as the user would, run the same setup/update/CLI/Gateway/API
...
- Prefer `MERCLAW_CURRENT_PACKAGE_TGZ` with Docker/package lanes when testing a
...
- Prefer deterministic arrow navigation over search typing for Clack-style
  ...
  sequences.
- Isolate mutable state with `MERCLAW_STATE_DIR=$(mktemp -d)`. Plugin npm
  installs live under that state dir (`npm/node_modules/...`), not under
  `MERCLAW_CONFIG_DIR`. Verify downloads by checking the state dir, package
  lock, and installed package metadata.
- To test automatic setup installs against local package artifacts, use
  `MERCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1` plus
  `MERCLAW_PLUGIN_INSTALL_OVERRIDES='{"plugin-id":"npm-pack:/tmp/plugin.tgz"}'`.
  Pack with `npm pack`, set an isolated `MERCLAW_STATE_DIR`, and verify the
...
- For OpenAI/Codex onboarding proof, the useful markers are the UI line
  `Installed Codex plugin`, `npm/node_modules/@merclaw/codex`, and the
...
- Bad local config: inspect `.crabbox.yaml`, `crabbox config show`, and
  `crabbox whoami`; normal MerClaw proof should use brokered AWS without
...
```sh
blacksmith auth login --non-interactive --organization merclaw
```
...
Use AWS for normal MerClaw remote proof. The repo `.crabbox.yaml` already
...
```sh
brew install merclaw/tap/crabbox
...
- If broker auth is missing, run `crabbox login --url https://crabbox.merclaw.ai --provider aws`.
- If the CLI asks for `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, or AWS
  profile setup during normal MerClaw validation, assume the agent selected
...
- Ask for AWS keys only for explicit direct-provider/account administration,
  not for normal brokered MerClaw proof.
- Trusted automation may still use
  `printf '%s' "$CRABBOX_COORDINATOR_TOKEN" | crabbox login --url https://crabbox.merclaw.ai --provider aws --token-stdin`.
...
for MerClaw lanes. Let that config drive normal validation.
...
Do not add MerClaw-specific setup to Crabbox itself. Put repo setup in the
