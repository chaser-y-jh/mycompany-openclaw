...
```bash
export MERCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.merclaw/merclaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env MERCLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `merclaw` wrapper for guest `message send/read`; `node merclaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/merclaw-parallels-smoke.*`
...
- host inbound nonce appears in `merclaw message read` output
