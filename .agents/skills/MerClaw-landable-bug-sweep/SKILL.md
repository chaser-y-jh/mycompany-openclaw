---
name: merclaw-landable-bug-sweep
description: "Find or repair small high-confidence non-SDK-boundary MerClaw bugfix PRs until five are landable."
---

# MerClaw Landable Bug Sweep

Autonomous maintainer workflow for producing five landable MerClaw bugfix PR URLs.
...
Use `$gitcrawl` for discovery/clustering, `$merclaw-pr-maintainer` for live GitHub mutation rules, `$github-author-context` when contributor trust matters, `$merclaw-testing` for proof choice, `$autoreview` before publishing/landing, and `$crabbox` for broad/E2E/live proof.
