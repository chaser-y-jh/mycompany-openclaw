...
@MainActor
enum MerClawAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class MerClawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.merclaw.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.merclaw.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.merclaw.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? MerClawAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "merclaw.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "merclaw.watch.promptId"
    static let sessionKeyKey = "merclaw.watch.sessionKey"
    static let actionPrimaryIDKey = "merclaw.watch.action.primary.id"
    static let actionPrimaryLabelKey = "merclaw.watch.action.primary.label"
    static let actionSecondaryIDKey = "merclaw.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "merclaw.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "merclaw.watch.action.primary"
    static let actionSecondaryIdentifier = "merclaw.watch.action.secondary"
    static let actionIdentifierPrefix = "merclaw.watch.action."
    static let actionIDKeyPrefix = "merclaw.watch.action.id."
    static let actionLabelKeyPrefix = "merclaw.watch.action.label."
    static let categoryPrefix = "merclaw.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: MerClawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> MerClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return MerClawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "MerClaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [MerClawWatchAction]) -> [UNNotificationAction] {
...
struct MerClawApp: App {
    ...
    init() {
        ...
        MerClawAppModelRegistry.appModel = appModel
...
extension MerClawApp {
    ...
    private static func installUncaughtExceptionLogger() {
        NSLog("MerClaw: installing uncaught exception handler")
