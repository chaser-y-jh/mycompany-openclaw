...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> MerClawDeviceStatusPayload {
        ...
        return MerClawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> MerClawDeviceInfoPayload {
        ...
        return MerClawDeviceInfoPayload(
    ...
    private func batteryStatus() -> MerClawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: MerClawBatteryState = switch device.batteryState {
        ...
        return MerClawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> MerClawThermalStatusPayload {
        let state: MerClawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return MerClawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> MerClawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return MerClawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
