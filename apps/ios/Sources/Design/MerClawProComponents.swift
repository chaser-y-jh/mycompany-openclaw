...
struct MerClawProBackground: View {
    @Environment(\.colorScheme) private var colorScheme

    var body: some View {
        LinearGradient(
            colors: MerClawBrand.canvasColors(for: self.colorScheme),
            startPoint: .top,
            endPoint: .bottom)
            .ignoresSafeArea()
            .overlay(alignment: .top) {
                if self.colorScheme == .light {
                    LinearGradient(
                        colors: [
                            MerClawBrand.accent.opacity(0.05),
                            .clear,
                        ],
            ...
            if let actionTitle {
                if let action {
                    Button(actionTitle, action: action)
                        .font(.caption.weight(.medium))
                        .foregroundStyle(MerClawBrand.accent)
                } else {
                    Text(actionTitle)
                        .font(.caption.weight(.medium))
                        .foregroundStyle(.secondary)
...
struct ProCard<Content: View>: View {
    ...
    var radius: CGFloat = MerClawProMetric.cardRadius
    ...
    private var borderStyle: AnyShapeStyle {
        ...
        let gradient = LinearGradient(
            colors: [
                ...
                (self.tint ?? MerClawBrand.accent).opacity(0.10),
...
extension View {
    func proPanelSurface(
        tint: Color? = nil,
        radius: CGFloat = MerClawProMetric.cardRadius,
...
struct MerClawProMark: View {
    ...
    var body: some View {
        Image("MerClawIcon")
            ...
            .shadow(color: MerClawBrand.accent.opacity(0.28), radius: self.shadowRadius, y: self.shadowRadius / 2)
            .accessibilityLabel("MerClaw")
    }
}

struct ProProgressBar: View {
    let progress: Double
    var color: Color = MerClawBrand.accentHot
        ...
        Button(action: self.action) {
            HStack(spacing: 12) {
                Image(systemName: self.systemImage)
                    ...
                    .background(MerClawBrand.accentHot, in: RoundedRectangle(cornerRadius: 13, style: .continuous))
                ...
                Image(systemName: "arrow.right")
                    ...
                    .foregroundStyle(MerClawBrand.accentHot)
            }
            .padding(12)
            .proGlassSurface(
                fill: self.colorScheme == .dark ? Color.white.opacity(0.045) : Color.white.opacity(0.68),
                stroke: MerClawBrand.accent.opacity(self.colorScheme == .dark ? 0.22 : 0.14),
                radius: 18,
                isProminent: true,
                interactive: true)
        }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            ProIconBadge(
                ...
                color: self.done ? MerClawBrand.ok : MerClawBrand.warn)
            VStack(alignment: .leading, spacing: 3) {
                Text(self.title)
