...
final class MotionService: MotionServicing {
    func activities(params: MerClawMotionActivityParams) async throws -> MerClawMotionActivityPayload {
        ...
        let mapped: [MerClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return MerClawMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = MerClawPedometerPayload(
