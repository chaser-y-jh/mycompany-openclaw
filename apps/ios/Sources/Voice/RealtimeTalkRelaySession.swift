...
final class RealtimeTalkRelaySession {
    private static let agentControlToolName = "merclaw_agent_control"
    ...
    private let logger = Logger(subsystem: "ai.merclaw", category: "RealtimeTalkRelay")
    ...
    private func handleToolCall(_ payload: [String: AnyCodable]) async {
        ...
        do {
            ...
            let result: [String: Any] = completion.failed
                ? ["error": "MerClaw tool call failed"]
                : ["text": completion.text ?? "MerClaw finished with no text."]
                ...
                for await event in stream {
                    ...
                    if chatEvent.state == "final" {
                        return ChatCompletionResult(
                            text: MerClawChatEventText.assistantText(from: chatEvent),
                            failed: false)
                    }
