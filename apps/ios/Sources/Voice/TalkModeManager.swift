...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__MERCLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<MerClawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.merclaw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = MerClawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> MerClawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = activePTTCaptureId {
            return MerClawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> MerClawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> MerClawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return MerClawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> MerClawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = MerClawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: MerClawTalkPTTStopPayload) {
            ...
            group.addTask { [runId] in
                ...
                for await evt in stream {
                    ...
                    if evt.event == "chat" {
                        ...
                        if let text = MerClawChatEventText.assistantText(from: chatEvent) {
