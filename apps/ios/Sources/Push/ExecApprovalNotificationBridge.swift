...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "merclaw.exec-approval"
    static let reviewActionIdentifier = "merclaw.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.merClawPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.merClawPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func merClawPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
