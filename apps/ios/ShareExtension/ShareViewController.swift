...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.merclaw.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to MerClaw", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to MerClaw gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to MerClaw.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "MerClawShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "MerClaw is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "MerClaw Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("merclaw-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
