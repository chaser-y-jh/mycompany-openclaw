# MerClaw iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an MerClaw Gateway as a `role: node`.
...
- Beta release uses canonical `ai.merclaw.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `MerClawPushTransport=relay`, `MerClawPushDistribution=official`, and `MerClawPushAPNsEnvironment=production`.
...
- Beta builds default to `https://ios-push-relay.merclaw.ai`.
- Optional custom relay override: `MERCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
3. Optional: set a custom official/TestFlight relay URL for the build. If unset, the beta flow uses `https://ios-push-relay.merclaw.ai`.

```bash
export MERCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/MerClaw-<version>.ipa`
   - `apps/ios/build/beta/MerClaw-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/MerClaw.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `MerClawPushTransport=direct` and `MerClawPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `MERCLAW_APNS_TEAM_ID`, `MERCLAW_APNS_KEY_ID`, and either `MERCLAW_APNS_PRIVATE_KEY_P8` or `MERCLAW_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.merclaw/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `MERCLAW_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `MerClawPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `merclaw.json`. `MERCLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine MerClaw official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as MerClaw node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
