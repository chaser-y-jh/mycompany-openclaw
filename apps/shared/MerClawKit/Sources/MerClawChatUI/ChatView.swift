...
@MainActor
public struct MerClawChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    @State private var viewModel: MerClawChatViewModel
    ...
    private let talkControl: MerClawChatTalkControl?
    ...
    public init(
        viewModel: MerClawChatViewModel,
        ...
        talkControl: MerClawChatTalkControl? = nil)
    {
    ...
    private var visibleMessages: [MerClawChatMessage] {
        let base: [MerClawChatMessage]
    ...
    private func mergeToolResults(in messages: [MerClawChatMessage]) -> [MerClawChatMessage] {
    ...
    private func isToolResultMessage(_ message: MerClawChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: MerClawChatMessage) -> Bool {
    ...
    private func primaryText(in message: MerClawChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            let kind = (content.type ?? "text").lowercased()
            guard kind == "text" || kind.isEmpty else { return nil }
            return content.text
        }
        return MerClawChatMessage.displayText(
    ...
    private func hasInlineAttachments(in message: MerClawChatMessage) -> Bool {
    ...
    private func toolCalls(in message: MerClawChatMessage) -> [MerClawChatMessageContent] {
    ...
    private func inlineToolResults(in message: MerClawChatMessage) -> [MerClawChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: MerClawChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: MerClawChatMessage) -> String {
...
private struct ChatAssistantIntroCard: View {
    ...
    var body: some View {
        Text(self.text)
            .font(.system(size: 15))
            .lineSpacing(4)
            .foregroundStyle(MerClawChatTheme.assistantText)
            .multilineTextAlignment(.leading)
            .padding(.vertical, 12)
            .padding(.horizontal, 14)
            .background(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .fill(MerClawChatTheme.assistantBubble)
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
