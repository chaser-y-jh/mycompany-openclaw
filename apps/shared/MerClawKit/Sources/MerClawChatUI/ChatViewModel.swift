...
private let chatUILogger = Logger(subsystem: "ai.merclaw", category: "MerClawChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class MerClawChatViewModel {
    public static let defaultModelSelectionID = "__default__"
    static let maxAttachmentBytes = 5_000_000

    public private(set) var messages: [MerClawChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var thinkingLevelOptions: [MerClawChatThinkingLevelOption]
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [MerClawChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [MerClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [MerClawChatPendingToolCall] = []
    public private(set) var sessions: [MerClawChatSessionEntry] = []
    private let transport: any MerClawChatTransport
    private var sessionDefaults: MerClawChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: MerClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any MerClawChatTransport,
    ...
    public var sessionChoices: [MerClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [MerClawChatSessionEntry] = []
    ...
    private static let baseThinkingLevelOptions: [MerClawChatThinkingLevelOption] = [
        MerClawChatThinkingLevelOption(id: "off", label: "off"),
        MerClawChatThinkingLevelOption(id: "minimal", label: "minimal"),
        MerClawChatThinkingLevelOption(id: "low", label: "low"),
        MerClawChatThinkingLevelOption(id: "medium", label: "medium"),
        MerClawChatThinkingLevelOption(id: "high", label: "high"),
    ]
    ...
    public func removeAttachment(_ id: MerClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [MerClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: MerClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: MerClawChatMessage) -> MerClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> MerClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return MerClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: MerClawChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: MerClawChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: MerClawChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [MerClawChatMessage],
        incoming: [MerClawChatMessage]) -> [MerClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [MerClawChatMessage]) -> [MerClawChatMessage] {
        var result: [MerClawChatMessage] = []
    ...
    private static func dedupeKey(for message: MerClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [MerClawChatMessageContent] = [
            MerClawChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> MerClawChatAttachmentPayload in
            MerClawChatAttachmentPayload(
                type: att.type,
    ...
    private static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "MerClawChatTransport"
    ...
    private func resolvedThinkingLevelOptions(
        for currentSession: MerClawChatSessionEntry?) -> [MerClawChatThinkingLevelOption]
    {
    ...
    private static func sessionModelMatchesDefaults(
        _ session: MerClawChatSessionEntry,
        defaults: MerClawChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [MerClawChatThinkingLevelOption]?) -> [MerClawChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return MerClawChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [MerClawChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return MerClawChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    private static func withCurrentThinkingOption(
        _ options: [MerClawChatThinkingLevelOption],
        current: String) -> [MerClawChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [MerClawChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [MerClawChatThinkingLevelOption]) -> [MerClawChatThinkingLevelOption]
    {
        var result: [MerClawChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    private func placeholderSession(key: String) -> MerClawChatSessionEntry {
        MerClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func updateCurrentSessionThinkingLevel(_ thinkingLevel: String?, sessionKey: String) {
        ...
        self.sessions[index] = MerClawChatSessionEntry(
            key: current.key,
            kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: MerClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleSessionMessageEvent(_ payload: MerClawSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: MerClawChatEventPayload) {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: MerClawChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = MerClawChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: MerClawChatMessage.self)
        }
    ...
    private static func isAssistantMessage(_ message: MerClawChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: MerClawChatMessage) -> MerClawChatMessage {
    ...
    private func handleAgentEvent(_ evt: MerClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = MerClawChatPendingToolCall(
    ...
    private func handleAgentLifecycleEvent(_ evt: MerClawAgentEventPayload, isPendingRun: Bool) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: MerClawAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
