...
public struct MerClawChatTalkControl {
...
@MainActor
struct MerClawChatComposer: View {
    @Bindable var viewModel: MerClawChatViewModel
    let style: MerClawChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: MerClawChatView.ComposerChrome
    let messagePlaceholder: String?
    let talkControl: MerClawChatTalkControl?
            ...
            if self.composerChrome == .full {
                ...
                if self.style == .standard {
                    let shape = UnevenRoundedRectangle(
                        ...
                        style: .continuous)
                    shape
                        .fill(MerClawChatTheme.composerBackground)
                        .overlay(shape.strokeBorder(MerClawChatTheme.composerBorder, lineWidth: 1))
    ...
    private var modelPicker: some View {
        ...
        {
            Text(self.viewModel.defaultModelLabel).tag(MerClawChatViewModel.defaultModelSelectionID)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \MerClawPendingAttachment.id)
                { (att: MerClawPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            MerClawPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var fullEditor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(MerClawChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(MerClawChatTheme.composerBorder)))
    ...
    private func talkButton(_ talkControl: MerClawChatTalkControl) -> some View {
        Button {
    ...
    private func compactTalkButton(_ talkControl: MerClawChatTalkControl) -> some View {
        Button {
            talkControl.toggle(self.viewModel.sessionKey)
        } label: {
            Image(systemName: talkControl.isEnabled ? "stop.fill" : "waveform")
    ...
    private func talkButtonFill(_ talkControl: MerClawChatTalkControl) -> AnyShapeStyle {
        if talkControl.isEnabled {
            return AnyShapeStyle(MerClawChatTheme.userBubble)
        }
        if !talkControl.isGatewayConnected {
            return AnyShapeStyle(Color.secondary.opacity(0.12))
        }
        return MerClawChatTheme.subtleCard
    }

    private func talkButtonStroke(_ talkControl: MerClawChatTalkControl) -> Color {
        if talkControl.isEnabled {
            return Color.white.opacity(0.18)
        }
        return MerClawChatTheme.composerBorder
    }

    private func talkAccessibilityValue(_ talkControl: MerClawChatTalkControl) -> String {
    ...
    private func talkHelpText(_ talkControl: MerClawChatTalkControl) -> String {
    ...
    private var connectionPill: some View {
        ...
        .background {
            if self.composerChrome == .full {
                Capsule()
                    .fill(MerClawChatTheme.subtleCard)
            }
        }
    }
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? MerClawChatTheme.userBubble
    }
