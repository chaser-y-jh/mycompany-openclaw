...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> MerClawChatHistoryPayload
{
    MerClawChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> MerClawChatSessionEntry {
...
private func thinkingOption(_ id: String, label: String? = nil) -> MerClawChatThinkingLevelOption {
    MerClawChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> MerClawChatSessionEntry
{
    MerClawChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> MerClawChatModelChoice {
    MerClawChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [MerClawChatHistoryPayload],
    sessionsResponses: [MerClawChatSessionsListResponse] = [],
    modelResponses: [[MerClawChatModelChoice]] = [],
    ...
    -> (TestChatTransport, MerClawChatViewModel)
{
...
private func sendUserMessage(_ vm: MerClawChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, MerClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [MerClawChatHistoryPayload]
    private let sessionsResponses: [MerClawChatSessionsListResponse]
    private let modelResponses: [[MerClawChatModelChoice]]
    ...
    private let stream: AsyncStream<MerClawChatTransportEvent>
    private let continuation: AsyncStream<MerClawChatTransportEvent>.Continuation
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<MerClawChatTransportEvent>.Continuation!
        ...
        parentSessionKey: String?) async throws -> MerClawChatCreateSessionResponse
    {
        ...
        return MerClawChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> MerClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? MerClawChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [MerClawChatAttachmentPayload]) async throws -> MerClawChatSendResponse
    {
        await self.state.sentSessionKeysAppend(sessionKey)
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return MerClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> MerClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? MerClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [MerClawChatModelChoice] {
    ...
    func emit(_ evt: MerClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func displaysErrorMessageFallbackOnlyForAssistantErrorTurns() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> MerClawChatMessage {
        ...
        #expect(
            MerClawChatMessage.displayText(
                contentText: "",
                role: assistantError.role,
                stopReason: assistantError.stopReason,
                errorMessage: assistantError.errorMessage) == "stale provider failure")
    ...
    @Test func appendsExternalSessionUserMessageForActiveSession() async throws {
        ...
        transport.emit(
            .sessionMessage(
                MerClawSessionMessageEventPayload(
                    sessionKey: "agent:aiden:main",
                    message: MerClawChatMessage(
                        role: "user",
                        content: [
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = MerClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = MerClawChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: MerClawChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = MerClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: MerClawChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(MerClawChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == MerClawChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = MerClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: MerClawChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = MerClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = MerClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = MerClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func decodesGatewayThinkingMetadataFromSessionList() throws {
        ...
        let decoded = try JSONDecoder().decode(
            MerClawChatSessionsListResponse.self,
            from: Data(json.utf8))
    ...
    @Test func stripsInboundMetadataFromHistoryMessages() async throws {
        ...
        let vm = await MainActor.run { MerClawChatViewModel(sessionKey: "main", transport: transport) }
