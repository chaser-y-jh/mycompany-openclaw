    ...
    if (showDiagnostics) {
      Surface(
        ...
        Column(
          ...
          if (pairingRequired) {
            Text(
              "Approve this phone on the gateway. MerClaw retries automatically while this screen stays open.",
              style = mobileCallout,
              color = mobileTextSecondary,
            )
            CommandBlock("merclaw devices list")
            CommandBlock("merclaw devices approve <requestId>")
          }
          Text("MerClaw Android ${merClawAndroidVersionLabel()}", style = mobileCaption1, color = mobileTextSecondary)
          Button(
            onClick = {
    ...
    AnimatedVisibility(visible = advancedOpen) {
      Surface(
        ...
        Column(
          ...
          CommandBlock("merclaw qr --setup-code-only")
          CommandBlock("merclaw qr --json")
          Text(
