...
internal fun merClawAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this MerClaw Android gateway connection failure.
    ...
    - tell me whether `merclaw devices list` should show a pending pairing request
    - if more signal is needed, ask for `merclaw qr --json`, `merclaw devices list`, and `merclaw nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${merClawAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("MerClaw gateway diagnostics", report))
