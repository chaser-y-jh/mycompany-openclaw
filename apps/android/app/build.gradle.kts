...
val androidStoreFile = providers.gradleProperty("MERCLAW_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("MERCLAW_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("MERCLAW_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("MERCLAW_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
  error(
    "Missing Android release signing properties. Set MERCLAW_ANDROID_STORE_FILE, " +
      "MERCLAW_ANDROID_STORE_PASSWORD, MERCLAW_ANDROID_KEY_ALIAS, and " +
      "MERCLAW_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
  )
}
...
android {
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/MerClawKit/Sources/MerClawKit/Resources")
    }
  }

  defaultConfig {
    applicationId = "ai.merclaw.app"
    minSdk = 31
    targetSdk = 36
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "merclaw-$versionName-$buildType.apk"
          } else {
            "merclaw-$versionName-$flavorName-$buildType.apk"
          }
