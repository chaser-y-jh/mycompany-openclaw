...
private let appDefaultsSuites = ["ai.merclaw.mac", "ai.merclaw.mac.debug"]
    ...
    do {
        ...
        if opts.help {
            ...
            Usage:
              ...
              merclaw-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote MerClaw checkout for CLI commands.
              --cli-path <path>   Remote merclaw executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
...
private func configureSSHRemote(_ opts: ConfigureRemoteOptions) throws -> ConfigureRemoteOutput {
    ...
    let configURL = merClawConfigURL()
...
private func merClawConfigURL() -> URL {
    if let raw = ProcessInfo.processInfo.environment["MERCLAW_CONFIG_PATH"],
       !raw.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    {
        return URL(fileURLWithPath: NSString(string: raw).expandingTildeInPath)
    }
    return FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".merclaw/merclaw.json")
}
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String) {
    for suite in appDefaultsSuites {
        ...
        defaults.set("remote", forKey: "merclaw.connectionMode")
        setDefaultString(defaults, key: "merclaw.remoteTarget", value: target)
        defaults.set(true, forKey: "merclaw.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "merclaw.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "merclaw.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "merclaw.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "merclaw.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("MerClaw macOS Remote Config")
