...
enum DebugActions {
    private static let verboseDefaultsKey = "merclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "merclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = MerClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "MerClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = MerClawPaths.configURL
        guard
