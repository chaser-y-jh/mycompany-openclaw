...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: MerClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.merclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: MerClawKit.AnyCodable]()) {
                $0[$1.key] = MerClawKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking MerClaw Local Network access.
                    Allow MerClaw in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: MerClawProtocol.AnyCodable?) -> [String: MerClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: MerClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: MerClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: MerClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("merclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("merclaw.control.agent")
}
