...
enum GatewayDiscoverySelectionSupport {
        ...
        state: AppState)
    {
        ...
        if preferredTransport == .direct {
            MerClawConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.direct.rawValue)
            if !state.remoteUrl.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                MerClawConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
            } else {
                MerClawConfigFile.clearRemoteGatewayUrl()
            }
        } else {
            MerClawConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.ssh.rawValue)
            MerClawConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
        }
    }
