...
struct GeneralSettings: View {
    ...
    private var generalPage: some View {
        VStack(alignment: .leading, spacing: 20) {
            SettingsPageHeader(
                title: "General",
                subtitle: "Everyday MerClaw app behavior.")

            self.merClawStatusPanel

            SettingsCardGroup("App") {
                SettingsCardToggleRow(
                    title: "Launch at login",
                    subtitle: "Automatically start MerClaw after you sign in.",
                    binding: self.$state.launchAtLogin)

                SettingsCardToggleRow(
                    title: "Show Dock icon",
                    subtitle: "Keep MerClaw visible in the Dock. When off, windows still show the Dock icon while open.",
    ...
    private var merClawStatusPanel: some View {
        HStack(alignment: .center, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(self.state.isPaused ? "MerClaw paused" : "MerClaw active")
                    .font(.headline)
            ...
            Toggle("MerClaw active", isOn: self.activeBinding)
                .labelsHidden()
                .toggleStyle(.switch)
        }
    ...
    private var connectionStatusSubtitle: String {
        switch self.state.connectionMode {
        case .local:
            return "MerClaw starts and monitors the Gateway on this Mac."
        case .remote:
        ...
        SettingsCardGroup("Advanced") {
            DisclosureGroup(isExpanded: self.$showRemoteAdvanced) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    self.advancedTextField(
                        "Project root",
                        placeholder: "/home/you/Projects/merclaw",
                        text: self.$state.remoteProjectRoot)
                    self.advancedTextField(
                        "CLI path",
                        placeholder: "/Applications/MerClaw.app/.../merclaw",
...
extension GeneralSettings {
    ...
    private func revealLogs() {
        ...
        Looked for merclaw logs in /tmp/merclaw/.
...
extension GeneralSettings {
    static func exerciseForTesting() {
        ...
        state.remoteProjectRoot = "/tmp/merclaw"
        state.remoteCliPath = "/tmp/merclaw"
