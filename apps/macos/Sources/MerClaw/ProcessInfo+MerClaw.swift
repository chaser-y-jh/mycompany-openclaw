...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.merclaw.mac`) even if the shipped
    ...
    {
        if environment["MERCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "merclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "merclaw.nixMode") { return true }
