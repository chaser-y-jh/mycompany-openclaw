...
enum MerClawLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = MerClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = MerClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        MerClawLogging.bootstrapIfNeeded()
        let label = MerClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct MerClawOSLogHandler: AppLogLevelBackedHandler {
...
struct MerClawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = MerClawLogging.parseLabel(self.label)
