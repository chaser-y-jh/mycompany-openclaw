...
enum CommandResolver {
    private static let projectRootDefaultsKey = "merclaw.gatewayProjectRootPath"
    private static let helperName = "merclaw"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let merclawEntry = root.appendingPathComponent("merclaw.mjs").path
        if FileManager().isReadableFile(atPath: merclawEntry) { return merclawEntry }
        let binEntry = root.appendingPathComponent("bin/merclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/merclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let merclawPaths = self.merclawManagedPaths(home: home)
        if !merclawPaths.isEmpty {
            extras.insert(contentsOf: merclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + merclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func merclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".merclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func merclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectMerClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("merclaw.mjs").path,
            root.appendingPathComponent("bin/merclaw.js").path,
        ]
    ...
    static func hasAnyMerClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.merclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func merclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let merclawPath = self.projectMerClawExecutable(projectRoot: root) {
            return [merclawPath, subcommand] + extraArgs
        }
        if let merclawPath = self.merclawExecutable(searchPaths: searchPaths) {
            return [merclawPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "merclaw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            merclaw entrypoint missing (looked for dist/index.js or merclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real merclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/merclaw"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/merclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/merclaw.mjs"
            node "$PRJ/merclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/merclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/merclaw.js"
            node "$PRJ/bin/merclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent merclaw"
          pnpm --silent merclaw \(quotedArgs);
        else
          echo "merclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? MerClawConfigFile.loadDict()
