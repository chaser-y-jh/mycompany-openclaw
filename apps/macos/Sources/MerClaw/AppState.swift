...
final class AppState {
    private static let logger = Logger(subsystem: "ai.merclaw", category: "app-state")
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = MerClawConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            MerClawConfigFile.canonicalHostForComparison(host) ==
                MerClawConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = MerClawConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: MerClawConfigFile.loadDict(),
            draft: .init(
        ...
        guard MerClawConfigFile.saveDict(synced.root) else {
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/merclaw"
        state.remoteCliPath = ""
        return state
    }
}
