...
private let webChatSwiftLogger = Logger(subsystem: "ai.merclaw", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "merclaw.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: MerClawChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> MerClawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(MerClawChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            MerClawChatSessionsDefaults(
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return MerClawChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> MerClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                MerClawGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "session.message":
                ...
                guard let message = try? JSONDecoder().decode(
                    MerClawSessionMessageEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: MerClawProtocol.ModelChoice) -> MerClawChatModelChoice {
        MerClawChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<MerClawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any MerClawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = MerClawChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: MerClawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<MerClawChatView>) -> NSViewController
    {
