...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["MERCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/merclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("merclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("merclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/merclaw/ (rolling or stdout), or nil if none exist.
