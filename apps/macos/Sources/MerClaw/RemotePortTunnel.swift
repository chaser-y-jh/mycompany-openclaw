...
final class RemotePortTunnel: @unchecked Sendable {
    private static let logger = Logger(subsystem: "ai.merclaw", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(defaultRemotePort: Int, for sshHost: String) -> Int? {
        ...
        guard let sshKey = MerClawConfigFile.canonicalHostForComparison(sshHost),
              let urlKey = MerClawConfigFile.canonicalHostForComparison(host)
        else {
            return nil
        }
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.merclaw.remote.tunnel.port", qos: .utility)
            do {
