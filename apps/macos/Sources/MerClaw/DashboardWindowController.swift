...
final class DashboardWindowController: NSWindowController, WKNavigationDelegate, NSWindowDelegate {
    ...
    private static func installNativeChromeScript(into userContentController: WKUserContentController) {
        let css = """
        html.merclaw-native-macos {
          --merclaw-native-titlebar-height: 50px;
        }
        @media (min-width: 700px) {
          html.merclaw-native-macos .sidebar-shell {
            padding-top: max(14px, var(--merclaw-native-titlebar-height)) !important;
          }
          html.merclaw-native-macos .sidebar-shell__header {
        ...
        (() => {
          try {
            if (document.getElementById("merclaw-native-macos-chrome")) return;
            const style = document.createElement("style");
            style.id = "merclaw-native-macos-chrome";
            style.textContent = \(Self.jsStringLiteral(css));
            document.documentElement.classList.add("merclaw-native-macos");
        ...
        auth: DashboardWindowAuth)
    {
        ...
        (() => {
          try {
            ...
            Object.defineProperty(window, "__MERCLAW_NATIVE_CONTROL_AUTH__", {
              value: \(json),
              configurable: true,
            });
          } catch {}
