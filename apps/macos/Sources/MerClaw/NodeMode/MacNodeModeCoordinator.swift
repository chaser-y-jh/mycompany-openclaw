...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.merclaw", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = MerClawConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: MerClawNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    nonisolated static func resolvedCaps(
        ...
        cameraEnabled: Bool,
        locationMode: MerClawLocationMode,
        connectionMode: AppState.ConnectionMode) -> [String]
    {
        var caps: [String] = [MerClawCapability.canvas.rawValue, MerClawCapability.screen.rawValue]
        if browserControlEnabled, connectionMode == .local {
            caps.append(MerClawCapability.browser.rawValue)
        }
        if cameraEnabled {
            caps.append(MerClawCapability.camera.rawValue)
        }
        if locationMode != .off {
            caps.append(MerClawCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCaps() -> [String] {
        ...
        return Self.resolvedCaps(
            browserControlEnabled: MerClawConfigFile.browserControlEnabled(),
            cameraEnabled: UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false,
            locationMode: MerClawLocationMode(rawValue: rawLocationMode) ?? .off,
            connectionMode: AppStateStore.shared.connectionMode)
    }
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            MerClawSystemCommand.which.rawValue,
            MerClawSystemCommand.run.rawValue,
            MerClawSystemCommand.execApprovalsGet.rawValue,
            MerClawSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(MerClawCapability.browser.rawValue) {
            commands.append(MerClawBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(MerClawCapability.camera.rawValue) {
        ...
        if capsSet.contains(MerClawCapability.location.rawValue) {
    ...
    private func buildSessionBox(url: URL, connectionMode: AppState.ConnectionMode) -> WebSocketSessionBox? {
        ...
        guard let params = Self.tlsParams(
            ...
            root: MerClawConfigFile.loadDict(),
