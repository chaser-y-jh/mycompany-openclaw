...
struct MacNodeModeCoordinatorTests {
    @Test func `remote mode does not advertise browser proxy`() {
        ...
        #expect(!caps.contains(MerClawCapability.browser.rawValue))
        #expect(!commands.contains(MerClawBrowserCommand.proxy.rawValue))
        #expect(commands.contains(MerClawCanvasCommand.present.rawValue))
