...
import Foundation
import Testing
@testable import MerClawDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("MERCLAW_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("MERCLAW_WIDE_AREA_DOMAIN", "merclaw.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("MERCLAW_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("MERCLAW_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._merclaw-gw._tcp.merclaw.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.merclaw.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (MerClaw)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/merclaw/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (MerClaw)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/merclaw/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._merclaw-gw._tcp.merclaw.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
