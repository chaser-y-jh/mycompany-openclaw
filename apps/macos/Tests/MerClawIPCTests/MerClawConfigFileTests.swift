...
@Suite(.serialized)
struct MerClawConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("merclaw-config-\(UUID().uuidString)")
            .appendingPathComponent("merclaw.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["MERCLAW_CONFIG_PATH": override]) {
            #expect(MerClawConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["MERCLAW_CONFIG_PATH": override]) {
            ...
            #expect(MerClawConfigFile.remoteGatewayPort() == 19999)
            #expect(MerClawConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(MerClawConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(MerClawConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(MerClawConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(MerClawConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["MERCLAW_CONFIG_PATH": override]) {
            ...
            MerClawConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:18789")
            let root = MerClawConfigFile.loadDict()
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["MERCLAW_CONFIG_PATH": override]) {
            ...
            MerClawConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = MerClawConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["MERCLAW_CONFIG_PATH": override]) {
            ...
            MerClawConfigFile.clearRemoteGatewayUrl()
            let root = MerClawConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        ...
        await TestIsolation.withEnvValues([
            "MERCLAW_CONFIG_PATH": nil,
            "MERCLAW_STATE_DIR": dir,
        ]) {
            #expect(MerClawConfigFile.stateDirURL().path == dir)
            #expect(MerClawConfigFile.url().path == "\(dir)/merclaw.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("merclaw-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("merclaw.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "MERCLAW_STATE_DIR": stateDir.path,
            "MERCLAW_CONFIG_PATH": configPath.path,
        ]) {
            MerClawConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-merclaw-config-file")
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let allowedRoot = MerClawConfigFile.loadDict()
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try MerClawConfigFile.withTestingFileLock {
                MerClawConfigFile.saveDict([
                    "update": ["channel": "beta"],
                ...
                let loaded = MerClawConfigFile.loadDict()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = MerClawConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [
