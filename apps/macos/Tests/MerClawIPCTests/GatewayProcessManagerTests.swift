...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["MERCLAW_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "merclaw-gateway",
                executablePath: "/tmp/merclaw-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 merclaw-gateway @ /tmp/merclaw-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
