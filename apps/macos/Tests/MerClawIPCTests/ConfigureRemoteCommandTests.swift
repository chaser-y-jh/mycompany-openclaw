...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("merclaw-configure-remote-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        let defaultSuites = ["ai.merclaw.mac", "ai.merclaw.mac.debug"]
        let keys = [
            "merclaw.connectionMode",
            "merclaw.remoteTarget",
            "merclaw.onboardingSeen",
            "merclaw.onboardingVersion",
            "merclaw.remoteCliPath",
        ]
        ...
        try await TestIsolation.withIsolatedState(env: ["MERCLAW_CONFIG_PATH": configURL.path]) {
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "merclaw.connectionMode") == "remote")
                #expect(defaults.string(forKey: "merclaw.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "merclaw.onboardingSeen") == true)
                #expect(defaults.string(forKey: "merclaw.remoteCliPath") == "/opt/homebrew/bin/merclaw")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("merclaw-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("merclaw-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("merclaw-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["MERCLAW_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
