...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: MerClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: MerClawProtocol.AnyCodable] = [
            "presence": .init([MerClawProtocol.AnyCodable(entry)]),
        ]
        let payload = MerClawProtocol.AnyCodable(payloadMap)
