...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(
      parsePluginReleaseSelection(" @merclaw/zalo, @merclaw/feishu  @merclaw/zalo "),
    ).toEqual(["@merclaw/feishu", "@merclaw/zalo"]);
  });
});
...
describe("collectPublishablePluginPackageErrors", () => {
  ...
  it("flags invalid publishable plugin metadata", () => {
    expect(
      ...
      'package name must start with "@merclaw/"; found "broken".',
      "package.json private must not be true.",
      `package.json repository.url must be "${MERCLAW_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "<missing>".`,
      'package.json version must match YYYY.M.D, YYYY.M.D-N, YYYY.M.D-alpha.N, or YYYY.M.D-beta.N; found "latest".',
      "merclaw.extensions must contain only non-empty strings.",
      "merclaw.install.npmSpec must be a non-empty string for publishable plugins.",
    ]);
  });
  ...
  it("requires npm install metadata for publishable plugins", () => {
    ...
    ).toEqual(["merclaw.install.npmSpec must be a non-empty string for publishable plugins."]);
  });
  ...
  it("requires the external plugin package compatibility contract for npm publish", () => {
    expect(
      ...
      "merclaw.build.merclawVersion is required for external code plugin packages.",
    ]);
  });
});
...
describe("collectPublishablePluginPackages", () => {
  ...
  it("collects publishable npm plugins from extension package manifests", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "merclaw-plugin-npm-release-");
    ...
    expect(collectPublishablePluginPackages(repoDir)).toEqual([
      {
        ...
        channel: "stable",
        publishTag: "latest",
        installNpmSpec: "@merclaw/demo-plugin",
      },
    ]);
  });
  ...
  it("does not validate unselected publishable plugin manifests", () => {
    ...
    writeJsonFile(join(repoDir, "extensions", "private-plugin", "package.json"), {
      ...
      merclaw: {
        ...
        ...externalPluginContract("2026.4.10-beta.1"),
        install: {
          npmSpec: "@merclaw/private-plugin",
        },
        release: {
          publishToNpm: true,
        },
...
describe("resolveSelectedPublishablePluginPackages", () => {
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@merclaw/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @merclaw/missing.");
  });
});
