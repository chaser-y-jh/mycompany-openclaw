...
function createTempHome(): string {
  return makeTempDir(tempDirs, "merclaw-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-merclaw.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.merclaw/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.merclaw/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".merclaw", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".merclaw", "external-plugins", "glueclaw", "merclaw.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".merclaw", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.MERCLAW_LIVE_TEST_QUIET = "1";
    process.env.MERCLAW_CONFIG_PATH = "~/custom-merclaw.json5";
    process.env.MERCLAW_TEST_HOME = priorIsolatedHome;
    process.env.MERCLAW_STATE_DIR = path.join(priorIsolatedHome, ".merclaw");
    ...
    expect(process.env.MERCLAW_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".merclaw", "merclaw.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".merclaw", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".merclaw", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.MERCLAW_LIVE_TEST = "1";
    process.env.MERCLAW_LIVE_USE_REAL_HOME = "1";
    process.env.MERCLAW_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.MERCLAW_LIVE_TEST;
    delete process.env.MERCLAW_LIVE_GATEWAY;
    delete process.env.MERCLAW_LIVE_USE_REAL_HOME;
    delete process.env.MERCLAW_LIVE_TEST_QUIET;
