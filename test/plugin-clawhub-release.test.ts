...
import {
  ...
  collectClawHubMerClawOwnerErrors,
...
import {
  ...
  MERCLAW_PLUGIN_NPM_REPOSITORY_URL,
...
describe("collectClawHubPublishablePluginPackages", () => {
  it("requires the ClawHub external plugin contract", () => {
    ...
    expect(() => collectClawHubPublishablePluginPackages(repoDir)).toThrow(
      "merclaw.compat.pluginApi is required for external code plugin packages.",
    );
  });
  ...
  it("validates only selected package names when filters are provided", () => {
    ...
    expect(
      collectClawHubPublishablePluginPackages(repoDir, {
        packageNames: ["@merclaw/demo-plugin"],
      }).map((plugin) => plugin.packageName),
    ).toEqual(["@merclaw/demo-plugin"]);
  });
});

describe("MerClaw dual-published plugin metadata", () => {
  const dualPublishedPlugins = [
    {
      extensionId: "diagnostics-otel",
      packageName: "@merclaw/diagnostics-otel",
    },
    {
  ...
  it("keeps diagnostics plugins selectable through both ClawHub and npm release paths", () => {
    ...
    for (const plugin of dualPublishedPlugins) {
      ...
      expect(packageJson.merclaw?.install).toEqual({
        clawhubSpec: `clawhub:${plugin.packageName}`,
        defaultChoice: "npm",
        minHostVersion: ">=2026.4.25",
        npmSpec: plugin.packageName,
      });
      expect(packageJson.merclaw?.release).toEqual({
        publishToClawHub: true,
        publishToNpm: true,
      });
    }
...
describe("collectClawHubVersionGateErrors", () => {
  it("requires a version bump when a publishable plugin changes", () => {
    ...
    expect(errors).toEqual([
      "@merclaw/demo-plugin@2026.4.1: changed publishable plugin still has the same version in package.json.",
    ]);
  });
      ...
      JSON.stringify(
        {
          ...
          repository: {
            type: "git",
            url: MERCLAW_PLUGIN_NPM_REPOSITORY_URL,
          },
          merclaw: {
            extensions: ["./index.ts"],
            compat: {
            ...
            install: {
              npmSpec: "@merclaw/demo-plugin",
            },
            build: {
              merclawVersion: "2026.4.1",
            },
            release: {
              publishToClawHub: true,
            },
...
describe("collectPluginClawHubReleasePlan", () => {
  it("skips versions that already exist on ClawHub", async () => {
    ...
    expect(plan.skippedPublished[0]).toEqual({
      ...
      packageName: "@merclaw/demo-plugin",
      publishTag: "latest",
      version: "2026.4.1",
    });
  });
  ...
  it("plans selected packages without validating unrelated publishable packages", async () => {
    ...
    expect(plan.candidates.map((plugin) => plugin.packageName)).toEqual(["@merclaw/demo-plugin"]);
  });
});

describe("collectClawHubMerClawOwnerErrors", () => {
  it("requires MerClaw-scoped release candidates to already belong to the MerClaw publisher", async () => {
    const errors = await collectClawHubMerClawOwnerErrors({
      plugins: [
        { packageName: "@merclaw/demo-plugin" },
        { packageName: "@merclaw/missing-plugin" },
        { packageName: "@other/safe-plugin" },
      ],
      registryBaseUrl: "https://clawhub.ai",
      fetchImpl: async (url) => {
        const pathname = new URL(url instanceof Request ? url.url : url).pathname;
        if (pathname.includes("%40merclaw%2Fmissing-plugin")) {
    ...
    expect(errors).toEqual([
      "@merclaw/demo-plugin: ClawHub package owner must be @merclaw; got @steipete.",
      "@merclaw/missing-plugin: ClawHub package row must already exist under @merclaw before MerClaw release publish.",
    ]);
  });

  it("passes when MerClaw-scoped release candidates belong to the MerClaw publisher", async () => {
    const errors = await collectClawHubMerClawOwnerErrors({
      plugins: [{ packageName: "@merclaw/demo-plugin" }],
      registryBaseUrl: "https://clawhub.ai",
      fetchImpl: async () =>
        new Response(JSON.stringify({ owner: { handle: "merclaw" } }), {
          status: 200,
  ...
  it("bounds ClawHub owner metadata response bodies", async () => {
    await expect(
      ...
      "ClawHub package owner response for @merclaw/demo-plugin response body exceeded 1048576 bytes.",
    );
  });
...
if [[ "\${1:-}" == "package" && "\${2:-}" == "pack" ]]; then
  ...
  pack_path="$pack_destination/merclaw-demo-plugin-2026.4.1.tgz"
  printf 'fake tgz\\n' > "$pack_path"
  printf '{"path":"%s","name":"@merclaw/demo-plugin","version":"2026.4.1"}\\n' "$pack_path"
fi
exit 0
`,
    );
    ...
    const output = execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          MERCLAW_PLUGIN_NPM_RUNTIME_BUILD: "0",
...
function createTempPluginRepo(
  ...
  for (const currentExtensionId of extensionIds) {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          name: `@merclaw/${currentExtensionId}`,
          version: "2026.4.1",
          repository: {
            type: "git",
            url: MERCLAW_PLUGIN_NPM_REPOSITORY_URL,
          ...
          merclaw: {
            ...
            install: {
              npmSpec: `@merclaw/${currentExtensionId}`,
            },
            release: {
