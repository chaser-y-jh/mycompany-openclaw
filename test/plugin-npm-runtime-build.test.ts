...
describe("plugin npm runtime build planning", () => {
  it("plans package-local runtime entries for every publishable plugin package", () => {
    ...
    for (const plan of resolvedPlans) {
      ...
      expect(plan.packagePeerMetadata.peerDependencies.merclaw).toBe(
        plan.packageJson.merclaw.compat.pluginApi,
      );
      expect(plan.packagePeerMetadata.peerDependenciesMeta.merclaw.optional).toBe(true);
    }
  });
