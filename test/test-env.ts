...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.MERCLAW_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "MERCLAW_TEST_FAST", value: process.env.MERCLAW_TEST_FAST },
    {
      key: "MERCLAW_STRICT_FAST_REPLY_CONFIG",
      value: process.env.MERCLAW_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "MERCLAW_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.MERCLAW_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "MERCLAW_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.MERCLAW_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "MERCLAW_STATE_DIR", value: process.env.MERCLAW_STATE_DIR },
    { key: "MERCLAW_CONFIG_PATH", value: process.env.MERCLAW_CONFIG_PATH },
    { key: "MERCLAW_GATEWAY_PORT", value: process.env.MERCLAW_GATEWAY_PORT },
    { key: "MERCLAW_BRIDGE_ENABLED", value: process.env.MERCLAW_BRIDGE_ENABLED },
    { key: "MERCLAW_BRIDGE_HOST", value: process.env.MERCLAW_BRIDGE_HOST },
    { key: "MERCLAW_BRIDGE_PORT", value: process.env.MERCLAW_BRIDGE_PORT },
    { key: "MERCLAW_CANVAS_HOST_PORT", value: process.env.MERCLAW_CANVAS_HOST_PORT },
    { key: "MERCLAW_TEST_HOME", value: process.env.MERCLAW_TEST_HOME },
    { key: "MERCLAW_AGENT_DIR", value: process.env.MERCLAW_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.MERCLAW_TEST_HOME = tempHome;
  process.env.MERCLAW_TEST_FAST = "1";
  process.env.MERCLAW_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.MERCLAW_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.MERCLAW_BRIDGE_ENABLED;
  delete process.env.MERCLAW_BRIDGE_HOST;
  delete process.env.MERCLAW_BRIDGE_PORT;
  delete process.env.MERCLAW_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.MERCLAW_STATE_DIR = path.join(tempHome, ".merclaw");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.MERCLAW_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.MERCLAW_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".merclaw");
  const priorIsolatedHome = params.env.MERCLAW_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".merclaw")
  ) {
    realStateDir = path.join(params.realHome, ".merclaw");
  }
  const tempStateDir = path.join(params.tempHome, ".merclaw");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.MERCLAW_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.MERCLAW_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "merclaw.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "merclaw.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.MERCLAW_LIVE_TEST === "1" ||
    process.env.MERCLAW_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.MERCLAW_LIVE_USE_REAL_HOME);
