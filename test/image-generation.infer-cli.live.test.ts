...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.MERCLAW_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through merclaw infer", () => {
    const outputBase = path.join(os.tmpdir(), `merclaw-infer-image-${process.pid}.png`);
