...
import { createMerClawTestInstance, testing } from "./merclaw-test-instance.js";
...
describe("merclaw test instance", () => {
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    const inst = await createMerClawTestInstance({
      name: "instance-unit",
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".merclaw"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "merclaw.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.MERCLAW_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.MERCLAW_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.MERCLAW_SKIP_CRON).toBe("0");
