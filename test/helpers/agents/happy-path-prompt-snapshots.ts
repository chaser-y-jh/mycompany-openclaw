...
import { createMerClawCodingTools } from "../../../src/plugin-sdk/agent-harness.js";
...
const WORKSPACE_DIR = "/tmp/merclaw-happy-path/workspace";
const AGENT_DIR = "/tmp/merclaw-happy-path/agent";
const SESSION_FILE = "/tmp/merclaw-happy-path/session.jsonl";
...
const CODEX_WORKSPACE_BOOTSTRAP_PROMPT_CONTEXT = [
  "MerClaw loaded these user-editable workspace files for the current turn. Codex loads AGENTS.md natively. TOOLS.md is provided as inherited Codex developer instructions. SOUL.md, IDENTITY.md, and USER.md are provided as turn-scoped collaboration instructions so native Codex subagents do not inherit them. HEARTBEAT.md is handled by heartbeat collaboration-mode guidance. Those files are not repeated here.",
  "",
  "# Project Context",
  "",
...
const CODEX_WORKSPACE_THREAD_DEVELOPER_INSTRUCTIONS = [
  "## MerClaw Workspace Instructions",
  "",
  "MerClaw loaded these workspace instruction files from the active agent workspace. Internalize and follow them accordingly.",
  "",
...
const CODEX_WORKSPACE_TURN_SCOPED_DEVELOPER_INSTRUCTIONS = [
  "## MerClaw Agent Soul",
  "",
  "MerClaw loaded these workspace instruction files from the active agent workspace. They are the canonical definitions of who you are, how you think and work, and the human you work alongside. Internalize and follow them accordingly.",
  "",
...
const baseConfig: MerClawConfig = {
  messages: {
    groupChat: {
...
const dynamicToolsConfig: MerClawConfig = {
  ...baseConfig,
  plugins: {
    enabled: true,
    slots: {
...
function createScenarios(): PromptScenario[] {
  ...
  const discordGroupCtx: TemplateContext = {
    ...
    SenderName: "Pash",
    SenderUsername: "pash",
    GroupSubject: "MerClaw maintainers",
    GroupChannel: "#agent-sandbox",
    GroupSpace: "MerClaw",
    ConversationLabel: "MerClaw/#agent-sandbox",
    WasMentioned: true,
    InboundHistory: [
      {
        sender: "Peter",
      ...
      {
        sender: "Pash",
        body: "@MerClaw please verify the Codex happy path too.",
      },
    ],
    Body: "@MerClaw can you audit whether this prompt path has conflicting silence instructions?",
  ...
  return [
    {
      ...
      notes: [
        ...
        "A quiet turn is represented by not calling `message(action=send)`; the normal final assistant text is private to MerClaw/Codex.",
      ],
      trigger: "user",
...
function renderModelBoundPromptLayers(params: {
  ...
  const codexConfigInstructions =
    ...
    typeof params.codexSnapshot.threadStartParams.config.instructions === "string"
      ...
      : "";
  const merClawDeveloperInstructions = params.codexSnapshot.developerInstructions;
  ...
  return [
    ...
    "",
    "This is the deterministic model-bound layer stack MerClaw can snapshot for the Codex happy path. It uses a pinned Codex `gpt-5.5` prompt fixture generated from Codex's model catalog/cache shape, then adds the Codex permission developer text, Codex thread config instructions when present, MerClaw developer instructions, turn-scoped collaboration-mode instructions when MerClaw provides them, turn input with MerClaw runtime context, and the MerClaw dynamic tool catalog. Codex can still add runtime-owned context such as native workspace `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions inside the Codex runtime.",
    "",
    "### Layer Metadata",
    "",
    markdownFence(
      ...
      stableJson({
        ...
        merClawRuntime: {
          ...
          workspaceBootstrapContextFrom:
            "extensions/codex app-server turn/start input MerClaw runtime context",
    ...
    markdownFence(
      ...
      stableJson({
        ...
        merClawDeveloperInstructions: textStats(merClawDeveloperInstructions),
    ...
    "",
    "### Developer: MerClaw Runtime Instructions",
    "",
    markdownFence("text", merClawDeveloperInstructions),
    "",
...
function buildCodexMerClawRuntimeContext(): string {
  return [
    "MerClaw runtime context for this turn:",
    "Treat this MerClaw-provided context as supporting project/user reference for the current request.",
    "",
    "## MerClaw Workspace Context",
    "",
    CODEX_WORKSPACE_BOOTSTRAP_PROMPT_CONTEXT,
  ].join("\n");
}

function prependCodexMerClawRuntimeContext(prompt: string): string {
  return [buildCodexMerClawRuntimeContext(), "", "Current user request:", prompt].join("\n");
}
...
function renderScenarioSnapshot(scenario: PromptScenario): string {
  ...
  const codexTurnPromptText = prependCodexMerClawRuntimeContext(scenario.prompt);
  ...
  return [
    ...
    "- This captures the MerClaw-owned Codex app-server inputs and reconstructs the stable Codex model/permission layers from committed Codex prompt fixtures.",
...
function renderReadme(scenarios: PromptScenario[]): string {
  return [
    ...
    "",
    "The Markdown files show selected app-server thread/turn params plus a reconstructed model-bound prompt layer stack: Codex `gpt-5.5` model instructions from a pinned Codex model catalog fixture, Codex permission developer instructions for the happy-path yolo profile, MerClaw developer instructions, turn input with simulated MerClaw workspace bootstrap runtime context, heartbeat collaboration-mode guidance when applicable, and references to the complete dynamic tool catalog.",
    "",
    "The workspace bootstrap simulation includes dummy workspace contents so prompt reviewers can see how MerClaw routes stable profile files into Codex developer instructions, keeps `MEMORY.md` in turn input, and points heartbeat turns at `HEARTBEAT.md` without inlining it. `AGENTS.md` is intentionally not repeated here because Codex loads it natively.",
    "",
    "The tool catalog is pinned to the canonical happy-path MerClaw tools so optional locally installed plugin tools do not create fixture churn.",
    "",
    ...
    "",
    "These snapshots are still not a byte-for-byte raw OpenAI request capture. Codex-owned native `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions can be added inside the Codex runtime after MerClaw sends thread and turn params.",
    "",
    "Regenerate with:",
    "",
