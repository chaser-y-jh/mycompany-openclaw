...
import {
  ...
  type MerClawTestStateOptions,
} from "../../src/test-utils/merclaw-test-state.js";
import { sleep } from "../../src/utils.js";

export type MerClawTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<MerClawTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

export type MerClawTestInstanceCommandResult = {
  code: number | null;
...
export type MerClawTestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  ) => Promise<MerClawTestInstanceCommandResult>;
...
export async function createMerClawTestInstance(
  options: MerClawTestInstanceOptions,
): Promise<MerClawTestInstance> {
  ...
  const instance: MerClawTestInstance = {
    name: options.name,
    port,
