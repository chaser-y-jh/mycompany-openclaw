...
const helperPath = path.resolve("scripts/lib/merclaw-e2e-instance.sh");
const hostPath = [
  ...
  return spawnSync(
    ...
    [
      ...
      [
        ...
        `merclaw_e2e_eval_test_state_from_b64 ${shellQuote(payload)}`,
        'printf "value=%s" "${MERCLAW_E2E_INSTANCE_TEST:-unset}"',
      ].join("; "),
    ],
    { encoding: "utf8" },
  );
...
function writePackageFixture(packagePath: string): void {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-e2e-package-"));
  try {
    const packageDir = path.join(root, "package");
    fs.mkdirSync(packageDir);
    fs.writeFileSync(
      path.join(packageDir, "package.json"),
      JSON.stringify({ name: "merclaw-e2e-fixture", version: "0.0.0" }),
    );
...
function expectNpmInstallObserved(argsPath: string, expectedArgs: string, prefix: string): void {
  ...
  expect(
    fs.existsSync(path.join(prefix, "lib/node_modules/merclaw-e2e-fixture/package.json")),
  ).toBe(true);
}

describe("scripts/lib/merclaw-e2e-instance.sh", () => {
  it("sources decoded test-state scripts", () => {
    const result = runHelper(base64('export MERCLAW_E2E_INSTANCE_TEST="ok"\n'));
  ...
  it("fails when the test-state payload is not valid base64", () => {
    ...
    expect(result.stderr).toContain("Invalid MerClaw test-state base64 payload");
  });
  ...
  it("wraps package installs with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-e2e-instance-"));
    try {
      ...
      const packagePath = path.join(tempDir, "merclaw.tgz");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `merclaw_e2e_install_package ${shellQuote(logPath)} ${shellQuote("fixture package")} ${shellQuote(prefixPath)}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
          env: shellTestEnv({
            ...
            MERCLAW_CURRENT_PACKAGE_TGZ: packagePath,
            MERCLAW_E2E_NPM_INSTALL_TIMEOUT: "42s",
            MERCLAW_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            MERCLAW_TEST_NPM_ARGS: npmArgsPath,
            MERCLAW_TEST_NPM_BIN: path.join(tempDir, "npm"),
          }),
        },
      );
  ...
  it("falls back to plain timeout when kill-after is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-e2e-instance-plain-timeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
  ...
  it("uses gtimeout when GNU timeout is not on PATH", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-e2e-instance-gtimeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
  ...
  it("uses the Node watchdog when timeout is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-e2e-instance-no-timeout-"));
    try {
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
      const packagePath = path.join(tempDir, "merclaw.tgz");
  ...
  it("bounds commands with the Node watchdog when timeout is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-e2e-instance-node-watchdog-"));
    try {
      writeNodeShim(tempDir);
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `merclaw_e2e_maybe_timeout 200ms ${shellQuote(process.execPath)} -e ${shellQuote("setInterval(() => {}, 1000)")}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
      ...
      expect(result.stderr).toContain("MerClaw E2E command timed out after 200ms");
    } finally {
      fs.rmSync(tempDir, { force: true, recursive: true });
    }
  });
  ...
  it("escalates Node watchdog children that ignore parent termination", () => {
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "merclaw-e2e-instance-node-watchdog-signal-"),
    );
    try {
      writeNodeShim(tempDir);
...
set -euo pipefail
source ${shellQuote(helperPath)}
export MERCLAW_E2E_TIMEOUT_KILL_GRACE_MS=100
merclaw_e2e_maybe_timeout 30s node ${shellQuote(childPath)} ${shellQuote(pidPath)} ${shellQuote(watchdogPidPath)} &
wrapper_pid="$!"
...
`;
  ...
  it("bounds HTTP readiness probes when a server accepts connections but never responds", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-e2e-http-probe-"));
    try {
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `source ${shellQuote(helperPath)}`,
            'merclaw_e2e_probe_http_status "http://127.0.0.1:${port}/health" 200 100',
          ].join("; "),
        ],
  ...
  it("wraps logged MerClaw E2E commands with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-e2e-instance-run-logged-"));
    const logLabel = path.basename(tempDir);
    const logPath = `/tmp/merclaw-onboard-${logLabel}.log`;
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `merclaw_e2e_run_logged ${shellQuote(logLabel)} fixture-command one two`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
          env: shellTestEnv({
            ...
            MERCLAW_E2E_COMMAND_TIMEOUT: "17s",
            MERCLAW_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            MERCLAW_TEST_COMMAND_ARGS: commandArgsPath,
            MERCLAW_TEST_COMMAND_BIN: path.join(tempDir, "fixture-command"),
          }),
        },
      );
  ...
  it("wraps package-installed MerClaw CLI calls with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-e2e-instance-merclaw-cli-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const commandArgsPath = path.join(tempDir, "merclaw-args.txt");
      fs.writeFileSync(
        path.join(tempDir, "timeout"),
        [
          "#!/usr/bin/env bash",
          ...
          'if [ "${1:-}" = "--kill-after=1s" ]; then exit 0; fi',
          'printf "%s\\n" "$*" >"$MERCLAW_TEST_TIMEOUT_ARGS"',
          `while [ "$#" -gt 0 ] && [ "$1" != ${shellQuote(path.join(tempDir, "merclaw"))} ]; do shift; done`,
          '[ "$#" -gt 0 ] || exit 127',
          "shift",
          'exec "$MERCLAW_TEST_MERCLAW_BIN" "$@"',
          "",
        ].join("\n"),
      );
      fs.writeFileSync(
        path.join(tempDir, "merclaw"),
        [
          "#!/usr/bin/env bash",
          "set -euo pipefail",
          'printf "%s\\n" "$*" >"$MERCLAW_TEST_COMMAND_ARGS"',
      ...
      fs.chmodSync(path.join(tempDir, "timeout"), 0o755);
      fs.chmodSync(path.join(tempDir, "merclaw"), 0o755);
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: shellTestEnv({
            PATH: `${tempDir}:${hostPath}`,
            MERCLAW_E2E_COMMAND_TIMEOUT: "23s",
            MERCLAW_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            MERCLAW_TEST_COMMAND_ARGS: commandArgsPath,
            MERCLAW_TEST_MERCLAW_BIN: path.join(tempDir, "merclaw"),
          }),
        },
      );
      ...
      expect(result.status).toBe(0);
      expect(fs.readFileSync(timeoutArgsPath, "utf8").trim()).toBe(
        `--kill-after=30s 23s ${path.join(tempDir, "merclaw")} plugins list --json`,
      );
  ...
  it("wraps interactive PTY scripts with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-e2e-instance-pty-timeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `merclaw_e2e_run_script_with_pty ${shellQuote("node /tmp/entry onboard")} ${shellQuote(logPath)}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
          env: shellTestEnv({
            PATH: `${tempDir}:${hostPath}`,
            MERCLAW_E2E_COMMAND_TIMEOUT: "31s",
            MERCLAW_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            MERCLAW_TEST_SCRIPT_ARGS: scriptArgsPath,
            MERCLAW_TEST_SCRIPT_BIN: path.join(tempDir, "script"),
          }),
        },
      );
