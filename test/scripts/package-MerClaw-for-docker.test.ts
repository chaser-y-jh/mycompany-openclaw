...
import {
  ...
  packMerClawPackageForDocker,
  runCommandForTest,
} from "../../scripts/package-merclaw-for-docker.mjs";
...
describe("package-merclaw-for-docker", () => {
  it("uses build-all as the single bounded package artifact build step", async () => {
    ...
    const previousTimeout = process.env.MERCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
    process.env.MERCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = "1234";
    ...
    try {
      await buildPackageArtifacts("/repo", {
        runImpl: async (
          ...
          calls.push({
            ...
            args,
            cwd,
            noPnpm: options.env?.MERCLAW_BUILD_ALL_NO_PNPM,
            skipDts: options.env?.MERCLAW_RUN_NODE_SKIP_DTS_BUILD,
      ...
      if (previousTimeout === undefined) {
        delete process.env.MERCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
      } else {
        process.env.MERCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = previousTimeout;
      }
    }
  ...
  it("trims and restores the changelog around ignore-scripts package artifacts", async () => {
    const calls: string[] = [];
    const tarball = await packMerClawPackageForDocker("/repo", "/out", {
      ...
      runCaptureImpl: async (
        ...
        return "merclaw-2026.5.28.tgz\n";
      },
    });

    expect(tarball).toBe(path.join("/out", "merclaw-2026.5.28.tgz"));
    expect(calls).toEqual([
      "prepare:/repo",
  ...
  it("restores the changelog when ignore-scripts packaging fails", async () => {
    ...
    await expect(
      packMerClawPackageForDocker("/repo", "/out", {
        prepareChangelog: async (cwd: string) => {
          calls.push(`prepare:${cwd}`);
        },
        restoreChangelog: async (cwd: string) => {
  ...
  it("kills timed-out child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-package-timeout-"));
    ...
    try {
      ...
      const parentScript = [
        ...
        "fs.writeFileSync(process.env.MERCLAW_TEST_CHILD_PID, String(child.pid));",
        "process.on('SIGTERM', () => {});",
        "setInterval(() => {}, 1000);",
      ].join("");

      const runPromise = runCommandForTest(process.execPath, ["-e", parentScript], process.cwd(), {
        env: { ...process.env, MERCLAW_TEST_CHILD_PID: childPidPath },
        killAfterMs: 25,
        timeoutMs: 500,
      });
  ...
  it("keeps fallback SIGKILL armed for descendants after the direct child exits", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-package-descendant-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid = 0;
    try {
      const childScript = ["process.on('SIGTERM', () => {});", "setInterval(() => {}, 1000);"].join(
  ...
  it("forwards external termination to active child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-package-signal-"));
    const childPidPath = path.join(tempDir, "child.pid");
    const scriptUrl = pathToFileURL(path.resolve("scripts/package-merclaw-for-docker.mjs")).href;
    let childPid = 0;
    let runnerPid = 0;
    try {
