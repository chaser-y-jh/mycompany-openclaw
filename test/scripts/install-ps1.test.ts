...
describe("install.ps1 failure handling", () => {
  ...
  it("keeps failure termination in the top-level completion handler", () => {
    ...
    expect(completeInstallBody).toMatch(/\bthrow "MerClaw installation failed with exit code/);
  ...
  it("runs npm install through the resolved command with quiet CI defaults", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-MerClaw");
  ...
  it("runs Windows command shims from a Windows-local cwd", () => {
    ...
    const merClawPathBody = extractFunctionBody(source, "Ensure-MerClawOnPath");
    ...
    expect(merClawPathBody).toContain('Invoke-NpmCommand -Arguments @("config", "get", "prefix")');
    ...
    expect(mainBody).toContain('Invoke-NpmCommand -Arguments @("uninstall", "-g", "merclaw")');
  ...
  it("rejects MerClaw GitHub source targets for npm installs", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-MerClaw");
    const sourceTargetBody = extractFunctionBody(source, "Test-MerClawSourcePackageInstallSpec");
    expect(sourceTargetBody).toContain('$normalizedTag -eq "main"');
    expect(sourceTargetBody).toContain("^github:merclaw/merclaw");
    expect(npmInstallBody).toContain("Test-MerClawSourcePackageInstallSpec -RequestedTag $Tag");
    expect(npmInstallBody).toContain("npm installs do not support MerClaw GitHub source targets");
  ...
  it("preserves caller-relative local tarball install specs before safe-cwd npm calls", () => {
    const resolveSpecBody = extractFunctionBody(source, "Resolve-NpmMerClawInstallSpec");
  ...
  it("falls back to a user-local portable Node.js bootstrap when package managers are absent", () => {
    ...
    const depsRootBody = extractFunctionBody(source, "Get-MerClawDepsRoot");
    ...
    expect(depsRootBody).toContain("MerClaw\\deps");
  ...
  it("persists user-local portable Git for future git-backed updates", () => {
    ...
    expect(portableGitRootBody).toContain("Get-MerClawDepsRoot");
  ...
  it("activates the repo-pinned pnpm version for git installs", () => {
    ...
    const ensurePnpmBody = extractFunctionBody(source, "Ensure-Pnpm");
    const gitInstallBody = extractFunctionBody(source, "Install-MerClawFromGit");
    ...
    expect(mainBody).toContain("$gitInstallResults = @(Install-MerClawFromGit");
    expect(mainBody).toContain("Test-BooleanSuccessResult -Results $gitInstallResults");
    expect(mainBody).toContain("$npmInstallResults = @(Install-MerClaw)");
    ...
    expect(gitInstallBody).toContain('Write-Host "[!] MerClaw build did not produce $entryPath"');
  ...
  it("launches interactive onboarding outside Main's captured output", () => {
    const interactiveCommandBody = extractFunctionBody(source, "Invoke-InteractiveMerClawCommand");
    const mainBody = extractFunctionBody(source, "Main");
    ...
    expect(mainBody).toContain("Invoke-InteractiveMerClawCommand onboard");
  });

  runIfPowerShell("exits non-zero when run as a script file", () => {
    const tempDir = harness.createTempDir("merclaw-install-ps1-");
  ...
  runIfPowerShell("throws without killing the caller when run as a scriptblock", () => {
    ...
    expect(result.stdout).toContain("caught=MerClaw installation failed with exit code 1.");
  ...
  runIfPowerShell("treats noisy Git install false as failure", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "function Check-ExistingMerClaw { return $false }",
        "function Get-NpmCommandPath { return $null }",
        "function Install-MerClawFromGit {",
        "  Write-Output 'pnpm stdout before failure'",
        "  return $false",
        "}",
        "function Ensure-MerClawOnPath { throw 'should not continue after failed git install' }",
        "$InstallMethod = 'git'",
        "$GitDir = 'C:\\\\merclaw-test'",
        "$NoOnboard = $true",
        "$result = Main",
  ...
  runIfPowerShell("keeps npm chatter out of Main's success return value", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "function Install-MerClaw { Write-Output 'npm stdout'; return $true }",
        "function Ensure-MerClawOnPath { return $true }",
        "function Refresh-GatewayServiceIfLoaded { }",
        "function Invoke-MerClawCommand { return 'MerClaw test-version' }",
        "$NoOnboard = $true",
        "$result = Main",
