...
const CHECK_SCRIPT = "scripts/check-merclaw-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "merclaw-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "merclaw", version }));
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><merclaw-app></merclaw-app>",
    ...
    const tarball = join(root, "merclaw.tgz");
...
describe("check-merclaw-package-tarball", () => {
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("MerClaw package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
