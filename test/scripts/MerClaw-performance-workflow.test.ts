...
const WORKFLOW = ".github/workflows/merclaw-performance.yml";
...
describe("MerClaw performance workflow", () => {
  it("uses the clawgrit reports token for every report repo push path", () => {
    ...
    expect(prepare.run).toContain(
      'remote add origin "https://x-access-token:${CLAWGRIT_REPORTS_TOKEN}@github.com/merclaw/clawgrit-reports.git"',
    );
    expect(publish.run).toContain(
      'remote set-url origin "https://x-access-token:${CLAWGRIT_REPORTS_TOKEN}@github.com/merclaw/clawgrit-reports.git"',
    );
