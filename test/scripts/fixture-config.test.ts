...
function makeTempRoot(): string {
  return mkdtempSync(path.join(tmpdir(), "merclaw-fixture-config-"));
}
...
function runFixture(
  ...
  return spawnSync(process.execPath, [fixturePath, command, ...args], {
    ...
    env: {
      ...process.env,
      MERCLAW_CONFIG_BATCH_PATH: path.join(root, "batch.json"),
      MERCLAW_CONFIG_PATH: path.join(root, "merclaw.json"),
      MERCLAW_GATEWAY_TOKEN: "test-token",
      MERCLAW_OPENWEBUI_MODEL: "openai/gpt-5.4-mini",
...
describe("scripts/e2e/lib/fixture.mjs config commands", () => {
  ...
  it("rejects loose Open WebUI provider timeout values", () => {
    ...
    try {
      const result = runFixture(root, "openwebui-config", ["test-key"], {
        MERCLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS: "300s",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain("invalid MERCLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS: 300s");
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("writes strict positive Open WebUI provider timeouts into generated config", () => {
    ...
    try {
      const result = runFixture(root, "openwebui-config", ["test-key"], {
        MERCLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS: "300",
      });
