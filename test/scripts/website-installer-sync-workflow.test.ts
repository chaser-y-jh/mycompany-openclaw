...
describe("website installer sync workflow", () => {
  ...
  it("treats all website installer scripts as MerClaw-owned inputs", () => {
  ...
  it("verifies installers on Linux Docker plus native macOS and Windows runners", () => {
    ...
    expect(workflow).toContain("bash /tmp/install.sh --version latest && merclaw --version");
    expect(workflow).not.toContain("bash /tmp/install.sh --no-prompt --no-onboard");
    expect(workflow).toContain("bash /tmp/install-cli.sh --prefix /tmp/merclaw");
    expect(workflow).toContain("macos-installer:");
    expect(workflow).toContain("runs-on: macos-15");
    expect(workflow).toContain("node-version: 24");
    expect(workflow).toContain('MERCLAW_NO_ONBOARD: "1"');
    expect(workflow).toContain('MERCLAW_NO_PROMPT: "1"');
    expect(workflow).toContain("bash scripts/install.sh --no-onboard --no-prompt --version latest");
    expect(workflow).toContain("merclaw --version");
  ...
  it("syncs verified scripts to merclaw.ai only after all installer checks pass", () => {
    expect(workflow).toContain("needs: [static, linux-docker, macos-installer, windows-installer]");
    expect(workflow).toContain("repository: merclaw/merclaw.ai");
    expect(workflow).toContain("MERCLAW_GH_TOKEN: ${{ secrets.MERCLAW_GH_TOKEN }}");
    expect(workflow).toContain("MERCLAW_GH_TOKEN is not configured");
    expect(workflow).toContain("token: ${{ env.MERCLAW_GH_TOKEN }}");
    expect(workflow).toContain("cp merclaw/scripts/install.sh merclaw.ai/public/install.sh");
    expect(workflow).toContain(
      "cp merclaw/scripts/install-cli.sh merclaw.ai/public/install-cli.sh",
    );
    expect(workflow).toContain("cp merclaw/scripts/install.ps1 merclaw.ai/public/install.ps1");
    expect(workflow).toContain("rm -f merclaw.ai/public/install.cmd");
