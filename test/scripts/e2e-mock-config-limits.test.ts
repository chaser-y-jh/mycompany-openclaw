...
const scrubbedEnvKeys = [
  "MOCK_PORT",
  "MOCK_REQUEST_LOG",
  "MERCLAW_CONFIG_RELOAD_LOG_MAX_READ_BYTES",
  "MERCLAW_CONFIG_RELOAD_LOG_PATH",
  "MERCLAW_CONFIG_RELOAD_LOG_TIMEOUT_MS",
  "MERCLAW_MOCK_OPENAI_PORT",
  "RAW_SCHEMA_ERROR",
  "SUCCESS_MARKER",
];
...
describe("e2e mock and config helper numeric limits", () => {
  it("rejects loose mock OpenAI port env values", () => {
    ...
    const fallbackPort = runScript(mockOpenAiPath, {
      MERCLAW_MOCK_OPENAI_PORT: "44080http",
    });
    expect(fallbackPort.status).not.toBe(0);
    expect(fallbackPort.stderr).toContain("invalid MERCLAW_MOCK_OPENAI_PORT: 44080http");
  });
  ...
  it("rejects loose config-reload log timeout env values", () => {
    const result = runScript(configReloadAssertPath, {
      MERCLAW_CONFIG_RELOAD_LOG_TIMEOUT_MS: "30000ms",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid MERCLAW_CONFIG_RELOAD_LOG_TIMEOUT_MS: 30000ms");
  });

  it("rejects loose config-reload log read caps", () => {
    const result = runScript(configReloadAssertPath, {
      MERCLAW_CONFIG_RELOAD_LOG_MAX_READ_BYTES: "256kb",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid MERCLAW_CONFIG_RELOAD_LOG_MAX_READ_BYTES: 256kb");
  });
});
