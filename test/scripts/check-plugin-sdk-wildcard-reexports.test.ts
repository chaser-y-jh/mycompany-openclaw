  ...
  it("flags wildcard re-exports from plugin-sdk subpaths", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export * from "merclaw/plugin-sdk/foo";',
          'export type * from "merclaw/plugin-sdk/bar";',
          'export { named } from "merclaw/plugin-sdk/foo";',
        ].join("\n"),
      ),
    ).toEqual([
      { line: 1, text: 'export * from "merclaw/plugin-sdk/foo";' },
      { line: 2, text: 'export type * from "merclaw/plugin-sdk/bar";' },
    ]);
  });
  ...
  it("allows explicit SDK exports and local wildcard barrels", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export { named } from "merclaw/plugin-sdk/foo";',
          'export type { Named } from "merclaw/plugin-sdk/foo";',
