...
function makeTempDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "merclaw-plugin-lifecycle-measure-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin lifecycle resource sampler", () => {
  it("rejects loose numeric env values instead of parsing prefixes", () => {
    ...
    const result = spawnSync("node", [scriptPath, summary, "invalid-env", "--", "node", "-e", ""], {
      ...
      env: {
        ...process.env,
        MERCLAW_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS: "150ms",
      },
      timeout: 5000,
    });
    ...
    expect(result.stderr).toContain(
      "MERCLAW_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS must be a positive integer; got: 150ms",
    );
  });
  ...
  it("rejects zero lifecycle timeouts instead of disabling the guard", () => {
    ...
    const result = spawnSync("node", [scriptPath, summary, "invalid-env", "--", "node", "-e", ""], {
      ...
      env: {
        ...process.env,
        MERCLAW_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS: "0",
      },
      timeout: 5000,
    });
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain(
      "MERCLAW_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS must be a positive integer; got: 0",
    );
  });
  ...
  it("configures a phase timeout with process-group cleanup", () => {
    ...
    expect(script).toContain("MERCLAW_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS");
    expect(script).toContain("MERCLAW_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS");
    ...
    () => {
      ...
      const result = spawnSync(
        ...
        [scriptPath, summary, "wedged", "--", "node", "-e", "setInterval(() => {}, 1000)"],
        {
          ...
          env: {
            ...process.env,
            MERCLAW_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS: "150",
            MERCLAW_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS: "50",
          },
          timeout: 5000,
        },
      );
        ...
        const result = spawnSync(
          ...
          ],
          {
            ...
            env: {
              ...process.env,
              MERCLAW_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS: "150",
              MERCLAW_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS: "100",
              PID_FILE: pidFile,
            },
            timeout: 5000,
          },
    ...
    try {
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            MERCLAW_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS: "5000",
            MERCLAW_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS: "100",
            PID_FILE: pidFile,
          },
          timeout: 5000,
