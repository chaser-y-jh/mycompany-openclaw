...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/merclaw-pr-maintainer/scripts/github-activity.sh",
);
...
describe("merclaw-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("merclaw/merclaw last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/merclaw\/merclaw\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/merclaw\/merclaw\/commits/g)).toHaveLength(1);
