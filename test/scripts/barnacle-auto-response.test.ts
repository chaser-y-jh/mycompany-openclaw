...
function realBehaviorProofBody(evidence: string, overrides: Record<string, string> = {}) {
  const fields = {
    ...
    environment: "macOS 15.4, Node 24, local MerClaw gateway, redacted Discord token.",
    steps: "pnpm merclaw gateway restart and pnpm merclaw gateway status",
    evidence,
...
describe("barnacle-auto-response", () => {
  ...
  it("does not classify a linked core plugin auto-enable fix as an external plugin candidate", () => {
    const labels = classifyPullRequestCandidateLabels(
      pr(
        ...
        [
          "- Problem: merclaw doctor --fix adds duplicate installed plugin entries",
  ...
  it("does not close automation PRs for the active PR limit", async () => {
    for (const automationPullRequest of [
      { head: { ref: "clawsweeper/merclaw-merclaw-73880" }, login: "app/merclaw-clawsweeper" },
      { headRefName: "clawsweeper/merclaw-merclaw-73880", login: "app/merclaw-clawsweeper" },
      {
        head: { ref: "clownfish/ghcrawl-156993-autonomous-smoke" },
        login: "app/merclaw-clownfish",
      },
      { headRefName: "clownfish/ghcrawl-156993-autonomous-smoke", login: "app/merclaw-clownfish" },
    ]) {
      const { calls, github } = barnacleGithub([]);
    ...
    await runBarnacleAutoResponse({
      ...
      context: barnacleContext(
        ...
        [PROOF_SUPPLIED_LABEL, PROOF_SUFFICIENT_LABEL],
        {
          ...
          sender: { login: "merclaw-clawsweeper[bot]", type: "Bot" },
        },
      ),
      core: {
        info: () => undefined,
  ...
  it("actions manually applied candidate labels", async () => {
    const { calls, github } = barnacleGithub([file("extensions/example/merclaw.plugin.json")]);
  ...
  it("keeps bot-applied candidate labels passive", async () => {
    ...
    await runBarnacleAutoResponse({
      ...
      context: barnacleContext({}, [candidateLabels.externalPluginCandidate], {
        action: "labeled",
        label: { name: candidateLabels.externalPluginCandidate },
        sender: { login: "merclaw-bot[bot]", type: "Bot" },
      }),
      core: {
        info: () => undefined,
      },
