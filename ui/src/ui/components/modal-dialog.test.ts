...
import type { MerClawModalDialog } from "./modal-dialog.ts";
...
function expectShadowElement(modal: MerClawModalDialog, id: string): HTMLElement {
...
describe("merclaw-modal-dialog", () => {
  ...
  it("opens a labelled modal dialog with an optional description", async () => {
    ...
    expect(labelId).toBe("merclaw-modal-dialog-label");
    expect(descriptionId).toBe("merclaw-modal-dialog-description");
    expect(dialog.getRootNode()).toBe(modal.shadowRoot);
    expect(dialog.ownerDocument.querySelector(`#${labelId}`)).toBeNull();
    expect(expectShadowElement(modal, "merclaw-modal-dialog-label").textContent).toBe(
      "Confirm action",
    );
    expect(expectShadowElement(modal, "merclaw-modal-dialog-description").textContent).toBe(
