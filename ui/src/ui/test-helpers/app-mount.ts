...
import "../app.ts";
import type { MerClawApp } from "../app.ts";
...
const mountedApps = new Set<MerClawApp>();

function collectMountedApps() {
  return new Set<MerClawApp>([
    ...mountedApps,
    ...document.querySelectorAll<MerClawApp>("merclaw-app"),
  ]);
}
...
async function waitForAppUpdates(apps: Iterable<MerClawApp>) {
  for (const app of apps) {
    await app.updateComplete;
  }
}

async function drainAppWork(apps: Iterable<MerClawApp>) {
  const snapshot = [...apps];
  await nextMicrotask();
...
export function mountApp(pathname: string) {
  ...
  const app = document.createElement("merclaw-app") as MerClawApp;
  mountedApps.add(app);
...
export function registerAppMountHooks() {
  beforeEach(async () => {
    ...
    window["__MERCLAW_CONTROL_UI_BASE_PATH__"] = undefined;
