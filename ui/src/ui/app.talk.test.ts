...
describe("MerClawApp Talk controls", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("retries Talk immediately when the previous session is already in error state", async () => {
    const { MerClawApp } = await import("./app.ts");
    const app = Object.create(MerClawApp.prototype) as {
      client: unknown;
      connected: boolean;
    ...
    await MerClawApp.prototype.toggleRealtimeTalk.call(app as never);
