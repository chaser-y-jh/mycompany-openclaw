---
summary: "Deploy MerClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run MerClaw on a Kubernetes cluster
  - You want to test MerClaw in a Kubernetes environment
title: "Kubernetes"
---

A minimal starting point for running MerClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

MerClaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/merclaw 18789:18789 -n merclaw
...
```bash
kubectl get secret merclaw-secrets -n merclaw -o jsonpath='{.data.MERCLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: merclaw (configurable via MERCLAW_NAMESPACE)
├── Deployment/merclaw        # Single pod, init container + gateway
├── Service/merclaw           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/merclaw-config  # merclaw.json + AGENTS.md
└── Secret/merclaw-secrets    # Gateway token + API keys
```
...
Edit `merclaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret merclaw-secrets -n merclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/merclaw -n merclaw
```

### Custom namespace

```bash
MERCLAW_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/merclaw/merclaw:latest # or pin to a specific version from https://github.com/merclaw/merclaw/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # merclaw.json + AGENTS.md
