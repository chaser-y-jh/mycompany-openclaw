---
summary: "Host MerClaw on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up MerClaw on Oracle Cloud
  - Looking for free VPS hosting for MerClaw
  - Want 24/7 MerClaw on a small server
title: "Oracle Cloud"
---

Run a persistent MerClaw Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname merclaw
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=merclaw
    ```

    From now on, connect via Tailscale: `ssh ubuntu@merclaw`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    merclaw config set gateway.bind loopback
    merclaw config set gateway.auth.mode token
    merclaw doctor --generate-gateway-token
    merclaw config set gateway.tailscale.mode serve
    merclaw config set gateway.trustedProxies '["127.0.0.1"]'
  ...
  <Step title="Verify">
    ...
    ```
    https://merclaw.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.merclaw` to restrict credential file permissions.
- `merclaw security audit` for an MerClaw-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most MerClaw features work fine; a small number of native binaries need ARM builds:
...
- `~/.merclaw/` — `merclaw.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
- `~/.merclaw/workspace/` — the agent workspace (SOUL.md, memory, artifacts).
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@merclaw
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=merclaw --reset` to re-authenticate.

**Gateway will not start** -- Run `merclaw doctor --non-interactive` and check logs with `journalctl --user -u merclaw-gateway.service -n 50`.
