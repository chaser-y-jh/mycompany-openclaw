---
summary: "Run MerClaw in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want MerClaw isolated from your main macOS environment
...
Run MerClaw in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
1. Install Lume
2. `lume create merclaw --os macos --ipsw latest`
3. Complete Setup Assistant, enable Remote Login (SSH)
4. `lume run merclaw --no-display`
5. SSH in, install MerClaw, configure channels
6. Done
...
```bash
lume create merclaw --os macos --ipsw latest
```
...
```bash
lume stop merclaw
lume run merclaw --no-display
```

The VM runs in the background. MerClaw's daemon keeps the gateway running.

To check status:

```bash
ssh youruser@192.168.64.X "merclaw status"
```
...
This is the killer feature of running on macOS. Use [iMessage](/channels/imessage) with `imsg` to add Messages to MerClaw.

Inside the VM:

1. Sign in to Messages.
2. Install `imsg`.
3. Grant Full Disk Access and Automation permission for the process running MerClaw/`imsg`.
4. Verify RPC support with `imsg rpc --help`.

Add to your MerClaw config:
...
```bash
lume stop merclaw
lume clone merclaw merclaw-golden
```

Reset anytime:

```bash
lume stop merclaw && lume delete merclaw
lume clone merclaw-golden merclaw
lume run merclaw --no-display
```
...
| VM IP not showing        | Wait for VM to fully boot, run `lume get merclaw` again                           |
| Lume command not found   | Add `~/.local/bin` to your PATH                                                    |
| WhatsApp QR not scanning | Ensure you're logged into the VM (not host) when running `merclaw channels login` |
