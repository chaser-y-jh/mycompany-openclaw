---
summary: "Deploy MerClaw on Render with Infrastructure-as-Code"
read_when:
  - Deploying MerClaw to Render
...
Deploy MerClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/merclaw/merclaw)
...
repository configures everything needed to run MerClaw:
...
services:
  - type: web
    ...
    envVars:
      - key: MERCLAW_GATEWAY_PORT
        value: "8080"
      - key: MERCLAW_STATE_DIR
        value: /data/.merclaw
      - key: MERCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: MERCLAW_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means MerClaw state
...
`MERCLAW_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original MerClaw repository, Render will not auto-deploy your MerClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For MerClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with MerClaw state plus any configured
...
- Missing `MERCLAW_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `MERCLAW_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `merclaw backup create` in the Render shell.
