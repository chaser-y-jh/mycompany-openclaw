---
summary: "Optional Docker-based setup and onboarding for MerClaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run MerClaw on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export MERCLAW_IMAGE="ghcr.io/merclaw/merclaw:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/merclaw/merclaw/pkgs/container/merclaw).
  ...
  <Step title="Complete onboarding">
    ...
    `merclaw-gateway` directly. `merclaw-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm merclaw-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm merclaw-cli channels login

    # Telegram
    docker compose run --rm merclaw-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm merclaw-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t merclaw:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node merclaw-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node merclaw-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d merclaw-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `MERCLAW_EXTRA_MOUNTS`
or `MERCLAW_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
...
<Note>
Because `merclaw-cli` shares `merclaw-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d merclaw-gateway`, run onboarding
and setup-time config writes through `merclaw-gateway` with
...
| Variable                                   | Purpose                                                               |
| ------------------------------------------ | --------------------------------------------------------------------- |
| `MERCLAW_IMAGE`                           | Use a remote image instead of building locally                        |
| `MERCLAW_IMAGE_APT_PACKAGES`              | Install extra apt packages during build (space-separated)             |
| `MERCLAW_IMAGE_PIP_PACKAGES`              | Install extra Python packages during build (space-separated)          |
| `MERCLAW_EXTENSIONS`                      | Pre-install plugin dependencies at build time (space-separated names) |
| `MERCLAW_EXTRA_MOUNTS`                    | Extra host bind mounts (comma-separated `source:target[:opts]`)       |
| `MERCLAW_HOME_VOLUME`                     | Persist `/home/node` in a named Docker volume                         |
| `MERCLAW_SANDBOX`                         | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)                |
| `MERCLAW_SKIP_ONBOARDING`                 | Skip the interactive onboarding step (`1`, `true`, `yes`, `on`)       |
| `MERCLAW_DOCKER_SOCKET`                   | Override Docker socket path                                           |
| `MERCLAW_DISABLE_BONJOUR`                 | Disable Bonjour/mDNS advertising (defaults to `1` for Docker)         |
| `MERCLAW_DISABLE_BUNDLED_SOURCE_OVERLAYS` | Disable bundled plugin source bind-mount overlays                     |
...
| `MERCLAW_OTEL_PRELOADED`                  | Skip starting a second OpenTelemetry SDK when one is preloaded        |

The official Docker image does not ship Homebrew. During onboarding, MerClaw
...
`MERCLAW_IMAGE_APT_PACKAGES` during image build. The legacy
`MERCLAW_DOCKER_APT_PACKAGES` name is still accepted.
For Python dependencies, use `MERCLAW_IMAGE_PIP_PACKAGES`. This runs
...
`MERCLAW_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`.
...
```bash
export MERCLAW_EXTENSIONS="diagnostics-otel"
export OTEL_EXPORTER_OTLP_ENDPOINT="http://otel-collector:4318"
export OTEL_SERVICE_NAME="merclaw-gateway"
./scripts/docker/setup.sh
```

Install the official `@merclaw/diagnostics-otel` plugin from ClawHub in
packaged Docker installs before enabling export. Custom source-built images can
still include the local plugin source with
`MERCLAW_EXTENSIONS=diagnostics-otel`. To enable export, allow and enable the
...
`clawhub:@merclaw/diagnostics-prometheus`, enable the
...
```bash
docker compose exec merclaw-gateway node dist/index.js health --token "$MERCLAW_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `MERCLAW_GATEWAY_BIND=lan` so host access to
...
When MerClaw runs in Docker, `127.0.0.1` inside the container is the container
...
`MERCLAW_DISABLE_BONJOUR=1` so the Gateway does not crash-loop or repeatedly
...
Set `MERCLAW_DISABLE_BONJOUR=0` only when running with host networking, macvlan,
...
Docker Compose bind-mounts `MERCLAW_CONFIG_DIR` to `/home/node/.merclaw`,
`MERCLAW_WORKSPACE_DIR` to `/home/node/.merclaw/workspace`, and
`MERCLAW_AUTH_PROFILE_SECRET_DIR` to `/home/node/.config/merclaw`, so those
...
That mounted config directory is where MerClaw keeps:

- `merclaw.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `MERCLAW_GATEWAY_TOKEN`
...
but separate from `MERCLAW_CONFIG_DIR`.

Installed downloadable plugins store their package state under the mounted
MerClaw home, so plugin install records and package roots survive container
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export MERCLAW_SANDBOX=1
    export MERCLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
    ...
    `workspace-write` while the MerClaw sandbox is active; do not mount the
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm merclaw-cli gateway probe
    docker compose run -T --rm merclaw-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `merclaw-cli` uses `network_mode: "service:merclaw-gateway"` so CLI
    ...
    `no-new-privileges` on both `merclaw-gateway` and `merclaw-cli`.
  </Accordion>

  <Accordion title="Docker Desktop DNS failures in merclaw-cli">
    Some Docker Desktop setups fail DNS lookups from the shared-network
    `merclaw-cli` sidecar after `NET_RAW` is dropped, which shows up as
    `EAI_AGAIN` during npm-backed commands such as `merclaw plugins install`.
    ...
    docker compose -f docker-compose.yml -f docker-compose.cli-no-dropped-caps.local.yml run --rm merclaw-cli plugins install <package>
    ```

    If you already created a long-running `merclaw-cli` container, recreate it
  ...
  <Accordion title="Permissions and EACCES">
    ...
    `/home/node/.merclaw`, make sure your host bind mounts are owned by uid 1000:
    ...
    `/path/to/merclaw-config/npm` to `root:root` if you intentionally run
    MerClaw as root long term.
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export MERCLAW_HOME_VOLUME="merclaw_home"`
    2. **Bake system deps**: `export MERCLAW_IMAGE_APT_PACKAGES="git curl jq"`
    3. **Bake Python deps**: `export MERCLAW_IMAGE_PIP_PACKAGES="requests==2.32.5 humanize==4.14.0"`
    4. **Bake Playwright Chromium**: `export MERCLAW_INSTALL_BROWSER=1`
    5. **Or install Playwright browsers into a persisted volume**:
       ```bash
       docker compose run --rm merclaw-cli \
         node /app/node_modules/playwright-core/cli.js install chromium
       ```
    6. **Persist browser downloads**: use `MERCLAW_HOME_VOLUME` or
       `MERCLAW_EXTRA_MOUNTS`. MerClaw auto-detects the Docker image's
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/merclaw/merclaw/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    MerClaw runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm merclaw-cli dashboard --no-open
    docker compose run --rm merclaw-cli devices list
    docker compose run --rm merclaw-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm merclaw-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm merclaw-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping MerClaw up to date
