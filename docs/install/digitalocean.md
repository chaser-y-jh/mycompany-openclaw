---
summary: "Host MerClaw on a DigitalOcean Droplet"
read_when:
  - Setting up MerClaw on DigitalOcean
  - Looking for a simple paid VPS for MerClaw
title: "DigitalOcean"
---

Run a persistent MerClaw Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    # Create the non-root user that will own MerClaw state and services.
    adduser merclaw
    usermod -aG sudo merclaw
    loginctl enable-linger merclaw

    su - merclaw
    merclaw --version
    ```

    Use the root shell only for system bootstrap. Run MerClaw commands as the non-root `merclaw` user so state lives under `/home/merclaw/.merclaw/` and the Gateway installs as that user's systemd service.
  ...
  <Step title="Access the Control UI">
    ...
    **Option C: Tailnet bind (no Serve)**
    ...
    ```bash
    merclaw config set gateway.bind tailnet
    merclaw gateway restart
    ```
...
DigitalOcean snapshots back the whole Droplet up; `merclaw backup create` is portable across hosts.
