...
MerClaw ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/merclaw`, or
  `$MERCLAW_HOME/merclaw` when `MERCLAW_HOME` is set; override with
  `MERCLAW_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
merclaw update --tag 2026.4.1-beta.1
...
merclaw update --tag merclaw@2026.4.1-beta.1
...
- The tag is not persisted. Your next `merclaw update` uses your configured
  channel as usual.
- For package installs, MerClaw pre-packs GitHub/git source specs into a
...
- Downgrade protection: if the target version is older than your current version,
  MerClaw prompts for confirmation (skip with `--yes`).
...
Preview what `merclaw update` would do without making changes:

```bash
merclaw update --dry-run
merclaw update --channel beta --dry-run
merclaw update --tag 2026.4.1-beta.1 --dry-run
merclaw update --dry-run --json
```
...
When you switch channels with `merclaw update`, MerClaw also syncs plugin
sources:
