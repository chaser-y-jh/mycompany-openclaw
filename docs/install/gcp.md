---
summary: "Run MerClaw Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want MerClaw running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent MerClaw Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "MerClaw 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the MerClaw Gateway in Docker
- Persist `~/.merclaw` + `~/.merclaw/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.merclaw` state includes `merclaw.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-merclaw-project --name="MerClaw Gateway"
    gcloud config set project my-merclaw-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create merclaw-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh merclaw-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the MerClaw repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    MERCLAW_IMAGE=merclaw:latest
    MERCLAW_GATEWAY_TOKEN=
    MERCLAW_GATEWAY_BIND=lan
    MERCLAW_GATEWAY_PORT=18789

    MERCLAW_CONFIG_DIR=/home/$USER/.merclaw
    MERCLAW_WORKSPACE_DIR=/home/$USER/.merclaw/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.merclaw
    ```

    Set `MERCLAW_GATEWAY_TOKEN` when you want to manage the stable gateway
    token through `.env`; otherwise configure `gateway.auth.token` before
    relying on clients across restarts. If neither source exists, MerClaw uses
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `MERCLAW_GATEWAY_TOKEN`.
    ...
    services:
      merclaw-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - MERCLAW_GATEWAY_BIND=${MERCLAW_GATEWAY_BIND}
          - MERCLAW_GATEWAY_PORT=${MERCLAW_GATEWAY_PORT}
        ...
        ports:
          ...
          - "127.0.0.1:${MERCLAW_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`MERCLAW_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm merclaw-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh merclaw-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop merclaw-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type merclaw-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start merclaw-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create merclaw-deploy \
     --display-name="MerClaw Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-merclaw-project \
     --member="serviceAccount:merclaw-deploy@my-merclaw-project.iam.gserviceaccount.com" \
