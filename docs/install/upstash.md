---
summary: "Host MerClaw on Upstash Box with keep-alive and SSH tunnel access"
read_when:
  - Deploying MerClaw to Upstash Box
  - You want a managed Linux environment for MerClaw with SSH-tunneled dashboard access
title: "Upstash Box"
---

Run a persistent MerClaw Gateway on Upstash Box, a managed Linux environment
...
Upstash maintains its current MerClaw Box walkthrough at
[MerClaw Setup](https://upstash.com/docs/box/guides/merclaw-setup).

## Connect with an SSH tunnel

Forward the MerClaw dashboard port to your local machine. Use your Box API key
...
```bash
merclaw config set gateway.bind lan
nohup merclaw gateway > gateway.log 2>&1 &
```
...
- [Updating MerClaw](/install/updating)
