---
summary: "Run MerClaw Gateway 24/7 on a cheap Hetzner VPS (Docker) with durable state and baked-in binaries"
read_when:
  - You want MerClaw running 24/7 on a cloud VPS (not your laptop)
  - You want a production-grade, always-on Gateway on your own VPS
  - You want full control over persistence, binaries, and restart behavior
  - You are running MerClaw in Docker on Hetzner or a similar provider
title: "Hetzner"
---

## Goal

Run a persistent MerClaw Gateway on a Hetzner VPS using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "MerClaw 24/7 for ~$5", this is the simplest reliable setup.
...
<Steps>
  ...
  <Step title="Create persistent host directories">
    ...
    ```bash
    mkdir -p /root/.merclaw/workspace

    # Set ownership to the container user (uid 1000):
    chown -R 1000:1000 /root/.merclaw
    ```
  ...
  <Step title="Configure environment variables">
    ...
    MERCLAW_CONFIG_DIR=/root/.merclaw
    MERCLAW_WORKSPACE_DIR=/root/.merclaw/workspace
...
- Infrastructure: [merclaw-terraform-hetzner](https://github.com/andreesg/merclaw-terraform-hetzner)
- Docker config: [merclaw-docker-config](https://github.com/andreesg/merclaw-docker-config)
