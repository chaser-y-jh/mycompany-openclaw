---
summary: "Public MerClaw App SDK for external apps, scripts, dashboards, CI jobs, and IDE extensions"
title: "MerClaw App SDK"
sidebarTitle: "App SDK"
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to MerClaw
...
The **MerClaw App SDK** is the public client API for apps outside the
MerClaw process. Use `@merclaw/sdk` when a script, dashboard, CI job, IDE
...
<Note>
  ...
  `@merclaw/sdk` talks to the Gateway from outside MerClaw.
  `merclaw/plugin-sdk/*` is only for plugins that run inside MerClaw and
  register providers, channels, tools, hooks, or trusted runtimes.
</Note>

## What ships today

`@merclaw/sdk` ships with:

| Surface                   | Status  | What it does                                                                      |
| ------------------------- | ------- | --------------------------------------------------------------------------------- |
| `MerClaw`                | Ready   | Main client entry point. Owns transport, connection, requests, and events.        |
...
`AgentRunParams`, `RunResult`, `RunStatus`, `MerClawEvent`,
`MerClawEventType`, `GatewayEvent`, `MerClawTransport`,
...
```typescript
import { MerClaw } from "@merclaw/sdk";

const oc = new MerClaw({
...
`new MerClaw({ gateway: "ws://..." })` is equivalent to `url`. The
...
For tests, pass an object that implements `MerClawTransport`:
...
The SDK normalizes raw Gateway events into a stable `MerClawEvent` envelope:
...
Task helpers use the durable task ledger that also backs `merclaw tasks`:
...
Use the App SDK when code lives outside MerClaw:
...
Use the Plugin SDK when code runs inside MerClaw:
...
App SDK code should import from `@merclaw/sdk`. Plugin code should import from
documented `merclaw/plugin-sdk/*` subpaths. Do not mix the two contracts.

## Related

- [MerClaw App SDK API design](/reference/merclaw-sdk-api-design)
