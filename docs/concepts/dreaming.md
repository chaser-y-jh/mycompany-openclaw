...
Dreaming is the background memory consolidation system in `memory-core`. It helps MerClaw move strong short-term signals into durable memory while keeping the process explainable and reviewable.
...
<Tabs>
  <Tab title="Promotion preview / apply">
    ```bash
    merclaw memory promote
    merclaw memory promote --apply
    merclaw memory promote --limit 5
...
If `merclaw memory status` reports `Dreaming status: blocked`, the managed cron exists but the default agent heartbeat is not firing. Check that heartbeat is enabled for the default agent and that its target is not `none`, then run `merclaw memory status --deep` again after the next heartbeat interval.
