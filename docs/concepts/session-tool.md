...
MerClaw gives agents tools to work across sessions, inspect status, and
...
After the target responds, MerClaw can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `merclaw-tui` are
not session keys.
...
`subagents` is the visibility helper for already spawned MerClaw
...
the completion origin only identifies a channel MerClaw can still reuse the
