---
summary: "Delegate architecture: running MerClaw as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run MerClaw as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an MerClaw agent that:
...
MerClaw's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of MerClaw's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.merclaw/workspace-delegate",
  tools: {
...
- Cron run history: MerClaw shared SQLite state database
- Session transcripts: `~/.merclaw/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through MerClaw's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
merclaw agents add delegate
```

This creates:

- Workspace: `~/.merclaw/workspace-delegate`
- State: `~/.merclaw/agents/delegate/agent`
- Sessions: `~/.merclaw/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.merclaw/workspace" },
      {
        id: "delegate",
        workspace: "~/.merclaw/workspace-delegate",
        tools: {
...
~/.merclaw/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.merclaw/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.merclaw/workspace-org",
        agentDir: "~/.merclaw/agents/org-assistant/agent",
...
recall view. MerClaw redacts credential/token-like text, truncates long
