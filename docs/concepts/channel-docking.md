---
summary: "Move one MerClaw session's reply route between linked chat channels"
...
Channel docking is call forwarding for one MerClaw session.
...
Alice can message MerClaw on Telegram and Discord:
...
MerClaw keeps the current session context and changes the reply route:
...
4. Keep the same MerClaw session, but receive future replies in Discord.
