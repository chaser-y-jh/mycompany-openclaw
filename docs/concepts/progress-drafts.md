...
read_when:
  ...
  - Explaining how MerClaw updates one channel message while work is in progress
...
When progress drafts are enabled, MerClaw creates one visible work-in-progress
...
That is usually enough. MerClaw will pick an automatic one-word label, wait
...
MerClaw sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from MerClaw's built-in
...
MerClaw renders only the `progress.text` in the channel progress UI. The
...
MerClaw uses the same formatter for progress drafts and `/verbose`:
...
MerClaw truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, MerClaw still suppresses the older standalone
...
When the final answer is ready, MerClaw tries to keep the chat clean:

- If the draft can safely become the final answer, MerClaw edits it in place.
- If the channel uses native progress streaming, MerClaw finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, MerClaw sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, MerClaw keeps the
