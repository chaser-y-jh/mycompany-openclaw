...
Paste this into `merclaw.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, MerClaw can show:
...
case-insensitive. If `allowedChatIds` is non-empty and MerClaw cannot resolve a
...
`config.promptOverride` replaces the default Active Memory prompt. MerClaw
...
5. Verify memory search itself works with `merclaw memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, MerClaw uses OpenAI embeddings. Set
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `merclaw memory status --deep` to inspect the memory-search backend
      and index health.
