...
[Honcho](https://honcho.dev) adds AI-native memory to MerClaw. It persists
...
```bash
merclaw plugins install @honcho-ai/merclaw-honcho
merclaw honcho setup
...
Settings live under `plugins.entries["merclaw-honcho"].config`:
    ...
    entries: {
      "merclaw-honcho": {
        config: {
          ...
          workspaceId: "merclaw", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `merclaw honcho setup` detects and
...
```bash
merclaw honcho setup                        # Configure API key and migrate files
merclaw honcho status                       # Check connection status
merclaw honcho ask <question>               # Query Honcho about the user
merclaw honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/merclaw-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho MerClaw integration guide](https://docs.honcho.dev/v3/guides/integrations/merclaw)
- [Memory](/concepts/memory) -- MerClaw memory overview
