---
summary: "How MerClaw manages conversation sessions"
read_when:
...
MerClaw organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `merclaw security audit`.
...
MerClaw automatically bounds session storage over time. By default, it runs
...
`merclaw sessions cleanup --dry-run --fix-dm-scope`. Applying the same flag
retires those old direct-DM rows and keeps their transcripts as deleted
archives.

Preview with `merclaw sessions cleanup --dry-run`.

## Inspecting sessions

- `merclaw status` -- session store path and recent activity.
- `merclaw sessions --json` -- all sessions (filter with `--active <minutes>`).
