...
read_when:
  - You want MerClaw to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, MerClaw can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. MerClaw may check in afterward.
- You say you are exhausted. MerClaw may ask later whether you slept.
- The agent says it will follow up after something changes. MerClaw may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: MerClaw remembers a
...
```bash
merclaw config set commitments.enabled true
merclaw config set commitments.maxPerDay 3
```

Equivalent `merclaw.json`:
...
After an agent reply, MerClaw may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, MerClaw stores a commitment with:
...
without MerClaw tools.

MerClaw never delivers an inferred commitment immediately after writing it.
...
merclaw commitments dismiss cm_abc123
```

See [`merclaw commitments`](/cli/commitments) for the command reference.
...
Stored commitments are local MerClaw state. They are operational memory, not
long-term memory. Disable the feature with:

```bash
merclaw config set commitments.enabled false
```
...
- Check `merclaw commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`merclaw commitments`](/cli/commitments)
