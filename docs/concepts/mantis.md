---
summary: "Mantis is the visual end-to-end verification system for reproducing MerClaw bugs on live transports, capturing before and after evidence, and attaching artifacts to PRs."
title: "Mantis"
read_when:
  - Building or running live visual QA for MerClaw bugs
...
Mantis is the MerClaw end-to-end verification system for bugs that need a real
...
Mantis lives in the MerClaw QA stack.

- MerClaw owns the scenario runtime, transport adapters, evidence schema, and
  local CLI under `pnpm merclaw qa mantis`.
...
- MerClaw agents drive Mantis through Codex when a scenario needs agentic setup,
  debugging, or stuck-state reporting.

This boundary keeps transport knowledge in MerClaw, machine scheduling in
...
```bash
pnpm merclaw qa mantis discord-smoke \
...
thread, calls MerClaw's `message.thread-reply` action with a repo-local
...
```bash
pnpm merclaw qa mantis desktop-browser-smoke \
...
`MERCLAW_MANTIS_CRABBOX_PROVIDER` when running against another Crabbox fleet.

Useful desktop smoke flags:

- `--lease-id <cbx_...>` or `MERCLAW_MANTIS_CRABBOX_LEASE_ID` reuses a warmed desktop.
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome user-data-dir archive from the named environment variable before launching the browser. Use this for logged-in witnesses such as Discord Web. The default env var is `MERCLAW_MANTIS_BROWSER_PROFILE_TGZ_B64`.
...
- `--keep-lease` or `MERCLAW_MANTIS_KEEP_VM=1` keeps a newly created passing lease open for VNC inspection. Failed runs keep the lease by default when one was created so an operator can reconnect.
...
REST. When `MERCLAW_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also
writes a Discord Web URL artifact. When `MERCLAW_QA_DISCORD_KEEP_THREADS=1` is
...
the VM, runs `pnpm merclaw qa slack` inside that VM, opens Slack Web in the VNC
...
shape where the SUT MerClaw gateway and the browser both live inside the same
Linux desktop VM.

With `--gateway-setup`, the command prepares a persistent disposable MerClaw
home at `$HOME/.merclaw-mantis/slack-merclaw`, patches Slack Socket Mode
configuration for the selected channel, starts `merclaw gateway run` on port
...
- `MERCLAW_LIVE_OPENAI_KEY` for the remote model lane. If only
  `OPENAI_API_KEY` is set locally, Mantis maps it to `MERCLAW_LIVE_OPENAI_KEY`
  before invoking Crabbox so Crabbox's `MERCLAW_*` env forwarding can carry it
  into the VM.
...
channel id, Socket Mode app token, and bot token as the `MERCLAW_MANTIS_SLACK_*`
...
- `--gateway-setup` starts a persistent MerClaw Slack gateway in the VM instead of only running the bot-to-bot QA lane.
...
- `MERCLAW_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome profile inside the VM. The default is `$HOME/.config/merclaw-mantis/slack-chrome-profile`, so a manual Slack Web login survives reruns on the same lease.
...
Crabbox CLI from
`merclaw/crabbox` main so it can use the current desktop/browser lease flags
...
runs `pnpm merclaw qa mantis slack-desktop-smoke --gateway-setup` against that
...
worktree, runs `pnpm merclaw qa telegram --credential-source convex
...
`@merclaw-mantis telegram desktop proof`, from the Actions UI with freeform
...
MerClaw with the leased Telegram SUT bot token, starts `merclaw gateway run`
...
token never logs Telegram Desktop in; it only configures MerClaw. The desktop
...
- `--no-gateway-setup` installs and opens Telegram Desktop without configuring MerClaw.
...
comment or creates one. The workflows publish to `merclaw-crabbox-artifacts`
with public URLs under `https://artifacts.merclaw.ai`. They provide bucket,
...
```text
@merclaw-mantis discord status reactions
```
...
```text
@merclaw-mantis discord status reactions baseline=origin/main candidate=HEAD
```
...
6. Start a child MerClaw Gateway with an isolated state directory.
...
- It exercises a real MerClaw Gateway, Discord bot auth, message dispatch,
...
- `pnpm merclaw qa discord` already runs a live Discord lane with driver and
  SUT bots.
...
- **Headless automation**: default for CI. Chrome runs with CDP enabled, and
  Playwright or MerClaw browser control captures screenshots.
...
- Node 22 and pnpm
- MerClaw checkout and dependency cache
- Playwright Chromium browser cache when Playwright is used
- enough CPU and memory for one MerClaw Gateway, one browser, and one model run
...
- `MERCLAW_QA_DISCORD_MANTIS_BOT_TOKEN`
...
- `MERCLAW_QA_DISCORD_NOTIFY_CHANNEL_ID`
- `MERCLAW_QA_REDACT_PUBLIC_METADATA=1` for public GitHub artifact uploads
- `MERCLAW_QA_CONVEX_SITE_URL`
- `MERCLAW_QA_CONVEX_SECRET_CI`
- `MERCLAW_QA_MANTIS_CRABBOX_COORDINATOR`
- `MERCLAW_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN`
...
`MERCLAW_QA_REDACT_PUBLIC_METADATA=1` for this reason.
