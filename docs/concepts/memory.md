---
summary: "How MerClaw remembers things across sessions"
...
MerClaw remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.merclaw/workspace`).
...
If `MEMORY.md` grows past the bootstrap file budget, MerClaw keeps the file on
...
`merclaw doctor` to see raw vs injected sizes and truncation status.
...
Memory can preserve approval context, but it does not enforce policy. Use MerClaw approval settings, sandboxing, and scheduled tasks for hard operational controls.
...
for that case. MerClaw infers them in a hidden background pass, scopes them to
...
<Info>
MerClaw uses OpenAI embeddings by default. Set
...
Before [compaction](/concepts/compaction) summarizes your conversation, MerClaw
...
```bash
merclaw memory rem-backfill --rollback
merclaw memory rem-backfill --rollback-short-term
```

## CLI

```bash
merclaw memory status          # Check index status and provider
merclaw memory search "query"  # Search from the command line
merclaw memory index --force   # Rebuild the index
```
