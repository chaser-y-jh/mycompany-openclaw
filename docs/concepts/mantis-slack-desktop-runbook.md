...
Linux desktop, VNC rescue, Slack Web, a real MerClaw gateway, screenshots,
...
- Mantis artifacts: owned by the MerClaw run. They live under
...
- remote logs such as `slack-desktop-command.log`, `merclaw-gateway.log`,
...
- `crabbox.remote_run`: sync, browser launch, MerClaw install/build or
...
remote status after Mantis has copied metadata proving that either the MerClaw
