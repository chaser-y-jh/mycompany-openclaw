...
- `/usage cost` in chats: local cost summary aggregated from MerClaw session logs.
- CLI: `merclaw status --usage` prints a full per-provider breakdown.
- CLI: `merclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. MerClaw treats
  ...
  quota, so MerClaw inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, MerClaw prefers the
...
can supply plugin-specific usage auth logic; otherwise MerClaw falls back to
