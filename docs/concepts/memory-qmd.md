...
alongside MerClaw. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, MerClaw falls back to the
...
MerClaw creates a self-contained QMD home under
`~/.merclaw/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- MerClaw creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so MerClaw skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, MerClaw retries with `qmd query`.
- With QMD releases that advertise multi-collection filters, MerClaw groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, MerClaw falls back to the builtin SQLite engine.
  ...
  `merclaw memory status` and one-shot CLI probes still recheck QMD directly.
...
MerClaw keeps the QMD search path compatible with both current and older QMD
installs.

On startup, MerClaw checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, MerClaw searches all
...
Older QMD builds only accept one collection filter. When MerClaw detects one
...
process, so you can tune QMD globally without adding new MerClaw config:
...
collection under `~/.merclaw/agents/<id>/qmd/sessions/`.
...
When scope denies a search, MerClaw logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If MerClaw
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but MerClaw still reports
...
with `merclaw memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs MerClaw uses.

**Many QMD subprocesses during search?** Update QMD if possible. MerClaw uses
...
`memory.qmd.searchMode = "search"`. MerClaw treats that mode as lexical-only,
...
the next startup reconciliation. MerClaw recreates stale managed collections
...
MerClaw's builtin symlink rules. Keep temporary monorepo checkouts under
