...
That shape fixes local bugs, but it leaves MerClaw with too many public
...
That gives MerClaw at-least-once recovery. Exactly-once behavior exists only
...
- Structured MerClaw-origin metadata for operational/system output so visible
...
MerClaw should borrow the vocabulary, not copy the surface.

What MerClaw needs beyond that model:
...
`thread.post()` style promises are not enough for MerClaw. They hide the
...
Origin describes who produced a message and how MerClaw should treat echoes of
that message. It is separate from relation: a message can be a reply to a user
and still be MerClaw-originated operational output.
...
Core owns the meaning of MerClaw-originated output. Channels own how that
...
MerClaw operational output must not be accepted as bot-authored input in shared
...
- **Transport ack:** tells the platform webhook or socket that MerClaw accepted
...
- **Inbound record ack:** confirms MerClaw persisted enough inbound metadata to
...
Before bot authorization, receive must apply the shared MerClaw echo policy
when the channel can decode message origin metadata:

```typescript
function shouldDropMerClawEcho(params: {
  origin?: MessageOrigin;
  isBotAuthor: boolean;
  isRoomish: boolean;
}): boolean {
...
same visible gateway-failure text but without MerClaw origin metadata still
...
middleware chain, but MerClaw persists only the safe completed update id after
...
source if we need platform-level redelivery beyond MerClaw's restart
...
process dies there, MerClaw cannot know whether the platform message exists
...
Before preflight authorization, core must run the shared MerClaw echo predicate
whenever `origin.decode` returns MerClaw-origin metadata. The receive adapter
...
- iMessage monitor delivery records sent messages in an echo cache after a
  ...
  MerClaw can re-ingest its own final replies as inbound user messages.
...
- Direct-DM helpers can have a reply callback that is the only correct transport
  ...
  that callback.
- MerClaw gateway failure output must stay visible to humans, but tagged
...
| Slack           | Send adapter handles normal chat posts. Live adapter chooses native stream when thread shape supports it, otherwise draft preview. Receipts preserve thread timestamps. Origin adapter maps MerClaw gateway failures to Slack `chat.postMessage.metadata` and drops tagged bot-room echoes before `allowBots` authorization.                                  |
...
| Matrix          | Live adapter owns draft event edits, finalization, redaction, encrypted media constraints, and reply-target mismatch fallback. Receive adapter owns encrypted event hydration and dedupe. Origin adapter should encode MerClaw gateway-failure origin into Matrix event content and drop configured-bot room echoes before `allowBots` handling.              |
...
| Google Chat     | Simple receive plus send adapter with thread relation mapped to spaces and thread ids. Audit `allowBots=true` room behavior for tagged MerClaw gateway-failure echoes.                                                                                                                                                                                        |
...
- Persist message origin through durable send intents so recovery, replay, and
  chunked sends preserve MerClaw operational provenance.
...
- Add `merclaw/plugin-sdk/channel-outbound`.
...
- Include `MessageOrigin`, origin encode/decode hooks, and the shared
  `shouldDropMerClawEcho` predicate in the channel-outbound SDK surface.
...
This is where the model stops being "agent replies" and becomes "MerClaw sends
messages".
...
- Gateway-failure origin factory and `shouldDropMerClawEcho` predicate.
...
- Slack tagged MerClaw gateway failure messages stay visible outbound, tagged
...
- Matrix tagged MerClaw gateway-failure room echoes from configured bot
...
- Whether Telegram should eventually replace the grammY runner source with a
  ...
  only MerClaw's persisted restart watermark.
...
- MerClaw-originated gateway failure output is visible to humans but tagged
