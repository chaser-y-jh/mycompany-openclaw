...
An owner is a ClawHub publisher handle, such as `@alice` or `@merclaw`.
...
`@merclaw/dronzer`, it can only be published as `@merclaw`. If you publish as
...
```text
Package scope "@merclaw" must match selected owner "@vintageayu".
Publish as "@merclaw" or rename this package to "@vintageayu/dronzer".
```
...
```sh
clawhub package transfer @opik/opik-merclaw --to opik
```
...
This protects org namespaces. A package named `@merclaw/dronzer` claims the
`@merclaw` namespace, so only publishers with access to the `@merclaw` owner
can publish it.
