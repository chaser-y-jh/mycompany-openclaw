...
MerClaw supports **camera capture** for agent workflows:
...
```bash
merclaw nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
merclaw nodes camera snap --node <id> --facing front
merclaw nodes camera clip --node <id> --duration 3000
merclaw nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`merclaw.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return "Camera disabled by user".

### CLI helper (node invoke)

Use the main `merclaw` CLI to invoke camera commands on the macOS node.

Examples:

```bash
merclaw nodes camera list --node <id>            # list camera ids
merclaw nodes camera snap --node <id>            # prints saved path
merclaw nodes camera snap --node <id> --max-width 1280
merclaw nodes camera snap --node <id> --delay-ms 2000
merclaw nodes camera snap --node <id> --device-id <id>
merclaw nodes camera clip --node <id> --duration 10s          # prints saved path
merclaw nodes camera clip --node <id> --duration-ms 3000      # prints saved path (legacy flag)
merclaw nodes camera clip --node <id> --device-id <id>
merclaw nodes camera clip --node <id> --no-audio
```

Notes:

- `merclaw nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
merclaw nodes screen record --node <id> --duration 10s --fps 15   # prints saved path
```
