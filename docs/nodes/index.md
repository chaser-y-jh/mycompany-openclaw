...
`merclaw nodes …` works against this Mac). In remote gateway mode, browser
automation is handled by the CLI node host (`merclaw node run` or the
...
merclaw devices reject <requestId>
...
`merclaw devices list` before approving.
...
- `node.pair.*` (CLI: `merclaw nodes pending/approve/reject/remove/rename`) is a separate gateway-owned
  node pairing store; it does **not** gate the WS `connect` handshake.
- `merclaw nodes remove --node <id|name|ip>` deletes stale entries from that
...
- **Approvals**: enforced on the node host via `~/.merclaw/exec-approvals.json`.

Approval note:

- Approval-backed node runs bind exact request context.
- For direct shell/runtime file executions, MerClaw also best-effort binds one concrete local
  file operand and denies the run if that file changes before execution.
- If MerClaw cannot identify exactly one concrete local file for an interpreter/runtime command,
...
```bash
merclaw node run --host <gateway-host> --port 18789 --display-name "Build Node"
```
...
export MERCLAW_GATEWAY_TOKEN="<gateway-token>"
merclaw node run --host 127.0.0.1 --port 18790 --display-name "Build Node"
```

Notes:

- `merclaw node run` supports token or password auth.
- Env vars are preferred: `MERCLAW_GATEWAY_TOKEN` / `MERCLAW_GATEWAY_PASSWORD`.
...
- Node-host auth resolution only honors `MERCLAW_GATEWAY_*` env vars.

### Start a node host (service)

```bash
merclaw node install --host <gateway-host> --port 18789 --display-name "Build Node"
merclaw node start
...
If the node retries with changed auth details, re-run `merclaw devices list`
and approve the current `requestId`.

Naming options:

- `--display-name` on `merclaw node run` / `merclaw node install` (persists in `~/.merclaw/node.json` on the node).
- `merclaw nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
```bash
merclaw approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
merclaw approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

Approvals live on the node host at `~/.merclaw/exec-approvals.json`.
...
```bash
merclaw config set tools.exec.host node
merclaw config set tools.exec.security allowlist
merclaw config set tools.exec.node "<id-or-name>"
```
...
```bash
merclaw nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
merclaw nodes canvas snapshot --node <idOrNameOrIp> --format png
merclaw nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas controls

```bash
merclaw nodes canvas present --node <idOrNameOrIp> --target https://example.com
merclaw nodes canvas hide --node <idOrNameOrIp>
merclaw nodes canvas navigate https://example.com --node <idOrNameOrIp>
merclaw nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
merclaw nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
merclaw nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
merclaw nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
merclaw nodes camera list --node <idOrNameOrIp>
merclaw nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
merclaw nodes camera snap --node <idOrNameOrIp> --facing front
```

Video clips (`mp4`):

```bash
merclaw nodes camera clip --node <idOrNameOrIp> --duration 10s
merclaw nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
merclaw nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
merclaw nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
merclaw nodes location get --node <idOrNameOrIp>
merclaw nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
```bash
merclaw nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from MerClaw"}'
```
...
```bash
merclaw nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
merclaw nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
merclaw nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
...
```bash
merclaw nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
merclaw nodes invoke --node <idOrNameOrIp> --command system.which --params '{"name":"git"}'
```
...
- On headless node host, `system.run` is gated by exec approvals (`~/.merclaw/exec-approvals.json`).
...
```bash
merclaw config set tools.exec.node "node-id-or-name"
```
...
```bash
merclaw config unset tools.exec.node
merclaw config unset 'agents.list[0].tools.exec.node'
```
...
MerClaw can run a **headless node host** (no UI) that connects to the Gateway
...
```bash
merclaw node run --host <gateway-host> --port 18789
```

Notes:

- Pairing is still required (the Gateway will show a device pairing prompt).
- The node host stores its node id, token, display name, and gateway connection info in `~/.merclaw/node.json`.
- Exec approvals are enforced locally via `~/.merclaw/exec-approvals.json`
  (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host executes `system.run` locally by default. Set
  `MERCLAW_NODE_EXEC_HOST=app` to route `system.run` through the companion app exec host; add
  `MERCLAW_NODE_EXEC_FALLBACK=0` to require the app host and fail closed if it is unavailable.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `merclaw nodes …` works against this Mac).
