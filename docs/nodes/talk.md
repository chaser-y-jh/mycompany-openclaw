...
- `consultThinkingLevel`: optional thinking level override for the full MerClaw agent run behind realtime `merclaw_agent_consult` calls.
- `consultFastMode`: optional fast-mode override for realtime `merclaw_agent_consult` calls.
...
- `realtime.consultRouting`: `provider-direct` preserves the provider's direct reply when it skips `merclaw_agent_consult`; `force-agent-consult` makes Gateway relay route finalized user transcripts through MerClaw instead.
- `realtime.instructions`: appends provider-facing system instructions to MerClaw's built-in realtime prompt. Use it for voice style and tone; MerClaw keeps the default `merclaw_agent_consult` guidance.
...
- Browser realtime Talk uses `talk.client.toolCall` for `merclaw_agent_consult` instead of exposing `chat.send` to provider-owned browser sessions.
...
- macOS local MLX playback uses the bundled `merclaw-mlx-tts` helper when present, or an executable on `PATH`. Set `MERCLAW_MLX_TTS_BIN` to point at a custom helper binary during development.
