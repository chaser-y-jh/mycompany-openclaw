...
```bash
merclaw nodes describe --node <idOrNameOrIp>
merclaw nodes canvas snapshot --node <idOrNameOrIp>
merclaw logs --follow
```
...
merclaw nodes status
merclaw approvals get --node <idOrNameOrIp>
merclaw approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`merclaw approvals get --node ...`), not in the gateway pairing record.
