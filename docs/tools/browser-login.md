...
When a site requires login, **sign in manually** in the **host** browser profile (the merclaw browser).
...
MerClaw controls a **dedicated Chrome profile** (named `merclaw`, orange-tinted UI). This is separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent should use its isolated `merclaw` browser.
...
merclaw browser open https://x.com
```

If you have multiple profiles, pass `--browser-profile <name>` (the default is `merclaw`).
...
```bash
merclaw browser open https://x.com --browser-profile merclaw
```
