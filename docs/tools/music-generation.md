...
For session-backed agent runs, MerClaw starts music generation as a
...
from the completion reply, MerClaw sends an idempotent direct fallback with
...
<Note>
Not all providers support all parameters. MerClaw still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, MerClaw
...
Provider request timeouts are operator configuration only. MerClaw uses
...
- **Status lookup:** `merclaw tasks list` or `merclaw tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** MerClaw injects an internal completion event back
...
```bash
MERCLAW_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
