...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in MerClaw
...
Today this applies to MerClaw embedded runs and MerClaw dynamic tools in the Codex
app-server harness. Tokenjuice hooks MerClaw's tool-result middleware and
...
```bash
merclaw plugins install clawhub:@merclaw/tokenjuice
```
...
```bash
merclaw config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
merclaw plugins disable tokenjuice
```
