...
MerClaw also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "MerClaw plugin SDK" });
    ```
...
Direct OpenAI Responses models use OpenAI's hosted `web_search` tool automatically when MerClaw web search is enabled and no managed provider is pinned. This is provider-owned behavior in the bundled OpenAI plugin and only applies to native OpenAI API traffic, not OpenAI-compatible proxy base URLs or Azure routes. Set `tools.web.search.provider` to another provider such as `brave` to keep the managed `web_search` tool for OpenAI models, or set `tools.web.search.enabled: false` to disable both managed search and native OpenAI search.

## Native Codex web search

Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of MerClaw's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, MerClaw keeps the normal managed `web_search` behavior.

## Network safety

Managed `web_search` provider calls use MerClaw's guarded fetch path. For
trusted provider API hosts, MerClaw allows Surge, Clash, and sing-box fake-IP
...
If no `provider` is set, MerClaw checks providers in this order and uses the
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, MerClaw resolves only the
...
Grok can also reuse an xAI OAuth auth profile from `merclaw models auth login
...
MerClaw keeps startup resilient and reports a warning so you can reinstall the
plugin or run `merclaw doctor --fix` to clean up the stale config.

`web_fetch` fallback provider selection is separate:

- choose it with `tools.web.fetch.provider`
- or omit that field and let MerClaw auto-detect the first ready web-fetch
...
When you choose **Kimi** during `merclaw onboard` or
`merclaw configure --section web`, MerClaw can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `merclaw doctor --fix`.
When you choose Grok during `merclaw onboard` or `merclaw configure --section web`,
MerClaw can also offer optional `x_search` setup with the same credential.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, MerClaw does not
...
<Tabs>
  <Tab title="Config file">
    Run `merclaw configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.merclaw/.env`.
...
optional structured filters. MerClaw only enables the built-in xAI `x_search`
