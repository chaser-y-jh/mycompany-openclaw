...
MerClaw supports Gemini models with built-in
...
For a gateway install, put env keys in `~/.merclaw/.env`.
...
`pd`, `pw`, `pm`, and `py`. MerClaw converts these values, or an explicit
