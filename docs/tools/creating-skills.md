...
summary: "Build, test, and publish custom SKILL.md workspace skills for your MerClaw agents."
read_when:
...
MerClaw loads skills from several roots in a defined [precedence order](/tools/skills#loading-order).
...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.merclaw/workspace/skills/hello-world
    ```
    ...
    ```bash
    mkdir -p ~/.merclaw/workspace/skills/personal/hello-world
  ...
  <Step title="Verify the skill loaded">
    ...
    MerClaw watches `SKILL.md` files under skills roots by default. If the
  ...
  <Step title="Test it">
    ...
    ```bash
    merclaw agent --message "give me a greeting"
    ```
...
metadata: { "merclaw": { "requires": { "bins": ["gemini"] }, "primaryEnv": "GEMINI_API_KEY" } }
---
```
...
<AccordionGroup>
  <Accordion title="Gating options">
    ...
    | `requires.config` | Each `merclaw.json` path must be truthy |
  ...
  <Accordion title="Environment and API keys">
    Wire an API key to a skill entry in `merclaw.json`:
...
merclaw skills workshop propose-update hello-world \
  --proposal ./PROPOSAL.md \
...
<Steps>
  <Step title="Ensure your SKILL.md is complete">
    Make sure `name`, `description`, and any `metadata.merclaw` gating fields
  ...
  <Step title="Install the ClawHub skill">
    ...
    ```bash
    merclaw skills install clawhub-publish
    ```
...
<Tip>
  ...
  - **Test locally** — use `merclaw agent --message "..."` before sharing.
