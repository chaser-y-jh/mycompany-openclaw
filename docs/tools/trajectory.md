---
summary: "Export redacted trajectory bundles for debugging an MerClaw agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an MerClaw session
...
Trajectory capture is MerClaw's per-session flight recorder. It records a
...
MerClaw writes the bundle under the workspace:

```text
.merclaw/trajectory-exports/merclaw-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.merclaw/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, MerClaw sends the
...
Trajectory capture is on by default for MerClaw agent runs.
...
```json
{
  "traceSchema": "merclaw-trajectory",
  "schemaVersion": 1
}
```
...
| `metadata.json`       | MerClaw version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
MerClaw also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `MERCLAW_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export MERCLAW_TRAJECTORY_DIR=/var/lib/merclaw/trajectories
```

When this variable is set, MerClaw writes one JSONL file per session id in that
directory.
...
Set `MERCLAW_TRAJECTORY=0` before starting MerClaw:

```bash
export MERCLAW_TRAJECTORY=0
```
...
MerClaw flushes runtime trajectory sidecars during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`MERCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting MerClaw:

```bash
export MERCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when MerClaw logs an `merclaw-trajectory-flush` timeout and continues.
It does not change the trajectory size caps. To tune all agent cleanup steps
that do not pass an explicit timeout, set `MERCLAW_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are designed for support and debugging, not public posting.
MerClaw redacts sensitive values before writing export files:
...
- confirm MerClaw was started without `MERCLAW_TRAJECTORY=0`
- check whether `MERCLAW_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.merclaw/trajectory-exports/`
