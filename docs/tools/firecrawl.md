...
MerClaw can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `merclaw configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `merclaw doctor --fix`.
...
when you run Firecrawl yourself. MerClaw accepts `http://` only for loopback,
...
MerClaw always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, MerClaw
