...
MerClaw supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), MerClaw reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.merclaw/.env`.

If you omit `baseUrl`, MerClaw defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, MerClaw defaults to `kimi-k2.6`.
...
MerClaw treats Kimi `web_search` as successful only after Moonshot returns
...
MerClaw returns a structured `kimi_web_search_ungrounded` error instead of
