...
MerClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, MerClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `merclaw configure --section web`. It stores the key in
`~/.merclaw/merclaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.merclaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
