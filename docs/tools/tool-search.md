---
summary: "Tool Search: compact large MerClaw tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want MerClaw agents to use a large tool catalog without adding every tool schema to the prompt
  - You want MerClaw tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for MerClaw runs
---

Tool Search is an experimental MerClaw agent runtime feature. It gives agents one
...
This page documents MerClaw Tool Search. It is not the Codex-native tool
...
When enabled for MerClaw runs, the model receives one `tool_search_code` tool by default.
That tool runs a short JavaScript body in an isolated Node subprocess with an
`merclaw.tools` bridge:

```js
const hits = await merclaw.tools.search("create a GitHub issue");
const tool = await merclaw.tools.describe(hits[0].id);
return await merclaw.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include MerClaw tools, plugin tools, MCP tools, and
...
needs the exact schema, and calls that tool through MerClaw.

Codex harness runs do not receive these experimental MerClaw Tool Search
controls. MerClaw passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the MerClaw embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible MerClaw and plugin tools.
...
6. Expose either the MerClaw code bridge or the structured fallback tools to the
   model.

At execution time every real tool call returns to MerClaw. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`merclaw.tools.call(...)` crosses the bridge back into the Gateway, where the
...
Both modes are experimental. Prefer direct tool exposure for small MerClaw tool
...
catalog includes eligible MerClaw, MCP, and client tools after normal policy
filtering.
...
`merclaw.tools.search(query, options?)`

Searches the effective catalog for the current run. Results are compact and safe
to put back into prompt context.

```js
const hits = await merclaw.tools.search("calendar event", { limit: 5 });
```

`merclaw.tools.describe(id)`

Loads full metadata for one search result, including the exact input schema.

```js
const calendarCreate = await merclaw.tools.describe("mcp:calendar:create_event");
```

`merclaw.tools.call(id, args)`

Calls a selected tool through MerClaw.

```js
await merclaw.tools.call(calendarCreate.id, {
  summary: "Planning",
...
network grants, and no child-process or worker grants. MerClaw enforces a
...
- `merclaw.tools.search`
- `merclaw.tools.describe`
- `merclaw.tools.call`

Normal MerClaw behavior still applies to final calls:
...
Enable Tool Search for MerClaw runs with the default code bridge:

```bash
merclaw config set tools.toolSearch true
```
...
Use the structured fallback tools instead for MerClaw runs:
...
- final tool calls executed through MerClaw
...
- whether the result came from MerClaw, MCP, or a client tool

## E2E validation

The gateway E2E runner proves both paths with the MerClaw runtime:
