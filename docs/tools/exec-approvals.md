...
host-local `ask: "always"` in `~/.merclaw/exec-approvals.json` keeps
...
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `merclaw approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                       |
| `merclaw exec-policy show`                                      | Local-machine merged view.                                                             |
| `merclaw exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals file in one step. |
...
- **Gateway host** → `merclaw` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, MerClaw also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.merclaw/exec-approvals.sock",
...
- `auto` - use allowlist policy, run deterministic matches directly, and send approval misses through MerClaw's native auto reviewer before falling back to a human approval route.
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, MerClaw treats inline code-eval forms as approval-only
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  ...
  MerClaw may attach parser-derived command spans so Web approval
...
**both** policy layers - requested exec policy in MerClaw config
...
- In YOLO mode, MerClaw does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when MerClaw's effective exec
policy is YOLO. For MerClaw-managed Claude live sessions, MerClaw's
...
If you want a more conservative setup, tighten MerClaw exec policy back to
...
<Steps>
  ...
  <Step title="Match the host approvals file">
    ```bash
    merclaw approvals set --stdin <<'EOF'
    {
      version: 1,
      defaults: {
        security: "full",
...
```bash
merclaw exec-policy preset yolo
```

That local shortcut updates both:

- Local `tools.exec.host/security/ask`.
- Local `~/.merclaw/exec-approvals.json` defaults.

It is intentionally local-only. To change gateway-host or node-host
approvals remotely, use `merclaw approvals set --gateway` or
`merclaw approvals set --node <id|name|ip>`.
...
```bash
merclaw approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `merclaw exec-policy` does not synchronize node approvals.
- `merclaw exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `merclaw approvals --node ...`.
...
specific argument shape. MerClaw evaluates the regular expression
...
entries instead of hand-editing the encoded value. If MerClaw cannot
...
edit its local `~/.merclaw/exec-approvals.json` directly.

CLI: `merclaw approvals` supports gateway or node editing - see
...
MerClaw posts the denial back into that session as an internal followup so the
agent can stop waiting on the async command and avoid a missing-result repair.
If there is no session or the session cannot be resumed, MerClaw can still
...
When an async exec approval is denied, MerClaw treats the host command as
...
session delivery is unavailable, MerClaw falls back to a concise operator or
