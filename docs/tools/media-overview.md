...
read_when:
  - Looking for an overview of MerClaw's media capabilities
...
MerClaw generates images, videos, and music, understands inbound media
...
For async tools, MerClaw submits the request to the provider, returns a task
...
MerClaw wakes the agent with the generated media paths so it can tell the
...
MerClaw sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    registered until MerClaw has dedicated provider contracts for those
    categories.
  </Accordion>
  <Accordion title="xAI">
    ...
    not registered in MerClaw until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
