...
read_when:
  - You want MerClaw to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current MerClaw session.
...
MerClaw waits for the next fresh session token snapshot and uses that as the
...
cost reporting, and context-window behavior still use the normal MerClaw
usage and model controls.

## Model tools

MerClaw exposes three core goal tools to agent harnesses:
...
The `/goal` command works in command-capable MerClaw sessions, including the
...
token snapshot yet. Usage refreshes as MerClaw records session usage and
