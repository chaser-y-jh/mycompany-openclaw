...
This is the full reference for `merclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.merclaw/merclaw.json` exists, choose **Keep current values**, **Review and update**, or **Reset before setup**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `merclaw doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though MerClaw now prefers Claude CLI reuse when available.
    ...
    - Auth profiles live in `~/.merclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.merclaw/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.merclaw/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$MERCLAW_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
    is only a legacy import source.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.merclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `merclaw pairing approve <channel> <code>` or use allowlists.
  ...
  <Step title="Web search">
    ...
    - Configure later: `merclaw configure --section web`.
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `merclaw health`.
    - Tip: `merclaw status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
...
- Stores it under `~/.merclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.merclaw/merclaw.json`:
...
`merclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.merclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.merclaw/agents/<agentId>/sessions/`.
