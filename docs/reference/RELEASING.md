...
MerClaw has three public release lanes:
...
- Every stable MerClaw release ships the npm package and macOS app together;
...
6. Run `MerClaw NPM Release` with `preflight_only=true`. Before a tag exists,
...
vYYYY.M.D-beta.N` from the matching `release/YYYY.M.D` branch. The helper runs
   ...
   `MerClaw Release Publish` command only after the evidence bundle is green.
   `MerClaw Release Publish` dispatches the selected or all-publishable plugin
   packages to npm and the same set to ClawHub in parallel, and then promotes the
   prepared MerClaw npm preflight artifact with the matching dist-tag as soon as
   plugin npm publish succeeds.
   After the MerClaw npm publish child succeeds, it creates or updates the
   ...
   `merclaw-<version>-dependency-evidence.zip` for post-release incident
   ...
   evidence as soon as MerClaw npm publish succeeds, waits for ClawHub whenever
   MerClaw npm is being published, then runs `pnpm release:verify-beta` and
   ...
   `merclaw@YYYY.M.D-beta.N` or
   `merclaw@beta` package. If a pushed or published prerelease needs a fix,
...
10. For stable, continue only after the vetted beta or release candidate has the
    ...
    `MerClaw Release Publish`, reusing the successful preflight artifact via
...
- Plugin version sync updates official plugin package versions and existing
  `merclaw.compat.pluginApi` floors to the MerClaw release version by
  ...
  compatible with older MerClaw hosts, keep the floor at the oldest supported
...
- Run the manual `Full Release Validation` workflow before release approval to
  ...
  `MerClaw Release Checks` for install smoke, package acceptance, cross-OS
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof
  ...
  `merclaw@beta`, `merclaw@latest`, or an exact release version; `source=ref`
  ...
  Example: `gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=merclaw@beta -f suite_profile=product -f published_upgrade_survivor_baseline=merclaw@2026.4.26 -f telegram_mode=mock-openai`
  Common profiles:
...
- `MerClaw NPM Release` preflight generates dependency release evidence before
...
- The preflight uploads dependency evidence as
  `merclaw-release-dependency-evidence-<tag>` and also embeds it under
  ...
  to the GitHub release as `merclaw-<version>-dependency-evidence.zip`.
- Run `MerClaw Release Publish` for the mutating publish sequence after the
  tag exists. Dispatch it from `release/YYYY.M.D` (or `main` when publishing a
  main-reachable tag), pass the release tag and successful MerClaw npm
  ...
  workflow serializes plugin npm publish, plugin ClawHub publish, and MerClaw
  npm publish so the core package is not published before its externalized
  plugins.
- Release checks now run in a separate manual workflow:
  `MerClaw Release Checks`
- `MerClaw Release Checks` also runs the QA Lab mock parity lane plus the fast
...
- Cross-OS install and upgrade runtime validation is part of public
  `MerClaw Release Checks` and `Full Release Validation`, which call the
  reusable workflow
  `.github/workflows/merclaw-cross-os-release-checks-reusable.yml` directly
...
- `MerClaw Release Checks` accepts a branch, tag, or full commit SHA as long
  as the resolved commit is reachable from an MerClaw branch or release tag
- `MerClaw NPM Release` validation-only preflight also accepts the current
...
- That workflow runs
  `MERCLAW_LIVE_TEST=1 MERCLAW_LIVE_CACHE_TEST=1 pnpm test:live:cache`
...
- Before tagging a release candidate locally, run
  ...
  UI build, and `release:merclaw:npm:check` in the order that catches common
  approval-blocking mistakes before the GitHub publish workflow starts.
- Run `RELEASE_TAG=vYYYY.M.D node --import tsx scripts/merclaw-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/merclaw-npm-postpublish-verify.ts YYYY.M.D`
  (or the matching beta/correction version) to verify the published registry
  install path in a fresh temp prefix
- After a beta publish, run `MERCLAW_NPM_TELEGRAM_PACKAGE_SPEC=merclaw@YYYY.M.D-beta.N MERCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex MERCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live`
  ...
  `MERCLAW_QA_TELEGRAM_*` env credentials directly.
...
- Maintainer release automation now uses preflight-then-promote:
  ...
  - token-based npm dist-tag mutation now lives in
    `merclaw/releases/.github/workflows/merclaw-npm-dist-tags.yml` because
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=merclaw@YYYY.M.D-beta.N
```

The workflow resolves the target ref, dispatches manual `CI` with
`target_ref=<release-ref>`, dispatches `MerClaw Release Checks`, prepares a
...
`npm_telegram_package_spec` is set. `MerClaw Release
...
`MerClaw Release Checks` uses the trusted workflow ref to resolve the target
...
After a beta is already on npm, set `release_package_spec=merclaw@YYYY.M.D-beta.N`
...
The cross-OS OpenAI install smoke uses `MERCLAW_CROSS_OS_OPENAI_MODEL` when the
...
gh workflow run full-release-validation.yml \
  ...
  -f release_package_spec=merclaw@YYYY.M.D-beta.N \
  -f evidence_package_spec=merclaw@YYYY.M.D-beta.N \
...
required MerClaw dynamic tools drift or disappear from the standard tier
summary.
...
The Docker box lives in `MerClaw Release Checks` through
`merclaw-live-and-e2e-checks-reusable.yml`, plus the release-mode
...
The QA Lab box is also part of `MerClaw Release Checks`. It is the agentic
...
`scripts/resolve-merclaw-package-candidate.mjs`. The resolver normalizes a
...
- `source=npm`: `merclaw@beta`, `merclaw@latest`, or an exact MerClaw release
  version
...
`MerClaw Release Checks` runs Package Acceptance with `source=artifact`, the
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=merclaw@2026.4.26
```
...
`MerClaw Release Publish` is the normal mutating publish entrypoint. It
...
6. Dispatch `MerClaw NPM Release` with the release tag, npm dist-tag, and
...
only for focused repair or republish work. `MerClaw Release Publish` rejects
`plugin_publish_scope=selected` when `publish_merclaw_npm=true` so the core
package cannot ship without every publishable official plugin, including
`@merclaw/diffs-language-pack`. For a selected plugin repair, set
`publish_merclaw_npm=false` with `plugin_publish_scope=selected` and
`plugins=@merclaw/name`, or dispatch the child workflow directly.

## NPM workflow inputs

`MerClaw NPM Release` accepts these operator-controlled inputs:
...
`MerClaw Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `MerClaw NPM Release` preflight run id;
  required when `publish_merclaw_npm=true`
- `npm_dist_tag`: npm target tag for the MerClaw package
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only
  for focused plugin-only repair work with `publish_merclaw_npm=false`
- `plugins`: comma-separated `@merclaw/*` package names when
  `plugin_publish_scope=selected`
- `publish_merclaw_npm`: defaults to `true`; set `false` only when using the
...
`MerClaw Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks
  require the resolved commit to be reachable from an MerClaw branch or
  release tag.
...
- For `MerClaw NPM Release`, full commit SHA input is allowed only when
  `preflight_only=true`
- `MerClaw Release Checks` and `Full Release Validation` are always
  validation-only
...
1. Run `MerClaw NPM Release` with `preflight_only=true`
...
6. Run `MerClaw Release Publish` with the same `tag`, the same `npm_dist_tag`,
   and the saved `preflight_run_id`; it publishes externalized plugins to npm
   and ClawHub before promoting the MerClaw npm package
7. If the release landed on `beta`, use the
   `merclaw/releases/.github/workflows/merclaw-npm-dist-tags.yml`
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/merclaw/merclaw/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/merclaw/merclaw/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/merclaw-npm-release.yml`](https://github.com/merclaw/merclaw/blob/main/.github/workflows/merclaw-npm-release.yml)
- [`.github/workflows/merclaw-release-checks.yml`](https://github.com/merclaw/merclaw/blob/main/.github/workflows/merclaw-release-checks.yml)
- [`.github/workflows/merclaw-cross-os-release-checks-reusable.yml`](https://github.com/merclaw/merclaw/blob/main/.github/workflows/merclaw-cross-os-release-checks-reusable.yml)
- [`scripts/resolve-merclaw-package-candidate.mjs`](https://github.com/merclaw/merclaw/blob/main/scripts/resolve-merclaw-package-candidate.mjs)
- [`scripts/merclaw-npm-release-check.ts`](https://github.com/merclaw/merclaw/blob/main/scripts/merclaw-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/merclaw/merclaw/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/merclaw/merclaw/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in
[`merclaw/maintainers/release/README.md`](https://github.com/merclaw/maintainers/blob/main/release/README.md)
