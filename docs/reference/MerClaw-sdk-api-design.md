---
summary: "Reference design for the public MerClaw App SDK API, event taxonomy, artifacts, approvals, and package structure"
title: "MerClaw App SDK API design"
sidebarTitle: "App SDK API design"
read_when:
  - You are implementing the proposed public MerClaw app SDK
  - You need the draft namespace, event, result, artifact, approval, or security contract for the app SDK
  - You are comparing Gateway protocol resources with the high-level MerClaw App SDK wrapper
---

This page is the detailed API reference design for the public
[MerClaw App SDK](/concepts/merclaw-sdk). It is intentionally separate from
the [Plugin SDK](/plugins/sdk-overview).

<Note>
  `@merclaw/sdk` is the external app/client package for talking to the
  Gateway. `merclaw/plugin-sdk/*` is the in-process plugin authoring contract.
...
2. A high-level ergonomic wrapper with `MerClaw`, `Agent`, `Session`, `Run`,
...
- source: MerClaw, MCP, plugin, channel, runtime, or app
- schema summary
- approval policy
...
| Package                 | Purpose                                                       |
| ----------------------- | ------------------------------------------------------------- |
| `@merclaw/sdk`         | Public high-level SDK and generated low-level Gateway client. |
| `@merclaw/sdk-react`   | Optional React hooks for dashboards and app builders.         |
| `@merclaw/sdk-testing` | Test helpers and fake Gateway server for app integrations.    |

The repo already has `merclaw/plugin-sdk/*` for plugins. Keep that namespace
...
4. High-level `MerClaw`, `Agent`, `Session`, `Run`, `Task`, and `Artifact`
   wrappers.
...
- [MerClaw App SDK](/concepts/merclaw-sdk)
