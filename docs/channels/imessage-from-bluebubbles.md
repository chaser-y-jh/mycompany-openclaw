...
BlueBubbles support was removed. MerClaw supports iMessage through `imsg` only. This guide is for migrating old `channels.bluebubbles` configs to `channels.imessage`; there is no other supported migration path.
...
5. With the Gateway stopped, enable `channels.imessage`, then run `merclaw channels status --probe --channel imessage`.
...
`imsg` is a local macOS CLI for Messages. MerClaw starts `imsg rpc` as a child process and talks JSON-RPC over stdin/stdout. There is no HTTP server, webhook URL, background daemon, launch agent, or port to expose.
...
- Linux builds can inspect a copied `chat.db`, but cannot send, watch the live Mac database, or drive Messages.app. For MerClaw iMessage, run `imsg` on the signed-in Mac or through an SSH wrapper to that Mac.
...
2. Verify the read, watch, send, and RPC surfaces before changing MerClaw config:

   ```bash
   imsg chats --limit 10 --json | jq -s
   imsg history --chat-id 42 --limit 10 --attachments --json | jq -s
   imsg watch --chat-id 42 --reactions --json
   imsg send --chat-id 42 --text "MerClaw imsg test"
   imsg rpc --help
   ```

   Replace `42` with a real chat id from `imsg chats`. Sending requires Automation permission for Messages.app. If MerClaw will run through SSH, run these commands through the same SSH wrapper or user context that MerClaw will use.
...
4. After you add an enabled `channels.imessage` config, verify the bridge through MerClaw:
...
5. Snapshot your config:
   ...
   ```bash
   cp ~/.merclaw/merclaw.json5 ~/.merclaw/merclaw.json5.bak
   ```
...
7. **Remove the BlueBubbles server and config** once iMessage DMs, groups, and actions are verified. MerClaw will not use `channels.bluebubbles`.
...
| Threaded reply (`reply_to_guid`)                           | ✅                                  | ✅ (closes [#51892](https://github.com/merclaw/merclaw/issues/51892))                                                 |
| Tapback (`react`)                                          | ✅                                  | ✅                                                                                                                      |
| Edit / unsend (macOS 13+ recipients)                       | ✅                                  | ✅                                                                                                                      |
| Send with screen effect                                    | ✅                                  | ✅ (closes part of [#9394](https://github.com/merclaw/merclaw/issues/9394))                                           |
...
| Catchup of inbound messages received while gateway is down | ✅ (webhook replay + history fetch) | ✅ (opt-in via `channels.imessage.catchup.enabled`; closes [#78649](https://github.com/merclaw/merclaw/issues/78649)) |
...
The reply cache lives at `~/.merclaw/state/imessage/reply-cache.jsonl` (mode `0600`, parent dir `0700`). It is safe to delete if you want a clean slate.
