...
Nextcloud Talk ships as a bundled plugin in current MerClaw releases, so
...
```bash
merclaw plugins install @merclaw/nextcloud-talk
```
...
```bash
merclaw plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "MerClaw" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure MerClaw:
   ...
   ```bash
   merclaw channels add --channel nextcloud-talk \
...
- Approve via:
  - `merclaw pairing list nextcloud-talk`
  - `merclaw pairing approve nextcloud-talk <CODE>`
