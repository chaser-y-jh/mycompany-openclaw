---
summary: "Synology Chat webhook setup and MerClaw config"
read_when:
  - Setting up Synology Chat with MerClaw
...
Synology Chat ships as a bundled plugin in current MerClaw releases, so normal
...
```bash
merclaw plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   - Current packaged MerClaw releases already bundle it.
   - Older/custom installs can add it manually from a source checkout with the command above.
   - `merclaw onboard` now shows Synology Chat in the same channel setup list as `merclaw channels add`.
   - Non-interactive setup: `merclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your MerClaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in MerClaw.
   - Guided: `merclaw onboard`
   - Direct: `merclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- MerClaw accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `merclaw pairing list synology-chat`
  - `merclaw pairing approve synology-chat <CODE>`
...
```bash
merclaw message send --channel synology-chat --target 123456 --text "Hello from MerClaw"
merclaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
merclaw message send --channel synology-chat --target synology:123456 --text "Short prefix"
```

Media sends are supported by URL-based file delivery.
Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before MerClaw forwards the URL to the NAS webhook.
...
Give each enabled account a distinct `webhookPath`. MerClaw now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached MerClaw
- `Rate limit exceeded`:
