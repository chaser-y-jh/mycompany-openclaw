...
read_when:
  - You want MerClaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables MerClaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current MerClaw releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`merclaw onboard`) and `merclaw channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install the npm package directly.

```bash
merclaw plugins install @merclaw/nostr
```
...
```bash
merclaw plugins install --link <path-to-local-nostr-plugin>
```
...
merclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "merclaw@example.com",
        lud16: "merclaw@example.com",
      },
    },
  },
}
