...
<Note>
Requires MerClaw 2026.5.29 or above. Run `merclaw --version` to check. Upgrade with `merclaw update`.
</Note>

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  merclaw channels login --channel feishu
  ```
...
```bash
merclaw pairing list feishu
merclaw pairing approve feishu <CODE>
```
...
4. Check logs: `merclaw logs --follow`
...
5. Ensure the gateway is running: `merclaw gateway status`
6. Check logs: `merclaw logs --follow`

### QR setup does not react in the Feishu mobile app

1. Rerun setup: `merclaw channels login --channel feishu`
...
3. Restart the gateway: `merclaw gateway restart`
  ...
  channels: {
    feishu: {
      ...
      dynamicAgentCreation: {
        enabled: true,
        workspaceTemplate: "~/.merclaw/workspace-{agentId}",
        agentDirTemplate: "~/.merclaw/agents/{agentId}/agent",
      },
    },
  },
  session: {
...
| `channels.feishu.dynamicAgentCreation.workspaceTemplate` | Path template for dynamic agent workspaces | `~/.merclaw/workspace-{agentId}`    |
| `channels.feishu.dynamicAgentCreation.agentDirTemplate`  | Agent directory name template              | `~/.merclaw/agents/{agentId}/agent` |
...
workspace: /Users/you/.merclaw/workspace-feishu-ou_xxxxxx
...
of raw `file_key` JSON. When `tools.media.audio` is configured, MerClaw
...
conversion fails, MerClaw falls back to a file attachment and logs the reason.
...
topic session key. If a native topic starter event omits `thread_id`, MerClaw
hydrates it from Feishu before routing the turn. Normal group replies that
MerClaw turns into threads keep using the reply root message ID (`om_*`) so the
