...
MerClaw routes replies **back to the channel where a message came from**. The
...
MerClaw infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, MerClaw still records inbound session metadata, but it
...
when a guarded path must not create a new MerClaw session. In that mode,
MerClaw may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when MerClaw would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.merclaw/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.merclaw`):

- `~/.merclaw/agents/<agentId>/sessions/sessions.json`
