...
Tlon is a decentralized messenger built on Urbit. MerClaw connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current MerClaw releases, so normal packaged
...
```bash
merclaw plugins install @merclaw/tlon
```
...
```bash
merclaw plugins install ./path/to/local/tlon-plugin
```
...
By default, MerClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `merclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, MerClaw replies in-thread.
