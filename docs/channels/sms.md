...
read_when:
  - You want to connect MerClaw to SMS through Twilio
  - You need SMS webhook or allowlist setup
title: "SMS"
---

MerClaw can receive and send SMS through a Twilio phone number or Messaging Service. The Gateway registers an inbound webhook route, validates Twilio request signatures by default, and sends replies back through Twilio's Messages API.
...
- A public HTTPS URL that reaches your MerClaw Gateway.
...
```bash
merclaw config patch --file ./sms.patch.json5 --dry-run
merclaw config patch --file ./sms.patch.json5
```
...
```bash
merclaw pairing list sms
merclaw pairing approve sms <CODE>
```
...
`authToken` can be a SecretRef. Use this when the Gateway should resolve the Twilio Auth Token from the MerClaw secrets runtime instead of storing plaintext config:
...
```bash
merclaw message send --channel sms --target sms:+15551234567 --message "hello"
```
...
```bash
merclaw message send --target twilio-sms:+15551234567 --message "hello"
```
...
SMS output is plain text. MerClaw strips markdown, flattens fenced code blocks, preserves readable links, and chunks long replies before sending them through Twilio.
...
```bash
merclaw channels capabilities --channel sms
merclaw channels status --channel sms --probe --json
```

3. Send an SMS to the Twilio number from your phone.
4. Run `merclaw pairing list sms`.
5. Approve the pairing code with `merclaw pairing approve sms <CODE>`.
6. Send another SMS and confirm the agent replies.

For outbound-only testing, use:

```bash
merclaw message send --channel sms --target sms:+15557654321 --message "MerClaw SMS test"
```
...
```bash
imsg send --to "+15551234567" --service sms --text "MerClaw SMS E2E $(date -u +%Y%m%dT%H%M%SZ)" --json
merclaw pairing list sms
merclaw pairing approve sms <CODE>
...
By default, MerClaw validates `X-Twilio-Signature` using `publicWebhookUrl` and `authToken`. Keep `publicWebhookUrl` byte-for-byte aligned with the URL configured in Twilio, including scheme, host, path, and query string.
...
### Twilio returns 403 or MerClaw rejects the webhook
