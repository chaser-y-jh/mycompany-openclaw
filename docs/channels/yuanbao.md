...
Tencent Yuanbao is Tencent's AI assistant platform. The MerClaw channel plugin
connects Yuanbao bots to MerClaw over WebSocket so they can interact with users
...
> **Requires MerClaw 2026.4.10 or above.** Run `merclaw --version` to check. Upgrade with `merclaw update`.

<Steps>
  <Step title="Add the Yuanbao channel with your credentials">
  ```bash
  merclaw channels add --channel yuanbao --token "appKey:appSecret"
  ```
...
```bash
merclaw channels login --channel yuanbao
```
...
```bash
merclaw pairing list yuanbao
merclaw pairing approve yuanbao <CODE>
```
