...
read_when:
  - You want to connect MerClaw to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to MerClaw via the LINE Messaging API. The plugin runs as a webhook
...
```bash
merclaw plugins install @merclaw/line
```

Local checkout (when running from a git repo):

```bash
merclaw plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so MerClaw applies strict pre-auth body limits and timeout before verification.
- MerClaw processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
merclaw pairing list line
merclaw pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.merclaw/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. MerClaw validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
