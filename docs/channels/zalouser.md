...
read_when:
  - Setting up Zalo Personal for MerClaw
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside MerClaw.
...
Zalo Personal ships as a bundled plugin in current MerClaw releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install the npm package directly:

- Install via CLI: `merclaw plugins install @merclaw/zalouser`
- Pinned version: `merclaw plugins install @merclaw/zalouser@2026.5.2`
- Or from a source checkout: `merclaw plugins install ./path/to/local/zalouser-plugin`
...
- `merclaw pairing list zalouser`
- `merclaw pairing approve zalouser <code>`
...
- On startup, MerClaw resolves group/user names in allowlists to IDs and logs the mapping only when `channels.zalouser.dangerouslyAllowNameMatching: true` is enabled.
...
- When a group message is skipped because mention is required, MerClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in MerClaw state. Example:
...
Profile names select the saved Zalo login credentials in MerClaw state. Resolution order is:
...
- MerClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, MerClaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `merclaw channels logout --channel zalouser && merclaw channels login --channel zalouser`
...
- The channel now runs fully in MerClaw without external CLI binaries.
