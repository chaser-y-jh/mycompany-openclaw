...
CLI onboarding is the **recommended** way to set up MerClaw on macOS,
...
`MERCLAW_LOCALE`, then `LC_ALL`, then `LC_MESSAGES`, then `LANG`, and falls
...
Names and stable identifiers stay literal: `MerClaw`, `Gateway`, `Tailscale`,
commands, config keys, URLs, provider IDs, model IDs, and plugin/channel labels
are not translated.

<Info>
Fastest first chat: open the Control UI (no channel setup needed). Run
`merclaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`merclaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.merclaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `merclaw doctor` first.
</Note>
...
Use `merclaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.merclaw/workspace-<agentId>`.
...
- CLI command reference: [`merclaw onboard`](/cli/onboard)
