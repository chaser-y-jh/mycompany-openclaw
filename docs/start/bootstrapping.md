...
On the first agent run, MerClaw bootstraps the workspace (default
`~/.merclaw/workspace`):
...
For embedded/local model runs, MerClaw keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `merclaw onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.merclaw/workspace`).
</Note>
