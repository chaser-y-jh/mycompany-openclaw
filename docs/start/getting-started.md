---
summary: "Get MerClaw installed and run your first chat in minutes."
read_when:
...
Install MerClaw, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.merclaw/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.merclaw/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run MerClaw as a service account or want custom paths:

- `MERCLAW_HOME` — home directory for internal path resolution
- `MERCLAW_STATE_DIR` — override the state directory
- `MERCLAW_CONFIG_PATH` — override the config file path
