---
summary: "Scripted onboarding and agent setup for the MerClaw CLI"
read_when:
...
Use `--non-interactive` to automate `merclaw onboard`.
...
<AccordionGroup>
  ...
  <Accordion title="Custom provider example">
    ...
    MerClaw marks common vision model IDs as image-capable automatically. Add `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
Anthropic setup-token remains available as a supported onboarding token path, but MerClaw now prefers Claude CLI reuse when available.
