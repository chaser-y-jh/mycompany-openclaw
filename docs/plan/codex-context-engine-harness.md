...
summary: "Specification for making the bundled Codex app-server harness honor MerClaw context-engine plugins"
read_when:
  ...
  - You are comparing embedded MerClaw and Codex app-server context behavior
---
...
Make the bundled Codex app-server harness honor the same MerClaw context-engine
lifecycle contract that embedded MerClaw turns already honor.
...
MerClaw-level compaction policy as far as the Codex app-server boundary allows.
...
as built-in MerClaw attempts:

- `extensions/codex/src/app-server/run-attempt.ts`

That means the required hook point is in MerClaw-controlled code. The external
boundary is the Codex app-server protocol itself: MerClaw can control what it
...
Built-in MerClaw attempts call the context-engine lifecycle directly:
...
For Codex harness turns, MerClaw should preserve this lifecycle:

1. Read the mirrored MerClaw session transcript.
...
8. Mirror the Codex result back into the MerClaw transcript.
...
11. Preserve Codex native compaction signals and MerClaw compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. MerClaw should
not try to mutate the app-server's internal history except through supported
protocol calls.

MerClaw's transcript mirror remains the source for MerClaw features:
...
The context-engine interface returns MerClaw `AgentMessage[]`, not a Codex
...
- Label it clearly as MerClaw assembled context.
...
```text
MerClaw assembled context for this turn:
...
inside MerClaw and preserves context-engine semantics.
...
we need strict MerClaw parity, run context-engine assembly before hook
...
1. MerClaw context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit MerClaw compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
MerClaw compaction should prefer the context engine's `compact()` result for
the MerClaw transcript mirror and plugin state.
...
the MerClaw session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
MerClaw session lifecycle paths. Do not add Codex-specific cleanup unless the
context-engine lifecycle currently misses reset/delete events for all harnesses.

### 10. Error handling

Follow built-in MerClaw semantics:
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole MerClaw compaction when the context engine is primary.
...
Avoid requiring lossless-claw in MerClaw core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for MerClaw/plugin state.
