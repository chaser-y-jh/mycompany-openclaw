---
summary: "Use Anthropic Claude via API keys or Claude CLI in MerClaw"
read_when:
  - You want to use Anthropic models in MerClaw
title: "Anthropic"
---

Anthropic builds the **Claude** model family. MerClaw supports two auth routes:
...
<Warning>
MerClaw's Claude CLI backend runs the installed Claude Code CLI in
    ...
    **Best for:** standard API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        merclaw onboard --anthropic-api-key "$ANTHROPIC_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        merclaw models list --provider anthropic
        ```
      </Step>
    </Steps>
    ...
    **Best for:** reusing an existing Claude CLI login without a separate API key.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        MerClaw detects and reuses the existing Claude CLI credentials.
      </Step>
      <Step title="Verify the model is available">
        ```bash
        merclaw models list --provider anthropic
    ...
    <Warning>
    Claude CLI reuse expects the MerClaw process to run on the same host as the
    ...
    there, or choose a provider with MerClaw-managed OAuth such as
    ...
    MerClaw uses Claude Code's non-interactive `claude -p` path for Claude CLI
    ...
    MerClaw release. Check `claude auth status`, `/status`, and
    ...
    Claude CLI. MerClaw also supports subscription-style options from
...
Claude Opus 4.8 keeps thinking off by default in MerClaw. When you explicitly enable adaptive thinking with `/think high|xhigh|max`, MerClaw sends Anthropic's Opus 4.8 effort values; Claude 4.6 models default to `adaptive`.
...
MerClaw supports Anthropic's prompt caching feature for API-key auth.
...
<AccordionGroup>
  <Accordion title="Fast mode">
    MerClaw's shared `/fast` toggle supports direct Anthropic traffic (API-key and OAuth to `api.anthropic.com`).
  ...
  <Accordion title="Media understanding (image and PDF)">
    The bundled Anthropic plugin registers image and PDF understanding. MerClaw
    ...
    When an image or PDF is attached to a conversation, MerClaw automatically
  ...
  <Accordion title="1M context window">
    ...
    such as Opus 4.8, Opus 4.7, Opus 4.6, and Sonnet 4.6. MerClaw sizes those models at
    1M automatically:
    ...
    Older configs can keep `params.context1m: true`, but MerClaw no longer sends
    ...
    <Warning>
    Requires long-context access on your Anthropic credential. OAuth/subscription token auth keeps its required Anthropic beta headers, but MerClaw strips the retired 1M beta header if it remains in older config.
    </Warning>
...
<AccordionGroup>
  ...
  <Accordion title='No API key found for provider "anthropic"'>
    Anthropic auth is **per agent** — new agents do not inherit the main agent's keys. Re-run onboarding for that agent (or configure an API key on the gateway host), then verify with `merclaw models status`.
  </Accordion>

  <Accordion title='No credentials found for profile "anthropic:default"'>
    Run `merclaw models status` to see which auth profile is active. Re-run onboarding, or configure an API key for that profile path.
  </Accordion>

  <Accordion title="No available auth profile (all in cooldown)">
    Check `merclaw models status --json` for `auth.unusableProfiles`. Anthropic rate-limit cooldowns can be model-scoped, so a sibling Anthropic model may still be usable. Add another Anthropic profile or wait for cooldown.
  </Accordion>
</AccordionGroup>
