---
summary: "Use Synthetic's Anthropic-compatible API in MerClaw"
read_when:
...
MerClaw registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    merclaw onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
MerClaw's Anthropic client appends `/v1` to the base URL automatically, so use
...
`merclaw models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that MerClaw appends `/v1` automatically.
