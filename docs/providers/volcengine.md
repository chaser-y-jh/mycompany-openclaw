...
read_when:
  - You want to use Volcano Engine or Doubao models with MerClaw
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    merclaw onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    merclaw models list --provider volcengine
    merclaw models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
Then enable it in `merclaw.json`:
...
For voice-note targets, MerClaw asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `merclaw onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, MerClaw falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.merclaw/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running MerClaw as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about MerClaw setup.
  </Card>
</CardGroup>
