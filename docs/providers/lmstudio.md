---
summary: "Run MerClaw with LM Studio"
read_when:
  - You want to run MerClaw with open source models via LM Studio
...
If LM Studio authentication is disabled, you can leave the API key blank during interactive MerClaw setup.
...
```bash
merclaw models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys follow a `author/model-name` format (e.g. `qwen/qwen3.5-9b`). MerClaw
...
For unauthenticated LM Studio servers, omit the key; MerClaw stores a local non-secret marker.
...
`usage` object, MerClaw recovers token counts from llama.cpp-style
...
options, MerClaw exposes the matching OpenAI-compatible `reasoning_effort`
...
on `/v1/chat/completions`; MerClaw normalizes that binary discovery shape to
...
LM Studio supports just-in-time (JIT) model loading, where models are loaded on first request. MerClaw preloads models through LM Studio's native load endpoint by default, which helps when JIT is disabled. To let LM Studio's JIT, idle TTL, and auto-evict behavior own model lifecycle, disable MerClaw's preload step:
