---
summary: "Use Mistral models and Voxtral transcription with MerClaw"
read_when:
  - You want to use Mistral models in MerClaw
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

MerClaw includes a bundled Mistral plugin that registers four contracts: chat completions, media understanding (Voxtral batch transcription), realtime STT for Voice Call (Voxtral Realtime), and memory embeddings (`mistral-embed`).
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    merclaw onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    merclaw onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    merclaw models list --provider mistral
    ```
  </Step>
</Steps>
...
MerClaw currently ships this bundled Mistral catalog:
...
```bash
merclaw models list --all --provider mistral --plain
```
...
<Note>
MerClaw defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning">
    ...
    MerClaw maps the session **thinking** level to Mistral's API:

    | MerClaw thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
    ...
    answers, turn thinking off/minimal so MerClaw sends
