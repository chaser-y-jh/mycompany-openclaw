---
summary: "Use NovitaAI's OpenAI-compatible API with MerClaw"
read_when:
  - You want to run MerClaw with NovitaAI models
...
API. In MerClaw it is a bundled model provider, so the provider id is
...
This provider uses Novita's OpenAI-compatible endpoint. MerClaw handles
...
```bash
merclaw onboard --auth-choice novita-api-key
```
...
The catalog is a starting point for MerClaw model selection. Your account,
...
```bash
merclaw models list --provider novita
```

## Troubleshooting

- `401` or `403`: verify the key in Novita's key management page and re-run
  `merclaw onboard --auth-choice novita-api-key` if the stored profile is
  stale.
