---
summary: "Use Ollama Cloud directly with MerClaw"
read_when:
...
Ollama Cloud is Ollama's hosted model API. It lets MerClaw call Ollama-hosted
...
`/api/chat` style, not the OpenAI-compatible `/v1` route. MerClaw registers it
...
- You want the same native Ollama chat API shape MerClaw uses for local
...
MerClaw discovers Ollama Cloud models from the live hosted catalog. Commonly
...
```bash
merclaw models list --provider ollama-cloud
merclaw models set ollama-cloud/kimi-k2.6
```
...
MERCLAW_LIVE_OLLAMA_MODEL=kimi-k2.6 \
...
- Unknown model errors: run `merclaw models list --provider ollama-cloud` and
