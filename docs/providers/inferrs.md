---
summary: "Run MerClaw through inferrs (OpenAI-compatible local server)"
read_when:
  - You want to run MerClaw against a local inferrs server
  - You are serving Gemma or another model through inferrs
  - You need the exact MerClaw compat flags for inferrs
title: "Inferrs"
---

[inferrs](https://github.com/ericcurtin/inferrs) can serve local models behind an OpenAI-compatible `/v1` API. MerClaw works with `inferrs` through the generic `openai-completions` path.

| Property           | Value                                                              |
| ------------------ | ------------------------------------------------------------------ |
| Provider id        | `inferrs` (custom; configure under `models.providers.inferrs`)     |
| Plugin             | none — `inferrs` is not a bundled MerClaw provider plugin         |
...
<Note>
  `inferrs` is currently best treated as a custom self-hosted OpenAI-compatible backend, not a dedicated MerClaw provider plugin. You configure it through `models.providers.inferrs` rather than an onboarding choice flag. If you need a true bundled plugin with auto-discovery, see [SGLang](/providers/sglang) or [vLLM](/providers/vllm).
</Note>
...
<Steps>
  ...
  <Step title="Add an MerClaw provider entry">
...
Inferrs can also be started by MerClaw only when an `inferrs/...` model is
...
<AccordionGroup>
  <Accordion title="Why requiresStringContent matters">
    ...
    <Warning>
    If MerClaw runs fail with an error like:
    ...
    MerClaw will flatten pure text content parts into plain strings before sending
    the request.
  ...
  <Accordion title="Gemma and tool-schema caveat">
    ...
    `/v1/chat/completions` requests but still fail on full MerClaw agent-runtime
    turns.
    ...
    That disables MerClaw's tool schema surface for the model and can reduce prompt
    pressure on stricter local backends.

    If tiny direct requests still work but normal MerClaw agent turns continue to
    crash inside `inferrs`, the remaining issue is usually upstream model/server
    behavior rather than MerClaw's transport layer.
...
<AccordionGroup>
  ...
  <Accordion title="Direct /v1/chat/completions calls pass but merclaw infer model run fails">
  ...
  <Accordion title="inferrs still crashes on larger agent turns">
    If MerClaw no longer gets schema errors but `inferrs` still crashes on larger
...
<CardGroup cols={2}>
  <Card title="Local models" href="/gateway/local-models" icon="server">
    Running MerClaw against local model servers.
  </Card>
