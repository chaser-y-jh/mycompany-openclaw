...
MerClaw sources most Go catalog rows from the bundled MerClaw model registry and
supplements current upstream rows while the registry catches up. Run
`merclaw models list --provider opencode-go` for the current model list.
...
<AccordionGroup>
  <Accordion title="Routing behavior">
    MerClaw handles per-model routing automatically when the model ref uses
