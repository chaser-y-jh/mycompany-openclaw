---
summary: "Use Xiaomi MiMo pay-as-you-go and Token Plan models with MerClaw"
read_when:
  - You want Xiaomi MiMo models in MerClaw
  - You need Xiaomi MiMo auth or Token Plan setup
title: "Xiaomi MiMo"
---

Xiaomi MiMo is the API platform for **MiMo** models. MerClaw includes a bundled Xiaomi plugin with two text-provider presets:
...
<Steps>
  ...
  <Step title="Run onboarding">
    ...
    ```bash
    merclaw onboard --auth-choice xiaomi-api-key
    ```

    Token Plan:

    ```bash
    merclaw onboard --auth-choice xiaomi-token-plan-sgp
    ```

    Or pass the keys directly:

    ```bash
    merclaw onboard --auth-choice xiaomi-api-key --xiaomi-api-key "$XIAOMI_API_KEY"
    merclaw onboard --auth-choice xiaomi-token-plan-sgp --xiaomi-token-plan-api-key "$XIAOMI_TOKEN_PLAN_API_KEY"
    ```

  </Step>
  <Step title="Verify the model is available">
    ```bash
    merclaw models list --provider xiaomi
    merclaw models list --provider xiaomi-token-plan
    ```
  </Step>
</Steps>
...
MerClaw sends `speakerVoice` for `mimo-v2.5-tts` and `mimo-v2-tts`.
...
`style` with the desired voice description; MerClaw sends it as the `user`
...
For voice-note targets such as Feishu and Telegram, MerClaw transcodes Xiaomi
...
<AccordionGroup>
  ...
  <Accordion title="Troubleshooting">
    ...
    - When the Gateway runs as a daemon, ensure the key is available to that process (for example in `~/.merclaw/.env` or via `env.shellEnv`).

    <Warning>
    Keys set only in your interactive shell are not visible to daemon-managed gateway processes. Use `~/.merclaw/.env` or `env.shellEnv` config for persistent availability.
    </Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full MerClaw configuration reference.
  </Card>
