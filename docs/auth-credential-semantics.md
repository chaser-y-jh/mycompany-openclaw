...
Explicit copy flows, such as `merclaw agents add`, use this portability policy:
...
has such a marker, `merclaw doctor --fix` moves it to `auth.profiles` and
...
- Probe targets can come from auth profiles, environment credentials, or
  `models.json`.
- If a provider has credentials but MerClaw cannot resolve a probeable model
