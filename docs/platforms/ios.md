...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `merclaw.internal.`), **or**
...
Run `merclaw devices list` again before approval.
...
merclaw gateway call node.list --params "{}"
```
...
By default, official/TestFlight builds and gateways use the hosted relay at `https://ios-push-relay.merclaw.ai`.
...
- `MERCLAW_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
- `MERCLAW_PUSH_RELAY_BASE_URL` still works as a temporary env override for official/TestFlight iOS builds.
...
- Only genuine MerClaw iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight MerClaw builds.
...
```bash
export MERCLAW_APNS_TEAM_ID="TEAMID"
export MERCLAW_APNS_KEY_ID="KEYID"
export MERCLAW_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.merclaw/credentials/apns
chmod 700 ~/.merclaw/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.merclaw/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.merclaw/credentials/apns/AuthKey_KEYID.p8
export MERCLAW_APNS_PRIVATE_KEY_PATH="$HOME/.merclaw/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_merclaw-gw._tcp` on `local.` and, when configured, the same
...
`merclaw.internal.`) and Tailscale split DNS.
...
```bash
merclaw nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__merclaw__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__merclaw__/canvas/` and `/__merclaw__/a2ui/`.
...
tools; the iOS app exposes iPhone capabilities through MerClaw node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through MerClaw by invoking node
...
```bash
merclaw nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__merclaw; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
merclaw nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `merclaw devices list` and approve manually.
