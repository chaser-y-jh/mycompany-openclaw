---
summary: "MerClaw macOS companion app (menu bar + gateway broker)"
read_when:
...
The macOS app is the **menu-bar companion** for MerClaw. It owns permissions,
...
- Installs the global CLI (`merclaw`) on request via npm, pnpm, or bun (the app prefers npm, then pnpm, then bun; Node remains the recommended Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `merclaw gateway install`.
...
The app manages a per-user LaunchAgent labeled `ai.merclaw.gateway`
(or `ai.merclaw.<profile>` when using `--profile`/`MERCLAW_PROFILE`; legacy `com.merclaw.*` still unloads).
...
Replace the label with `ai.merclaw.<profile>` when running a named profile.
...
The app registers the `merclaw://` URL scheme for local actions.
...
```bash
open 'merclaw://agent?message=Hello%20from%20deep%20link'
```
...
1. Install and launch **MerClaw.app**.
...
Avoid putting your MerClaw state dir in iCloud or other cloud-synced folders.
...
If `merclaw doctor` detects state under:
...
```bash
cd apps/macos
swift run merclaw-mac connect --json
swift run merclaw-mac discover --timeout 3000 --json
```
...
<Tip>
Compare against `merclaw gateway discover --json` to see whether the macOS app's discovery pipeline (`local.` plus the configured wide-area domain, with wide-area and Tailscale Serve fallbacks) differs from the Node CLI's `dns-sd` based discovery.
</Tip>
