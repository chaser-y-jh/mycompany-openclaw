...
service via the external `merclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.merclaw.gateway`
  (or `ai.merclaw.<profile>` when using `--profile`/`MERCLAW_PROFILE`; legacy `com.merclaw.*` is supported).
