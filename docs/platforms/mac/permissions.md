...
- Same path: run the app from a fixed location (for MerClaw, `dist/MerClaw.app`).
...
Prefer granting Accessibility to MerClaw.app, Peekaboo.app, or another signed
...
```bash
sudo tccutil reset Accessibility ai.merclaw.mac
sudo tccutil reset ScreenCapture ai.merclaw.mac
...
Workaround: move files into the MerClaw workspace (`~/.merclaw/workspace`) if you want to avoid per-folder grants.
