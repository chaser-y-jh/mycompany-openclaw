...
2. `npm i -g merclaw@latest`
3. `merclaw onboard --install-daemon`
...
MerClaw installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `merclaw gateway install` and
`merclaw onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/merclaw-gateway[-<profile>].service`:

```
[Unit]
Description=MerClaw Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. MerClaw therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, MerClaw starts the child through a short
...
- MerClaw-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `MERCLAW_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
