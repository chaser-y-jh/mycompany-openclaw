...
- repo: `merclaw/docs`
- local checkout: `~/Projects/merclaw-docs`

## Source of truth

- English docs are authored in `merclaw/merclaw`.
...
1. Edit English docs in `merclaw/merclaw`.
2. Push to `main`.
3. `merclaw/merclaw/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `merclaw/docs`.
...
5. `merclaw/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `merclaw/docs`.
...
- Do not treat missing `th` or `fa` entries in generated `docs/docs.json` as a pipeline failure. Verify their generated folders in `merclaw/docs` instead.
...
- Sync metadata is written to `.merclaw-sync/source.json` in the publish repo.
- Source repo secret: `MERCLAW_DOCS_SYNC_TOKEN`
- Publish repo secret: `MERCLAW_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `merclaw/docs` first.
