...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/merclaw`)
...
If the page fails to load, start the Gateway first: `merclaw gateway`.
...
If the browser retries pairing with changed auth details (role/scopes/public key), the previous pending request is superseded and a new `requestId` is created. Re-run `merclaw devices list` before approval.

If the browser is already paired and you change it from read access to write/admin access, this is treated as an approval upgrade, not a silent reconnect. MerClaw keeps the old approval active, blocks the broader reconnect, and asks you to approve the new scope set explicitly.

Once approved, the device is remembered and won't require re-approval unless you revoke it with `merclaw devices revoke --device <id> --role <role>`. See [Devices CLI](/cli/devices) for token rotation and revocation.

Paperclip agents that connect through the `merclaw_gateway` adapter use the same first-run approval flow. After the initial connection attempt, run `merclaw devices approve --latest` to preview the pending request, then rerun the printed `merclaw devices approve <requestId>` command to approve it. Pass explicit `--url` and `--token` values for a remote gateway. To keep approvals stable across restarts, configure a persistent `adapterConfig.devicePrivateKeyPem` in Paperclip instead of letting it generate a new ephemeral device identity each run.
...
<AccordionGroup>
  ...
  <Accordion title="Config">
    - View/edit `~/.merclaw/merclaw.json` (`config.get`, `config.set`).
  ...
  <Accordion title="Cron jobs panel notes">
    ...
    - Deprecated fallback: run `merclaw doctor --fix` to migrate stored legacy jobs with `notify: true` from `cron.webhook` to explicit per-job webhook or completion delivery.
...
The dedicated MCP page is an operator view for MerClaw-managed MCP servers under `mcp.servers`. It does not start MCP transports by itself; use it to inspect and edit saved config, then use `merclaw mcp doctor --probe` when you need live server proof.
...
7. Run `merclaw mcp status --verbose`, `merclaw mcp doctor --probe`, or `merclaw mcp reload` from a terminal when the edited process needs static diagnostics, live proof, or cached-runtime disposal.
...
<AccordionGroup>
  ...
  <Accordion title="Stop and abort">
    ...
    - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop merclaw`, `please stop`) to abort out-of-band.
...
If the page shows **Protocol mismatch** right after an MerClaw update, first reopen the dashboard with `merclaw dashboard` and hard-refresh the page. If it still fails, clear site data for the dashboard origin or test in a private browser window; an old tab or browser service-worker cache can keep running a pre-update Control UI bundle against the newer Gateway.
...
| `push/vapid-keys.json` (under the MerClaw state dir) | Auto-generated VAPID keypair used to sign Web Push payloads.       |
...
- `MERCLAW_VAPID_PUBLIC_KEY`
- `MERCLAW_VAPID_PRIVATE_KEY`
- `MERCLAW_VAPID_SUBJECT` (defaults to `https://merclaw.ai`)
...
<Tabs>
  ...
  <Tab title="Bind to tailnet + token">
    ```bash
    merclaw gateway --bind tailnet --token "$(openssl rand -hex 32)"
    ```
...
If you open the dashboard over plain HTTP (`http://<lan-ip>` or `http://<tailscale-ip>`), the browser runs in a **non-secure context** and blocks WebCrypto. By default, MerClaw **blocks** Control UI connections without device identity.
...
- `GET /__merclaw__/assistant-media?meta=1&source=<path>` requires the normal Control UI operator auth. The browser sends the gateway token as a bearer header when checking availability.
...
```bash
MERCLAW_CONTROL_UI_BASE_PATH=/merclaw/ pnpm ui:build
```
...
If the browser loads a blank dashboard and DevTools shows no useful error, an extension or early content script may have prevented the JavaScript module app from evaluating. The static page includes a plain HTML recovery panel that appears when `<merclaw-app>` is not registered after startup.
