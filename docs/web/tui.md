...
```bash
merclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- `merclaw chat` and `merclaw terminal` are aliases for `merclaw tui --local`.
- `--local` cannot be combined with `--url`, `--token`, or `--password`.
- Local mode uses the embedded agent runtime directly. Most local tools work, but Gateway-only features are unavailable.
- After a config file has authored settings, `merclaw` and `merclaw crestodian` also use this TUI shell, with Crestodian as the local setup and repair chat backend.
...
- Turn delivery on:
  ...
  - or start with `merclaw tui --deliver`
...
- Local shell commands receive `MERCLAW_SHELL=tui-local` in their environment.
...
If `merclaw config validate` is already failing, start with `merclaw configure`
or `merclaw doctor --fix` first. `merclaw chat` does not bypass the invalid-
config guard.
...
4. Apply narrow changes with `merclaw config set` or `merclaw configure`, then rerun `!merclaw config validate`.
5. If Doctor recommends an automatic migration or repair, review it and run `!merclaw doctor --fix`.

Tips:

- Prefer `merclaw config set` or `merclaw configure` over hand-editing `merclaw.json`.
- `merclaw docs "<query>"` searches the live docs index from the same machine.
- `merclaw config validate --json` is useful when you want structured schema and SecretRef/resolvability errors.
...
- If your terminal uses a light background and auto-detection is wrong, set `MERCLAW_THEME=light` before launching `merclaw tui`.
- To force the original dark palette instead, set `MERCLAW_THEME=dark`.
...
- Check the Gateway logs: `merclaw logs --follow`.
- Confirm the agent can run: `merclaw status` and `merclaw models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `merclaw agents list` and your routing config.
- Empty session picker: you might be in global scope or have no sessions yet.

## Related

- [Control UI](/web/control-ui) — web-based control interface
- [Config](/cli/config) — inspect, validate, and edit `merclaw.json`
