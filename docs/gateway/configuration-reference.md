---
summary: "Gateway config reference for core MerClaw keys, defaults, and links to dedicated subsystem references"
...
Core config reference for `~/.merclaw/merclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Covers the main MerClaw config surfaces and links out when a subsystem has its own deeper reference. Channel- and plugin-owned command catalogs and deep memory/QMD knobs live on their own pages rather than on this one.

Code truth:

- `merclaw config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in when available
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional - MerClaw uses safe defaults when omitted.
...
- `talk.*` (Talk mode)
  - `talk.consultThinkingLevel`: thinking level override for the full MerClaw agent run behind Control UI Talk realtime consults
  ...
  - `talk.realtime.consultRouting`: Gateway relay fallback for finalized realtime Talk transcripts that skip `merclaw_agent_consult`
...
- `models.providers.*.localService`: optional on-demand process manager for
  local model servers. MerClaw probes the configured health endpoint, starts
...
MerClaw-managed MCP server definitions live under `mcp.servers` and are
consumed by embedded MerClaw and other runtime adapters. The `merclaw mcp list`,
...
- `mcp.servers`: named stdio or remote MCP server definitions for runtimes that
  ...
  `type: "http"` is a CLI-native alias that `merclaw mcp set` and
  `merclaw doctor --fix` normalize into the canonical `transport` field.
- `mcp.servers.<name>.enabled`: set `false` to keep a saved server definition
  while excluding it from embedded MerClaw MCP discovery and tool projection.
...
- `mcp.servers.<name>.auth`: set `"oauth"` for HTTP MCP servers that require
  OAuth. Run `merclaw mcp login <name>` to store tokens under MerClaw state.
...
- `mcp.servers.<name>.codex`: optional Codex app-server projection controls.
  This block is MerClaw metadata for Codex app-server threads only; it does not
  affect ACP sessions, generic Codex harness config, or other runtime adapters.
  Non-empty `codex.agents` limits the server to the listed MerClaw agent ids.
  ...
  `default_tools_approval_mode` for that server. MerClaw strips the `codex`
...
See [MCP](/cli/mcp#merclaw-as-an-mcp-client-registry) and
...
- `install.nodeManager`: node installer preference for `metadata.merclaw.install`
...
- Loaded from package or bundle directories under `~/.merclaw/extensions` and `<workspace>/.merclaw/extensions`, plus files or directories listed in `plugins.load.paths`.
...
- Discovery accepts native MerClaw plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native MerClaw plugin schema when available).
- Channel plugin account/runtime settings live under `channels.<id>` and should be described by the owning plugin's manifest `channelConfigs` metadata, not by a central MerClaw option registry.
...
It does not enable Codex plugins for MerClaw provider runs, ACP
...
- Enabled Claude bundle plugins can also contribute embedded MerClaw defaults from `settings.json`; MerClaw applies those as sanitized agent settings, not as raw MerClaw config patches.
...
`commitments` controls inferred follow-up memory: MerClaw can detect check-ins from conversation turns and deliver them through heartbeat runs.
...
```json5
{
  browser: {
    ...
    profiles: {
      merclaw: { cdpPort: 18800, color: "#FF4500" },
      work: {
        cdpPort: 18801,
        color: "#0066CC",
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want MerClaw to discover `/json/version`; use WS(S)
...
- If an externally managed CDP service is reachable through loopback, set that
  profile's `attachOnly: true`; otherwise MerClaw treats the loopback port as a
...
- Local managed `merclaw` profiles auto-assign `cdpPort` and `cdpUrl`; only
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or MERCLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `MERCLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `tailscale.preserveFunnel`: when `true` and `tailscale.mode = "serve"`, MerClaw
...
- `MERCLAW_APNS_RELAY_BASE_URL` / `MERCLAW_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `MERCLAW_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
- `gateway.handshakeTimeoutMs`: pre-auth Gateway WebSocket handshake timeout in milliseconds. Default: `15000`. `MERCLAW_HANDSHAKE_TIMEOUT_MS` takes precedence when set. Increase this on loaded or low-powered hosts where local clients can connect while startup warmup is still settling.
...
```bash
MERCLAW_CONFIG_PATH=~/.merclaw/a.json \
MERCLAW_STATE_DIR=~/.merclaw-a \
merclaw gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.merclaw-dev` + port `19001`), `--profile <name>` (uses `~/.merclaw-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/merclaw/tls/server.crt",
      keyPath: "/etc/merclaw/tls/server.key",
      caPath: "/etc/merclaw/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.merclaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-merclaw-token: <token>`.
Query-string hook tokens are rejected.

Validation and safety notes:

- `hooks.enabled=true` requires a non-empty `hooks.token`.
- `hooks.token` must be distinct from `gateway.auth.token` / `MERCLAW_GATEWAY_TOKEN`; reusing the Gateway token fails startup validation.
- `merclaw security audit` also flags `hooks.token` reuse of active Gateway password auth (`gateway.auth.password` / `MERCLAW_GATEWAY_PASSWORD`, or `--auth password --password <password>`) as a critical finding; password-mode reuse stays startup-compatible and should be repaired by rotating one of the secrets.
...
- `transform` can point to a JS/TS module returning a hook action.
  ...
  - Keep `hooks.transformsDir` under `~/.merclaw/hooks/transforms`; workspace skill directories are rejected. If `merclaw doctor` reports this path as invalid, move the transform module into the hooks transforms directory or remove `hooks.transformsDir`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `MERCLAW_SKIP_GMAIL_WATCHER=1` to disable.
  ...
  plugins: {
    entries: {
      canvas: {
        config: {
          host: {
            root: "~/.merclaw/workspace/canvas",
            liveReload: true,
            // enabled: false, // or MERCLAW_SKIP_CANVAS_HOST=1
          },
        },
      },
    },
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__merclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__merclaw__/a2ui/`
...
- Also serves A2UI at `/__merclaw__/a2ui/`.
...
- Hostname defaults to the system hostname when it is a valid DNS label, falling back to `merclaw`. Override with `MERCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.merclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `merclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.merclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${MERCLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `merclaw.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.merclaw/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/merclaw-vault-resolver",
...
- Legacy flat `auth-profiles.json` maps such as `{ "provider": { "apiKey": "..." } }` are not a runtime format; `merclaw doctor --fix` rewrites them to canonical `provider:default` API-key profiles with a `.legacy-flat.*.bak` backup.
...
- Legacy OAuth imports from `~/.merclaw/credentials/oauth.json`.
...
- Default log file: `/tmp/merclaw/merclaw-YYYY-MM-DD.log`.
- Set `logging.file` for a stable path.
- `consoleLevel` bumps to `debug` when `--verbose`.
- `maxFileBytes`: maximum active log file size in bytes before rotation (positive integer; default: `104857600` = 100 MB). MerClaw keeps up to five numbered archives beside the active file.
...
- `stuckSessionAbortMs`: no-progress age threshold in ms before eligible stalled active work may be abort-drained for recovery. When unset, MerClaw uses the safer extended embedded-run window of at least 5 minutes and 3x `stuckSessionWarnMs`.
...
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`: environment toggle for latest experimental GenAI inference span shape, including `{gen_ai.operation.name} {gen_ai.request.model}` span names, `CLIENT` span kind, and `gen_ai.provider.name` instead of legacy `gen_ai.system`. By default spans keep `merclaw.model.call` and `gen_ai.system` for compatibility; GenAI metrics use bounded semantic attributes.
- `MERCLAW_OTEL_PRELOADED=1`: environment toggle for hosts that already registered a global OpenTelemetry SDK. MerClaw then skips plugin-owned SDK startup/shutdown while keeping diagnostic listeners active.
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$MERCLAW_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one MerClaw.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `MERCLAW_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `merclaw doctor --fix` can strip unknown keys).
...
- `webhook`: deprecated legacy fallback webhook URL (http/https) used by `merclaw doctor --fix` to migrate stored jobs that still have `notify: true`; runtime delivery uses per-job `delivery.mode="webhook"` plus `delivery.to`, or `delivery.completionDestination` when preserving announce delivery.
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `merclaw.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary. Paths must not contain null bytes and must be strictly shorter than 4096 characters before and after resolution.
- MerClaw-owned writes that change only one top-level section backed by a single-file include write through to that included file. For example, `plugins install` updates `plugins: { $include: "./plugins.json5" }` in `plugins.json5` and leaves `merclaw.json` intact.
- Root includes, include arrays, and includes with sibling overrides are read-only for MerClaw-owned writes; those writes fail closed instead of flattening the config.
