---
summary: "Use OpenShell as a managed sandbox backend for MerClaw agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for MerClaw. Instead of running Docker
containers locally, MerClaw delegates sandbox lifecycle to the `openshell` CLI,
...
2. Restart the Gateway. On the next agent turn, MerClaw creates an OpenShell
...
- You edit files locally outside MerClaw and want those changes visible in the
...
- When the sandbox is first created, MerClaw seeds the remote workspace from
...
- MerClaw does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside MerClaw after the initial seed, the remote sandbox does **not** see those changes. Use `merclaw sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"merclaw"`  | Sandbox source for first-time create                  |
...
1. MerClaw calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. MerClaw calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `merclaw sandbox` commands
