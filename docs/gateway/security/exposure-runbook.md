---
summary: "Pre-flight and rollback checklist before exposing an MerClaw Gateway beyond loopback"
...
- Backup location for `~/.merclaw/merclaw.json` and credentials.
...
```bash
merclaw gateway probe --url ws://127.0.0.1:18789 --token "$MERCLAW_GATEWAY_TOKEN"
```
...
Run `merclaw security audit --deep` after proxy changes. Trusted-proxy findings
...
1. Re-run `merclaw security audit --deep`.
...
5. Re-run `merclaw security audit --deep`.
