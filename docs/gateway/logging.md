...
- Default rolling log file is under `/tmp/merclaw/` (one file per day): `merclaw-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.merclaw/merclaw.json`:
  - `logging.file`
  - `logging.level`
...
MerClaw can mask sensitive tokens before log or transcript output leaves the
...
`merclaw gateway` supports a per-gateway style switch:
