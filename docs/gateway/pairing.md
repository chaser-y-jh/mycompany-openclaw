...
merclaw nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), MerClaw treats
...
Pairing state is stored under the Gateway state directory (default `~/.merclaw`):

- `~/.merclaw/nodes/paired.json`
- `~/.merclaw/nodes/pending.json`

If you override `MERCLAW_STATE_DIR`, the `nodes/` folder moves with it.
