...
MerClaw can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **MerClaw tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
merclaw agent --message "hi" --model claude-cli/claude-sonnet-4-6
```
...
gateway host, MerClaw now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), MerClaw will
    ...
    defaults: {
      cliBackends: {
        "my-cli": {
          ...
          // from bounded raw MerClaw transcript history before compaction.
...
2. **Builds a system prompt** using the same MerClaw prompt + workspace context.
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
   The bundled `claude-cli` backend keeps a Claude stdio process alive per
   MerClaw session and sends follow-up turns over stream-json stdin.
4. **Parses output** (JSON or plain text) and returns the final text.
5. **Persists session ids** per backend, so follow-ups reuse the same CLI session.

<Note>
The bundled Anthropic `claude-cli` backend is supported again. Anthropic staff
told us MerClaw-style Claude CLI usage is allowed again, so MerClaw treats
...
resolver for MerClaw skills. When the current skills snapshot includes at least
one selected skill with a materialized path, MerClaw passes a temporary Claude
Code plugin with `--plugin-dir` and omits the duplicate MerClaw skills catalog
from the appended system prompt. If the snapshot has no materialized plugin
skill, MerClaw keeps the prompt catalog as a fallback. Skill env/API key
overrides are still applied by MerClaw to the child process environment for the
run.

Claude CLI also has its own noninteractive permission mode. MerClaw maps that
to the existing exec policy instead of adding Claude-specific policy config.
For MerClaw-managed Claude live sessions, the effective MerClaw exec policy is
...
Claude launches normalize that flag to match the effective MerClaw exec policy.

The bundled Anthropic `claude-cli` backend also maps MerClaw `/think` levels
...
Before MerClaw can use the bundled `claude-cli` backend, Claude Code itself
must already be logged in on the same host:

```bash
claude auth login
claude auth status --text
merclaw models auth login --provider anthropic --method cli --set-default
```
...
- `claude-cli` defaults to `liveSession: "claude-stdio"`, `output: "jsonl"`,
  ...
  exits, MerClaw resumes from the stored Claude session id. Stored session
...
- Claude live sessions keep bounded JSONL output guards. Defaults allow up to
  ...
  and `maxTurnLines`; MerClaw clamps those settings to 64 MiB and 100,000
  lines.
- Stored CLI sessions are provider-owned continuity. The implicit daily session
  ...
  do.
- Fresh CLI sessions normally reseed only from MerClaw's compaction summary
  plus post-compaction tail. To recover short sessions that are invalidated
  before compaction, a backend can opt in with
  `reseedFromRawTranscriptWhenUncompacted: true`. MerClaw still keeps raw
...
- MerClaw drops stored CLI session reuse when the selected auth identity changes,
  ...
  stable OAuth account id, MerClaw lets that CLI enforce resume permissions.

## Fallback prelude from claude-cli sessions

When a `claude-cli` attempt fails over to a non-CLI candidate in
[`agents.defaults.model.fallbacks`](/concepts/model-failover), MerClaw seeds
...
provider would start cold because MerClaw's own session transcript is empty
for `claude-cli` runs.
...
MerClaw will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, MerClaw appends the
...
- For Gemini CLI JSON output, MerClaw reads reply text from `response` and
...
agent runs use the Codex app-server harness through `openai/*`; MerClaw no
...
- `stats.cached` is normalized into MerClaw `cacheRead`.
- If `stats.input` is missing, MerClaw derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before MerClaw handles
...
CLI backends do **not** receive MerClaw tool calls directly, but a backend can
...
When bundle MCP is enabled, MerClaw:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`MERCLAW_MCP_TOKEN`)
...
If no MCP servers are enabled, MerClaw still injects a strict config when a
...
When a fresh CLI session is seeded from a prior MerClaw transcript (for
...
- **No direct MerClaw tool calls.** MerClaw does not inject tool calls into
