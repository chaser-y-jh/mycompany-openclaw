...
read_when:
  - Running MerClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of MerClaw
...
- You run MerClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth).
...
<Steps>
  ...
  <Step title="Gateway verifies trusted source">
    MerClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`).
  </Step>
  <Step title="Gateway extracts identity">
    MerClaw extracts the user identity from the configured header.
  </Step>
...
cannot create the device identity that MerClaw uses to bind operator scopes,
...
This is a break-glass flag (`merclaw security audit` reports it as critical).
...
- Internal Gateway clients that do not travel through the reverse proxy should use `gateway.auth.password` / `MERCLAW_GATEWAY_PASSWORD`, not trusted-proxy identity headers.
...
<Tabs>
  <Tab title="Proxy TLS termination (recommended)">
    ...
    - MerClaw can stay on loopback HTTP behind the proxy.
  ...
  <Tab title="Gateway TLS termination">
    If MerClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
<AccordionGroup>
  <Accordion title="Pomerium">
    ...
    ```yaml
    routes:
      - from: https://merclaw.example.com
        to: http://merclaw-gateway:18789
        policy:
          - allow:
              or:
                - email:
...
MerClaw rejects ambiguous configurations where both a `gateway.auth.token` (or `MERCLAW_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
Loopback trusted-proxy identity headers still fail closed: same-host callers are not silently authenticated as proxy users. Internal MerClaw callers that bypass the proxy may authenticate with `gateway.auth.password` / `MERCLAW_GATEWAY_PASSWORD` instead. Token fallback remains intentionally unsupported in trusted-proxy mode.

## Operator scopes header

Trusted-proxy auth is an **identity-bearing** HTTP mode, so callers may optionally declare operator scopes with `x-merclaw-scopes`.

Note: `x-merclaw-scopes` applies to HTTP endpoints only. WebSocket scopes are
...
- `x-merclaw-scopes: operator.read,operator.write`
- `x-merclaw-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, MerClaw honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-merclaw-scopes` is absent, their runtime scope falls back to `operator.write`.
...
Practical rule: send `x-merclaw-scopes` explicitly when you want a trusted-proxy request to be narrower than the defaults, or when a gateway-auth plugin route needs something stronger than write scope.
...
`merclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
<AccordionGroup>
  ...
  <Accordion title="trusted_proxy_loopback_source">
    MerClaw rejected a loopback-source trusted-proxy request.
...
<Steps>
  ...
  <Step title="Update MerClaw config">
    Update MerClaw config with trusted-proxy auth.
  </Step>
  ...
  <Step title="Audit">
    Run `merclaw security audit` and review findings.
  </Step>
</Steps>
