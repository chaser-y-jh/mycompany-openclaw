...
Healthy baseline: `Runtime: running`, `Connectivity probe: ok`, and `Capability: ...` that matches what you expect. Use `merclaw gateway status --require-rpc` when you need read-scope RPC proof, not just reachability.
...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `MERCLAW_CONFIG_PATH` when set).
...
- Auth is required by default. Shared-secret setups use
  ...
  `MERCLAW_GATEWAY_TOKEN` / `MERCLAW_GATEWAY_PASSWORD`), and non-loopback
  reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

MerClaw's highest-leverage compatibility surface is now:
...
- `/v1/models` is agent-first: it returns `merclaw`, `merclaw/default`, and `merclaw/<agentId>`.
- `merclaw/default` is the stable alias that always maps to the configured default agent.
- Use `x-merclaw-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` → `MERCLAW_GATEWAY_PORT` → `gateway.port` → `18789` |
| Bind mode    | CLI/override → `gateway.bind` → `loopback`                    |

Installed gateway services record the resolved `--port` in supervisor metadata. After changing `gateway.port`, run `merclaw doctor --fix` or `merclaw gateway install --force` so launchd/systemd/schtasks starts the process on the new port.
...
- Unique `gateway.port`
- Unique `MERCLAW_CONFIG_PATH`
- Unique `MERCLAW_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
MERCLAW_CONFIG_PATH=~/.merclaw/a.json MERCLAW_STATE_DIR=~/.merclaw-a merclaw gateway --port 19001
MERCLAW_CONFIG_PATH=~/.merclaw/b.json MERCLAW_STATE_DIR=~/.merclaw-b merclaw gateway --port 19002
```
...
Use `merclaw gateway restart` for restarts. Do not chain `merclaw gateway stop` and `merclaw gateway start` as a restart substitute.

On macOS, `gateway stop` uses `launchctl bootout` by default — this removes the LaunchAgent from the current boot session without persisting a disable, so KeepAlive auto-recovery still works after unexpected crashes and `gateway start` re-enables cleanly. To persistently suppress auto-respawn across reboots, pass `--disable`: `merclaw gateway stop --disable`.

LaunchAgent labels are `ai.merclaw.gateway` (default) or `ai.merclaw.<profile>` (named profile). `merclaw doctor` audits and repairs service config drift.
...
```bash
merclaw gateway install
systemctl --user enable --now merclaw-gateway[-<profile>].service
merclaw gateway status
```
...
[Service]
ExecStart=/usr/local/bin/merclaw gateway --port 18789
Restart=always
RestartSec=5
TimeoutStopSec=30
TimeoutStartSec=30
...
Native Windows managed startup uses a Scheduled Task named `MerClaw Gateway`
(or `MerClaw Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, MerClaw falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now merclaw-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/merclaw-gateway[-<profile>].service` and adjust
`ExecStart=` if your `merclaw` binary lives elsewhere.

Do not also let `merclaw doctor --fix` install a user-level gateway service for the same profile/port. Doctor refuses that automatic install when it finds a system-level MerClaw gateway service; use `MERCLAW_SERVICE_REPAIR_POLICY=external` when the system unit owns the lifecycle.
...
```bash
merclaw --dev setup
merclaw --dev gateway --allow-unconfigured
merclaw --dev status
```
