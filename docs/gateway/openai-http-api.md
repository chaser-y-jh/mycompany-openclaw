...
MerClaw's Gateway can serve a small OpenAI-compatible Chat Completions endpoint.
...
Under the hood, requests are executed as a normal Gateway agent run (same codepath as `merclaw agent`), so routing/permissions/config match your Gateway.
...
- When `gateway.auth.mode="token"`, use `gateway.auth.token` (or `MERCLAW_GATEWAY_TOKEN`).
- When `gateway.auth.mode="password"`, use `gateway.auth.password` (or `MERCLAW_GATEWAY_PASSWORD`).
...
- Internal same-host callers that bypass the proxy can use
  `gateway.auth.password` / `MERCLAW_GATEWAY_PASSWORD` as a local direct
...
- There is no separate non-owner/per-user tool boundary on this endpoint; once a caller passes Gateway auth here, MerClaw treats that caller as a trusted operator for this gateway.
- For shared-secret auth modes (`token` and `password`), the endpoint restores the normal full operator defaults even if the caller sends a narrower `x-merclaw-scopes` header.
- Trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`) honor `x-merclaw-scopes` when present and otherwise fall back to the normal operator default scope set.
...
- `gateway.auth.mode="token"` or `"password"` + `Authorization: Bearer ...`
  ...
  - ignores narrower `x-merclaw-scopes`
...
- trusted identity-bearing HTTP modes (for example trusted proxy auth, or `gateway.auth.mode="none"` on private ingress)
  ...
  - honor `x-merclaw-scopes` when the header is present
...
MerClaw treats the OpenAI `model` field as an **agent target**, not a raw provider model id.

- `model: "merclaw"` routes to the configured default agent.
- `model: "merclaw/default"` also routes to the configured default agent.
- `model: "merclaw/<agentId>"` routes to a specific agent.

Optional request headers:

- `x-merclaw-model: <provider/model-or-bare-id>` overrides the backend model for the selected agent.
- `x-merclaw-agent-id: <agentId>` remains supported as a compatibility override.
- `x-merclaw-session-key: <sessionKey>` fully controls session routing.
- `x-merclaw-message-channel: <channel>` sets the synthetic ingress channel context for channel-aware prompts and policies.

Compatibility aliases still accepted:

- `model: "merclaw:<agentId>"`
- `model: "agent:<agentId>"`
...
For custom apps, the safest default is to reuse the same `user` value per conversation thread. Avoid account-level identifiers unless you explicitly want multiple conversations or devices to share one MerClaw session. Use `x-merclaw-session-key` when you need explicit routing control across multiple clients or threads.
...
<AccordionGroup>
  <Accordion title="What does `/v1/models` return?">
    An MerClaw agent-target list.

    The returned ids are `merclaw`, `merclaw/default`, and `merclaw/<agentId>` entries.
  ...
  <Accordion title="Why is `merclaw/default` included?">
    `merclaw/default` is the stable alias for the configured default agent.
  ...
  <Accordion title="How do I override the backend model?">
    ...
    Examples:
    `x-merclaw-model: openai/gpt-5.4`
    `x-merclaw-model: gpt-5.5`
  ...
  <Accordion title="How do embeddings fit this contract?">
    ...
    Use `model: "merclaw/default"` or `model: "merclaw/<agentId>"`.
    When you need a specific embedding model, send it in `x-merclaw-model`.
...
- `GET /v1/models` should list `merclaw/default`
- Open WebUI should use `merclaw/default` as the chat model id
- If you want a specific backend provider/model for that agent, set the agent's normal default model or send `x-merclaw-model`
...
If that returns `merclaw/default`, most Open WebUI setups can connect with the same base URL and token.
...
curl -N http://127.0.0.1:18789/v1/chat/completions \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-merclaw-model: openai/gpt-5.4' \
  -d '{
    "model": "merclaw/research",
    "stream": true,
    "messages": [{"role":"user","content":"hi"}]
  }'
```
...
```bash
curl -sS http://127.0.0.1:18789/v1/models/merclaw%2Fdefault \
  -H 'Authorization: Bearer YOUR_TOKEN'
```
...
curl -sS http://127.0.0.1:18789/v1/embeddings \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-merclaw-model: openai/text-embedding-3-small' \
  -d '{
    "model": "merclaw/default",
    "input": ["alpha", "beta"]
  }'
...
- `/v1/models` returns MerClaw agent targets, not raw provider catalogs.
- `merclaw/default` is always present so one stable id works across environments.
- Backend provider/model overrides belong in `x-merclaw-model`, not the OpenAI `model` field.
