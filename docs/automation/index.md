...
read_when:
  - Deciding how to automate work with MerClaw
  - Choosing between heartbeat, cron, commitments, hooks, and standing orders
  - Looking for the right automation entry point
title: "Automation"
---

MerClaw runs work in the background through tasks, scheduled jobs, inferred
...
| Audit what ran and when                 | Background Tasks       | `merclaw tasks list` and `merclaw tasks audit` |
...
The background task ledger tracks all detached work: ACP runs, subagent spawns, isolated cron executions, and CLI operations. Tasks are records, not schedulers. Use `merclaw tasks list` and `merclaw tasks audit` to inspect them.

See [Background Tasks](/automation/tasks).

### Inferred commitments

Commitments are opt-in, short-lived follow-up memories. MerClaw infers them
...
Task Flow is the flow orchestration substrate above background tasks. It manages durable multi-step flows with managed and mirrored sync modes, revision tracking, and `merclaw tasks flow list|show|cancel` for inspection.
...
with `merclaw hooks`. For in-process tool-call interception, use
