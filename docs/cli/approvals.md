---
summary: "CLI reference for `merclaw approvals` and `merclaw exec-policy`"
read_when:
...
`merclaw exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.merclaw/exec-approvals.json`

Examples:

```bash
merclaw exec-policy show
merclaw exec-policy show --json

merclaw exec-policy preset yolo
merclaw exec-policy preset cautious --json

merclaw exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `merclaw exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `merclaw approvals set --gateway`
or `merclaw approvals set --node <id|name|ip>`.

## Common commands

```bash
merclaw approvals get
merclaw approvals get --node <id|name|ip>
merclaw approvals get --gateway
```

`merclaw approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
merclaw approvals set --file ./exec-approvals.json
merclaw approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
merclaw approvals set --node <id|name|ip> --file ./exec-approvals.json
merclaw approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested MerClaw policy aligned, also set:
...
```bash
merclaw approvals allowlist add "~/Projects/**/bin/rg"
merclaw approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
merclaw approvals allowlist add --agent "*" "/usr/bin/uname"

merclaw approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `merclaw nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.merclaw/exec-approvals.json`.
