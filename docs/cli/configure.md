---
summary: "CLI reference for `merclaw configure` (interactive configuration prompts)"
read_when:
...
Use `merclaw onboard` for the full guided first-run journey, `merclaw setup` for the baseline config/workspace only, and `merclaw channels add` when you only need channel account setup.
...
<Tip>
`merclaw config` without a subcommand opens the same wizard. Use `merclaw config get|set|unset` for non-interactive edits.
</Tip>

For web search, `merclaw configure --section web` lets you choose a provider
...
merclaw configure --section model --section channels
merclaw configure --section gateway --section daemon
```
