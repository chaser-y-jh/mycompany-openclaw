---
summary: "CLI reference for `merclaw migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into MerClaw
...
```bash
merclaw migrate list
merclaw migrate claude --dry-run
merclaw migrate codex --dry-run
merclaw migrate codex --skill gog-vault77-google-workspace
merclaw migrate codex --plugin google-calendar --dry-run
merclaw migrate codex --plugin google-calendar --verify-plugin-apps --dry-run
merclaw migrate hermes --dry-run
merclaw migrate hermes
merclaw migrate apply codex --yes --skill gog-vault77-google-workspace
merclaw migrate apply codex --yes --plugin google-calendar
merclaw migrate apply codex --yes
merclaw migrate apply claude --yes
merclaw migrate apply hermes --yes
merclaw migrate apply hermes --include-secrets --yes
merclaw onboard --flow import
merclaw onboard --import-from claude --import-source ~/.claude
merclaw onboard --import-from hermes --import-source ~/.hermes
```

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `merclaw migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local MerClaw state exists.
</ParamField>
...
`merclaw migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `merclaw migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an MerClaw backup before applying the migration. If no local MerClaw state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the MerClaw agent workspace.
...
- Claude command Markdown files converted into MerClaw skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. MerClaw does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the MerClaw Codex harness and you want to
...
Running `merclaw migrate codex` in an interactive terminal previews the full
...
start checked unless the target MerClaw Codex plugin config already has that
...
```bash
merclaw migrate codex --dry-run --skill gog-vault77-google-workspace
merclaw migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
```bash
merclaw migrate codex --dry-run --plugin google-calendar
merclaw migrate apply codex --yes --plugin google-calendar
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  MerClaw agent workspace when you want per-agent ownership.
- Source-installed `openai-curated` Codex plugins discovered through Codex
  ...
  native Codex harness; they are not exposed to MerClaw provider runs,
...
- `SOUL.md` and `AGENTS.md` into the MerClaw agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for MerClaw file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that MerClaw cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live MerClaw config or credentials. This preserves opaque or unsafe state without pretending MerClaw can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `merclaw.plugin.json`:
...
Provider plugins can use `merclaw/plugin-sdk/migration` for item construction and summary counts, plus `merclaw/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `merclaw onboard --flow import` and `merclaw setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh MerClaw setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move MerClaw to a new machine.
