...
read_when:
  - You run merclaw with no command after setup and want to understand Crestodian
  - You need a configless-safe way to inspect or repair MerClaw
...
Crestodian is MerClaw's local setup, repair, and configuration helper. It is
designed to stay reachable when the normal agent path is broken.

Running `merclaw` with no command starts classic onboarding first when the
active config file is missing or has no authored settings (empty or
metadata-only). After a config file has authored settings, running `merclaw`
with no command starts Crestodian in an interactive terminal. Running
`merclaw crestodian` starts the same helper explicitly.

## What Crestodian shows

On startup, interactive Crestodian opens the same TUI shell used by
`merclaw tui`, with a Crestodian chat backend. The chat log starts with a short
greeting:
...
Crestodian uses the same MerClaw reference discovery as regular agents. In a Git checkout,
...
[https://github.com/merclaw/merclaw](https://github.com/merclaw/merclaw), with explicit
...
merclaw
merclaw crestodian
merclaw crestodian --json
merclaw crestodian --message "models"
merclaw crestodian --message "validate config"
merclaw crestodian --message "setup workspace ~/Projects/work model openai/gpt-5.5" --yes
merclaw crestodian --message "set default model openai/gpt-5.5" --yes
merclaw onboard --modern
```
...
config set-ref gateway.auth.token env MERCLAW_GATEWAY_TOKEN
gateway status
restart gateway
agents
...
plugin install clawhub:merclaw-codex-app-server
plugin uninstall merclaw-codex-app-server
talk to work agent
...
- `merclaw.json` is missing
- `merclaw.json` is invalid
- the Gateway is down
- plugin command registration is unavailable
- no agent has been configured yet

`merclaw --help` and `merclaw --version` still use the normal fast paths.
Noninteractive bare `merclaw` exits with a short message instead of printing
...
```text
~/.merclaw/audit/crestodian.jsonl
```

Discovery is not audited. Only applied operations and writes are logged.

`merclaw onboard --modern` starts Crestodian as the modern onboarding preview.
Plain `merclaw onboard` still runs classic onboarding.
...
planner turn through MerClaw's normal runtime paths. It first uses the
configured MerClaw model. If no configured model is usable yet, it can fall
...
`merclaw tui`, `merclaw chat`, and `merclaw terminal` still open the normal
...
MerClaw: Crestodian rescue mode. Gateway reachable: no. Config valid: no.
You: /crestodian restart gateway
MerClaw: Plan: restart the Gateway. Reply /crestodian yes to apply.
You: /crestodian yes
MerClaw: Applied. Audit entry written.
```
...
- Remote rescue cannot open the local TUI or switch into an interactive agent
  session. Use local `merclaw` for agent handoff.
