---
summary: "CLI reference for `merclaw webhooks` (Gmail Pub/Sub setup and runner)"
read_when:
  - You want to wire Gmail Pub/Sub events into MerClaw
...
```bash
merclaw webhooks gmail setup --account <email> [...]
merclaw webhooks gmail run   [--account <email>] [...]
```

| Subcommand    | Description                                                                                  |
| ------------- | -------------------------------------------------------------------------------------------- |
| `gmail setup` | Configure Gmail watch, Pub/Sub topic/subscription, and the MerClaw webhook delivery target. |
| `gmail run`   | Run `gog watch serve` plus the watch auto-renew loop.                                        |

## `webhooks gmail setup`

Configure Gmail watch, Pub/Sub, and MerClaw webhook delivery.

```bash
merclaw webhooks gmail setup --account you@example.com
merclaw webhooks gmail setup --account you@example.com --project my-gcp-project --json
merclaw webhooks gmail setup --account you@example.com --hook-url https://gateway.example.com/hooks/gmail
```
...
### MerClaw delivery options

| Flag                   | Default | Description                                |
| ---------------------- | ------- | ------------------------------------------ |
| `--hook-url <url>`     | (none)  | MerClaw webhook URL.                      |
| `--hook-token <token>` | (none)  | MerClaw webhook token.                    |
...
```bash
merclaw webhooks gmail run --account you@example.com
```

`run` accepts the same `gog watch serve`, MerClaw delivery, Pub/Sub, and Tailscale flags as `setup`, except:
...
| MerClaw delivery | `--hook-url`, `--hook-token`, `--push-token`                                     |
