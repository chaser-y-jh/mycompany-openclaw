---
summary: "CLI reference for `merclaw devices` (device pairing + token rotation/revocation)"
read_when:
...
### `merclaw devices remove <deviceId>`
...
```
merclaw devices remove <deviceId>
merclaw devices remove <deviceId> --json
```

### `merclaw devices clear --yes [--pending]`

Clear paired devices in bulk.

```
merclaw devices clear --yes
merclaw devices clear --yes --pending
merclaw devices clear --yes --pending --json
```

### `merclaw devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, MerClaw only prints the selected pending
request and exits; rerun approval with the exact request ID after verifying
the details.

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), MerClaw supersedes the previous pending entry and issues a new `requestId`. Run `merclaw devices list` right before approval to use the current ID.
</Note>

If the device is already paired and asks for broader scopes or a broader role,
MerClaw keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `merclaw devices list`
or use `merclaw devices approve --latest` to preview the exact upgrade before
approving it.
...
```
merclaw devices approve
merclaw devices approve <requestId>
merclaw devices approve --latest
```

## Paperclip / `merclaw_gateway` first-run approval

When a new Paperclip agent connects through the `merclaw_gateway` adapter for the first time, the Gateway may require a one-time device pairing approval before runs can succeed. If Paperclip reports `merclaw_gateway_pairing_required`, approve the pending device and retry.
...
The preview prints the exact `merclaw devices approve <requestId>` command. Verify the request details, then rerun that command with the request ID to approve it.

For remote gateways or explicit credentials, pass the same options while previewing and approving:

```bash
merclaw devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```
...
If approval keeps failing, run `merclaw devices list` first to confirm a pending request exists.
...
### `merclaw devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
merclaw devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `merclaw devices revoke --device <id> --role <role>`
...
```
merclaw devices revoke --device <deviceId> --role node
```
...
```bash
merclaw devices rotate --device <deviceId> --role operator
```
