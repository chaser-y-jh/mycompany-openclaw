---
summary: "CLI reference for `merclaw config` (get/set/patch/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `merclaw.json`: get/set/patch/unset/file/schema/validate values by path and print the active config file. Run without a subcommand to open the configure wizard (same as `merclaw configure`).

<Note>
When `MERCLAW_NIX_MODE=1`, MerClaw treats `merclaw.json` as immutable. Read-only commands such as `config get`, `config file`, `config schema`, and `config validate` still work, but config writers refuse. Agents should edit the Nix source for the install instead; for the first-party nix-merclaw distribution, use [nix-merclaw Quick Start](https://github.com/merclaw/nix-merclaw#quick-start) and set values under `programs.merclaw.config` or `instances.<name>.config`.
</Note>

## Root options

<ParamField path="--section <section>" type="string">
  Repeatable guided-setup section filter when you run `merclaw config` without a subcommand.
</ParamField>
...
```bash
merclaw config file
merclaw config --section model
merclaw config --section gateway --section daemon
merclaw config schema
merclaw config get browser.executablePath
...
merclaw config set agents.defaults.heartbeat.every "2h"
merclaw config set 'agents.list[0].tools.exec.node' "node-id-or-name"
merclaw config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
merclaw config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
merclaw config set secrets.providers.vaultfile --provider-source file --provider-path /etc/merclaw/secrets.json --provider-mode json
merclaw config patch --file ./merclaw.patch.json5 --dry-run
merclaw config unset plugins.entries.brave.config.webSearch.apiKey
merclaw config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
merclaw config validate
merclaw config validate --json
```

### `config schema`

Print the generated JSON schema for `merclaw.json` to stdout as JSON.
...
```bash
merclaw config schema > merclaw.schema.json
```

### Paths

Paths use dot or bracket notation. Quote bracket-notation paths in shell examples so shells such as zsh do not expand `[0]` as a glob before MerClaw receives the path:

```bash
merclaw config get agents.defaults.workspace
merclaw config get 'agents.list[0].id'
```

Use the agent list index to target a specific agent:

```bash
merclaw config get agents.list
merclaw config set 'agents.list[1].tools.exec.node' "node-id-or-name"
```
...
```bash
merclaw config set agents.defaults.heartbeat.every "0m"
merclaw config set gateway.port 19001 --strict-json
merclaw config set channels.whatsapp.groups '["*"]' --strict-json
```
...
```bash
merclaw config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
merclaw config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
```
...
`merclaw config set` supports four assignment styles:

<Tabs>
  <Tab title="Value mode">
    ```bash
    merclaw config set <path> <value>
    ```
  </Tab>
  <Tab title="SecretRef builder mode">
    ```bash
    merclaw config set channels.discord.token \
  ...
  <Tab title="Provider builder mode">
    ...
    ```bash
    merclaw config set secrets.providers.vault \
      --provider-source exec \
      --provider-command /usr/local/bin/merclaw-vault \
      --provider-arg read \
  ...
  <Tab title="Batch mode">
    ...
    ```bash
    merclaw config set --batch-file ./config-set.batch.json --dry-run
    ```
...
```bash
merclaw config patch --file ./merclaw.patch.json5 --dry-run
merclaw config patch --file ./merclaw.patch.json5
```

You can also pipe a patch over stdin, which is useful for remote setup scripts:

```bash
ssh merclaw-host 'merclaw config patch --stdin --dry-run' < ./merclaw.patch.json5
ssh merclaw-host 'merclaw config patch --stdin' < ./merclaw.patch.json5
```
...
```bash
merclaw config patch --file ./discord.patch.json5 --replace-path 'channels.discord.guilds["123"].channels'
```
...
merclaw config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/merclaw/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `merclaw.json`.
...
<Tabs>
  <Tab title="Success example">
    ```json
    {
      "ok": true,
      "operations": 1,
      "configPath": "~/.merclaw/merclaw.json",
      "inputModes": ["builder"],
      "checks": {
        "schema": false,
        "resolvability": true,
...
`merclaw config set` and other MerClaw-owned config writers validate the full post-change config before committing it to disk. If the new payload fails schema validation or looks like a destructive clobber, the active config is left alone and the rejected payload is saved beside it as `merclaw.json.rejected.*`.

<Warning>
The active config path must be a regular file. Symlinked `merclaw.json` layouts are unsupported for writes; use `MERCLAW_CONFIG_PATH` to point directly at the real file instead.
</Warning>

Prefer CLI writes for small edits:

```bash
merclaw config set gateway.reload.mode hybrid --dry-run
merclaw config set gateway.reload.mode hybrid
merclaw config validate
```
...
Direct editor writes are still allowed, but the running Gateway treats them as untrusted until they validate. Invalid direct edits fail startup or are skipped by hot reload; Gateway does not rewrite `merclaw.json`. Run `merclaw doctor --fix` to repair prefixed/clobbered config or restore the last-known-good copy. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-rejected-invalid-config).
...
- `config file`: Print the active config file path (resolved from `MERCLAW_CONFIG_PATH` or default location). The path should name a regular file, not a symlink.
...
After `merclaw config validate` is passing, you can use the local TUI to have an embedded agent compare the active config against the docs while you validate each change from the same terminal:

<Note>
If validation is already failing, start with `merclaw configure` or `merclaw doctor --fix`. `merclaw chat` does not bypass the invalid-config guard.
</Note>
...
```text
!merclaw config file
!merclaw docs gateway auth token secretref
!merclaw config validate
!merclaw doctor
```
...
<Steps>
  ...
  <Step title="Apply targeted edits">
    Apply targeted edits with `merclaw config set` or `merclaw configure`.
  </Step>
  <Step title="Re-validate">
    Rerun `merclaw config validate` after each change.
  </Step>
  <Step title="Doctor for runtime issues">
    If validation passes but the runtime is still unhealthy, run `merclaw doctor` or `merclaw doctor --fix` for migration and repair help.
  </Step>
</Steps>
