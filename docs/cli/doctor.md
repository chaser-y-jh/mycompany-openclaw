---
summary: "CLI reference for `merclaw doctor` (health checks + guided repairs)"
read_when:
...
`merclaw doctor` is the MerClaw health surface. Use it when the gateway,
...
| ------- | ------------------------ | ------------------------------------------------------------------------------- |
| Inspect | `merclaw doctor`        | Human-oriented checks and guided prompts.                                       |
| Repair  | `merclaw doctor --fix`  | Applies supported repairs, using prompts unless non-interactive repair is safe. |
| Lint    | `merclaw doctor --lint` | Read-only structured findings for CI, preflight, and review gates.              |
...
merclaw doctor --lint --allow-exec
merclaw doctor --deep
merclaw doctor --fix
merclaw doctor --fix --non-interactive
merclaw doctor --generate-gateway-token
```

For channel-specific permissions, use the channel probes instead of `doctor`:

```bash
merclaw channels capabilities --channel discord --target channel:<channel-id>
...
`merclaw doctor --lint` is the read-only automation posture for doctor checks.
...
doctor --lint: ran 6 check(s), 1 finding(s)
  [warning] core/doctor/gateway-config gateway.mode - gateway.mode is unset; gateway start will be blocked.
    fix: Run `merclaw configure` and set Gateway mode (local/remote), or `merclaw config set gateway.mode local`.
```
  ...
  "findings": [
    {
      ...
      "path": "gateway.mode",
      "fixHint": "Run `merclaw configure` and set Gateway mode (local/remote), or `merclaw config set gateway.mode local`."
    }
  ]
}
```
...
example, `merclaw doctor --lint --severity-min error` can print no findings and
...
health path. The `merclaw/plugin-sdk/health` subpath exposes the same
...
```bash
merclaw doctor --lint --only core/doctor/gateway-config --json
merclaw doctor --lint --skip core/doctor/skills-readiness
```
...
- In Nix mode (`MERCLAW_NIX_MODE=1`), read-only doctor checks still work, but `doctor --fix`, `doctor --repair`, `doctor --yes`, and `doctor --generate-gateway-token` are disabled because `merclaw.json` is immutable. Edit the Nix source for this install instead; for nix-merclaw, use the agent-first [Quick Start](https://github.com/merclaw/nix-merclaw#quick-start).
...
- By default, doctor does not execute `exec` SecretRefs while checking secrets. Use `merclaw doctor --allow-exec` or `merclaw doctor --lint --allow-exec` only when you intentionally want doctor to run those configured secret resolvers.
- `--fix` (alias for `--repair`) writes a backup to `~/.merclaw/merclaw.json.bak` and drops unknown config keys, listing each removal.
...
- `doctor --fix --non-interactive` reports missing or stale gateway service definitions but does not install or rewrite them outside update repair mode. Run `merclaw gateway install` for a missing service, or `merclaw gateway install --force` when you intentionally want to replace the launcher.
...
- Doctor also scans `~/.merclaw/cron/jobs.json` (or `cron.store`) for legacy cron job shapes and rewrites them before importing canonical rows into SQLite.
...
- On Linux, doctor warns when the user's crontab still runs legacy `~/.merclaw/bin/ensure-whatsapp.sh`; that script is no longer maintained and can log false WhatsApp gateway outages when cron lacks the systemd user-bus environment.
- When WhatsApp is enabled, doctor checks for a degraded Gateway event loop with local `merclaw-tui` clients still running. `doctor --fix` stops only verified local TUI clients so WhatsApp replies are not queued behind stale TUI refresh loops.
...
- Doctor cleans legacy plugin dependency staging state created by older MerClaw versions and relinks the host `merclaw` package for managed npm plugins that declare it as a peer dependency. It also repairs missing downloadable plugins that are referenced by config, such as `plugins.entries`, configured channels, configured provider/search settings, or configured agent runtimes. During package updates, doctor skips package-manager plugin repair until the package swap is complete; rerun `merclaw doctor --fix` afterward if a configured plugin still needs recovery. If the download fails, doctor reports the install error and preserves the configured plugin entry for the next repair attempt.
...
- Set `MERCLAW_SERVICE_REPAIR_POLICY=external` when another supervisor owns the gateway lifecycle. Doctor still reports gateway/service health and applies non-service repairs, but skips service install/start/restart/bootstrap and legacy service cleanup.
- On Linux, doctor ignores inactive extra gateway-like systemd units and does not rewrite command/entrypoint metadata for a running systemd gateway service during repair. Stop the service first or use `merclaw gateway install --force` when you intentionally want to replace the active launcher.
...
- Doctor includes a memory-search readiness check and can recommend `merclaw configure --section model` when embedding credentials are missing.
...
- Doctor reports an info note when Codex-mode agents are configured and personal Codex CLI assets exist in the operator's Codex home. Local Codex app-server launches use isolated per-agent homes, so install the Codex plugin first if needed, then use `merclaw migrate plan codex` to inventory assets that should be promoted deliberately.
...
- If sandbox mode is enabled but Docker is unavailable, doctor reports a high-signal warning with remediation (`install Docker` or `merclaw config set agents.defaults.sandbox.mode off`).
- If legacy sandbox registry files (`~/.merclaw/sandbox/containers.json` or `~/.merclaw/sandbox/browsers.json`) are present, doctor reports them; `merclaw doctor --fix` migrates valid entries into sharded registry directories and quarantines invalid legacy files.
...
If you previously ran `launchctl setenv MERCLAW_GATEWAY_TOKEN ...` (or `...PASSWORD`), that value overrides your config file and can cause persistent "unauthorized" errors.

```bash
launchctl getenv MERCLAW_GATEWAY_TOKEN
launchctl getenv MERCLAW_GATEWAY_PASSWORD

launchctl unsetenv MERCLAW_GATEWAY_TOKEN
launchctl unsetenv MERCLAW_GATEWAY_PASSWORD
```
