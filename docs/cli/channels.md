---
summary: "CLI reference for `merclaw channels` (accounts, status, login/logout, logs)"
read_when:
...
Do not use `merclaw sessions`, Gateway `sessions.list`, or the agent
...
```bash
merclaw channels add --channel telegram --token <bot-token>
merclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
merclaw channels remove --channel telegram --delete
```

<Tip>
`merclaw channels add --help` shows per-channel flags (token, private key, app token, signal-cli paths, etc).
</Tip>
...
If a channel plugin needs to be installed during a flag-driven add command, MerClaw uses the channel's default install source without opening the interactive plugin install prompt.

When you run `merclaw channels add` without flags, the interactive wizard can prompt:
...
You can also manage the same routing rules later with `merclaw agents bindings`, `merclaw agents bind`, and `merclaw agents unbind` (see [agents](/cli/agents)).

When you add a non-default account to a channel that is still using single-account top-level settings, MerClaw promotes account-scoped top-level values into the channel's account map before writing the new account. Most channels land those values in `channels.<channel>.accounts.default`, but bundled channels can preserve an existing matching promoted account instead. Matrix is the current example: if one named account already exists, or `defaultAccount` points at an existing named account, promotion preserves that account instead of creating a new `accounts.default`.
...
- Run `merclaw status --deep` for a broad probe.
- Use `merclaw doctor` for guided fixes.
- `merclaw channels list` no longer prints model provider usage/quota snapshots. For those, use `merclaw status` (overview) or `merclaw models list` (per-provider).
- `merclaw channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
merclaw channels resolve --channel slack "#general" "@jane"
merclaw channels resolve --channel discord "My Server/#support" "@someone"
merclaw channels resolve --channel matrix "Project Room"
```
