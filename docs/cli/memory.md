---
summary: "CLI reference for `merclaw memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
merclaw memory status
merclaw memory status --deep
merclaw memory status --fix
merclaw memory index --force
merclaw memory search "meeting notes"
merclaw memory search --query "deployment" --max-results 20
merclaw memory promote --limit 10 --min-score 0.75
merclaw memory promote --apply
merclaw memory promote --json --min-recall-count 0 --min-unique-queries 0
merclaw memory promote-explain "router vlan"
merclaw memory promote-explain "router vlan" --json
merclaw memory rem-harness
merclaw memory rem-harness --json
merclaw memory status --json
merclaw memory status --deep --index
merclaw memory status --deep --index --verbose
merclaw memory status --agent main
merclaw memory index --agent main --verbose
```
...
```bash
merclaw memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `merclaw cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
merclaw memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
merclaw memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
