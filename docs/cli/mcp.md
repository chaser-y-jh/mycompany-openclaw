---
summary: "Expose MerClaw channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to MerClaw-backed channels
  - Running `merclaw mcp serve`
  - Managing MerClaw-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`merclaw mcp` has two jobs:

- run MerClaw as an MCP server with `merclaw mcp serve`
- manage MerClaw-owned outbound MCP server definitions with `list`, `show`, `status`, `doctor`, `probe`, `add`, `set`, `configure`, `tools`, `login`, `logout`, `reload`, and `unset`

In other words:

- `serve` is MerClaw acting as an MCP server
- the other subcommands are MerClaw acting as an MCP client-side registry for MCP servers its runtimes may consume later

Use [`merclaw acp`](/cli/acp) when MerClaw should host a coding harness session itself and route that runtime through ACP.

## Choose the right MCP path

MerClaw has several MCP surfaces. Pick the one that matches who owns the agent runtime and who owns the tools.

| Goal                                                                | Use                                                                  | Why                                                                                                             |
| ------------------------------------------------------------------- | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Let an external MCP client read/send MerClaw channel conversations | `merclaw mcp serve`                                                 | MerClaw is the MCP server and exposes Gateway-backed conversations over stdio.                                 |
| Save third-party MCP servers for MerClaw-managed agent runs        | `merclaw mcp add`, `set`, `configure`, `tools`, `login`             | MerClaw is the MCP client-side registry and later projects those servers into eligible runtimes.               |
| Check a saved server without running an agent turn                  | `merclaw mcp status`, `doctor`, `probe`                             | `status` and `doctor` inspect config; `probe` opens a live MCP connection and lists capabilities.               |
...
| Run ACP-hosted harness sessions                                     | [`merclaw acp`](/cli/acp) and [ACP Agents](/tools/acp-agents-setup) | ACP bridge mode does not accept per-session MCP server injection; configure gateway/plugin bridges instead.     |

<Tip>
If you are not sure which path you need, start with `merclaw mcp status --verbose`. It shows what MerClaw has saved without starting any MCP servers.
</Tip>

## MerClaw as an MCP server

This is the `merclaw mcp serve` path.

### When to use `serve`

Use `merclaw mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to MerClaw-backed channel conversations
- you already have a local or remote MerClaw Gateway with routed sessions
- you want one MCP server that works across MerClaw's channel backends instead of running separate per-channel bridges

Use [`merclaw acp`](/cli/acp) instead when MerClaw should host the coding runtime itself and keep the agent session inside MerClaw.

### How it works

`merclaw mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote MerClaw Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `merclaw mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the MerClaw Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `merclaw agent` and `merclaw infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by MerClaw (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when MerClaw already has session state with a known route such as:
...
<Tabs>
  ...
  <Tab title="Remote Gateway (token)">
    ```bash
    merclaw mcp serve --url wss://gateway-host:18789 --token-file ~/.merclaw/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    merclaw mcp serve --url wss://gateway-host:18789 --password-file ~/.merclaw/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    merclaw mcp serve --verbose
    merclaw mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the MerClaw equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying MerClaw channel configuration
...
MerClaw ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `merclaw mcp serve`
...
## MerClaw as an MCP client registry

This is the `merclaw mcp list`, `show`, `status`, `doctor`, `probe`, `add`, `set`,
`configure`, `tools`, `login`, `logout`, `reload`, and `unset` path.

These commands do not expose MerClaw over MCP. They manage MerClaw-owned MCP server definitions under `mcp.servers` in MerClaw config.

Those saved definitions are for runtimes that MerClaw launches or configures later, such as embedded MerClaw and other runtime adapters. MerClaw stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write MerClaw config
    ...
    - embedded MerClaw exposes configured MCP tools in normal `coding` and `messaging` tool profiles; `minimal` still hides them, and `tools.deny: ["bundle-mcp"]` disables them explicitly
    - per-server `toolFilter.include` and `toolFilter.exclude` filter discovered MCP tools before they become MerClaw tools
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded MerClaw consumes MerClaw `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

Codex app-server also honors an optional `codex` block on each server. This is
MerClaw projection metadata for Codex app-server threads only; it does not
change ACP sessions, generic Codex harness config, or other runtime adapters.
Use non-empty `codex.agents` to project a server only into specific MerClaw
...
MerClaw strips the `codex` metadata before handing the native `mcp_servers`
config to Codex.

### Saved MCP server definitions

MerClaw also stores a lightweight MCP server registry in config for surfaces that want MerClaw-managed MCP definitions.

Commands:

- `merclaw mcp list`
- `merclaw mcp show [name]`
- `merclaw mcp status [--verbose]`
- `merclaw mcp doctor [name] [--probe]`
- `merclaw mcp probe [name]`
- `merclaw mcp add <name> [flags]`
- `merclaw mcp set <name> <json>`
- `merclaw mcp configure <name> [flags]`
- `merclaw mcp tools <name> [--include csv] [--exclude csv] [--clear]`
- `merclaw mcp login <name> [--code code]`
- `merclaw mcp logout <name>`
- `merclaw mcp reload`
- `merclaw mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `merclaw mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
merclaw mcp list
merclaw mcp show context7 --json
merclaw mcp status --verbose
merclaw mcp doctor --probe
merclaw mcp probe context7 --json
merclaw mcp add memory --command npx --arg -y --arg @modelcontextprotocol/server-memory
merclaw mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
merclaw mcp tools context7 --include 'resolve-library-id,get-library-docs'
merclaw mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
merclaw mcp configure docs --timeout 20 --connect-timeout 5 --include 'search,read_*'
merclaw mcp configure docs --auth oauth --oauth-scope 'docs.read'
merclaw mcp login docs
merclaw mcp logout docs
merclaw mcp unset context7
```

### Common server recipes

These examples save server definitions only. Run `merclaw mcp doctor --probe` afterward to prove that the server starts and exposes tools.

<Tabs>
  <Tab title="Filesystem">
    ```bash
    merclaw mcp add files \
      --command npx \
      --arg -y \
      --arg @modelcontextprotocol/server-filesystem \
      --arg "$HOME/Documents" \
      --include 'read_file,list_directory,search_files'
    merclaw mcp doctor files --probe
    ```
  ...
  <Tab title="Memory">
    ```bash
    merclaw mcp add memory \
      --command npx \
      --arg -y \
      --arg @modelcontextprotocol/server-memory
    merclaw mcp probe memory --json
    ```
  ...
  <Tab title="Local script">
    ```bash
    merclaw mcp add local-tools \
      --command node \
      --arg ./dist/mcp-server.js \
      --cwd /srv/merclaw-tools \
  ...
  <Tab title="Desktop/CUA">
    ```bash
    merclaw mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
    merclaw mcp tools cua-driver --include 'list_apps,observe,click,type'
    merclaw mcp doctor cua-driver --probe
    ```
...
<AccordionGroup>
  <Accordion title="status --json">
    ```json
    {
      "path": "/home/user/.merclaw/merclaw.json",
      "servers": [
        {
          "name": "docs",
          "configured": true,
      ...
      "servers": [
        {
          ...
          "issues": [
            {
              "level": "error",
              "message": "OAuth credentials are not authorized; run merclaw mcp login docs"
            }
          ]
        }
      ]
...
| `auth: "oauth"`                | Use MCP OAuth token storage and `merclaw mcp login`             |
...
Sensitive values in `url` (userinfo) and `headers` are redacted in logs and status output. `merclaw mcp doctor` warns when sensitive-looking `headers` or `env` entries contain literal values, so operators can move those values out of committed config.
...
<Steps>
  <Step title="Save the server">
    ...
    ```bash
    merclaw mcp set docs '{"url":"https://mcp.example.com/mcp","transport":"streamable-http","auth":"oauth","oauth":{"scope":"docs.read"}}'
    ```
  ...
  <Step title="Start login">
    ...
    MerClaw prints the authorization URL and stores temporary OAuth verifier state under the MerClaw state directory.

  </Step>
  <Step title="Finish with the code">
    After approving in the browser, pass the returned code back to MerClaw.

    ```bash
    merclaw mcp login docs --code abc123
    ```
...
If the provider rotates tokens or the authorization state gets stuck, run `merclaw mcp logout <name>`, then repeat `login`. `logout` can clear credentials for a saved HTTP server even after `auth: "oauth"` has been removed from config, as long as the server name and URL still identify the credential store entry.
...
| ------------------------------ | -------------------------------------------------------------------------------------- |
| `url`                          | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`                    | Set to `"streamable-http"` to select this transport; when omitted, MerClaw uses `sse` |
| `headers`                      | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs`          | Per-server connection timeout in ms (optional)                                         |
| `connectTimeout`               | Per-server connection timeout in seconds (optional)                                    |
| `timeout` / `requestTimeoutMs` | Per-server MCP request timeout in seconds or ms                                        |
...
MerClaw config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `merclaw mcp set` and repaired by `merclaw doctor --fix` in existing config, but `transport` is what embedded MerClaw consumes directly.
...
Use the page for operator edits and quick inventory. Use `merclaw mcp doctor --probe` or `merclaw mcp probe` when you need live server proof.
...
7. Run `merclaw mcp doctor --probe` when you need live proof that the edited server starts and lists tools.
...
- active runtimes may need `merclaw mcp reload`, Gateway config publish, or process restart depending on which process owns the MCP clients
