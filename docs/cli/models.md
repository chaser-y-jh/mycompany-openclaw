---
summary: "CLI reference for `merclaw models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
merclaw models list
merclaw models set <model-or-alias>
merclaw models scan
```

`merclaw models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise MerClaw falls back to matching OAuth/API-key
...
the command uses `MERCLAW_AGENT_DIR` if set, otherwise the
configured default agent.
Probe rows can come from auth profiles, env credentials, or `models.json`.
For OpenAI ChatGPT/Codex OAuth troubleshooting, `merclaw models status`,
`merclaw models auth list --provider openai`, and
`merclaw config get agents.defaults.model --json` are the quickest way to
...
- If you omit the provider, MerClaw resolves the input as an alias first, then
  ...
  If that provider no longer exposes the configured default model, MerClaw
...
By default MerClaw tries to probe tool and image support with live model calls.
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `MERCLAW_AGENT_DIR`)
...
- `no_model`: provider auth exists, but MerClaw could not resolve a probeable
...
```bash
merclaw models auth add
merclaw models auth list [--provider <id>] [--json]
merclaw models auth login --provider <id>
merclaw models auth login --provider openai --profile-id openai:work
merclaw models auth paste-api-key --provider <id>
merclaw models auth setup-token --provider <id>
merclaw models auth paste-token
```
...
`merclaw plugins list` to see which providers are installed.
Use `merclaw models auth --agent <id> <subcommand>` to write auth results to a
...
usually as a backup for Codex subscription limits. Run `merclaw doctor --fix`
to migrate older legacy OpenAI Codex prefix auth/profile state to `openai`.

Examples:

```bash
merclaw models auth login --provider openai --set-default
merclaw models auth login --provider openai --method api-key
merclaw models auth paste-api-key --provider openai
...
- `paste-api-key` accepts API keys generated elsewhere, prompts for the key
  ...
  `printf "%s\n" "$OPENAI_API_KEY" | merclaw models auth paste-api-key --provider openai`.
...
- Anthropic note: Anthropic staff told us MerClaw-style Claude CLI usage is allowed again, so MerClaw treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic `setup-token` / `paste-token` remain available as a supported MerClaw token path, but MerClaw now prefers Claude CLI reuse and `claude -p` when available.
