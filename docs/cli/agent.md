---
summary: "CLI reference for `merclaw agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
merclaw agent --to +15555550123 --message "status update" --deliver
merclaw agent --agent ops --message "Summarize logs"
merclaw agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
merclaw agent --session-key agent:ops:incident-42 --message "Summarize status"
merclaw agent --agent ops --session-key incident-42 --message "Summarize status"
merclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium
merclaw agent --to +15555550123 --message "Trace logs" --verbose on --json
merclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
merclaw agent --agent ops --message "Run locally" --local
```
