---
summary: "CLI reference for `merclaw sessions` (list stored sessions + usage)"
read_when:
...
until a message is processed. Use `merclaw channels status --probe`,
`merclaw status --deep`, or `merclaw health --verbose` when you need live
channel connectivity.

`merclaw sessions` and Gateway `sessions.list` responses are bounded by
...
```bash
merclaw sessions
merclaw sessions --agent work
merclaw sessions --all-agents
merclaw sessions --active 120
merclaw sessions --limit 25
merclaw sessions --verbose
merclaw sessions --json
```
...
```bash
merclaw sessions tail
merclaw sessions tail --follow
merclaw sessions tail --session-key "agent:main:telegram:direct:123" --tail 25
merclaw sessions --agent work tail --follow
merclaw sessions --all-agents tail --follow
```

`merclaw sessions tail` renders recent trajectory JSONL events as compact progress lines. Without `--session-key`, it tails running sessions first, then the latest stored session. `--tail <count>` controls how many existing events print before follow mode; the default is `80`, and `0` starts at the current end. `--follow` keeps watching the selected trajectory files, including relocated files referenced by `<session>.trajectory-path.json`.
...
```bash
merclaw sessions export-trajectory --session-key "agent:main:telegram:direct:123" --workspace .
merclaw sessions export-trajectory --session-key "agent:main:telegram:direct:123" --output bug-123 --json
```
...
inside `.merclaw/trajectory-exports/` under the selected workspace.

`merclaw sessions --all-agents` reads configured agent stores. Gateway and ACP
...
`merclaw sessions --all-agents --json`:

```json
{
  "path": null,
  "stores": [
    { "agentId": "main", "path": "/home/user/.merclaw/agents/main/sessions/sessions.json" },
    { "agentId": "work", "path": "/home/user/.merclaw/agents/work/sessions/sessions.json" }
  ],
  "allAgents": true,
  "count": 2,
  "totalCount": 2,
...
merclaw sessions cleanup --agent work --dry-run
merclaw sessions cleanup --all-agents --dry-run
merclaw sessions cleanup --enforce
merclaw sessions cleanup --enforce --active-key "agent:main:telegram:direct:123"
merclaw sessions cleanup --dry-run --fix-dm-scope
merclaw sessions cleanup --json
```

`merclaw sessions cleanup` uses `session.maintenance` settings from config:

- Scope note: `merclaw sessions cleanup` maintains session stores, transcripts, and trajectory sidecars. It does not prune cron run history, which is managed by `cron.runLog.keepLines` in [Cron configuration](/automation/cron-jobs#configuration) and explained in [Cron maintenance](/automation/cron-jobs#maintenance).
...
`merclaw sessions cleanup --all-agents --dry-run --json`:
...
```json
{
  ...
  "stores": [
    {
      "agentId": "main",
      "storePath": "/home/user/.merclaw/agents/main/sessions/sessions.json",
      "beforeCount": 120,
      "afterCount": 80,
      "missing": 0,
      "dmScopeRetired": 0,
    ...
    {
      "agentId": "work",
      "storePath": "/home/user/.merclaw/agents/work/sessions/sessions.json",
      "beforeCount": 18,
      "afterCount": 18,
      "missing": 0,
      "dmScopeRetired": 0,
