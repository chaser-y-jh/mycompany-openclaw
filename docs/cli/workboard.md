---
summary: "CLI reference for `merclaw workboard` cards, dispatch, and worker runs"
read_when:
...
`merclaw workboard` is the terminal surface for the bundled
...
```bash
merclaw workboard list [--board <id>] [--status <status>] [--json]
merclaw workboard create <title...> [--notes <text>] [--status <status>] [--priority <priority>] [--agent <id>] [--board <id>] [--labels <items>] [--json]
merclaw workboard show <id> [--json]
merclaw workboard dispatch [--url <url>] [--token <token>] [--timeout <ms>] [--json]
```
...
merclaw workboard list --board default --status ready
merclaw workboard list --json
```
...
```bash
merclaw workboard create "Fix stale worker heartbeat" --priority high --labels bug,workboard
merclaw workboard create "Write Workboard docs" --status ready --agent docs-agent --board docs --notes "Cover CLI, slash command, dispatch, and SQLite state."
```
...
```bash
merclaw workboard show 7f4a2c10
merclaw workboard show 7f4a2c10 --json
```
...
merclaw workboard dispatch --json
merclaw workboard dispatch --url http://127.0.0.1:18789 --token "$MERCLAW_GATEWAY_TOKEN"
```
...
Local `list`, `create`, and `show` commands operate on the local MerClaw state
directory used by the current profile. Use `--dev` or `--profile <name>` on the
top-level `merclaw` command when you need a different state root.
...
Then retry `merclaw workboard dispatch`. Data-only fallback is useful for local
