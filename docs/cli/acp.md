...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an MerClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`merclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to MerClaw channel
conversations instead of hosting an ACP harness session, use
[`merclaw mcp serve`](/cli/mcp) instead.
...
- MerClaw acts as an ACP server
- an IDE or ACP client connects to MerClaw
- MerClaw forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where MerClaw runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to MerClaw: use `merclaw acp`
- MerClaw should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include MerClaw parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                                |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the MerClaw gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
merclaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
merclaw acp --url wss://gateway-host:18789 --token-file ~/.merclaw/gateway.token

# Attach to an existing session key
merclaw acp --session agent:main:main

# Attach by label (must already exist)
merclaw acp --session-label "support inbox"

# Reset the session key before the first prompt
merclaw acp --session agent:main:main --reset-session
```
...
merclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.merclaw/gateway.token

# Override the server command (default: merclaw)
merclaw acp client --server "node" --server-args merclaw.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run MerClaw through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`merclaw acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `merclaw gateway call sessions.list` as the only ACP proof. That
...
it to drive an MerClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `merclaw acp` over stdio.

Example config (persisted):

```bash
merclaw config set gateway.remote.url wss://gateway-host:18789
merclaw config set gateway.remote.token <token>
```
...
```bash
merclaw acp --session agent:main:main
merclaw acp --session agent:design:main
merclaw acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see MerClaw plugin tools or selected
...
[MerClaw tools MCP bridge](/tools/acp-agents-setup#merclaw-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
MerClaw bot over ACP, use `acpx` with its built-in `merclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx merclaw` at `merclaw acp`.
3. Target the MerClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default MerClaw ACP session
acpx merclaw exec "Summarize the active MerClaw session state."

# Persistent named session for follow-up turns
acpx merclaw sessions ensure --name codex-bridge
acpx merclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my MerClaw work agent for recent context relevant to this repo."
```

If you want `acpx merclaw` to target a specific Gateway and session key every
time, override the `merclaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "merclaw": {
      "command": "env MERCLAW_HIDE_BANNER=1 MERCLAW_SUPPRESS_NOTES=1 merclaw acp --url ws://127.0.0.1:18789 --token-file ~/.merclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local MerClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env MERCLAW_HIDE_BANNER=1 MERCLAW_SUPPRESS_NOTES=1 node merclaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an MerClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select "MerClaw ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`MERCLAW_GATEWAY_TOKEN`, `MERCLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`MERCLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `MERCLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `merclaw acp client` sets `MERCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `merclaw`).
