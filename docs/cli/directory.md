---
summary: "CLI reference for `merclaw directory` (self, peers, groups)"
read_when:
...
- `directory` is meant to help you find IDs you can paste into other commands (especially `merclaw message send --target ...`).
...
```bash
merclaw directory peers list --channel slack --query "U0"
merclaw message send --channel slack --target user:U012ABCDEF --message "hello"
```
...
```bash
merclaw directory peers list --channel zalouser
merclaw directory peers list --channel zalouser --query "name"
merclaw directory peers list --channel zalouser --limit 50
```

## Groups

```bash
merclaw directory groups list --channel zalouser
