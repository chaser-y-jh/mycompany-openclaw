---
summary: "CLI reference for `merclaw update` (safe-ish source update + gateway auto-restart)"
read_when:
  - You want to update a source checkout safely
  - You are debugging `merclaw update` output or options
...
Safely update MerClaw and switch between stable/beta/dev channels.
...
merclaw update wizard
merclaw update --channel beta
merclaw update --channel dev
merclaw update --tag beta
merclaw update --tag main
merclaw update --dry-run
...
- `--tag <dist-tag|version|spec>`: override the package target for this update only. For package installs, `main` maps to `github:merclaw/merclaw#main`; GitHub/git source specs are packed into a temporary tarball before the staged global npm install.
...
`merclaw update` does not have a `--verbose` flag. Use `--dry-run` to preview
the planned channel/tag/install/restart actions, `--json` for machine-readable
results, and `merclaw update status --json` when you only need channel and
...
<Note>
In Nix mode (`MERCLAW_NIX_MODE=1`), mutating `merclaw update` runs are disabled. Update the Nix source or flake input for this install instead; for nix-merclaw, use the agent-first [Quick Start](https://github.com/merclaw/nix-merclaw#quick-start). `merclaw update status` and `merclaw update --dry-run` remain read-only.
</Note>
...
merclaw update status --timeout 10
```
...
When you switch channels explicitly (`--channel ...`), MerClaw also keeps the
install method aligned:

- `dev` → ensures a git checkout (default: `~/merclaw`, or `$MERCLAW_HOME/merclaw` when
  `MERCLAW_HOME` is set; override with `MERCLAW_GIT_DIR`),
...
and the helper runs the normal `merclaw update --yes --json` CLI path from
...
For package-manager installs, `merclaw update` resolves the target package
version before invoking the package manager. npm global installs use a staged
install: MerClaw installs the new package into a temporary npm prefix, verifies
...
installed MerClaw build while leaving full plugin-command completion rebuilds to
explicit `merclaw completion --write-state` runs.
...
not supervising it, MerClaw re-bootstraps the LaunchAgent automatically, then
...
manual `merclaw gateway restart` hint. With `--no-restart`,
...
- `ok: true`, `result.status: "skipped"`,
  ...
  `merclaw update --yes --json` outside the live service process.
- `ok: false`, `result.reason: "managed-service-handoff-unavailable"`, and
  `handoff.status: "unavailable"` mean MerClaw could not find a supervising
...
result. `merclaw status` and `merclaw status --all` show an `Update restart`
...
<Steps>
  ...
  <Step title="Preflight build (dev only)">
    Runs the TypeScript build in a temp worktree. If the tip fails, walks back up to 10 commits to find the newest buildable commit. Set `MERCLAW_UPDATE_PREFLIGHT_LINT=1` to also run lint during this preflight; lint runs in constrained serial mode because user update hosts are often smaller than CI runners.
  </Step>
  <Step title="Rebase">
  ...
  <Step title="Run doctor">
    `merclaw doctor` runs as the final safe-update check.
  </Step>
...
beta release, MerClaw falls back to the recorded default/latest spec and reports
that as a warning. For npm plugins, MerClaw also falls back when the beta
...
<Warning>
If an exact pinned npm plugin update resolves to an artifact whose integrity differs from the stored install record, `merclaw update` aborts that plugin artifact update instead of installing it. Reinstall or update the plugin explicitly only after verifying that you trust the new artifact.
</Warning>

<Note>
Post-update plugin sync failures that are scoped to a managed plugin and that the sync path can route around (e.g. an unreachable npm registry for a non-essential plugin) are reported as warnings after the core update succeeds. The JSON result keeps the top-level update `status: "ok"` and reports `postUpdate.plugins.status: "warning"` with `merclaw doctor --fix` and `merclaw plugins inspect <id> --runtime --json` guidance. Unexpected updater or sync exceptions still fail the update result. Fix the plugin install or update error, then rerun `merclaw doctor --fix` or `merclaw update`.

After the per-plugin sync step, `merclaw update` runs a mandatory **post-core convergence** pass before the gateway is restarted: it repairs missing configured plugin payloads, validates each _active_ tracked install record on disk, and statically verifies its `package.json` is parseable (and any explicitly-declared `main` exists). Failures from this pass — and an invalid MerClaw config snapshot — return `postUpdate.plugins.status: "error"` and flip the top-level update `status` to `"error"`, so `merclaw update` exits non-zero and the gateway is _not_ restarted with an unverified plugin set. The error includes structured `postUpdate.plugins.warnings[].guidance` lines pointing at `merclaw doctor --fix` and `merclaw plugins inspect <id> --runtime --json` for follow-up. Disabled plugin entries and records that are not trusted-source-linked official sync targets are skipped here, mirroring the `skipDisabledPlugins` policy used by the missing-payload check, so a stale disabled plugin record cannot block an otherwise valid update.
...
`merclaw --update` rewrites to `merclaw update` (useful for shells and launcher scripts).

## Related

- `merclaw doctor` (offers to run update first on git checkouts)
