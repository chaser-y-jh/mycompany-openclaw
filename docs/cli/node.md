---
summary: "CLI reference for `merclaw node` (headless node host)"
read_when:
...
`merclaw node run` and `merclaw node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `MERCLAW_GATEWAY_TOKEN` / `MERCLAW_GATEWAY_PASSWORD` are checked first.
...
trusted private-DNS names, set `MERCLAW_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `merclaw.json` config
key.
`merclaw node install` persists it into the supervised node service when it is
...
```bash
merclaw node install --host <gateway-host> --port 18789
```
...
merclaw node uninstall
```

Use `merclaw node run` for a foreground node host (no service).
...
- `merclaw approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, MerClaw prepares a canonical `systemRunPlan`
