---
summary: "CLI reference for `merclaw cron` (schedule and run background jobs)"
read_when:
...
<Tip>
Run `merclaw cron --help` for the full command surface. See [Cron jobs](/automation/cron-jobs) for the conceptual guide.
</Tip>

## Create jobs quickly

`merclaw cron create` is an alias for `merclaw cron add`. For new jobs, put the schedule first and the prompt second:
...
`merclaw cron list` and `merclaw cron show <job-id>` preview the resolved delivery route. For `channel: "last"`, the preview shows whether the route resolved from the main or current session, or will fail closed.
...
If an isolated run times out before the first model request, `merclaw cron show`
and `merclaw cron runs` include a phase-specific error such as
...
Skipped runs are tracked separately from execution errors. They do not affect retry backoff, but `merclaw cron edit <job-id> --failure-alert-include-skipped` can opt failure alerts into repeated skipped-run notifications.
...
Note: cron jobs, pending runtime state, and run history live in the shared SQLite state database. Legacy `jobs.json`, `jobs-state.json`, and `runs/*.jsonl` files are imported once and renamed with a `.migrated` suffix. After import, edit schedules with `merclaw cron add|edit|remove` instead of editing JSON files.

### Manual runs

`merclaw cron run <job-id>` force-runs by default and returns as soon as the manual run is queued. Successful responses include `{ ok: true, enqueued: true, runId }`. Use the returned `runId` to inspect the later result:

```bash
merclaw cron run <job-id>
merclaw cron runs --id <job-id> --run-id <run-id>
```

Add `--wait` when a script should block until that exact queued run records a terminal status:

```bash
merclaw cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
```
...
- When a job has `--model` but no fallback list is configured, MerClaw passes an explicit empty fallback override so the agent primary is not appended as a hidden retry target.
- Local-provider preflight checks walk configured fallbacks before marking a cron run `skipped`.

`merclaw doctor` reports jobs that already have `payload.model` set, including provider namespace counts and mismatches against `agents.defaults.model`. Use that check when auth, provider, or billing behavior looks different between live chat and scheduled jobs.
...
```bash
merclaw cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
merclaw cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
merclaw cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
merclaw cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```

Announce to a Telegram forum topic:

```bash
merclaw cron edit <job-id> --announce --channel telegram --to "-1001234567890" --thread-id 42
```
...
```bash
merclaw cron list
merclaw cron list --agent ops
merclaw cron get <job-id>
merclaw cron show <job-id>
merclaw cron run <job-id>
merclaw cron run <job-id> --due
merclaw cron run <job-id> --wait --wait-timeout 10m
merclaw cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
merclaw cron runs --id <job-id> --limit 50
merclaw cron runs --id <job-id> --run-id <run-id>
```

`merclaw cron list` shows all matching jobs by default. Pass `--agent <id>` to show only jobs whose effective normalized agent id matches; jobs without a stored agent id count as the configured default agent.

`merclaw cron get <job-id>` returns the stored job JSON directly. Use `cron show <job-id>` when you want the human-readable view with delivery-route preview.
...
```bash
merclaw cron edit <job-id> --agent ops
merclaw cron edit <job-id> --clear-agent
merclaw cron edit <job-id> --session current
merclaw cron edit <job-id> --session "session:daily-brief"
```

`merclaw cron add` warns when `--agent` is omitted on agent-turn jobs and falls back to the default agent (`main`). Pass `--agent <id>` at create time to pin a specific agent.

Delivery tweaks:

```bash
merclaw cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
merclaw cron edit <job-id> --webhook "https://example.invalid/merclaw/cron"
merclaw cron edit <job-id> --best-effort-deliver
merclaw cron edit <job-id> --no-best-effort-deliver
merclaw cron edit <job-id> --no-deliver
```
