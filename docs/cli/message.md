---
summary: "CLI reference for `merclaw message` (send + channel actions)"
read_when:
...
```
merclaw message <subcommand> [flags]
```
...
- `merclaw message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, MerClaw will attempt a live directory lookup when the provider supports it.
...
- `merclaw message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, MerClaw does not force a `default` account SecretRef scope
...
```
merclaw message send --channel discord \
...
```bash
merclaw message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
merclaw message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
merclaw message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
merclaw message poll --channel msteams \
...
```
merclaw message react --channel slack \
...
```
merclaw message react --channel signal \
...
```
merclaw message send --channel telegram --target @mychat --message "Choose:" \
...
```
merclaw message send --channel telegram --target 123456789 --message "Open app:" \
...
```bash
merclaw message send --channel telegram --target @mychat \
