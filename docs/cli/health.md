---
summary: "CLI reference for `merclaw health` (gateway health snapshot via RPC)"
read_when:
...
merclaw health
merclaw health --json
merclaw health --timeout 2500
merclaw health --verbose
merclaw health --debug
```

Notes:

- Default `merclaw health` asks the running gateway for its health snapshot. When the
