---
summary: "CLI reference for `merclaw dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You're setting up split DNS for a custom discovery domain (example: merclaw.internal)
title: "DNS"
---
...
```bash
merclaw dns setup
merclaw dns setup --domain merclaw.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `merclaw.internal`)
...
- zone file path
- current tailnet IPs
- recommended `merclaw.json` discovery config
...
- If `--domain` is omitted, MerClaw uses `discovery.wideArea.domain` from config.
