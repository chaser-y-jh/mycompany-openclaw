---
summary: "MerClaw Gateway CLI (`merclaw gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is MerClaw's WebSocket server (channels, nodes, sessions, hooks). Subcommands in this page live under `merclaw gateway …`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How MerClaw advertises and finds gateways.
  </Card>
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    - By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.merclaw/merclaw.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
    - `merclaw onboard --mode local` and `merclaw setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
<ParamField path="--token <token>" type="string">
  Token override (also sets `MERCLAW_GATEWAY_TOKEN` for the process).
</ParamField>
...
<ParamField path="--bind custom + gateway.customBindHost" type="string">
  Expects an IPv4 address today. For IPv6-only BYOH, place an IPv4 sidecar or proxy in front of the Gateway and point MerClaw at that IPv4 endpoint.
</ParamField>
...
merclaw gateway restart --safe
merclaw gateway restart --safe --skip-deferral
merclaw gateway restart --force
```

`merclaw gateway restart --safe` asks the running Gateway to preflight active MerClaw work before restarting. If queued operations, reply delivery, embedded runs, or task runs are active, the Gateway reports the blockers, coalesces duplicate safe restart requests, and restarts once the active work drains. Plain `restart` keeps the existing service-manager behavior for compatibility. Use `--force` only when you explicitly want the immediate override path.

`merclaw gateway restart --safe --skip-deferral` runs the same MerClaw-aware coordinated restart as `--safe`, but bypasses the active-work deferral gate so the Gateway emits the restart immediately even when blockers are reported. Use it as the operator escape hatch when a deferral has been pinned by a stuck task run and `--safe` alone would wait indefinitely. `--skip-deferral` requires `--safe`.
...
- Set `MERCLAW_GATEWAY_STARTUP_TRACE=1` to log phase timings during Gateway startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings for installed-index, manifest registry, startup planning, and owner-map work.
- Set `MERCLAW_GATEWAY_RESTART_TRACE=1` to log restart-scoped `restart trace:` lines for restart signal handling, active-work drain, shutdown phases, next start, ready timing, and memory metrics.
- Set `MERCLAW_DIAGNOSTICS=timeline` with `MERCLAW_DIAGNOSTICS_TIMELINE_PATH=<path>` to write a best-effort JSONL startup diagnostics timeline for external QA harnesses. You can also enable the flag with `diagnostics.flags: ["timeline"]` in config; the path is still env-provided. Add `MERCLAW_DIAGNOSTICS_EVENT_LOOP=1` to include event-loop samples.
...
```bash
merclaw gateway health --url ws://127.0.0.1:18789
```
...
```bash
merclaw gateway usage-cost
merclaw gateway usage-cost --days 7
merclaw gateway usage-cost --json
```
...
```bash
merclaw gateway stability
merclaw gateway stability --type payload.large
merclaw gateway stability --bundle latest
merclaw gateway stability --bundle latest --export
merclaw gateway stability --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - On fatal Gateway exits, shutdown timeouts, and restart startup failures, MerClaw writes the same diagnostic snapshot to `~/.merclaw/logs/stability/merclaw-stability-*.json` when the recorder has events. Inspect the newest bundle with `merclaw gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` also apply to bundle output.
...
merclaw gateway diagnostics export --output merclaw-diagnostics.zip
merclaw gateway diagnostics export --json
```
...
merclaw gateway status --require-rpc
```
...
```bash
merclaw gateway call status
merclaw gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
responsible for eventually exec'ing `merclaw` or Node with those args.

```bash
cat > ~/.local/bin/merclaw-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- merclaw "$@"
EOF
chmod +x ~/.local/bin/merclaw-doppler

merclaw gateway install --wrapper ~/.local/bin/merclaw-doppler --force
merclaw gateway restart
```
...
`MERCLAW_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor
repairs.

```bash
MERCLAW_WRAPPER="$HOME/.local/bin/merclaw-doppler" merclaw gateway install --force
merclaw doctor
```

To remove a persisted wrapper, clear `MERCLAW_WRAPPER` while reinstalling:

```bash
MERCLAW_WRAPPER= merclaw gateway install --force
merclaw gateway restart
```
...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - `gateway restart --safe` asks the running Gateway to preflight active MerClaw work and defer the restart until reply delivery, embedded runs, and task runs drain. `--safe` cannot be combined with `--force` or `--wait`.
    ...
    - `gateway restart --safe --skip-deferral` runs the MerClaw-aware safe restart but bypasses the deferral gate so the Gateway emits the restart immediately even when blockers are reported. Operator escape hatch for stuck-task-run deferrals; requires `--safe`.
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `MERCLAW_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `MERCLAW_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
`gateway discover` scans for Gateway beacons (`_merclaw-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `merclaw.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
...
```bash
merclaw gateway discover --timeout 4000
merclaw gateway discover --json | jq '.beacons[].wsUrl'
```
