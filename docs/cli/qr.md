---
summary: "CLI reference for `merclaw qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
merclaw qr
merclaw qr --setup-code-only
merclaw qr --json
merclaw qr --remote
merclaw qr --url wss://gateway.example/ws
```
...
- With `--remote`, MerClaw requires either `gateway.remote.url` or
