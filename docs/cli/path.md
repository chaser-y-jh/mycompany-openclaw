---
summary: "CLI reference for `merclaw path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
merclaw plugins enable oc-path
```

## Why use it

MerClaw state is spread across human-edited markdown, commented JSONC config,
...
`merclaw path` gives those callers a stable address instead of a one-off grep,
...
You probably do not need `merclaw path` for ordinary whole-file edits, rich
...
```bash
merclaw path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
merclaw path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
merclaw path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
merclaw path resolve 'oc://AGENTS.md/runtime-safety/merclaw-gateway'
```

Validate a path in CI or a preflight script before the script reads or writes:

```bash
merclaw path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
# Read a leaf
merclaw path resolve 'oc://gateway.jsonc/version'

# Wildcard search
merclaw path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
merclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
merclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
merclaw path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
merclaw path emit ./AGENTS.md
```
...
merclaw path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
merclaw path set 'oc://merclaw.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
merclaw path set 'oc://merclaw.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
merclaw path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
merclaw path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
merclaw path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
merclaw path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
merclaw path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
merclaw path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
merclaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
merclaw path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
merclaw path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
merclaw path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
merclaw path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ merclaw path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ merclaw path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ merclaw path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ merclaw path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ merclaw path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ merclaw path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ merclaw path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: merclaw.invoke
```

```bash
$ merclaw path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ merclaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
merclaw path resolve 'oc://AGENTS.md/tools/gh/risk' --human
merclaw path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
merclaw path find 'oc://AGENTS.md/tools/**/risk'
merclaw path find 'oc://session.jsonl/[event=action]/userId'
merclaw path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
merclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
merclaw path set 'oc://gateway.jsonc/version' '2.0'
merclaw path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ merclaw path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
merclaw path emit ./AGENTS.md
merclaw path emit ./gateway.jsonc --json
```
...
`merclaw path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__MERCLAW_REDACTED__` (verbatim or as a substring) is refused at write
  time.
