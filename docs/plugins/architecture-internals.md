...
At startup, MerClaw does roughly this:
...
The manifest is the control-plane source of truth. MerClaw uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, MerClaw
...
MerClaw does not cache plugin discovery results or direct manifest registry
...
MerClaw still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, MerClaw calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that MerClaw no longer calls, such as
...
| --  | _(built-in model lookup)_         | MerClaw tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
| 24  | `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared MerClaw refreshers                                                                                          |
...
that still runs on MerClaw's normal inference loop.
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of MerClaw's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as MerClaw adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  product plugins own their routes and schemas while MerClaw owns the
...
- MerClaw only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "MerClaw plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-merclaw-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-merclaw-scopes` only when the header is explicitly present
  - if `x-merclaw-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-merclaw-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `merclaw/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `merclaw/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `merclaw/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `merclaw/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `merclaw/plugin-sdk/config-schema` | Root `merclaw.json` Zod schema (`MerClawSchema`) |
...
<Info>
`merclaw/plugin-sdk/channel-runtime`, `merclaw/plugin-sdk/channel-lifecycle`,
small channel helper facades, `merclaw/plugin-sdk/outbound-runtime`,
`merclaw/plugin-sdk/outbound-send-deps`, `merclaw/plugin-sdk/config-runtime`,
and `merclaw/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `merclaw/plugin-sdk/*` subpaths. Never
...
`merclaw/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape MerClaw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to MerClaw's
...
- if both `catalog` and `discovery` are registered, MerClaw uses `catalog`
...
- Read-only command paths such as `merclaw status`, `merclaw status --all`,
  `merclaw channels status`, `merclaw channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `merclaw.extensions`:
...
Security guardrail: every `merclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `merclaw plugins install` installs plugin dependencies with a
...
Optional: `merclaw.setupEntry` can point at a lightweight setup-only module.
When MerClaw needs setup surfaces for a disabled channel plugin, or
...
Optional: `merclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let MerClaw load the
...
Channel plugins can advertise setup/discovery metadata via `merclaw.channel` and
install hints via `merclaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@merclaw/nextcloud-talk",
  "merclaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@merclaw/nextcloud-talk",
...
Useful `merclaw.channel` fields beyond the minimal example:
...
MerClaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.merclaw/mpm/plugins.json`
- `~/.merclaw/mpm/catalog.json`
- `~/.merclaw/plugins/catalog.json`

Or point `MERCLAW_PLUGIN_CATALOG_PATHS` (or `MERCLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "merclaw": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `merclaw.install` block. The
...
2. add typed plugin registration/runtime surfaces
   Extend `MerClawPluginApi` and/or `api.runtime` with the smallest useful
...
This is how MerClaw stays opinionated without becoming hardcoded to one
