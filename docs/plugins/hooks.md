...
Plugin hooks are in-process extension points for MerClaw plugins. Use them
...
current plugin options; MerClaw injects it per handler without mutating the
...
- `subagent_spawned` includes `resolvedModel` and `resolvedProvider` when MerClaw has resolved the child session's native model before launch.
...
use `merclaw sessions` or the Gateway session/status surfaces. When debugging
...
- MerClaw strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, MerClaw stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when MerClaw can
...
identifier when MerClaw can derive one from the session key or delivery
metadata.
...
forever. A timeout is logged and MerClaw continues; it does not cancel
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when MerClaw can derive a
...
Codex native `Stop` hooks are relayed into this hook as MerClaw
...
reach the next model turn exactly once. MerClaw drains queued injections before
...
context when syncing external wake schedulers, and keep MerClaw as the
