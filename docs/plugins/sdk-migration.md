...
read_when:
  - You see the MERCLAW_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the MERCLAW_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before MerClaw 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external MerClaw plugin
---

MerClaw has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`merclaw/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`merclaw/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`merclaw/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`merclaw/extension-api`** - a bridge that gave plugins direct access to
...
MerClaw does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`merclaw/plugin-sdk/\<subpath\>`)
...
`merclaw/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `merclaw doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `merclaw/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `MerClawConfig` | `merclaw/plugin-sdk/config-contracts` |
    | Already-loaded config assertions and plugin-entry config lookup | `merclaw/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `merclaw/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `merclaw/plugin-sdk/config-mutation` |
    | Session store helpers | `merclaw/plugin-sdk/session-store-runtime` |
    | Markdown table config | `merclaw/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `merclaw/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `merclaw/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `merclaw/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate embedded tool-result extensions to middleware">
    ...
    ```typescript
    // MerClaw and Codex runtime dynamic tools
    api.registerAgentToolResultMiddleware(async (event) => {
      return compactToolResult(event);
    }, {
      runtimes: ["merclaw", "codex"],
    });
    ```
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `merclaw/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `merclaw/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "merclaw/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "merclaw/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "merclaw/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "merclaw/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "merclaw/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedAgent } from "merclaw/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `merclaw/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `merclaw/plugin-sdk/system-event-runtime` |
    | Heartbeat wake, event, and visibility helpers | `merclaw/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `merclaw/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `merclaw/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `merclaw/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `merclaw/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `merclaw/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `merclaw/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `merclaw/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `merclaw/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `merclaw/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `merclaw/plugin-sdk/error-runtime` |
    | Transport readiness waits | `merclaw/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `merclaw/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `merclaw/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `merclaw/plugin-sdk/number-runtime` |
    | Process-local async lock | `merclaw/plugin-sdk/async-lock-runtime` |
    | File locks | `merclaw/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `merclaw/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `MerClawSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | MerClaw-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@merclaw/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`merclaw/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`merclaw/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "merclaw/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "merclaw/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `merclaw/plugin-sdk/channel-inbound` or
    `merclaw/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `merclaw/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `merclaw/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `merclaw/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `merclaw/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    MerClaw no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. MerClaw downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="MerClawSchemaType alias → MerClawConfig">
    `MerClawSchemaType` re-exported from `merclaw/plugin-sdk` is now a
    one-line alias for `MerClawConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { MerClawSchemaType } from "merclaw/plugin-sdk";
    // After
    import type { MerClawConfig } from "merclaw/plugin-sdk/config-schema";
    ```
...
```bash
MERCLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 merclaw gateway run
MERCLAW_SUPPRESS_EXTENSION_API_WARNING=1 merclaw gateway run
```
