---
summary: "Step-by-step guide to building a messaging channel plugin for MerClaw"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect MerClaw to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects MerClaw to a
...
<Info>
  If you have not built any MerClaw plugin before, read
...
Channel plugins do not need their own send/edit/react tools. MerClaw keeps one
...
`defineChannelMessageAdapter` from `merclaw/plugin-sdk/channel-outbound`. The
...
receive/send lifecycle helpers on `merclaw/plugin-sdk/channel-outbound`.

Channels migrating inbound authorization can use the experimental
`merclaw/plugin-sdk/channel-ingress-runtime` subpath from runtime receive
...
Use `merclaw/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `merclaw/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If custom approval auth intentionally allows only same-chat fallback, return `markImplicitSameChatApprovalAuthorization({ authorized: true })` from `merclaw/plugin-sdk/approval-auth-runtime`; otherwise core treats the result as explicit approver authorization.
...
- `merclaw/plugin-sdk/approval-auth-runtime`
- `merclaw/plugin-sdk/approval-client-runtime`
- `merclaw/plugin-sdk/approval-delivery-runtime`
- `merclaw/plugin-sdk/approval-gateway-runtime`
- `merclaw/plugin-sdk/approval-handler-adapter-runtime`
- `merclaw/plugin-sdk/approval-handler-runtime`
- `merclaw/plugin-sdk/approval-native-runtime`
- `merclaw/plugin-sdk/approval-reply-runtime`
- `merclaw/plugin-sdk/channel-runtime-context`

Likewise, prefer `merclaw/plugin-sdk/setup-runtime`,
`merclaw/plugin-sdk/setup-runtime`,
`merclaw/plugin-sdk/reply-runtime`,
`merclaw/plugin-sdk/reply-dispatch-runtime`,
`merclaw/plugin-sdk/reply-reference`, and
`merclaw/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `merclaw/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `merclaw/plugin-sdk/setup-runtime` includes the env-aware adapter seam for
  `createEnvPatchedAccountSetupAdapter`
- `merclaw/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `merclaw.setupEntry` in
...
- use the broader `merclaw/plugin-sdk/setup` seam only when you also need the
...
- `merclaw/plugin-sdk/account-core`,
  `merclaw/plugin-sdk/account-id`,
  `merclaw/plugin-sdk/account-resolution`, and
  `merclaw/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `merclaw/plugin-sdk/inbound-envelope` and
  `merclaw/plugin-sdk/channel-inbound` for inbound route/envelope and
  record-and-dispatch wiring
- `merclaw/plugin-sdk/channel-targets` for target parsing helpers
- `merclaw/plugin-sdk/outbound-media` for media loading and
  `merclaw/plugin-sdk/channel-outbound` for outbound identity/send delegates
  and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `merclaw/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `merclaw/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `merclaw/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `merclaw/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `merclaw/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `merclaw/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`merclaw/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#merclaw-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/merclaw-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "merclaw": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect MerClaw to Acme Chat."
        }
      }
    }
    ```

    ```json merclaw.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { MerClawConfig } from "merclaw/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "merclaw/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so MerClaw
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "merclaw/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    MerClaw loads this instead of the full entry when the channel is disabled
    ...
    `merclaw/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    MerClaw. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to MerClaw.
...
├── package.json              # merclaw.channel metadata
├── merclaw.plugin.json      # Manifest with config schema
