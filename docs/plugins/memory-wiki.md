...
If the active memory plugin exposes shared recall artifacts, MerClaw can search
...
currently exposing public bridge inputs yet. Run `merclaw wiki doctor` first,
...
`merclaw wiki status`, `merclaw wiki doctor`, and `merclaw wiki bridge
...
merclaw wiki apply synthesis "Alpha Summary" --body "..." --source-id source.alpha
