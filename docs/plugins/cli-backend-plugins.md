...
read_when:
  ...
  - You need to map a third-party CLI into MerClaw's text fallback runner
---

CLI backend plugins let MerClaw call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points MerClaw at the plugin runtime module              |
| Manifest ownership   | `merclaw.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ...
    entry is `./src/index.ts`, add `merclaw.runtimeExtensions` that points at
  ...
  <Step title="Declare backend ownership">
    ```json merclaw.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through MerClaw",
    ...
    `cliBackends` is the runtime ownership list. It lets MerClaw auto-load the
  ...
  <Step title="Register the backend">
    ...
    } from "merclaw/plugin-sdk/cli-backend";
...
`CliBackendConfig` describes how MerClaw should launch and parse the CLI:
...
| `modelAliases`                            | Map MerClaw model ids to CLI-native ids                    |
| `sessionArg` / `sessionArgs`              | How to pass a session id                                    |
| `sessionMode`                             | `always`, `existing`, or `none`                             |
| `sessionIdFields`                         | JSON fields MerClaw reads from CLI output                  |
...
| `defaultAuthProfileId`             | Prefer a specific MerClaw auth profile                |
| `authEpochMode`                    | Decide how auth changes invalidate stored CLI sessions |
| `nativeToolMode`                   | Declare whether the CLI has always-on native tools     |
| `bundleMcp` / `bundleMcpMode`      | Opt into MerClaw's loopback MCP tool bridge           |
...
CLI backends do not receive MerClaw tools by default. If the CLI can consume an
...
"always-on"` so MerClaw can fail closed when a caller requires no native tools.
...
```bash
merclaw plugins inspect acme-cli --runtime --json
merclaw agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `merclaw.extensions` and built runtime entries for published packages</Check>
<Check>`merclaw.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
