...
Message presentation is MerClaw's shared contract for rich outbound chat UI.
...
```bash
merclaw message send --channel slack \
  --target channel:C123 \
...
```bash
merclaw message send --channel telegram \
...
Use helpers from `merclaw/plugin-sdk/interactive-runtime` when bridging older
code:
