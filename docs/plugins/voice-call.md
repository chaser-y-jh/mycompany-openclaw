...
read_when:
  - You want to place an outbound voice call from MerClaw
...
Voice calls for MerClaw via a plugin. Supports outbound notifications,
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    `streaming.*` OpenAI keys are rewritten by `merclaw doctor --fix`.
    Runtime fallback still accepts the old voice-call keys for now, but
    the rewrite path is `merclaw doctor --fix` and the compat shim is
    temporary.
...
- Voice Call exposes the shared `merclaw_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal MerClaw tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `merclaw_agent_consult`.
...
configured MerClaw agent without paying a full agent-consult round trip on
...
`merclaw_agent_consult` still run the full MerClaw agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call merclaw_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `merclaw doctor --fix`; committed config should use `tts.providers.<provider>`.
...
Provider signatures are checked against the public URL MerClaw reconstructs
...
Use `merclaw logs --follow` for the live phase trace. A healthy Twilio Meet
...
`merclaw voicecall tail` still shows persisted call records; it is useful for
...
- `merclaw logs --follow` shows realtime TwiML served, the realtime bridge
