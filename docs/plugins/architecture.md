...
read_when:
  - Building or debugging native MerClaw plugins
...
This is the **deep architecture reference** for the MerClaw plugin system. For practical guides, start with one of the focused pages below.
...
Capabilities are the public **native plugin** model inside MerClaw. Every native MerClaw plugin registers against one or more capability types:
...
MerClaw classifies every loaded plugin into a shape based on its actual registration behavior (not just static metadata):
...
Use `merclaw plugins inspect <id>` to see a plugin's shape and capability breakdown. See [CLI reference](/cli/plugins#inspect) for details.
...
When you run `merclaw doctor` or `merclaw plugins inspect <id>`, you may see one of these labels:
...
Neither `hook-only` nor `before_agent_start` will break your plugin today: `hook-only` is advisory, and `before_agent_start` only triggers a warning. These signals also appear in `merclaw status --all` and `merclaw plugins doctor`.

## Architecture overview

MerClaw's plugin system has four layers:

<Steps>
  <Step title="Manifest + discovery">
    MerClaw finds candidate plugins from configured paths, workspace roots, global plugin roots, and bundled plugins. Discovery reads native `merclaw.plugin.json` manifests plus supported bundle manifests first.
  </Step>
  ...
  <Step title="Runtime loading">
    Native MerClaw plugins are loaded in-process and register capabilities into a central registry. Packaged JavaScript loads through native `require`; third-party local source TypeScript is the emergency Jiti fallback. Compatible bundles are normalized into registry records without importing runtime code.
  </Step>
  <Step title="Surface consumption">
    The rest of MerClaw reads the registry to expose tools, channels, provider setup, hooks, HTTP routes, CLI commands, and services.
  </Step>
</Steps>
...
That keeps plugin-owned CLI code inside the plugin while still letting MerClaw reserve root command names before parsing.
...
That split lets MerClaw validate config, explain missing/disabled plugins, and build UI/schema hints before the full runtime is active.
...
Channel plugins do not need to register a separate send/edit/react tool for normal chat actions. MerClaw keeps one shared `message` tool in core, and channel plugins own the channel-specific discovery and execution behind it.
...
Bundled plugins follow the same rule. A bundled plugin's `runtime-api.ts` should not re-export its own branded `merclaw/plugin-sdk/<plugin-id>` facade. Those branded facades remain compatibility shims for external plugins and older consumers, but bundled plugins should use local exports plus narrow generic SDK subpaths such as `merclaw/plugin-sdk/channel-policy`, `merclaw/plugin-sdk/runtime-store`, or `merclaw/plugin-sdk/webhook-ingress`. New code should not add plugin-id-specific SDK facades unless the compatibility boundary for an existing external ecosystem requires it.
...
MerClaw treats a native plugin as the ownership boundary for a **company** or a **feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's MerClaw-facing surfaces
...
So if MerClaw adds a new domain such as video, the first question is not "which provider should hardcode video handling?" The first question is "what is the core video capability contract?" Once that contract exists, vendor plugins can register against it and channel/feature plugins can consume it.
...
A company plugin should feel cohesive from the outside. If MerClaw has shared contracts for models, speech, realtime transcription, realtime voice, media understanding, image generation, video generation, web fetch, and web search, a vendor can own all of its surfaces in one place:

```ts
import type { MerClawPluginDefinition } from "merclaw/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "merclaw/plugin-sdk/media-understanding";

const plugin: MerClawPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
MerClaw already treats image/audio/video understanding as one shared capability. The same ownership model applies there:
...
The plugin API surface is intentionally typed and centralized in `MerClawPluginApi`. That contract defines the supported registration points and the runtime helpers a plugin may rely on.
...
<AccordionGroup>
  ...
  <Accordion title="Contract tests">
    Bundled plugins are captured in contract registries during test runs so MerClaw can assert ownership explicitly. Today this is used for model providers, speech providers, web search providers, and bundled registration ownership.
  </Accordion>
</AccordionGroup>

The practical effect is that MerClaw knows, up front, which plugin owns which surface. That lets core and channels compose seamlessly because ownership is declared, typed, and testable rather than implicit.
...
<Tabs>
  ...
  <Tab title="Bad contracts">
    ...
    - ad hoc runtime objects that are not part of `MerClawPluginApi` or `api.runtime`
...
Native MerClaw plugins run **in-process** with the Gateway. They are not sandboxed. A loaded native plugin has the same process-level trust boundary as core code.

<Warning>
Native plugin implications: a plugin can register tools, network handlers, hooks, and services; a plugin bug can crash or destabilize the gateway; and a malicious native plugin is equivalent to arbitrary code execution inside the MerClaw process.
</Warning>

Compatible bundles are safer by default because MerClaw currently treats them as metadata/content packs. In current releases, that mostly means bundled skills.
...
For bundled workspace package names, keep the plugin id anchored in the npm name: `@merclaw/<id>` by default, or an approved typed suffix such as `-provider`, `-plugin`, `-speech`, `-sandbox`, or `-media-understanding` when the package intentionally exposes a narrower plugin role.
...
MerClaw exports capabilities, not implementation convenience.
