...
when MerClaw loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, MerClaw will use a
...
**Import:** `merclaw/plugin-sdk/tool-plugin`
...
schemas, wraps plain return values in the MerClaw tool-result format, and
exposes static metadata that `merclaw plugins build` writes into the plugin
manifest.
...
- `configSchema` is optional. When omitted, MerClaw uses a strict empty object
...
- Tool names are static. `merclaw plugins build` derives `contracts.tools`
...
- Runtime loading stays strict. Installed plugins still need
  `merclaw.plugin.json` and `package.json` `merclaw.extensions`; MerClaw does
  not execute plugin code to infer missing manifest data.

## `definePluginEntry`

**Import:** `merclaw/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | -                   |
| `configSchema` | `MerClawPluginConfigSchema \| () => MerClawPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: MerClawPluginApi) => void`                               | Yes      | -                   |

- `id` must match your `merclaw.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- MerClaw resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `merclaw/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: MerClawPluginApi) => void`                               | No       | -                   |
| `registerFull`        | `(api: MerClawPluginApi) => void`                               | No       | -                   |
...
- Discovery registration is non-activating, not import-free. MerClaw may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and MerClaw
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `merclaw nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; MerClaw resolves it to the
...
MerClaw loads this instead of the full entry when a channel is disabled,
...
- `merclaw/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
...
- `merclaw/plugin-sdk/channel-setup` for optional-install setup surfaces
- `merclaw/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`merclaw/plugin-sdk/channel-entry-contract` instead. That contract lets the
setup entry keep setup-safe plugin/secrets exports while still exposing a
runtime setter:

```typescript
import { defineBundledChannelSetupEntry } from "merclaw/plugin-sdk/channel-entry-contract";
...
the plugin entry and the channel plugin object so MerClaw can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want MerClaw to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; MerClaw rejects descriptor names outside
...
MerClaw classifies loaded plugins by their registration behavior:
...
Use `merclaw plugins inspect <id>` to see a plugin's shape.
