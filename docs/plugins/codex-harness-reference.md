...
By default, MerClaw starts the managed Codex binary shipped with the bundled
plugin:
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after MerClaw builds its inherited environment.                                                                                                                                                                                |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active MerClaw sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows MerClaw sandbox egress.                                                                                                          |
...
unattended MerClaw turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, MerClaw treats the implicit default as guardian
...
When an MerClaw sandbox is active, the local Codex app-server process still
runs on the Gateway host. MerClaw therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the MerClaw sandbox
backend. Shell access is exposed through MerClaw sandbox-backed dynamic tools
...
`workspace-write` without active MerClaw sandboxing. If you see
`bwrap: setting up uid map: Permission denied` or
`bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted`, run
`merclaw doctor` and fix the reported host namespace policy for the MerClaw
...
The stable default is fail-closed: active MerClaw sandboxing disables native
...
try Codex's remote environment support with MerClaw's sandbox backend. This
...
When the flag is on and the current MerClaw session is sandboxed, MerClaw
...
MerClaw-owned environment. If the app-server cannot register the environment,
...
loopback exec-server unless it is running on the same host, so MerClaw rejects
that combination.
...
1. An explicit MerClaw Codex auth profile for the agent.
...
Stdio app-server launches inherit MerClaw's process environment by default.
MerClaw owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's MerClaw state. That keeps Codex config,
accounts, plugin cache/data, and thread state scoped to the MerClaw agent
instead of leaking in from the operator's personal `~/.codex` home.

MerClaw does not rewrite `HOME` for normal local app-server launches. Codex-run
subprocesses such as `merclaw`, `gh`, `git`, cloud CLIs, and shell commands see
...
MerClaw plugins and MerClaw skill snapshots still flow through MerClaw's own
...
part of an MerClaw agent, inventory them explicitly:

```bash
merclaw migrate codex --dry-run
merclaw migrate apply codex --yes
```
...
Most remaining MerClaw integration tools, such as messaging, media, cron,
browser, nodes, gateway, `heartbeat_respond`, and `web_search`, are available
through Codex tool search under the `merclaw` namespace. This keeps the initial
...
surface, while explicit MerClaw or ACP delegation is still available through
the `merclaw` dynamic tool namespace.
...
Dynamic tool budgets are capped at 600000 ms. On timeout, MerClaw aborts the
...
post-tool idle budget: after MerClaw returns an `item/tool/call` response, after
...
`turn/completed`, MerClaw best-effort interrupts the native turn and releases
...
availability is owned by Codex app-server, so the list can change when MerClaw
...
If discovery fails or times out, MerClaw uses a bundled fallback catalog for:
...
Codex handles `AGENTS.md` itself through native project-doc discovery. MerClaw
...
For MerClaw workspace parity, the Codex harness resolves the other bootstrap
files. `SOUL.md`, `IDENTITY.md`, `TOOLS.md`, and `USER.md` are forwarded as
MerClaw Codex developer instructions because they define the active agent,
available workspace guidance, and user profile. The compact MerClaw skills
...
`BOOTSTRAP.md` when present is forwarded as MerClaw turn input reference
context.
