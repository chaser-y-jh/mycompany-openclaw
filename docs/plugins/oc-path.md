---
summary: "Bundled `oc-path` plugin: ships the `merclaw path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`merclaw path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the MerClaw repo under
...
- **Local automation**: shell scripts can resolve or update one workspace value
  with `merclaw path … --json` instead of carrying separate markdown, JSONC,
...
merclaw path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
merclaw path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `merclaw` CLI** on the host where you
...
The plugin metadata lives in `extensions/oc-path/merclaw.plugin.json`:
...
`merclaw path …`, so installs that never use the verb pay no cost.
...
state. Bare `merclaw path` invocations work immediately on the same host —
the CLI loads the plugin on demand.

Disable with:

```bash
merclaw plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `merclaw path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__MERCLAW_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`merclaw path` CLI reference](/cli/path)
