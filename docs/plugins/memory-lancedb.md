...
The plugin is published to npm and is not bundled into the MerClaw runtime image.
...
Set `dimensions` for non-standard embedding models. MerClaw knows the
...
`memory_forget` query path, and `merclaw ltm search`. Auto-recall prefers the
...
```bash
merclaw ltm list
merclaw ltm search "project preferences"
merclaw ltm stats
```
...
```bash
merclaw ltm query --cols id,text,createdAt --limit 20
merclaw ltm query --filter "category = 'preference'" --order-by createdAt:desc
```
...
By default, LanceDB data lives under `~/.merclaw/memory/lancedb`. Override the
path with `dbPath`:
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "s3://memory-bucket/merclaw",
          storageOptions: {
...
MerClaw treats that package as part of the plugin package. Gateway startup
...
`@lancedb/lancedb` error during plugin load, upgrade MerClaw and restart the
Gateway.
...
```bash
merclaw ltm stats
merclaw ltm search "recent preference"
```
