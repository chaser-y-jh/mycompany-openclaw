...
An **agent harness** is the low level executor for one prepared MerClaw agent
...
runtime and the normal MerClaw provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the MerClaw
  session transcript
...
Before a harness is selected, MerClaw has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the MerClaw transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an MerClaw-owned
policy bundle for runtime decisions that must stay shared across MerClaw and native
harnesses:
...
Harnesses may use the plan for decisions that need to match MerClaw behavior, but
should still treat it as host-owned attempt state. Do not mutate it or use it to
switch providers/models inside a turn.

## Register a harness

**Import:** `merclaw/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "merclaw/plugin-sdk/agent-harness";
...
MerClaw chooses a harness after provider/model resolution:
...
3. `auto` asks registered harnesses if they support the resolved
   provider/model.
4. If no registered harness matches, MerClaw uses its embedded runtime.
...
provider/model. Once a plugin harness has claimed a run, MerClaw does not
...
`agents.list[].agentRuntime`, and `MERCLAW_AGENT_RUNTIME`. `/status` shows the
...
MerClaw. The harness then claims that provider in `supports(...)`.
...
- app-server request: MerClaw sends the bare model id to Codex and lets the
...
MerClaw requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
MerClaw only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning MerClaw to the newer tested stable line.
...
seam is for async tool-result transforms that must run before MerClaw or Codex feeds
...
`merclaw/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so MerClaw's fallback policy can decide whether to retry on a
...
The bundled `codex` harness is the native Codex mode for embedded MerClaw
...
`merclaw doctor --fix`, and legacy `codex/*` model refs remain compatibility
aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. MerClaw still owns the chat channel,
...
By default, MerClaw uses `auto` provider/model runtime policy: registered
...
fail hard. This does not block an explicit provider/model `agentRuntime.id: "merclaw"`.
...
Keep that binding explicitly associated with the MerClaw session, and keep
mirroring user-visible assistant/tool output into the MerClaw transcript.

The MerClaw transcript remains the compatibility layer for:

- channel-visible session history
- transcript search and indexing
- switching back to the built-in MerClaw harness on a later turn
- generic `/new`, `/reset`, and session deletion behavior

If your harness stores a sidecar binding, implement `reset(...)` so MerClaw can
clear it when the owning MerClaw session is reset.

## Tool and media results

Core constructs the MerClaw tool list and passes it into the prepared attempt.
...
on the same delivery path as MerClaw-backed runs.
