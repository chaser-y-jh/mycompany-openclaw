...
The plugin is included with MerClaw, but it is off by default. When disabled, the route is not registered. When enabled, it adds:
...
Use the WebSocket RPC path for MerClaw clients and interactive tools that can keep a Gateway WebSocket connection open.
...
<Tabs>
  <Tab title="CLI">
    ```bash
    merclaw plugins enable admin-http-rpc
...
```bash
merclaw plugins disable admin-http-rpc
merclaw gateway restart
```
...
- For `token` and `password` auth, narrower `x-merclaw-scopes` headers are ignored and the normal full operator defaults are restored.
- Trusted identity-bearing HTTP modes honor `x-merclaw-scopes` when present.
...
The normal Gateway WebSocket RPC path remains the preferred control-plane API for MerClaw clients. Use admin HTTP RPC only for host tooling that needs a request/response HTTP surface.
