---
summary: "Build simple typed agent tools with defineToolPlugin and merclaw plugins init/build/validate"
title: "Tool plugins"
sidebarTitle: "Tool Plugins"
read_when:
  - You want to build a simple MerClaw plugin that only adds agent tools
...
Tool plugins add agent-callable tools to MerClaw without adding a channel,
model provider, hook, service, or setup backend. Use `defineToolPlugin` when the
plugin owns a fixed list of tools and you want MerClaw to generate the manifest
metadata that keeps those tools discoverable without loading runtime code.

The recommended flow is:

1. Scaffold a package with `merclaw plugins init`.
2. Write tools with `defineToolPlugin`.
3. Build JavaScript.
4. Generate `merclaw.plugin.json` and `package.json` metadata with
   `merclaw plugins build`.
...
- `merclaw >=2026.5.17`, the first MerClaw version that exports
  `merclaw/plugin-sdk/tool-plugin`.
- A package root that can ship `dist/`, `merclaw.plugin.json`, and
  `package.json`.
...
- `merclaw.plugin.json`: generated manifest metadata for the initial tool.
...
`merclaw plugins build` writes the matching `toolMetadata.<tool>.optional`
manifest entry, so MerClaw can discover the tool without loading plugin
runtime code.
...
`defineToolPlugin` wraps plain return values into the MerClaw tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and MerClaw to keep the original value in `details`.
...
`configSchema` is optional. If you omit it, MerClaw uses a strict empty object
...
MerClaw reads plugin config from the plugin entry in the Gateway config. Do not
...
MerClaw discovers installed plugins from cold metadata. It must be able to read
the plugin manifest before importing plugin runtime code. `defineToolPlugin`
therefore exposes static metadata, and `merclaw plugins build` writes that
...
`contracts.tools` is the important discovery contract. It tells MerClaw which
...
For the simple tool-plugin workflow, `merclaw plugins build` aligns
...
- `merclaw.plugin.json` exists and passes the normal manifest loader.
...
- `package.json` points `merclaw.extensions` at the selected runtime entry.

## Install and inspect locally

From a separate MerClaw checkout or installed CLI, install the package path:

```bash
merclaw plugins install ./stock-quotes
merclaw plugins inspect stock-quotes --runtime
```
...
```bash
npm pack
merclaw plugins install npm-pack:./merclaw-plugin-stock-quotes-0.1.0.tgz
merclaw plugins inspect stock-quotes --runtime --json
```
...
```bash
merclaw plugins install clawhub:your-org/stock-quotes
```

Bare npm package specs remain supported during the launch cutover, but ClawHub
is the preferred discovery and distribution surface for MerClaw plugins.
...
`merclaw plugins build --entry ./dist/index.js` or
`merclaw plugins validate --entry ./dist/index.js`.
...
### `merclaw.plugin.json generated metadata is stale`
...
Commit both `merclaw.plugin.json` and `package.json` changes.

### `package.json merclaw.extensions must include ./dist/index.js`

The package metadata points at a different runtime entry. Run
`merclaw plugins build --entry ./dist/index.js` so the generator aligns the
...
1. `merclaw plugins inspect <plugin-id> --runtime`
2. `merclaw plugins validate --root <plugin-root> --entry ./dist/index.js`
3. `merclaw.plugin.json` has `contracts.tools` with the expected tool names.
4. `package.json` has `merclaw.extensions: ["./dist/index.js"]`.
