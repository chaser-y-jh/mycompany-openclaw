---
summary: "Install and use Codex, Claude, and Cursor bundles as MerClaw plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how MerClaw maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

MerClaw can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
MerClaw maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native MerClaw plugins. Native plugins run
...
of requiring authors to rewrite them as native MerClaw plugins, MerClaw
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    merclaw plugins install ./my-bundle

    # Archive
    merclaw plugins install ./my-bundle.tgz
...
## What MerClaw maps from bundles

Not every bundle feature runs in MerClaw today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal MerClaw skills                                                 | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                        | Claude, Cursor |
| Hook packs    | MerClaw-style `HOOK.md` + `handler.ts` layouts                                                   | Codex          |
| MCP tools     | Bundle MCP config merged into embedded MerClaw settings; supported stdio and HTTP servers loaded | All formats    |
| LSP servers   | Claude `.lsp.json` and manifest-declared `lspServers` merged into embedded MerClaw LSP defaults  | Claude         |
| Settings      | Claude `settings.json` imported as embedded MerClaw defaults                                     | Claude         |

#### Skill content

- bundle skill roots load as normal MerClaw skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal MerClaw skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal MerClaw hook-pack
  layout. Today this is primarily the Codex-compatible case:
  - `HOOK.md`
  - `handler.ts` or `handler.js`

#### MCP for embedded MerClaw

- enabled bundles can contribute MCP server config
- MerClaw merges bundle MCP config into the effective embedded MerClaw settings as
  `mcpServers`
- MerClaw exposes supported bundle MCP tools during embedded MerClaw agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, MerClaw uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in MerClaw config. `merclaw mcp set` and `merclaw doctor --fix` normalize the common alias.
...
MerClaw registers bundle MCP tools with provider-safe names in the form
...
#### Embedded MerClaw settings

- Claude `settings.json` is imported as default embedded MerClaw settings when the
  bundle is enabled
- MerClaw sanitizes shell override keys before applying them
...
#### Embedded MerClaw LSP

- enabled Claude bundles can contribute LSP server config
- MerClaw loads `.lsp.json` plus any manifest-declared `lspServers` paths
- bundle LSP config is merged into the effective embedded MerClaw LSP defaults
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `merclaw plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but MerClaw does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit MerClaw best when they use skill roots and MerClaw-style
  ...
  <Accordion title="Claude bundles">
    ...
    - `settings.json` is imported into embedded MerClaw settings (shell override keys are sanitized)
    - `.mcp.json` exposes supported stdio tools to embedded MerClaw
    - `.lsp.json` plus manifest-declared `lspServers` paths load into embedded MerClaw LSP defaults
...
MerClaw checks for native plugin format first:

1. `merclaw.plugin.json` or valid `package.json` with `merclaw.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, MerClaw uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `merclaw plugins install` and ship everything
  they need in the installed plugin directory.
- MerClaw-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `merclaw doctor --fix` removes legacy staged dependency directories and can
...
- MerClaw does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `merclaw plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded MerClaw settings from `settings.json` are supported. MerClaw does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    MerClaw hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
