...
[`merclaw plugins install`](/cli/plugins).
...
```bash
export MERCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1
export MERCLAW_PLUGIN_INSTALL_OVERRIDES='{
  "codex": "npm-pack:/tmp/merclaw-codex-2026.5.8.tgz",
  "merclaw-web-search": "npm:@merclaw/web-search@2026.5.8"
}'
```
...
MerClaw uses the override source instead of the catalog, bundled, or default
...
entry normally represents an MerClaw-owned package, an override is treated as
operator-supplied test input.

Workspace `.env` files cannot enable install overrides. Set these variables in
the trusted shell, CI job, or remote test command that launches MerClaw.

## Package E2E

Use an isolated state directory so package installs and install records do not
touch your normal MerClaw state:

```bash
npm pack extensions/codex --pack-destination /tmp

MERCLAW_STATE_DIR="$(mktemp -d)" \
MERCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1 \
MERCLAW_PLUGIN_INSTALL_OVERRIDES='{"codex":"npm-pack:/tmp/merclaw-codex-2026.5.8.tgz"}' \
pnpm merclaw onboard --mode local
```

Verify the installed package under the state directory:

```bash
find "$MERCLAW_STATE_DIR/npm/projects" -path '*/node_modules/@merclaw/codex/package.json' -print
grep -R '"@merclaw/codex"' "$MERCLAW_STATE_DIR/npm/projects"/*/package-lock.json
```
