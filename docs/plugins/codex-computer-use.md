---
summary: "Set up Codex Computer Use for Codex-mode MerClaw agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode MerClaw agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. MerClaw
...
Use this page when MerClaw is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## MerClaw.app and Peekaboo

MerClaw.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want MerClaw.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode MerClaw agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an MerClaw node and exposes mobile
...
MerClaw-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through MerClaw's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the MerClaw command:

```bash
cua-driver mcp-config --client merclaw
```
...
available as a normal MerClaw MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. MerClaw
...
Computer Use in and lets MerClaw install or re-enable it before the turn:
...
With this config, MerClaw checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, MerClaw asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, MerClaw also tries to
...
plugin command surface is available. These are MerClaw chat/runtime commands,
not `merclaw codex ...` CLI subcommands:
...
MerClaw uses the same app-server API that Codex itself exposes. The
...
During install, MerClaw polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, MerClaw prefers
...
`computer-use` is registered, MerClaw tries to add the standard bundled
...
MerClaw reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If MerClaw says Computer Use
...
MerClaw intentionally fails closed when `computerUse.enabled` is true. A
...
fresh MerClaw session.

**A Computer Use tool says `Native hook relay unavailable`.** The Codex-native
tool hook could not reach an active MerClaw relay through the local bridge or
Gateway fallback. Start a fresh MerClaw session with `/new` or `/reset`. If it
works once and then fails again on a later tool call, `/new` is only clearing the
current attempt; restart the Codex app-server or MerClaw Gateway so old threads
