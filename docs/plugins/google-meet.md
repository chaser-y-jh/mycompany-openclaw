...
read_when:
  - You want an MerClaw agent to join a Google Meet call
  - You want an MerClaw agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for MerClaw — the plugin is explicit by design:
...
- `agent` is the default talk-back mode: realtime transcription listens, the
  configured MerClaw agent answers, and regular MerClaw TTS speaks into Meet.
...
provider plus regular MerClaw TTS. OpenAI is the default transcription
...
```bash
merclaw googlemeet setup --transport chrome-node --mode transcribe
```
...
before asking an agent to join. Use `merclaw googlemeet setup --json` for
...
```bash
merclaw googlemeet setup --transport twilio
```
...
```bash
merclaw googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
merclaw googlemeet create --transport chrome-node --mode agent
```
...
```bash
merclaw googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
- Browser fallback: used when OAuth credentials are absent. MerClaw uses the
  ...
  the MerClaw Chrome profile on the node to already be signed in to Google.
...
MerClaw's microphone/camera permission grant and avoid the Meet **Use
...
Use `merclaw googlemeet test-listen <meet-url> --transport chrome-node` when
...
Local Chrome joins through the signed-in MerClaw browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by MerClaw. For
...
You do **not** need a full MerClaw Gateway or model API key inside a macOS VM
...
- Gateway host: MerClaw Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: MerClaw CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update MerClaw in the VM, then enable the bundled plugin there:

```bash
merclaw plugins enable google-meet
```

Start the node host in the VM:

```bash
merclaw node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
```bash
MERCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  merclaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
MERCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  merclaw node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
merclaw node restart
```

`MERCLAW_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`merclaw.json` setting. `merclaw node install` stores it in the LaunchAgent
...
```bash
merclaw googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, MerClaw browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, MerClaw auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `merclaw node run` in the VM,
  approve pairing, and make sure `merclaw plugins enable google-meet` and
  `merclaw plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses MerClaw
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. MerClaw
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by MerClaw; use separate virtual devices or Loopback-style routing
...
MerClaw does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with MerClaw, review BlackHole's
...
Chrome transport opens the Meet URL through MerClaw browser control and joins
as the signed-in MerClaw browser profile. On macOS, the plugin checks for
...
```bash
merclaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
merclaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local MerClaw audio
...
meeting; MerClaw does not discover those from the Meet page.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an MerClaw agent. Be brief.",
...
```bash
merclaw config validate
merclaw plugins list | grep -E 'google-meet|voice-call'
merclaw googlemeet setup
```
...
`MERCLAW_GOOGLE_MEET_*` environment variables.
...
4. Add the scopes MerClaw requests:
...
`meetings.space.readonly` lets MerClaw resolve Meet URLs/codes to spaces.
`meetings.space.settings` lets MerClaw pass `SpaceConfig` settings such as
...
```bash
merclaw googlemeet auth login --json
```
...
```bash
MERCLAW_GOOGLE_MEET_CLIENT_ID="your-client-id" \
MERCLAW_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
merclaw googlemeet auth login --json
```
...
MERCLAW_GOOGLE_MEET_CLIENT_ID="your-client-id" \
MERCLAW_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
merclaw googlemeet auth login --json --manual
```
...
support existed, rerun `merclaw googlemeet auth login --json` so the refresh
...
```bash
merclaw googlemeet doctor --oauth --json
```
...
```bash
merclaw googlemeet doctor --oauth --create-space --json
merclaw googlemeet create --no-join --json
```
...
```bash
merclaw googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
merclaw googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `merclaw googlemeet auth login
...
- `MERCLAW_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `MERCLAW_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `MERCLAW_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `MERCLAW_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `MERCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `MERCLAW_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `MERCLAW_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
merclaw googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
merclaw googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
merclaw googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
merclaw googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
merclaw googlemeet latest --today
merclaw googlemeet calendar-events --today --json
merclaw googlemeet artifacts --event "Weekly sync"
merclaw googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
merclaw googlemeet latest --meeting https://meet.google.com/abc-defg-hij
merclaw googlemeet artifacts --conference-record conferenceRecords/abc123 --json
merclaw googlemeet attendance --conference-record conferenceRecords/abc123 --json
```
...
```bash
merclaw googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```
...
MerClaw accepts a Meet URL, meeting code, or `spaces/{id}` input and resolves it
to the API space resource before ending the active conference.
It is separate from `googlemeet leave`: `leave` stops MerClaw's local/session
...
```bash
merclaw googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
merclaw googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
merclaw googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
merclaw googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
MERCLAW_LIVE_TEST=1 \
MERCLAW_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
...
```bash
merclaw googlemeet setup --transport chrome-node --mode transcribe
merclaw googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

Live smoke environment:

- `MERCLAW_LIVE_TEST=1` enables guarded live tests.
- `MERCLAW_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `MERCLAW_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `MERCLAW_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `MERCLAW_GOOGLE_MEET_CLIENT_SECRET`,
  `MERCLAW_GOOGLE_MEET_ACCESS_TOKEN`, and
  `MERCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `MERCLAW_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the MerClaw browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the MerClaw browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, MerClaw reports `manualActionRequired: true` or a
...
realtime transcription provider key, and a configured MerClaw TTS provider.
...
- `chrome.guestName: "MerClaw Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through MerClaw browser automation on `chrome-node`
...
- `mode: "agent"`: default talk-back mode. Participant speech is transcribed by
  ...
  MerClaw agent in a per-meeting sub-agent session, and spoken back through the
  normal MerClaw TTS runtime.
- `mode: "bidi"`: fallback direct bidirectional realtime model mode. The
  ...
  `merclaw_agent_consult` for deeper/tool-backed answers.
...
- `realtime.instructions`: brief spoken replies, with
  `merclaw_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional MerClaw agent id for
  `merclaw_agent_consult`; defaults to `main`
...
VM. In both cases the model providers and `merclaw_agent_consult` run on the
Gateway host, so model credentials stay there. With the default `mode: "agent"`,
the realtime transcription provider handles listening, the configured MerClaw
agent produces the answer, and regular MerClaw TTS speaks it into Meet. Use
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: whether
  managed Chrome speech is allowed now. `speechReady: false` means MerClaw did
...
transcripts are routed through the configured MerClaw agent, and the answer is
spoken through the normal MerClaw TTS runtime. Set `mode: "bidi"` when you want
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured MerClaw agent | Normal MerClaw TTS runtime            | You want "my agent is in the meeting" behavior        |
...
information, or normal MerClaw tools, it can call `merclaw_agent_consult`.

The consult tool runs the regular MerClaw agent behind the scenes with recent
meeting transcript context and returns a concise spoken answer. In `agent` mode,
MerClaw sends that answer directly to the TTS runtime; in `bidi` mode, the
...
Meet lane should consult a dedicated MerClaw agent workspace, model defaults,
...
```bash
merclaw googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
```bash
merclaw googlemeet setup
merclaw nodes status --connected
merclaw nodes invoke \
  --node parallels-macos \
...
- `merclaw logs --follow` shows DTMF TwiML served before realtime TwiML, then a
...
```bash
merclaw plugins list | grep google-meet
merclaw googlemeet setup
```
...
```bash
merclaw plugins enable google-meet
merclaw plugins enable browser
MERCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  merclaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; MerClaw
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, MerClaw
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `MERCLAW_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `merclaw googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the MerClaw Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. MerClaw should click **Use microphone** or, for
...
```bash
merclaw googlemeet setup
merclaw googlemeet doctor
```

Use `mode: "agent"` for the normal STT -> MerClaw agent -> TTS talk-back path,
...
run `merclaw googlemeet status --json <session-id>` after participants speak
...
utterance but MerClaw did not see new output bytes reach the Chrome audio
bridge.
...
- Meet microphone and speaker are routed through the virtual audio path used by
  MerClaw. `doctor` should show `meet output routed: yes` for local Chrome
  realtime joins.
...
```bash
merclaw googlemeet recover-tab
merclaw googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
merclaw voicecall smoke --to "+15555550123"
```
...
```bash
merclaw voicecall smoke --to "+15555550123" --yes
```
...
- Run `merclaw googlemeet doctor <session-id>` to confirm the delegated Twilio
  call ID, whether DTMF was queued, and whether the intro greeting was requested.
- Run `merclaw voicecall status --call-id <id>` and confirm the call is still
  active.
- Run `merclaw voicecall tail` and check that Twilio webhooks are arriving at
  the Gateway.
- Run `merclaw logs --follow` and look for the Twilio Meet sequence: Google
...
- Re-run `merclaw googlemeet setup --transport twilio`; a green setup check is
...
- If the participant joins but you do not hear the greeting, check
  `merclaw logs --follow` for the post-DTMF `voicecall.speak` request and
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: MerClaw owns the
