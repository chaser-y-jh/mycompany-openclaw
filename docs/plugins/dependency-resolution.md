---
summary: "How MerClaw installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged MerClaw installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

MerClaw keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the MerClaw
package directory.
...
- SDK/core imports are peer or supplied MerClaw imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into MerClaw-owned package roots

MerClaw owns only the plugin lifecycle:
...
MerClaw uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.merclaw/npm/projects/<encoded-package>`
- git packages clone under `~/.merclaw/git`
...
```bash
cd ~/.merclaw/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`merclaw plugins install npm-pack:<path.tgz>` uses that same per-plugin npm
project root for a local npm-pack tarball. MerClaw reads the tarball's npm
...
`node_modules` beside the plugin package. MerClaw scans the managed project
...
publishable lockfile during install, and MerClaw's managed npm project root
supports it through the normal npm install path. MerClaw-owned publishable
...
MerClaw does not require it for community packages, but npm will respect it
when present.

MerClaw-owned npm plugin packages can also publish with explicit
...
with `merclaw.release.bundleRuntimeDependencies: false`; those packages still
...
`merclaw` package does not bundle its full dependency tree.

Plugins that import `merclaw/plugin-sdk/*` declare `merclaw` as a peer
dependency. MerClaw does not let npm install a separate registry copy of the
...
resolution/materialization and MerClaw reasserts plugin-local
`node_modules/merclaw` links for installed packages that declare the host peer
...
Local plugins are treated as developer-controlled directories. MerClaw does not
...
merclaw plugins install <source>
merclaw doctor --fix
```

`doctor --fix` can clean legacy MerClaw-generated dependency state and recover
...
Lightweight and core-critical bundled plugins are shipped as part of MerClaw.
...
In source checkouts, MerClaw treats the repository as a pnpm monorepo. After
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g merclaw`        | Built runtime tree inside the package | MerClaw package and explicit plugin install/update/doctor flows     |
...
| `merclaw plugins install ...`   | Managed npm project/git/ClawHub root  | The plugin install/update flow                                       |

## Legacy cleanup

Older MerClaw versions generated bundled-plugin dependency roots at startup or
...
`.merclaw-runtime-deps*` manifests, generated plugin `node_modules`, install
...
Older npm installs also used a shared `~/.merclaw/npm/node_modules` root.
