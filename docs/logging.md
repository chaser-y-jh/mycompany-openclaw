...
read_when:
  - You need a beginner-friendly overview of MerClaw logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

MerClaw has two main log surfaces:
...
MerClaw keeps up to five numbered archives beside the active file, such as
`merclaw-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.merclaw/merclaw.json`:

```json
{
  "logging": {
    "file": "/path/to/merclaw.log"
  }
}
```
...
or times out before `logs.tail` answers, `merclaw logs` falls back to the
configured Gateway file log automatically. Explicit `--url` targets do not use
this fallback. `merclaw logs --follow` is stricter: on Linux it uses the active
...
```bash
merclaw channels logs --channel whatsapp
```
...
MerClaw preserves the original structured log arguments alongside these fields
...
`merclaw gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
merclaw gateway
merclaw gateway --verbose --ws-log compact
merclaw gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.merclaw/merclaw.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/merclaw/merclaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`MERCLAW_LOG_LEVEL`** environment variable (e.g. `MERCLAW_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `merclaw.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `merclaw --log-level debug gateway run`), which overrides the environment variable for that command.
...
```bash
MERCLAW_DEBUG_MODEL_TRANSPORT=1 merclaw gateway
MERCLAW_DEBUG_MODEL_PAYLOAD=tools MERCLAW_DEBUG_SSE=events merclaw gateway
```

Available flags:

- `MERCLAW_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `MERCLAW_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `MERCLAW_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `MERCLAW_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `MERCLAW_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `MERCLAW_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `MERCLAW_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal MerClaw logging, so `merclaw logs --follow`
...
MerClaw writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
MerClaw can redact sensitive tokens before they hit console output, file logs,
...
policy. MerClaw still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `MERCLAW_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `merclaw doctor` first.
