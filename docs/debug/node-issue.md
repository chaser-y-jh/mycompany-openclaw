...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in MerClaw
...
Running MerClaw via Node with `tsx` fails at startup with:

```
[merclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `merclaw status` and `gateway:watch` worked.
