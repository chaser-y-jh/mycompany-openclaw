...
This is handy when you need to toggle obscure settings without editing `merclaw.json`.

Examples:

```
/debug show
/debug set messages.responsePrefix="[merclaw]"
...
Use `MERCLAW_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
MERCLAW_PLUGIN_LIFECYCLE_TRACE=1 merclaw plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm merclaw ...`
...
`MERCLAW_RUN_NODE_CPU_PROF_DIR`:

```bash
MERCLAW_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm merclaw status
```
...
```bash
MERCLAW_TRACE_SYNC_IO=1 pnpm merclaw gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched
Gateway child. Set `MERCLAW_TRACE_SYNC_IO=1` when you explicitly want Node
...
`merclaw-gateway-watch-main` (or a profile/port-specific variant such as
`merclaw-gateway-watch-dev-19001`) and auto-attaches from interactive terminals.
...
```bash
tmux attach -t merclaw-gateway-watch-main
```
...
# or
MERCLAW_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Disable auto-attach while keeping tmux management:

```bash
MERCLAW_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
`MERCLAW_TRACE_SYNC_IO=1` with `--benchmark` when you explicitly want both CPU
...
`MERCLAW_PROFILE`, `MERCLAW_CONFIG_PATH`, `MERCLAW_STATE_DIR`,
`MERCLAW_GATEWAY_PORT`, and `MERCLAW_SKIP_CHANNELS` into the pane. Put
...
`merclaw doctor --fix --non-interactive` once and restarts the Gateway child.
Use `MERCLAW_GATEWAY_WATCH_AUTO_DOCTOR=0` when you want the original startup
...
extension `package.json` and `merclaw.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.merclaw-dev` and
...
```bash
pnpm gateway:dev
MERCLAW_PROFILE=dev merclaw tui
```

If you don't have a global install yet, run the CLI via `pnpm merclaw ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `MERCLAW_PROFILE=dev`
   - `MERCLAW_STATE_DIR=~/.merclaw-dev`
   - `MERCLAW_CONFIG_PATH=~/.merclaw-dev/merclaw.json`
   - `MERCLAW_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`MERCLAW_SKIP_CHANNELS=1`).
...
```bash
MERCLAW_PROFILE=dev merclaw gateway --dev --reset
```
...
## Raw stream logging (MerClaw)

MerClaw can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.merclaw/logs/raw-stream.jsonl
```

Equivalent env vars:

```bash
MERCLAW_RAW_STREAM=1
MERCLAW_RAW_STREAM_PATH=~/.merclaw/logs/raw-stream.jsonl
```

Default file:

`~/.merclaw/logs/raw-stream.jsonl`
...
```bash
MERCLAW_RAW_STREAM_PATH=~/.merclaw/logs/raw-openai-completions.jsonl
```

Default file:

`~/.merclaw/logs/raw-openai-completions.jsonl`
...
- If you need to use the built MerClaw CLI for other tasks (i.e. `dashboard --no-open` if your debug session spawns a new auth token), you can execute it in another terminal as `node ./merclaw.mjs` or create a shell alias like `alias merclaw-build="node $(pwd)/merclaw.mjs"`
