...
MerClaw has three Vitest suites (unit/integration, e2e, live) and a small set
...
- [Matrix QA](/concepts/qa-matrix) - reference for `pnpm merclaw qa matrix`.
...
- Linux VM-backed QA lane: `pnpm merclaw qa suite --runner multipass --scenario channel-chat-baseline`
...
- Runtime performance reports: dispatch `MerClaw Performance` with
  ...
  `merclaw/clawgrit-reports` when `CLAWGRIT_REPORTS_TOKEN` is configured. The
...
- Docker live model sweep: `pnpm test:docker:live-models`
  - Each selected model now runs a text turn plus a small file-read-style probe.
    ...
    Disable the extra probes with `MERCLAW_LIVE_MODEL_FILE_PROBE=0` or
    `MERCLAW_LIVE_MODEL_IMAGE_PROBE=0` when isolating provider failures.
  - CI coverage: daily `MerClaw Scheduled Live And E2E Checks` and manual
    `MerClaw Release Checks` both call the reusable live/E2E workflow with
    `include_live_suites: true`, which includes separate Docker live model
    matrix jobs sharded by provider.
  - For focused CI reruns, dispatch `MerClaw Live And E2E Checks (Reusable)`
  ...
  - Add new high-signal provider secrets to `scripts/ci-hydrate-live-auth.sh`
    plus `.github/workflows/merclaw-live-and-e2e-checks-reusable.yml` and its
...
- Codex app-server harness smoke: `pnpm test:docker:live-codex-harness`
  - Runs gateway agent turns through the plugin-owned Codex app-server harness,
    ...
    `MERCLAW_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0` when isolating other Codex
    app-server failures. For a focused sub-agent check, disable the other probes:
    `MERCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 MERCLAW_LIVE_CODEX_HARNESS_MCP_PROBE=0 MERCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0 MERCLAW_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=1 pnpm test:docker:live-codex-harness`.
    This exits after the sub-agent probe unless
    `MERCLAW_LIVE_CODEX_HARNESS_SUBAGENT_ONLY=0` is set.
- Codex on-demand install smoke: `pnpm test:docker:codex-on-demand`
  - Installs the packaged MerClaw tarball in Docker, runs OpenAI API-key
...
- Crestodian first-run Docker smoke: `pnpm test:docker:crestodian-first-run`
  - Starts from an empty MerClaw state dir, verifies the modern onboard
    ...
    `pnpm merclaw qa suite --scenario crestodian-ring-zero-setup`.
- Moonshot/Kimi cost smoke: with `MOONSHOT_API_KEY` set, run
  `merclaw models list --provider moonshot --json`, then run an isolated
  `merclaw agent --local --session-id live-kimi-cost --message 'Reply exactly: KIMI_LIVE_OK' --thinking off --json`
...
`media`, `e2ee-smoke`, `e2ee-deep`, and `e2ee-cli` jobs. `MerClaw Release
...
`ghcr.io/merclaw/merclaw-live-media-runner:ubuntu-24.04`, which already has
`ffmpeg` and `ffprobe`. Docker live model/backend shards use the shared
`ghcr.io/merclaw/merclaw-live-test:<sha>` image built once per selected
commit, then pull it with `MERCLAW_SKIP_DOCKER_BUILD=1` instead of rebuilding
inside every shard.

- `pnpm merclaw qa suite`
  ...
  - Supports provider modes `live-frontier`, `mock-openai`, and `aimock`.
    ...
    `mock-openai` lane.
- `pnpm merclaw qa coverage --match <query>`
...
- `pnpm test:plugins:kitchen-sink-live`
  - Runs the live OpenAI Kitchen Sink plugin gauntlet through QA Lab. It
    ...
    `merclaw-testbox-env` helper is present.
...
- `pnpm merclaw qa suite --runner multipass`
...
- `pnpm test:docker:npm-onboard-channel-agent`
  ...
  - Use `MERCLAW_NPM_ONBOARD_CHANNEL=discord` to run the same packaged-install
    lane with Discord.
- `pnpm test:docker:session-runtime-context`
  - Runs a deterministic built-app Docker smoke for embedded runtime context
    transcripts. It verifies hidden MerClaw runtime context is persisted as a
    ...
    `merclaw doctor --fix` rewrites it to the active branch with a backup.
- `pnpm test:docker:npm-telegram-live`
  - Installs an MerClaw package candidate in Docker, runs installed-package
  ...
  - The wrapper mounts only the `qa-lab` harness source from the checkout; the
    installed package owns `dist`, `merclaw/plugin-sdk`, and bundled plugin
    runtime so the lane does not mix current checkout plugins into the package
    under test.
  - Defaults to `MERCLAW_NPM_TELEGRAM_PACKAGE_SPEC=merclaw@beta`; set
    `MERCLAW_NPM_TELEGRAM_PACKAGE_TGZ=/path/to/merclaw-current.tgz` or
    `MERCLAW_CURRENT_PACKAGE_TGZ` to test a resolved local tarball instead of
    installing from the registry.
  - Uses the same Telegram env credentials or Convex credential source as
    `pnpm merclaw qa telegram`. For CI/release automation, set
    `MERCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex` plus
    `MERCLAW_QA_CONVEX_SITE_URL` and the role secret. If
    `MERCLAW_QA_CONVEX_SITE_URL` and a Convex role secret are present in CI,
  ...
  - The wrapper validates Telegram or Convex credential env on the host before
    Docker build/install work. Set `MERCLAW_NPM_TELEGRAM_SKIP_CREDENTIAL_PREFLIGHT=1`
    only when deliberately debugging pre-credential setup.
  - `MERCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE=ci|maintainer` overrides the shared
    `MERCLAW_QA_CREDENTIAL_ROLE` for this lane only.
...
- GitHub Actions also exposes `Package Acceptance` for side-run product proof
  ...
  the normalized `merclaw-current.tgz` as `package-under-test`, then runs the
...
gh workflow run package-acceptance.yml --ref main \
  -f source=url \
  -f package_url=https://registry.npmjs.org/merclaw/-/merclaw-VERSION.tgz \
...
gh workflow run package-acceptance.yml --ref main \
  ...
  -f package_url=https://packages.example.internal:8443/artifactory/merclaw/merclaw-VERSION.tgz \
  -f package_sha256=<sha256> \
  -f suite_profile=package
```

`source=trusted-url` reads `.github/package-trusted-sources.json` from the trusted workflow ref and does not accept URL credentials or a workflow-input private-network bypass. If the named policy declares bearer auth, configure the fixed `MERCLAW_TRUSTED_PACKAGE_TOKEN` secret.
...
- `pnpm test:docker:plugins`
  - Packs and installs the current MerClaw build in Docker, starts the Gateway
  ...
  - Also installs a known older npm baseline, enables Telegram before running
    `merclaw update --tag <candidate>`, and verifies the candidate's
...
- `pnpm test:parallels:npm-update`
  - Runs the native packaged-install update smoke across Parallels guests. Each
    ...
    the installed `merclaw update` command in the same guest and verifies the
  ...
  - The OpenAI lane uses `openai/gpt-5.5` for the live agent-turn proof by
    ...
    `MERCLAW_PARALLELS_OPENAI_MODEL` when deliberately validating another
    OpenAI model.
  ...
  - The script writes nested lane logs under `/tmp/merclaw-parallels-npm-update.*`.
...
- `pnpm merclaw qa telegram`
  - Runs the Telegram live QA lane against a real private group using the driver and SUT bot tokens from env.
  - Requires `MERCLAW_QA_TELEGRAM_GROUP_ID`, `MERCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and `MERCLAW_QA_TELEGRAM_SUT_BOT_TOKEN`. The group id must be the numeric Telegram chat id.
  - Supports `--credential-source convex` for shared pooled credentials. Use env mode by default, or set `MERCLAW_QA_CREDENTIAL_SOURCE=convex` to opt into pooled leases.
...
```text
@merclaw-mantis telegram
@merclaw-mantis telegram scenario=telegram-status-command
@merclaw-mantis telegram scenarios=telegram-status-command,telegram-mentioned-message-reply
```
...
```text
@merclaw-mantis telegram desktop proof
```
...
- `pnpm merclaw qa mantis telegram-desktop-builder`
  - Leases or reuses a Crabbox Linux desktop, installs native Telegram Desktop, configures MerClaw with a leased Telegram SUT bot token, starts the gateway, and records screenshot/MP4 evidence from the visible VNC desktop.
  - Defaults to `--credential-source convex` so workflows only need the Convex broker secret. Use `--credential-source env` with the same `MERCLAW_QA_TELEGRAM_*` variables as `pnpm merclaw qa telegram`.
  - Telegram Desktop still needs a user login/profile. The bot token configures MerClaw only. Use `--telegram-profile-archive-env <name>` for a base64 `.tgz` profile archive, or use `--keep-lease` and log in manually through VNC once.
...
When `--credential-source convex` (or `MERCLAW_QA_CREDENTIAL_SOURCE=convex`) is enabled for
...
- `MERCLAW_QA_CONVEX_SITE_URL` (for example `https://your-deployment.convex.site`)
- One secret for the selected role:
  - `MERCLAW_QA_CONVEX_SECRET_MAINTAINER` for `maintainer`
  - `MERCLAW_QA_CONVEX_SECRET_CI` for `ci`
- Credential role selection:
  - CLI: `--credential-role maintainer|ci`
  - Env default: `MERCLAW_QA_CREDENTIAL_ROLE` (defaults to `ci` in CI, `maintainer` otherwise)

Optional env vars:

- `MERCLAW_QA_CREDENTIAL_LEASE_TTL_MS` (default `1200000`)
- `MERCLAW_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS` (default `30000`)
- `MERCLAW_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS` (default `90000`)
- `MERCLAW_QA_CREDENTIAL_HTTP_TIMEOUT_MS` (default `15000`)
- `MERCLAW_QA_CONVEX_ENDPOINT_PREFIX` (default `/qa-credentials/v1`)
- `MERCLAW_QA_CREDENTIAL_OWNER_ID` (optional trace id)
- `MERCLAW_QA_ALLOW_INSECURE_HTTP=1` allows loopback `http://` Convex URLs for local-only development.

`MERCLAW_QA_CONVEX_SITE_URL` should use `https://` in normal operation.

Maintainer admin commands (pool add/remove/list) require
`MERCLAW_QA_CONVEX_SECRET_MAINTAINER` specifically.

CLI helpers for maintainers:

```bash
pnpm merclaw qa credentials doctor
pnpm merclaw qa credentials add --kind telegram --payload-file qa/telegram-credential.json
pnpm merclaw qa credentials list --kind telegram
pnpm merclaw qa credentials remove --credential-id <credential-id>
```
...
Default endpoint contract (`MERCLAW_QA_CONVEX_SITE_URL` + `/qa-credentials/v1`):
...
- Compare native opus performance in the `libopus-wasm` benchmark repo, not in default MerClaw install/test loops. Do not set `@discordjs/opus` to `true` in the default `allowBuilds`; that makes unrelated install/test loops compile native code.
...
<AccordionGroup>
  <Accordion title="Projects, shards, and scoped lanes">
    ...
    - `pnpm test:changed` expands changed git paths into cheap scoped lanes by default: direct test edits, sibling `*.test.ts` files, explicit source mappings, and local import-graph dependents. Config/setup/package edits do not broad-run tests unless you explicitly use `MERCLAW_TEST_CHANGED_BROAD=1 pnpm test:changed`.
    ...
    - Import-light unit tests from agents, commands, plugins, auto-reply helpers, `plugin-sdk`, and similar pure utility areas route through the `unit-fast` lane, which skips `test/setup-merclaw-runtime.ts`; stateful/runtime-heavy files stay on the existing lanes.
  ...
  <Accordion title="Vitest pool and isolation defaults">
    ...
    - `scripts/run-vitest.mjs` adds `--no-maglev` for Vitest child Node
      ...
      Set `MERCLAW_VITEST_ENABLE_MAGLEV=1` to compare against stock V8
      behavior.
    - `scripts/run-vitest.mjs` terminates explicit non-watch Vitest runs after
      5 minutes with no stdout or stderr output. Set
      `MERCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS=0` to disable the watchdog for an
  ...
  <Accordion title="Fast local iteration">
    ...
    - `pnpm test:changed` routes through cheap scoped lanes by default. Use
      `MERCLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` only when the agent
    ...
    - The base Vitest config marks the projects/config files as
      ...
      wiring changes.
    - The config keeps `MERCLAW_VITEST_FS_MODULE_CACHE` enabled on supported
      hosts; set `MERCLAW_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want
...
- Useful overrides:
  - `MERCLAW_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `MERCLAW_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises MerClaw's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `MERCLAW_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `MERCLAW_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `MERCLAW_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.merclaw`.
- Set `MERCLAW_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` defaults to a quieter mode: it keeps `[live] ...` progress output and mutes gateway bootstrap logs/Bonjour chatter. Set `MERCLAW_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `MERCLAW_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `MERCLAW_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `MERCLAW_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Docker live runners keep their own practical caps where needed:
  ...
  `test:docker:live-gateway` defaults to `MERCLAW_LIVE_GATEWAY_SMOKE=1`,
  `MERCLAW_LIVE_GATEWAY_MAX_MODELS=8`,
  `MERCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `MERCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Set `MERCLAW_LIVE_MAX_MODELS`
  or the gateway env vars when you explicitly want a smaller cap or larger scan.
- `test:docker:all` builds the live Docker image once via `test:docker:live-build`, packs MerClaw once as an npm tarball through `scripts/package-merclaw-for-docker.mjs`, then builds/reuses two `scripts/e2e/Dockerfile` images. The bare image is only the Node/Git runner for install/update/plugin-dependency lanes; those lanes mount the prebuilt tarball. The functional image installs the same tarball into `/app` for built-app functionality lanes. Docker lane definitions live in `scripts/lib/docker-e2e-scenarios.mjs`; planner logic lives in `scripts/lib/docker-e2e-plan.mjs`; `scripts/test-docker-all.mjs` executes the selected plan. The aggregate uses a weighted local scheduler: `MERCLAW_DOCKER_ALL_PARALLELISM` controls process slots, while resource caps keep heavy live, npm-install, and multi-service lanes from all starting at once. If a single lane is heavier than the active caps, the scheduler can still start it when the pool is empty and then keeps it running alone until capacity is available again. Defaults are 10 slots, `MERCLAW_DOCKER_ALL_LIVE_LIMIT=9`, `MERCLAW_DOCKER_ALL_NPM_LIMIT=10`, and `MERCLAW_DOCKER_ALL_SERVICE_LIMIT=7`; tune `MERCLAW_DOCKER_ALL_WEIGHT_LIMIT` or `MERCLAW_DOCKER_ALL_DOCKER_LIMIT` only when the Docker host has more headroom. The runner performs a Docker preflight by default, removes stale MerClaw E2E containers, prints status every 30 seconds, stores successful lane timings in `.artifacts/docker-tests/lane-timings.json`, and uses those timings to start longer lanes first on later runs. Use `MERCLAW_DOCKER_ALL_DRY_RUN=1` to print the weighted lane manifest without building or running Docker, or `node scripts/test-docker-all.mjs --plan-json` to print the CI plan for selected lanes, package/image needs, and credentials.
...
- Docker/Bash E2E lanes that install the packed MerClaw tarball through `scripts/lib/merclaw-e2e-instance.sh` cap `npm install` at `MERCLAW_E2E_NPM_INSTALL_TIMEOUT` (default `600s`; set `0` to disable the wrapper for debugging).
...
- Npm tarball onboarding/channel/agent smoke: `pnpm test:docker:npm-onboard-channel-agent` installs the packed MerClaw tarball globally in Docker, configures OpenAI via env-ref onboarding plus Telegram by default, runs doctor, and runs one mocked OpenAI agent turn. Reuse a prebuilt tarball with `MERCLAW_CURRENT_PACKAGE_TGZ=/path/to/merclaw-*.tgz`, skip the host rebuild with `MERCLAW_NPM_ONBOARD_HOST_BUILD=0`, or switch channel with `MERCLAW_NPM_ONBOARD_CHANNEL=discord` or `MERCLAW_NPM_ONBOARD_CHANNEL=slack`.

- Release user journey smoke: `pnpm test:docker:release-user-journey` installs the packed MerClaw tarball globally in a clean Docker home, runs onboarding, configures a mocked OpenAI provider, runs an agent turn, installs/uninstalls external plugins, configures ClickClack against a local fixture, verifies outbound/inbound messaging, restarts Gateway, and runs doctor.
- Release typed onboarding smoke: `pnpm test:docker:release-typed-onboarding` installs the packed tarball, drives `merclaw onboard` through a real TTY, configures OpenAI as an env-ref provider, verifies no raw key persistence, and runs a mocked agent turn.
...
- Release upgrade user journey smoke: `pnpm test:docker:release-upgrade-user-journey` installs `merclaw@latest` by default, configures provider/plugin/ClickClack state on the published package, upgrades to the candidate tarball, then reruns the core agent/plugin/channel journey. Override the baseline with `MERCLAW_RELEASE_UPGRADE_BASELINE_SPEC=merclaw@<version>`.
...
- Skill install smoke: `pnpm test:docker:skill-install` installs the packed MerClaw tarball globally in Docker, disables uploaded archive installs in config, resolves the current live ClawHub skill slug from search, installs it with `merclaw skills install`, and verifies the installed skill plus `.clawhub` origin/lock metadata.
- Update channel switch smoke: `pnpm test:docker:update-channel-switch` installs the packed MerClaw tarball globally in Docker, switches from package `stable` to git `dev`, verifies the persisted channel and plugin post-update work, then switches back to package `stable` and checks update status.
- Upgrade survivor smoke: `pnpm test:docker:upgrade-survivor` installs the packed MerClaw tarball over a dirty old-user fixture with agents, channel config, plugin allowlists, stale plugin dependency state, and existing workspace/session files. It runs package update plus non-interactive doctor without live provider or channel keys, then starts a loopback Gateway and checks config/state preservation plus startup/status budgets.
- Published upgrade survivor smoke: `pnpm test:docker:published-upgrade-survivor` installs `merclaw@latest` by default, seeds realistic existing-user files, configures that baseline with a baked command recipe, validates the resulting config, updates that published install to the candidate tarball, runs non-interactive doctor, writes `.artifacts/upgrade-survivor/summary.json`, then starts a loopback Gateway and checks configured intents, state preservation, startup, `/healthz`, `/readyz`, and RPC status budgets. Override one baseline with `MERCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC`, ask the aggregate scheduler to expand exact local baselines with `MERCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS` such as `merclaw@2026.5.2 merclaw@2026.4.23 merclaw@2026.4.15`, and expand issue-shaped fixtures with `MERCLAW_UPGRADE_SURVIVOR_SCENARIOS` such as `reported-issues`; the reported-issues set includes `configured-plugin-installs` for automatic external MerClaw plugin install repair. Package Acceptance exposes those as `published_upgrade_survivor_baseline`, `published_upgrade_survivor_baselines`, and `published_upgrade_survivor_scenarios`, resolves meta baseline tokens such as `last-stable-4` or `all-since-2026.4.23`, and Full Release Validation expands the release-soak package gate to `last-stable-4 2026.4.23 2026.5.2 2026.4.15` plus `reported-issues`.
...
- Bun global install smoke: `bash scripts/e2e/bun-global-install-smoke.sh` packs the current tree, installs it with `bun install -g` in an isolated home, and verifies `merclaw infer image providers --json` returns bundled image providers instead of hanging. Reuse a prebuilt tarball with `MERCLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ=/path/to/merclaw-*.tgz`, skip the host build with `MERCLAW_BUN_GLOBAL_SMOKE_HOST_BUILD=0`, or copy `dist/` from a built Docker image with `MERCLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE=merclaw-dockerfile-smoke:local`.
- Installer Docker smoke: `bash scripts/test-install-sh-docker.sh` shares one npm cache across its root, update, and direct-npm containers. Update smoke defaults to npm `latest` as the stable baseline before upgrading to the candidate tarball. Override with `MERCLAW_INSTALL_SMOKE_UPDATE_BASELINE=2026.4.22` locally, or with the Install Smoke workflow's `update_baseline_version` input on GitHub. Non-root installer checks keep an isolated npm cache so root-owned cache entries do not mask user-local install behavior. Set `MERCLAW_INSTALL_SMOKE_NPM_CACHE_DIR=/path/to/cache` to reuse the root/update/direct-npm cache across local reruns.
- Install Smoke CI skips the duplicate direct-npm global update with `MERCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1`; run the script locally without that env when direct `npm install -g` coverage is needed.
- Agents delete shared workspace CLI smoke: `pnpm test:docker:agents-delete-shared-workspace` (script: `scripts/e2e/agents-delete-shared-workspace-docker.sh`) builds the root Dockerfile image by default, seeds two agents with one workspace in an isolated container home, runs `agents delete --json`, and verifies valid JSON plus retained workspace behavior. Reuse the install-smoke image with `MERCLAW_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE=merclaw-dockerfile-smoke:local MERCLAW_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD=1`.
...
- MerClaw bundle MCP tools (real stdio MCP server + embedded MerClaw profile allow/deny smoke): `pnpm test:docker:agent-bundle-mcp-tools` (script: `scripts/e2e/agent-bundle-mcp-tools-docker.sh`)
...
- Plugins (install/update smoke for local path, `file:`, npm registry with hoisted dependencies, malformed npm package metadata, git moving refs, ClawHub kitchen-sink, marketplace updates, and Claude-bundle enable/inspect): `pnpm test:docker:plugins` (script: `scripts/e2e/plugins-docker.sh`)
  Set `MERCLAW_PLUGINS_E2E_CLAWHUB=0` to skip the ClawHub block, or override the default kitchen-sink package/runtime pair with `MERCLAW_PLUGINS_E2E_CLAWHUB_SPEC` and `MERCLAW_PLUGINS_E2E_CLAWHUB_ID`. Without `MERCLAW_CLAWHUB_URL`/`CLAWHUB_URL`, the test uses a hermetic local ClawHub fixture server.
- Plugin update unchanged smoke: `pnpm test:docker:plugin-update` (script: `scripts/e2e/plugin-update-unchanged-docker.sh`)
- Plugin lifecycle matrix smoke: `pnpm test:docker:plugin-lifecycle-matrix` installs the packed MerClaw tarball in a bare container, installs an npm plugin, toggles enable/disable, upgrades and downgrades it through a local npm registry, deletes the installed code, then verifies uninstall still removes stale state while logging RSS/CPU metrics for each lifecycle phase.
...
```bash
MERCLAW_DOCKER_E2E_IMAGE=merclaw-docker-e2e-functional:local pnpm test:docker:e2e-build
MERCLAW_DOCKER_E2E_IMAGE=merclaw-docker-e2e-functional:local MERCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels
```

Suite-specific image overrides such as `MERCLAW_GATEWAY_NETWORK_E2E_IMAGE` still win when set. When `MERCLAW_SKIP_DOCKER_BUILD=1` points at a remote shared image, the scripts pull it if it is not already local. The QR and installer Docker tests keep their own Dockerfiles because they validate package/install behavior rather than the shared built-app runtime.
...
`.pnpm-store`, `.worktrees`, `__merclaw_vitest__`, and app-local `.build` or
Gradle output directories so Docker live runs do not spend minutes copying
machine-specific artifacts.
They also set `MERCLAW_SKIP_CHANNELS=1` so gateway live probes do not start
...
`MERCLAW_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
live coverage from that Docker lane.
`test:docker:openwebui` is a higher-level compatibility smoke: it starts an
MerClaw gateway container with the OpenAI-compatible HTTP endpoints enabled,
starts a pinned Open WebUI container against that gateway, signs in through
Open WebUI, verifies `/api/models` exposes `merclaw/default`, then sends a
...
environment, staged auth profiles, or an explicit `MERCLAW_PROFILE_FILE`.
...
container, starts a second container that spawns `merclaw mcp serve`, then
...
inside the container, materializes that server through the embedded MerClaw bundle
...
- `MERCLAW_CONFIG_DIR=...` (default: `~/.merclaw`) mounted to `/home/node/.merclaw`
- `MERCLAW_WORKSPACE_DIR=...` (default: `~/.merclaw/workspace`) mounted to `/home/node/.merclaw/workspace`
- `MERCLAW_PROFILE_FILE=...` mounted and sourced before running tests
- `MERCLAW_DOCKER_PROFILE_ENV_ONLY=1` to verify only env vars sourced from `MERCLAW_PROFILE_FILE`, using temporary config/workspace dirs and no external CLI auth mounts
- `MERCLAW_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/merclaw/docker-cli-tools`) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `MERCLAW_LIVE_PROVIDERS` / `MERCLAW_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `MERCLAW_DOCKER_AUTH_DIRS=all`, `MERCLAW_DOCKER_AUTH_DIRS=none`, or a comma list like `MERCLAW_DOCKER_AUTH_DIRS=.claude,.codex`
- `MERCLAW_LIVE_GATEWAY_MODELS=...` / `MERCLAW_LIVE_MODELS=...` to narrow the run
- `MERCLAW_LIVE_GATEWAY_PROVIDERS=...` / `MERCLAW_LIVE_PROVIDERS=...` to filter providers in-container
- `MERCLAW_SKIP_DOCKER_BUILD=1` to reuse an existing `merclaw:local-live` image for reruns that do not need a rebuild
- `MERCLAW_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `MERCLAW_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `MERCLAW_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
