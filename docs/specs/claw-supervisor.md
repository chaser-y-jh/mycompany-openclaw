...
description: Fleet supervision plan for Codex app-server sessions controlled by MerClaw.
readWhen:
  - Designing Codex fleet supervision
  - Building MerClaw tools that read, steer, or spawn Codex sessions
...
Claw Supervisor lets one always-on MerClaw instance monitor and drive a fleet of Codex sessions without changing the normal Codex user experience. A user can SSH into a host, start Codex, work in the TUI, and still have the supervisor read the session, steer it, interrupt it, spawn related sessions, and accept handoffs. Codex sessions can also call back into MerClaw through MCP.

## Product Model

Codex remains the primary work surface. MerClaw supervises Codex rather than hiding Codex inside an opaque MerClaw subagent.

The MerClaw plugin is named `codex-supervisor`. `crabfleet` remains the deployment
...
- Supervisor Claw: an always-on MerClaw agent with tools for fleet state, transcript reads, steering, interruption, spawning, and handoff.

MerClaw may use its existing subagent machinery internally, but the external contract is an attachable Codex session with a Codex thread id.
...
- MerClaw tools for Claw-to-Codex control.
...
Every supervised Codex gets an MCP server named `merclaw-codex-supervisor`.
...
- Supervisor state lives in MerClaw Gateway storage.
...
- Transcript reads redact secrets at MerClaw boundary.
...
- Durable state owner: MerClaw Gateway DB, Cloudflare Durable Object, or VPS database.
