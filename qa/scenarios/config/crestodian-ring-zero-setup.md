...
coverage:
  ...
  secondary:
    ...
    - agents.create
objective: Verify Crestodian can bootstrap a fresh MerClaw config, set the default model, create an agent, configure Discord through a SecretRef, validate config, and leave an audit trail.
successCriteria:
...
steps:
  - name: bootstraps config through Crestodian CLI
    actions:
      ...
      - set: configPath
        value:
          expr: "path.join(stateDir, 'merclaw.json')"
      ...
      - set: crestodianEnv
        value:
          expr: "({ MERCLAW_STATE_DIR: stateDir, MERCLAW_CONFIG_PATH: configPath, MERCLAW_BUNDLED_PLUGINS_DIR: path.join(env.repoRoot, 'dist', 'extensions'), [setupSpec.discordEnv]: setupSpec.discordToken })"
      - call: fs.rm
        args:
          - ref: stateDir
          - recursive: true
