...
steps:
  - name: emits protected low-cardinality prometheus metrics
    actions:
      ...
      - set: gatewayToken
        value:
          expr: "String(env.gateway.token ?? env.gateway.runtimeEnv.MERCLAW_GATEWAY_TOKEN ?? '')"
      - assert:
      ...
      - assert:
          expr: "prometheusText.includes('# TYPE merclaw_run_completed_total counter')"
          message: "missing run completion counter"
      - assert:
          expr: "prometheusText.includes('# TYPE merclaw_run_duration_seconds histogram')"
          message: "missing run duration histogram"
      - assert:
          expr: "prometheusText.includes('# TYPE merclaw_model_call_total counter')"
          message: "missing model call counter"
      - assert:
          expr: "prometheusText.includes('# TYPE merclaw_harness_run_total counter')"
      ...
      - assert:
          expr: "!prometheusText.includes('merclaw.content.')"
          message: "prometheus output leaked content attributes"
      - assert:
          expr: "!/merclaw_prometheus_series_dropped_total(?:\\{[^}]*\\})?\\s+(?!0(?:\\.0+)?(?:\\s|$))/.test(prometheusText)"
