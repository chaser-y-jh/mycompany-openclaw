...
execution:
  ...
  config:
    ...
    toolCoverage:
      ...
      qaImpact: P1
      action: split native search/command behavior from MerClaw dynamic tool parity
      reason: Codex app-server intentionally owns command execution natively; current MerClaw coding surface routes grep-style searches through exec.
    knownHarnessGap:
      issue: "#80319"
      reason: QA tool-defaults currently needs native search/command behavior coverage instead of MerClaw dynamic exec exposure.
