...
coverage:
  primary:
    - tools.apply-patch
objective: Verify apply_patch behavior is tracked across MerClaw and Codex while Codex owns patching natively.
successCriteria:
  - MerClaw may expose MerClaw apply_patch while Codex app-server mode may omit duplicate MerClaw dynamic apply_patch.
...
execution:
  ...
  config:
    ...
    knownHarnessGap:
      issue: "#80320"
      reason: Codex-native apply_patch is intentionally not an MerClaw dynamic tool; QA fault injection still uses synthetic failure-path inputs.
