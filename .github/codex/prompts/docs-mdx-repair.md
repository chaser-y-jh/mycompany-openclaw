# MerClaw Docs MDX Repair Agent

You are repairing generated MerClaw documentation after a fast MDX validation failure.
...
1. Read `.merclaw-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".merclaw-sync/mdx/${LOCALE}.json"`.
