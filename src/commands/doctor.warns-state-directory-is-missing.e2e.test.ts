...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.MERCLAW_STATE_DIR = missingDir;
    await doctorCommand(createDoctorRuntime(), {
      nonInteractive: true,
      workspaceSuggestions: false,
    });
  ...
  it("skips gateway auth warning when MERCLAW_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    const prevToken = process.env.MERCLAW_GATEWAY_TOKEN;
    process.env.MERCLAW_GATEWAY_TOKEN = "env-token-1234567890";
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
  ...
  it("warns when token and password are both configured and gateway.auth.mode is unset", async () => {
    ...
    expect(String(gatewayAuthNote[0])).toContain("merclaw config set gateway.auth.mode token");
    expect(String(gatewayAuthNote[0])).toContain("merclaw config set gateway.auth.mode password");
  });
  ...
  it("keeps doctor read-only when gateway token is SecretRef-managed but unresolved", async () => {
    ...
    const previousToken = process.env.MERCLAW_GATEWAY_TOKEN;
    delete process.env.MERCLAW_GATEWAY_TOKEN;
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
      ...
      if (previousToken === undefined) {
        ...
        process.env.MERCLAW_GATEWAY_TOKEN = previousToken;
      }
    }
  ...
  it("does not let MERCLAW_GATEWAY_TOKEN hide an unresolved SecretRef-managed token", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: {
          mode: "local",
    ...
    process.env.MERCLAW_GATEWAY_TOKEN = "fallback-token-1234567890";
  ...
  it("skips token-mode exec token probes even when env password is set", async () => {
    ...
    const previousPassword = process.env.MERCLAW_GATEWAY_PASSWORD;
    process.env.MERCLAW_GATEWAY_PASSWORD = "fallback-password";
    try {
      callGateway.mockClear();
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
      ...
      if (previousPassword === undefined) {
        ...
        process.env.MERCLAW_GATEWAY_PASSWORD = previousPassword;
      }
    }
  ...
  it("skips gateway auth warning when SecretRef-managed token resolves", async () => {
    ...
    const previousToken = process.env.MERCLAW_GATEWAY_TOKEN;
    process.env.MERCLAW_GATEWAY_TOKEN = "resolved-token-1234567890";
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
