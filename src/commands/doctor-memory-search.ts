...
function isOpenAICompatibleMemoryProvider(providerId: string, cfg: MerClawConfig): boolean {
...
function isKeyOptionalMemoryProvider(providerId: string, cfg: MerClawConfig): boolean {
  return (
    providerId === "local" ||
    providerId === "ollama" ||
    providerId === "lmstudio" ||
...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("merclaw doctor --fix")} or ${formatCliCommand("merclaw memory status --fix")}`
    : `Verify: ${formatCliCommand("merclaw memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: MerClawConfig): Promise<void> {
  try {
...
function hasActiveAlternateMemoryPluginSlot(cfg: MerClawConfig): boolean {
 ...
 * Runs as part of `merclaw doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
      ...
      note(
        [
          ...
          workspaceProbeFailed
            ...
            : `- Set an explicit binary path: ${formatCliCommand("merclaw config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("merclaw config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("merclaw memory status --deep")}`,
        ]
          .filter(Boolean)
    ...
    note(
      [
        ...
        suggestedRemoteProvider
          ? `- Switch to a remote provider: ${formatCliCommand(`merclaw config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
  ...
  if (
    ...
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("merclaw config set agents.defaults.memorySearch.remote.baseUrl http://127.0.0.1:1234/v1")}`,
        "",
        `Verify: ${formatCliCommand("merclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isOpenAICompatibleMemoryProvider(provider, cfg) && !normalizeOptionalString(resolved.model)) {
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("merclaw config set agents.defaults.memorySearch.model text-embedding-bge-m3")}`,
        "",
        `Verify: ${formatCliCommand("merclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isKeyOptionalMemoryProvider(provider, cfg)) {
    ...
    // not warn. A skipped probe means the user ran `merclaw doctor` without
  ...
  note(
    [
      ...
      `- Configure credentials: ${formatCliCommand("merclaw configure --section model")}`,
      `- To disable: ${formatCliCommand("merclaw config set agents.defaults.memorySearch.enabled false")}`,
      "",
      `Verify: ${formatCliCommand("merclaw memory status --deep")}`,
    ].join("\n"),
    "Memory search",
