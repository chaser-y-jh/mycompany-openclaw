...
export async function channelsListCommand(
  ...
  // there are no configured channels even though merclaw.json has one.
    ...
    lines.push(
      theme.muted(
        showAll
          ...
          : "- no configured chat channels (run `merclaw channels list --all` to see installable channels)",
      ),
    );
  } else {
  ...
  runtime.log(
    theme.muted(
      "Model provider usage moved out of `channels list` — see `merclaw status` or `merclaw models list`.",
    ),
  );
