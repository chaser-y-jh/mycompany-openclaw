...
async function resolveCatalogChannelEntry(raw: string, cfg: MerClawConfig | null) {
...
async function channelsAddCommandImpl(
  ...
  const cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as MerClawConfig;
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run merclaw doctor --fix, then retry merclaw channels add.`
  ...
  if (!plugin?.setup?.applyAccountConfig) {
    runtime.error(
      ...
      })} Run ${formatCliCommand("merclaw channels add")} with no flags for guided setup.`,
    );
    runtime.exit(1);
    return;
  }
