...
vi.mock("node:child_process", async () => {
  const { mockNodeChildProcessSpawnSync } = await import("merclaw/plugin-sdk/test-node-mocks");
...
describe("doctor WhatsApp responsiveness", () => {
  ...
  it("lists only verified local TUI processes", () => {
    spawnSyncMock.mockReturnValue({
      ...
      stdout: [
        " 101 merclaw-tui",
        " 102 /usr/bin/node /usr/lib/node_modules/merclaw/dist/index.js gateway --port 18789",
        " 103 merclaw channels",
        " 104 merclaw tui --local",
        " 105 /usr/bin/merclaw chat",
        " 106 helper --note 'merclaw tui'",
        " 107 merclaw-helper merclaw terminal",
        " 108 merclaw --flag tui",
      ].join("\n"),
    });

    expect(listLocalTuiProcesses()).toEqual([
      { pid: 101, command: "merclaw-tui" },
      { pid: 104, command: "merclaw tui --local" },
      { pid: 105, command: "/usr/bin/merclaw chat" },
    ]);
  });
  ...
  it("terminates stale local TUI processes with a kill fallback", async () => {
    ...
    await expect(
      terminateLocalTuiProcesses({
        processes: [{ pid: 101, command: "merclaw-tui" }],
        controller,
        graceMs: 0,
      }),
  ...
  it("warns and repairs local TUI pressure when WhatsApp is enabled and the gateway is degraded", async () => {
    ...
    const cfg = { channels: { whatsapp: { enabled: true } } } as MerClawConfig;
    ...
    await noteWhatsappResponsivenessHealth({
      ...
      shouldRepair: true,
      listLocalTuiProcesses: () => [{ pid: 101, command: "merclaw-tui" }],
