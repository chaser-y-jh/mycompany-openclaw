...
import { normalizeNullableString as normalizeId } from "@merclaw/normalization-core/string-coerce";
...
import { compareMerClawVersions } from "../../../config/version.js";
...
function isPluginsGloballyDisabled(cfg: MerClawConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: MerClawConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: MerClawConfig): string[] {
...
function isPluginEntryDisabled(cfg: MerClawConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: MerClawConfig, channelId: string): boolean {
  const channels = asObjectRecord(cfg.channels);
  const entry = asObjectRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: MerClawConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: MerClawConfig): string[] {
  const entries = asObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: MerClawConfig): string[] {
...
function collectConfiguredChannelIds(cfg: MerClawConfig, env: NodeJS.ProcessEnv): string[] {
  const ids = new Set<string>();
  const channels = asObjectRecord(cfg.channels);
  if (channels) {
...
function collectConfiguredProviderIds(cfg: MerClawConfig): Set<string> {
  const ids = new Set<string>();
...
function collectProviderPluginIds(cfg: MerClawConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectWebSearchPluginIds(cfg: MerClawConfig): string[] {
...
function collectAcpRuntimePluginIds(cfg: MerClawConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: MerClawConfig): string[] {
...
function addEligiblePluginId(cfg: MerClawConfig, pluginIds: Set<string>, pluginId: string): void {
...
export function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareMerClawVersions(
  ...
  const touchedComparedToRelease = compareMerClawVersions(params.touchedVersion, releaseVersion);
