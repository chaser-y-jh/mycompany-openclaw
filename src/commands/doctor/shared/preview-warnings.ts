...
import { isRecord as hasRecord } from "@merclaw/normalization-core/record-coerce";
...
function listAgentRecords(cfg: MerClawConfig): Record<string, unknown>[] {
  return Array.isArray(cfg.agents?.list) ? cfg.agents.list.filter(hasRecord) : [];
}

function hasChannels(cfg: MerClawConfig): boolean {
  return hasRecord(cfg.channels);
}

function hasPlugins(cfg: MerClawConfig): boolean {
  return hasRecord(cfg.plugins);
}

function hasPluginLoadPaths(cfg: MerClawConfig): boolean {
...
function hasSubagentAllowlistConfig(cfg: MerClawConfig): boolean {
...
function hasConfiguredSafeBins(cfg: MerClawConfig): boolean {
...
function sourceReplyRuntimeMayAllowMessageTool(cfg: MerClawConfig): boolean {
...
function resolveGroupVisibleReplyProvenance(cfg: MerClawConfig): {
...
export function collectVisibleReplyToolPolicyWarnings(cfg: MerClawConfig): string[] {
  const groupPolicy = resolveGroupVisibleReplyProvenance(cfg);
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    warnings.push(
      `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; MerClaw falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
    );
  }
  ...
  if (globalVisibleReplies === "message_tool" && groupPolicy.path !== "messages.visibleReplies") {
    ...
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; MerClaw falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  }
  return warnings;
}
...
export function collectChannelBoundMessageToolPolicyWarnings(cfg: MerClawConfig): string[] {
...
export function collectProfileConfiguredToolSectionWarnings(cfg: MerClawConfig): string[] {
  const warnings: string[] = [];
