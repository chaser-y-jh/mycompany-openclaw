...
export const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "MERCLAW_DISABLE_PLUGIN_REGISTRY_MIGRATION";
export const FORCE_PLUGIN_REGISTRY_MIGRATION_ENV = "MERCLAW_FORCE_PLUGIN_REGISTRY_MIGRATION";
...
export type PluginRegistryInstallMigrationParams = LoadInstalledPluginIndexParams &
  InstalledPluginIndexStoreOptions & {
    ...
    readConfig?: () => Promise<MerClawConfig> | MerClawConfig;
  };
...
function listConfiguredChannelIds(config: MerClawConfig): Set<string> {
...
function listConfiguredModelProviderIds(config: MerClawConfig): Set<string> {
...
export async function migratePluginRegistryForInstall(
  ...
  const config = stripShippedPluginInstallConfigRecords(rawConfig) as MerClawConfig;
