...
function buildLegacyRealtimeTalkCompat(
  talk: Record<string, unknown>,
  normalizedTalk: NonNullable<MerClawConfig["talk"]>,
): Record<string, unknown> | undefined {
  ...
  return normalizeTalkSection({ realtime: compat } as MerClawConfig["talk"])?.realtime;
}

export function normalizeLegacyTalkConfig(cfg: MerClawConfig, changes: string[]): MerClawConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as MerClawConfig["talk"]) ?? {};
