...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(merclaw completion ...)` */
    ...
    return [
      {
        ...
        path,
        fixHint: "Run `merclaw doctor --fix` to upgrade to cached completion.",
      },
    ];
  }
    ...
    return [
      {
        ...
        path,
        fixHint: `Run \`merclaw completion --write-state\` or \`merclaw doctor --fix\` to regenerate ${status.cachePath}.`,
      },
    ];
  }
  return [];
 ...
 */
export async function ensureCompletionCacheExists(binName = "merclaw"): Promise<boolean> {
  const shell = resolveShellFromEnv() as CompletionShell;
  const cacheExists = await completionCacheExists(shell, binName);
