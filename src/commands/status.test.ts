...
beforeAll(() => {
  envSnapshot = captureEnv(["MERCLAW_PROFILE"]);
  process.env.MERCLAW_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByMerClaw: Boolean(command),
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/merclaw/pnpm-lock.yaml",
        markerPath: "/tmp/merclaw/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.MERCLAW_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.merclaw.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.merclaw.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.MERCLAW_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/merclaw-root.js", () => ({
  resolveMerClawPackageRoot: vi.fn().mockResolvedValue("/tmp/merclaw"),
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expectLogsMatch(logs, /merclaw (?:--profile isolated )?status --all/);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("merclaw --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("MERCLAW_GATEWAY_TOKEN", "abcd1234", async () => {
