...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.merclaw.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.merclaw.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        MERCLAW_STATE_DIR: "/tmp/merclaw-service",
        MERCLAW_CONFIG_PATH: "/tmp/merclaw-service/merclaw.json",
      },
    });
    ...
    const [handoffEnv] = readGatewayRestartHandoffSync.mock.calls[0] as unknown as [
      { MERCLAW_STATE_DIR?: string; MERCLAW_CONFIG_PATH?: string },
    ];
    expect(handoffEnv?.MERCLAW_STATE_DIR).toBe("/tmp/merclaw-service");
    expect(handoffEnv?.MERCLAW_CONFIG_PATH).toBe("/tmp/merclaw-service/merclaw.json");
    expect(note).toHaveBeenCalledWith(
      ...
      connections: [
        {
          pid: 4242,
          command: "node",
          commandLine: "/tmp/newer-merclaw/bin/merclaw logs --follow",
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "merclaw-gateway", address: "0.0.0.0:18789" }];
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners,
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      status: "busy",
      listeners: [
        { pid: 5001, commandLine: "merclaw-gateway", address: "0.0.0.0:18789" },
        { pid: 5002, commandLine: "merclaw-gateway", address: "127.0.0.1:18789" },
      ],
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      `Run ${formatCliCommand("merclaw gateway install")} when you want to install the gateway service.`,
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system MerClaw gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        platform: "linux",
        label: "merclaw-gateway.service",
        detail: "unit: /etc/systemd/system/merclaw-gateway.service",
        scope: "system",
        marker: "merclaw",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        "System-level MerClaw gateway service detected while the user gateway service is not installed.",
        "- merclaw-gateway.service (unit: /etc/systemd/system/merclaw-gateway.service)",
        "MerClaw will not install a second user-level gateway service automatically.",
        "Run `merclaw gateway status --deep` or `merclaw doctor --deep` to inspect duplicate services.",
