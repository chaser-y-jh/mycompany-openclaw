...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/merclaw-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = MerClawConfig>(
  overrides: T = {} as T,
): MerClawConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): MerClawConfig {
