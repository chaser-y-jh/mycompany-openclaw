...
describe("onboard-hooks", () => {
  ...
  const createMockHook = (
    ...
    ...params,
    source: "merclaw-bundled" as const,
    pluginId: undefined,
    homepage: undefined,
    always: false,
  ...
  const createMockHookReport = (eligible = true): HookStatusReport => ({
    ...
    managedHooksDir: "/mock/.merclaw/hooks",
    hooks: [
      createMockHook(
        {
          name: "session-memory",
  ...
  describe("setupInternalHooks", () => {
    ...
    it("localizes built-in hook prompts when MERCLAW_LOCALE is set", async () => {
      ...
      expect(noteCalls).toEqual([
        [
          [
            ...
            "",
            "Learn more: https://docs.merclaw.ai/automation/hooks",
          ].join("\n"),
          "Hooks",
        ],
        [
