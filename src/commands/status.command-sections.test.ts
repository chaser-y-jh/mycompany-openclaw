...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:merclaw security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:merclaw security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:merclaw status --all",
      "  Need to debug live? cmd:merclaw logs --follow",
      "  Fix reachability first: cmd:merclaw gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:merclaw devices approve req-123)",
      "muted(Fallback: cmd:merclaw devices approve --latest)",
      "muted(Inspect: cmd:merclaw devices list)",
    ]);
  });
