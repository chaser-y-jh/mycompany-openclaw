...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "merclaw/plugin-sdk/core";
import { readFileWithinRoot, writeFileWithinRoot } from "merclaw/plugin-sdk/file-access-runtime";
import {
  loadSecretFileSync,
  type SecretFileReadResult,
} from "merclaw/plugin-sdk/secret-file-runtime";
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTempDir({ prefix: "merclaw-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps deprecated root-bounded read/write helpers on file-access-runtime", async () => {
    await withTempDir({ prefix: "merclaw-sdk-file-access-compat-" }, async (root) => {
