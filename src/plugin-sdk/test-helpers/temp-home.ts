...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked MERCLAW_HOME.
  delete process.env.MERCLAW_HOME;
  process.env.MERCLAW_STATE_DIR = path.join(base, ".merclaw");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "merclaw-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".merclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
