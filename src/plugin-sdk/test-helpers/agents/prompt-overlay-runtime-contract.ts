...
export function openAiPluginPersonalityConfig(personality: "friendly" | "off"): MerClawConfig {
  return {
    plugins: {
      entries: {
        openai: {
...
export function sharedGpt5PersonalityConfig(personality: "friendly" | "off"): MerClawConfig {
  return {
    agents: {
      defaults: {
        promptOverlays: {
...
export function codexPromptOverlayContext(params?: {
  ...
  return {
    ...
    promptMode: "full",
    agentDir: "/tmp/merclaw-codex-prompt-contract-agent",
    workspaceDir: "/tmp/merclaw-codex-prompt-contract-workspace",
