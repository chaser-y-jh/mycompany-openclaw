...
export type { MerClawConfig, ModelApi, ModelDefinitionConfig, ModelProviderConfig };
export {
...
export type ProviderOnboardPresetAppliers<TArgs extends unknown[]> = {
  applyProviderConfig: (cfg: MerClawConfig, ...args: TArgs) => MerClawConfig;
  applyConfig: (cfg: MerClawConfig, ...args: TArgs) => MerClawConfig;
};
...
function hasAgentDefaultModelPrimary(cfg: MerClawConfig): boolean {
...
>(params: {
  ...
  applyPreset: (cfg: MerClawConfig, preset: TParams) => MerClawConfig;
  primaryModelRef: string;
...
export function applyOpencodeZenModelDefault(cfg: MerClawConfig): {
  next: MerClawConfig;
  changed: boolean;
} {
