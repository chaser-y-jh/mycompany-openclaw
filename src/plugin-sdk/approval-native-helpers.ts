...
type ChannelApprovalForwardingEvaluatorParams = {
  channel: string;
  isTransportEnabled: (params: { cfg: MerClawConfig; accountId?: string | null }) => boolean;
...
type NativeApprovalChannelRouteGateParams<TTarget extends NativeApprovalTarget> = {
  ...
  isTransportEnabled: (params: { cfg: MerClawConfig; accountId?: string | null }) => boolean;
  listAccountIds: (cfg: MerClawConfig) => readonly string[];
  resolveDefaultAccountId: (cfg: MerClawConfig) => string;
  normalizeForwardTarget: (target: NativeApprovalForwardTarget) => TTarget | null;
...
export function shouldSuppressLocalNativeExecApprovalPrompt(params: {
  ...
  isTransportEnabled?: (params: { cfg: MerClawConfig; accountId?: string | null }) => boolean;
  isNativeDeliveryEnabled?: (params: { cfg: MerClawConfig; accountId?: string | null }) => boolean;
