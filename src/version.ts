...
declare const __MERCLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "merclaw";
...
function readInjectedVersion(): string | undefined {
  return typeof __MERCLAW_VERSION__ === "string" ? __MERCLAW_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["MERCLAW_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["MERCLAW_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["MERCLAW_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.MERCLAW_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current MerClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: readInjectedVersion(),
  bundledVersion: process.env.MERCLAW_BUNDLED_VERSION,
});
