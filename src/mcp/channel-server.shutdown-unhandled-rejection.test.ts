...
vi.mock("./channel-bridge.js", () => ({
  MerClawChannelBridge: class MockMerClawChannelBridge {
...
describe("serveMerClawChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveMerClawChannelMcp } = await import("./channel-server.js");

    const servePromise = serveMerClawChannelMcp({ verbose: false });
