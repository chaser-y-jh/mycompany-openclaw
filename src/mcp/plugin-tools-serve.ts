/**
 * Standalone MCP server that exposes MerClaw plugin-registered tools
...
function resolvePluginToolPolicy(config: MerClawConfig): {
...
function resolveTools(config: MerClawConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "merclaw-plugin-tools", tools });
}
