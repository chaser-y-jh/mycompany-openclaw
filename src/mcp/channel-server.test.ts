...
import { createMerClawChannelMcpServer, MerClawChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createMerClawChannelMcpServer({
...
describe("merclaw channel mcp server", () => {
  ...
  describe("gateway-backed flows", () => {
    describe("gateway integration", () => {
      ...
      test("lists conversations and reads messages", async () => {
        ...
        const bridge = new MerClawChannelBridge({} as never, {
        ...
        expect((messages[1]?.["__merclaw"] as { id?: string } | undefined)?.id).toBe(
          "msg-attachment",
        );
        expect(
