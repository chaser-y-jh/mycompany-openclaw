...
export class MerClawChannelBridge {
  ...
  constructor(
    private readonly cfg: MerClawConfig,
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`merclaw mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `merclaw mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
