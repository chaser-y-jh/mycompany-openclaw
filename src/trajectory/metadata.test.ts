...
describe("trajectory metadata", () => {
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/merclaw.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.merclaw/merclaw.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          MERCLAW_STATE_DIR: "/Users/tester/.merclaw",
        },
      ...
      expect(harness.invocation).toEqual([
        ...
        "--api-key",
        "<redacted>",
        "--config=$MERCLAW_STATE_DIR/merclaw.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/merclaw.js");
  ...
  it("redactPathForSupport returns empty string for null/undefined input", () => {
    const ctx: SupportRedactionContext = { env: {}, stateDir: "/tmp/.merclaw" };
