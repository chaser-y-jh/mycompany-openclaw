...
export function createTrajectoryRuntimeRecorder(
  ...
  // override so operators can still disable recording with MERCLAW_TRAJECTORY=0.
  const enabled = parseBooleanValue(env.MERCLAW_TRAJECTORY) ?? true;
  if (!enabled) {
    return null;
  }
