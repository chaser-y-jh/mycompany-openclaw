...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<MerClawConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<MerClawConfig["tools"]>["web"]>["search"]>;
...
const WEB_SEARCH_DOCS_URL = "https://docs.merclaw.ai/tools/web";
...
function rawKeyValue(config: MerClawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: MerClawConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: MerClawConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: MerClawConfig, result: MerClawConfig): MerClawConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as MerClawConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "MerClaw will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.merclaw.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — MerClaw will store a reference instead of the API key.",
        `Env var: ${ref.id}${envAvailable ? " (detected)" : ""}.`,
        ...(envAvailable ? [] : [`Set ${ref.id} in the Gateway environment.`]),
        "Docs: https://docs.merclaw.ai/tools/web",
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
