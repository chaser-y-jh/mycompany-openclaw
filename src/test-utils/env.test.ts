...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "MERCLAW_ENV_TEST_A";
    const keyB = "MERCLAW_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "MERCLAW_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "MERCLAW_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "MERCLAW_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "MERCLAW_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "MERCLAW_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "MERCLAW_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "MERCLAW_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "merclaw-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousMerClawHome = process.env.MERCLAW_HOME;
    const previousStateDir = process.env.MERCLAW_STATE_DIR;
    const previousBundledDir = process.env.MERCLAW_BUNDLED_PLUGINS_DIR;
    process.env.MERCLAW_HOME = "/srv/merclaw-home";
    process.env.MERCLAW_STATE_DIR = "/srv/merclaw-state";
    process.env.MERCLAW_BUNDLED_PLUGINS_DIR = "/srv/merclaw-bundled";
    ...
    try {
      ...
      expect(env.MERCLAW_HOME).toBeUndefined();
      expect(env.MERCLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.MERCLAW_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("MERCLAW_HOME", previousMerClawHome);
      restoreEnvKey("MERCLAW_STATE_DIR", previousStateDir);
      restoreEnvKey("MERCLAW_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processMerClawHome: process.env.MERCLAW_HOME,
          processBundledDir: process.env.MERCLAW_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.MERCLAW_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processMerClawHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.MERCLAW_HOME).toBe("/srv/merclaw-home");
    } finally {
      restoreEnvKey("MERCLAW_HOME", previousMerClawHome);
    }
  });
