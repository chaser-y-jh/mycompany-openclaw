...
type MerClawTestStateLayout = "home" | "state-only" | "split";

type MerClawTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type MerClawTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: MerClawTestStateLayout;
  scenario?: MerClawTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "merclaw-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "home") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(stateDir, "merclaw.json"),
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "merclaw.json"),
...
function scenarioConfig(options: MerClawTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "merclaw-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: MerClawTestStateOptions): Record<string, string | undefined> {
...
export async function createMerClawTestState(
  options: MerClawTestStateOptions = {},
): Promise<MerClawTestState> {
  ...
  const state: MerClawTestState = {
    ...
    applyEnv: () => {
      for (const [key, value] of Object.entries(envVars)) {
        // Test fixtures apply a fixed MerClaw env set, not plugin-provided host env.
        if (value === undefined) {
...
export async function withMerClawTestState<T>(
  options: MerClawTestStateOptions,
  fn: (state: MerClawTestState) => Promise<T>,
): Promise<T> {
  const state = await createMerClawTestState(options);
  try {
