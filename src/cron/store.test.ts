...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses MERCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("MERCLAW_HOME", "/srv/merclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/merclaw-home", "cron", "jobs.json"));
  });
});
