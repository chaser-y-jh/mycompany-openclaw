...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: MerClawConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride =
    cliUrlOverride || options.ignoreEnvUrlOverride
      ? undefined
      : normalizeOptionalString(process.env.MERCLAW_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.MERCLAW_ALLOW_INSECURE_PRIVATE_WS === "1";
