...
export function warnLegacyMerClawEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but MerClaw only reads MERCLAW_* names now.`,
      "Rename them by replacing the legacy prefix with MERCLAW_; the old names are ignored.",
    ].join("\n"),
    { code: "MERCLAW_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}

export function resetLegacyMerClawEnvWarningForTest(): void {
  warned = false;
}
