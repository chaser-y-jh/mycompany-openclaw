...
export function resolveCliBackendLiveModelSelection(params: {
  ...
  if (!parsed) {
    throw new Error(
      `MERCLAW_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${params.rawModel}`,
    );
  }

  if (parsed.provider === "codex-cli") {
    throw new Error(
      "MERCLAW_LIVE_CLI_BACKEND_MODEL=codex-cli/... is no longer supported. Use a supported CLI backend such as claude-cli or google-gemini-cli.",
    );
  }
...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("MERCLAW_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.MERCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.MERCLAW_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  const args =
    parseJsonStringArray(
      "MERCLAW_LIVE_CLI_BACKEND_ARGS",
      process.env.MERCLAW_LIVE_CLI_BACKEND_ARGS,
    ) ?? params.defaultArgs;
  if (!args || args.length === 0) {
    throw new Error(
      `MERCLAW_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`,
    );
  }
  const resumeArgs =
    parseJsonStringArray(
      "MERCLAW_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.MERCLAW_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.MERCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.MERCLAW_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.MERCLAW_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.MERCLAW_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.MERCLAW_TEST_MINIMAL_GATEWAY,
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("MERCLAW_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("MERCLAW_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("MERCLAW_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("MERCLAW_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("MERCLAW_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("MERCLAW_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("MERCLAW_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("MERCLAW_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("MERCLAW_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("MERCLAW_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("MERCLAW_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
