...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([MERCLAW_MODEL_ID, MERCLAW_DEFAULT_MODEL_ID]);
  ids.add(`merclaw/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`merclaw/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== MERCLAW_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
