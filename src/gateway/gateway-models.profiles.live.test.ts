...
import { ensureMerClawModelsJson } from "../agents/models-config.js";
import { STREAM_ERROR_FALLBACK_TEXT } from "../agents/stream-message-shared.js";
import { clearRuntimeConfigSnapshot, getRuntimeConfig } from "../config/io.js";
import type { ModelsConfig, ModelProviderConfig, MerClawConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.MERCLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.MERCLAW_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.MERCLAW_LIVE_GATEWAY_SMOKE);
const THINKING_LEVEL = resolveGatewayLiveThinkingLevel({
  raw: process.env.MERCLAW_LIVE_GATEWAY_THINKING,
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.MERCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_SETUP_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.MERCLAW_LIVE_GATEWAY_SETUP_TIMEOUT_MS, 60_000),
);
...
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.MERCLAW_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.MERCLAW_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["MERCLAW_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.MERCLAW_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.MERCLAW_LIVE_GATEWAY_MODELS?.trim();
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.MERCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.MERCLAW_LIVE_MODEL_TIMEOUT_MS,
...
function enterProductionEnvForLiveRun() {
  const previous = {
    ...
    testFast: process.env.MERCLAW_TEST_FAST,
  };
...
function restoreProductionEnvForLiveRun(previous: {
  ...
  if (previous.testFast === undefined) {
    ...
    process.env.MERCLAW_TEST_FAST = previous.testFast;
  }
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.MERCLAW_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.MERCLAW_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.MERCLAW_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("MERCLAW_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("MERCLAW_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("MERCLAW_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.MERCLAW_LIVE_GATEWAY_MODELS;
    delete process.env.MERCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.MERCLAW_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.MERCLAW_LIVE_GATEWAY_MODELS = "openai/gpt-5.5,anthropic/claude-opus-4-6";
    delete process.env.MERCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.MERCLAW_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.MERCLAW_LIVE_GATEWAY_MAX_MODELS = "2";
...
describe("buildLiveGatewayConfig", () => {
  it("pins selected live gateway models to the MerClaw runtime", () => {
...
describe("enterProductionEnvForLiveRun", () => {
  it("clears Vitest fast-reply flags while preserving caller state", () => {
    ...
    try {
      ...
      expect(process.env.MERCLAW_TEST_FAST).toBeUndefined();
    } finally {
      ...
      restoreOptionalEnv("MERCLAW_TEST_FAST", previous.testFast);
    }
  });
});
      ...
      defaults: {
        ...
        models: Object.fromEntries(
          params.candidates.map((m) => [
            `${m.provider}/${m.id}`,
            { agentRuntime: { id: "merclaw" as const } },
          ]),
        ),
      },
    },
...
async function sanitizeAuthConfig(params: {
  cfg: MerClawConfig;
  agentDir: string;
}): Promise<MerClawConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<MerClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const runtimeEnv = enterProductionEnvForLiveRun();
  const previous = {
    configPath: process.env.MERCLAW_CONFIG_PATH,
    token: process.env.MERCLAW_GATEWAY_TOKEN,
    skipChannels: process.env.MERCLAW_SKIP_CHANNELS,
    skipGmail: process.env.MERCLAW_SKIP_GMAIL_WATCHER,
    skipCron: process.env.MERCLAW_SKIP_CRON,
    skipCanvas: process.env.MERCLAW_SKIP_CANVAS_HOST,
    disableBonjour: process.env.MERCLAW_DISABLE_BONJOUR,
    logLevel: process.env.MERCLAW_LOG_LEVEL,
    agentDir: process.env.MERCLAW_AGENT_DIR,
    stateDir: process.env.MERCLAW_STATE_DIR,
  };
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.MERCLAW_DISABLE_BONJOUR = "1";
    process.env.MERCLAW_LOG_LEVEL = "silent";
  }
  ...
  const tempStateDir: string | undefined = await fs.mkdtemp(
    path.join(os.tmpdir(), "merclaw-live-state-"),
  );
  ...
  process.env.MERCLAW_AGENT_DIR = tempAgentDir;

  const workspaceDir = resolveAgentWorkspaceDir(params.cfg, agentId);
  await fs.mkdir(workspaceDir, { recursive: true });
  await fs.mkdir(path.join(workspaceDir, ".merclaw"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".merclaw", "workspace-state.json"),
    `${JSON.stringify(
      {
        version: 1,
  ...
  const toolProbePath = path.join(workspaceDir, `.merclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveDefaultAgentDir(params.cfg);
  const sanitizedCfg: MerClawConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-live-"));
  const tempConfigPath = path.join(tempDir, "merclaw.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.MERCLAW_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            (async () => {
              ...
              for (
                ...
                toolText = await requestGatewayAgentText({
                  ...
                  modelKey,
                  message: strictReply
                    ? "MerClaw live tool probe (local, safe): " +
    ...
    process.env.MERCLAW_CONFIG_PATH = previous.configPath;
    process.env.MERCLAW_GATEWAY_TOKEN = previous.token;
    process.env.MERCLAW_SKIP_CHANNELS = previous.skipChannels;
    process.env.MERCLAW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.MERCLAW_SKIP_CRON = previous.skipCron;
    process.env.MERCLAW_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.MERCLAW_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.MERCLAW_LOG_LEVEL = previous.logLevel;
    process.env.MERCLAW_AGENT_DIR = previous.agentDir;
    process.env.MERCLAW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        const modelsJsonResult = await withGatewayLiveSetupTimeout(
          ensureMerClawModelsJson(cfg, undefined, {
            workspaceDir,
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via MERCLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    await ensureMerClawModelsJson(cfg);
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.merclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
