...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("merclaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(async () => {
    ...
    delete process.env.MERCLAW_PROXY_LOOPBACK_MODE;
  ...
  it("allows ws:// hostnames with MERCLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.MERCLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://merclaw-gateway.ai:18789",
      onConnectError,
    });
...
describe("GatewayClient close handling", () => {
  ...
  it("clears stale token on device token mismatch close", () => {
    const onClose = vi.fn();
    const env = { MERCLAW_HOME: "/tmp/custom-merclaw-home" };
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      MERCLAW_STATE_DIR: "/tmp/merclaw-client-service-state",
