...
async function writeConfig(config: MerClawConfig): Promise<void> {
...
describe("gateway startup web fetch config", () => {
  it("binds HTTP with credential-free tools.web.fetch config without fetch provider discovery", async () => {
    const previousMinimal = process.env.MERCLAW_TEST_MINIMAL_GATEWAY;
    process.env.MERCLAW_TEST_MINIMAL_GATEWAY = "0";
    ...
    try {
      ...
      if (previousMinimal === undefined) {
        ...
        process.env.MERCLAW_TEST_MINIMAL_GATEWAY = previousMinimal;
      }
    }
  });
});
