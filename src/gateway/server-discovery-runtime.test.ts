...
const mocks = vi.hoisted(() => ({
  ...
  resolveWideAreaDiscoveryDomain: vi.fn<ResolveWideAreaDiscoveryDomain>(() => "merclaw.internal."),
  writeWideAreaGatewayZone: vi.fn<WriteWideAreaGatewayZone>(async () => ({
    changed: true,
    zonePath: "/tmp/merclaw.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (MerClaw)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/merclaw"),
...
describe("startGatewayDiscovery", () => {
  ...
  it("starts registered local discovery services with gateway advertisement context", async () => {
    ...
    process.env.MERCLAW_SSH_PORT = "2222";
  ...
  it("omits invalid SSH discovery ports", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.MERCLAW_SSH_PORT = "2222abc";
  ...
  it("omits out-of-range SSH discovery ports", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.MERCLAW_SSH_PORT = "65536";
  ...
  it("continues startup when a local discovery service never settles", async () => {
    ...
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.MERCLAW_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = "10";
  ...
  it("uses the default discovery timeout for partial timeout env values", async () => {
    ...
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.MERCLAW_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = "10abc";
  ...
  it("skips local discovery services for truthy MERCLAW_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.MERCLAW_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "merclaw.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
    ...
    expect(zoneParams.domain).toBe("merclaw.internal.");
    expect(zoneParams.gatewayPort).toBe(18789);
    expect(zoneParams.gatewayDirectReachable).toBe(true);
    expect(zoneParams.displayName).toBe("Lab Mac (MerClaw)");
    ...
    expect(logs.info.mock.calls).toEqual([
      ["wide-area DNS-SD updated (merclaw.internal. → /tmp/merclaw.internal.db)"],
    ]);
    expect(result.bonjourStop).toBeNull();
  });
