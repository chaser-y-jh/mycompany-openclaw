...
function cfg(input: Partial<MerClawConfig>): MerClawConfig {
  return input as MerClawConfig;
}
...
const DEFAULT_ENV = {
  ...
  MERCLAW_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when MERCLAW env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when MERCLAW env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when MERCLAW env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
