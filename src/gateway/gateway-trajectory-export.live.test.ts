...
const CODEX_HARNESS_LIVE = process.env.MERCLAW_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.MERCLAW_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["MERCLAW_TRAJECTORY", "MERCLAW_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-merclaw-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.MERCLAW_TRAJECTORY = "1";
      process.env.MERCLAW_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".merclaw", "trajectory-exports", "bundle");
