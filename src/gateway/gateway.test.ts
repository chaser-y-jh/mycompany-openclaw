...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "merclaw-test-empty-bundled-plugins");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".merclaw", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["MERCLAW_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  const workspaceDir = path.join(tempHome, "merclaw");
  await fs.mkdir(workspaceDir, { recursive: true });
  process.env.MERCLAW_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const configDir = path.join(tempHome, ".merclaw");
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "merclaw-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-wizard-home-"));
      const configPath = path.join(tempHome, ".merclaw", "merclaw.json");
      process.env.HOME = tempHome;
      process.env.MERCLAW_STATE_DIR = path.join(tempHome, ".merclaw");
      process.env.MERCLAW_CONFIG_PATH = configPath;
      process.env.MERCLAW_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-minimal-gateway-home-"));
      const configPath = path.join(tempHome, ".merclaw", "merclaw.json");
      const bundledPluginsDir = path.join(tempHome, "merclaw-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.MERCLAW_STATE_DIR = path.join(tempHome, ".merclaw");
      process.env.MERCLAW_CONFIG_PATH = configPath;
      process.env.MERCLAW_SKIP_CHANNELS = "1";
