...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("MERCLAW_CONFIG_PATH is required for gateway config tests");
  }
...
async function postResponses(port: number, body: unknown, headers?: Record<string, string>) {
  const res = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
    ...
    headers: {
      ...
      "x-merclaw-scopes": "operator.write",
      ...headers,
    },
...
describe("OpenResponses HTTP API (e2e)", () => {
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "merclaw", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `merclaw` or `merclaw/<agentId>`.",
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "merclaw/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "merclaw/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "merclaw", input: "hi" },
        { "x-merclaw-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-merclaw-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("accepts write-scoped and admin-scoped HTTP callers", async () => {
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "merclaw", input: "hi" },
      { "x-merclaw-scopes": "operator.admin, operator.write" },
    );
