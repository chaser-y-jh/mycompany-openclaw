...
const { createMerClawTools } = await import("../agents/merclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createMerClawTools>[number];
...
beforeAll(async () => {
  envSnapshot = captureEnv(["MERCLAW_GATEWAY_PORT", "MERCLAW_GATEWAY_TOKEN"]);
  ...
  process.env.MERCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.MERCLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  ...
  it(
    ...
    async () => {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-sessions-send-route-"));
