...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "merclaw-test-owner-display-secret",
    },
  };
}
...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.MERCLAW_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as MerClawConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "merclaw-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.MERCLAW_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
