...
/**
 * Lazily resolve the local node host's nodeId from ~/.merclaw/node.json.
 * Process-stable: only changes on `merclaw node install`, which requires restart.
 */
...
function resolveTrustedProxyControlUiScopes(params: {
  ...
  const rawHeader = firstHeaderValue(params.upgradeReq.headers["x-merclaw-scopes"]);
  ...
  const handleMessage = async (data: RawData) => {
    ...
    try {
      ...
      if (!client) {
        ...
        // matching instanceId against ~/.merclaw/node.json nodeId). SSH-tunneled remote
