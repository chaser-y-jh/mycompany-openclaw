...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.MERCLAW_CONFIG_PATH) {
    configPaths.add(process.env.MERCLAW_CONFIG_PATH);
  }
  if (process.env.MERCLAW_STATE_DIR) {
    configPaths.add(path.join(process.env.MERCLAW_STATE_DIR, "merclaw.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.MERCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "merclaw-test-no-bundled-extensions")
    : "merclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".merclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".merclaw-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".merclaw-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>merclaw-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__merclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__merclawTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["MERCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.MERCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.MERCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.MERCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
