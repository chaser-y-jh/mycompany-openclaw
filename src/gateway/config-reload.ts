...
function asPluginInstallConfig(records: PluginInstallRecords): MerClawConfig {
  return {
    plugins: {
      installs: records,
    },
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: MerClawConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: MerClawConfig) => void | Promise<void>;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: MerClawConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (
    ...
    nextCompareConfig: MerClawConfig,
    afterWrite?: ConfigWriteNotification["afterWrite"],
  ) => {
