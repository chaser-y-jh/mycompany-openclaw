export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current MerClaw session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsMerClawStatus =
    normalized.includes("merclaw is running on") ||
    /merclaw\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("merclaw status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsMerClawStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from merclaw status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live merclaw config shows") ||
