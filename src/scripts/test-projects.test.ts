...
const resolveExpectedVitestNodeArgs = (env: NodeJS.ProcessEnv) =>
  ["1", "true", "yes", "on"].includes(env.MERCLAW_VITEST_ENABLE_MAGLEV?.trim().toLowerCase() ?? "")
    ? []
    : ["--no-maglev"];
...
describe("test-projects args", () => {
  ...
  it("routes boundary targets to the boundary config", () => {
    expect(buildVitestRunPlans(["src/infra/merclaw-root.test.ts"])).toEqual([
      {
        config: "test/vitest/vitest.boundary.config.ts",
        forwardedArgs: [],
        includePatterns: ["src/infra/merclaw-root.test.ts"],
        watchMode: false,
      },
    ]);
  });
  ...
  it("caps project-level parallelism when the Vitest worker budget is conservative", () => {
    ...
    expect(
      resolveParallelFullSuiteConcurrency(58, {
        MERCLAW_TEST_WORKERS: "1",
      }),
    ).toBe(1);
  });

  it("keeps conservative core full-suite runs on aggregate shards", () => {
    const originalVitestMaxWorkers = process.env.MERCLAW_VITEST_MAX_WORKERS;
    const originalTestWorkers = process.env.MERCLAW_TEST_WORKERS;
    const originalProjectParallel = process.env.MERCLAW_TEST_PROJECTS_PARALLEL;
    const originalLeafShards = process.env.MERCLAW_TEST_PROJECTS_LEAF_SHARDS;
    try {
      ...
      if (originalVitestMaxWorkers === undefined) {
        ...
        process.env.MERCLAW_VITEST_MAX_WORKERS = originalVitestMaxWorkers;
      }
      if (originalTestWorkers === undefined) {
        delete process.env.MERCLAW_TEST_WORKERS;
      } else {
        process.env.MERCLAW_TEST_WORKERS = originalTestWorkers;
      }
      if (originalProjectParallel === undefined) {
        delete process.env.MERCLAW_TEST_PROJECTS_PARALLEL;
      } else {
        process.env.MERCLAW_TEST_PROJECTS_PARALLEL = originalProjectParallel;
      }
      if (originalLeafShards === undefined) {
        delete process.env.MERCLAW_TEST_PROJECTS_LEAF_SHARDS;
      } else {
        process.env.MERCLAW_TEST_PROJECTS_LEAF_SHARDS = originalLeafShards;
      }
    }
  });
  ...
  it("uses a bounded local default for full-suite project parallelism", () => {
    expect(
      resolveParallelFullSuiteConcurrency(
        58,
        {
          MERCLAW_TEST_PROJECTS_LEAF_SHARDS: "1",
        },
        {
          cpuCount: 8,
          loadAverage1m: 0,
  ...
  it("gives parallel Vitest shards separate filesystem module caches", () => {
    ...
    expect(firstEnv?.MERCLAW_VITEST_FS_MODULE_CACHE_PATH).toBe(
      "/repo/node_modules/.experimental-vitest-cache/0-test-vitest-vitest.gateway.config.ts",
    );
    expect(specs[1]?.env.MERCLAW_VITEST_FS_MODULE_CACHE_PATH).toBe(
  ...
  it("preserves explicit Vitest filesystem module cache paths", () => {
    ...
    expect(
      applyParallelVitestCachePaths(specs, {
        cwd: "/repo",
        env: {
          MERCLAW_VITEST_FS_MODULE_CACHE_PATH: "/tmp/cache",
        },
      }),
    ).toBe(specs);
  });
  ...
  it("routes top-level test helpers to importing repo tests", () => {
    expect(buildVitestRunPlans(["test/helpers/temp-dir.ts"])).toEqual([
      ...
      },
      {
        ...
        forwardedArgs: ["test/merclaw-launcher.e2e.test.ts"],
  ...
  it("writes scoped include files for routed extension runs", () => {
    ...
    expect(spec?.includeFilePath).toContain("merclaw-vitest-include-");
    expect(spec?.env.MERCLAW_VITEST_INCLUDE_FILE).toBe(spec?.includeFilePath);
  });

  it("rejects explicit test file targets that do not exist", () => {
    expect(findUnmatchedExplicitTestTargets(["src/not-a-real-merclaw-test.test.ts"])).toEqual([
      {
        target: "src/not-a-real-merclaw-test.test.ts",
  ...
  it("skips channel contract configs with no matching external include patterns", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-contract-include-"));
    try {
