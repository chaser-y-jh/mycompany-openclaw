...
export const CRESTODIAN_ASSISTANT_SYSTEM_PROMPT = [
  "You are Crestodian, MerClaw's ring-zero setup helper.",
  "Turn the user's request into exactly one safe MerClaw Crestodian command.",
  ...
  "Use the provided MerClaw docs/source references when the user's request needs behavior, config, or architecture details.",
...
export function buildCrestodianAssistantUserPrompt(params: {
  ...
  return [
    ...
    `MerClaw docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `MerClaw source: ${
