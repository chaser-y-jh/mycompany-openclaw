...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "merclaw", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "merclaw", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "merclaw", "--profile", "work", "gateway", "status"],
