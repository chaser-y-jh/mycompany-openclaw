...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["merclaw channels list", "List configured channels."],
          ["merclaw channels list --all", "Show configured, bundled, and installable channels."],
          ["merclaw channels add", "Open guided channel setup."],
          ["merclaw channels status --probe", "Run channel status checks and probes."],
          [
            "merclaw channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["merclaw channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.merclaw.ai/cli/channels",
        )}\n`,
    );
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["merclaw channels add", "Open guided setup for available chat channels."],
          [
            "merclaw channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["merclaw channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)
