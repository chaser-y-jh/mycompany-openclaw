...
const loadConfig = vi.fn<() => MerClawConfig>(() => ({
  gateway: {
    auth: {
      token: "config-token",
    },
...
function stubServiceGatewayTokenEnv() {
  service.readCommand.mockResolvedValue({
    programArguments: [],
    environment: {
      MERCLAW_GATEWAY_TOKEN: "service-token",
...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { MERCLAW_GATEWAY_TOKEN: "service-token" },
    });
    stubEmptyGatewayEnv();
  });
  ...
  it("prints the container restart hint when restart is requested for a not-loaded service", async () => {
    ...
    vi.stubEnv("MERCLAW_CONTAINER_HINT", "merclaw-demo-container");

    await runServiceRestart({
      serviceNoun: "Gateway",
      service,
      renderStartHints: () => [
        "Restart the container or the service that manages it for merclaw-demo-container.",
    ...
    expect(runtimeLogs).toContain(
      "Start with: Restart the container or the service that manages it for merclaw-demo-container.",
    );
  });
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { MERCLAW_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("MERCLAW_GATEWAY_TOKEN", "env-token");
  ...
  it("repairs stale loaded services during start before reporting success", async () => {
    service.readCommand.mockResolvedValue({
      programArguments: ["merclaw", "gateway"],
      environment: { MERCLAW_SERVICE_VERSION: "2026.4.24" },
    });
    const repairLoadedService = vi.fn(async () => ({
      result: "started" as const,
      message: "Gateway service definition repaired and started.",
      warnings: ["service was installed by MerClaw 2026.4.24, current CLI is 2026.5.2"],
      loaded: true,
    }));
    ...
    expect(payload.warnings?.[0]).toContain("service was installed by MerClaw");
    expect(payload.service?.loaded).toBe(true);
  });
  ...
  it("fails start with an install hint when a stale loaded service has no repair callback", async () => {
    ...
    expect(payload.hints).toEqual(["merclaw gateway install --force"]);
    expect(service.restart).not.toHaveBeenCalled();
  });
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    await runServiceStart({
      serviceNoun: "Gateway",
      service,
      renderStartHints: () => ["merclaw gateway install"],
      opts: { json: true },
    });
    ...
    expect(payload.hints?.includes("merclaw gateway install")).toBe(true);
    expect(
      payload.hintItems?.some(
        (item) => item.kind === "install" && item.text === "merclaw gateway install",
      ),
    ).toBe(true);
    expect(service.restart).not.toHaveBeenCalled();
  });
