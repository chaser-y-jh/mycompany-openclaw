...
describe("renderGatewayServiceStartHints", () => {
  it("resolves daemon container context from either env key", () => {
    expect(
      resolveDaemonContainerContext({
        MERCLAW_CONTAINER: "merclaw-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("merclaw-demo-container");
    expect(
      resolveDaemonContainerContext({
        MERCLAW_CONTAINER_HINT: "merclaw-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("merclaw-demo-container");
  });

  it("prepends a single container restart hint when MERCLAW_CONTAINER is set", () => {
    expect(
      renderGatewayServiceStartHints({
        MERCLAW_CONTAINER: "merclaw-demo-container",
      } as NodeJS.ProcessEnv),
  ...
  it("prepends a single container restart hint when MERCLAW_CONTAINER_HINT is set", () => {
    expect(
      renderGatewayServiceStartHints({
        MERCLAW_CONTAINER_HINT: "merclaw-demo-container",
      } as NodeJS.ProcessEnv),
...
describe("filterContainerGenericHints", () => {
  it("drops the generic container foreground hint when MERCLAW_CONTAINER is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
          "If you're in a container, run the gateway in the foreground instead of `merclaw gateway`.",
        ],
        { MERCLAW_CONTAINER: "merclaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });

  it("drops the generic container foreground hint when MERCLAW_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
        ...
        { MERCLAW_CONTAINER_HINT: "merclaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });
});
