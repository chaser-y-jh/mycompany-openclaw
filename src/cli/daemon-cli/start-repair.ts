...
export async function repairLoadedGatewayServiceForStart(params: {
  ...
  const wrapperPath = await resolveMerClawWrapperPath(installEnv[MERCLAW_WRAPPER_ENV_KEY]);
  ...
  return {
    result: "started",
    message:
      "Gateway service definition repaired and started. Reopen the Control UI with `merclaw dashboard` or copy a fresh auth URL with `merclaw dashboard --no-open`.",
