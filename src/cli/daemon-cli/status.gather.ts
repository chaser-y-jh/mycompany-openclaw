...
type DaemonConfigContext = {
  ...
  cliCfg: MerClawConfig;
  daemonCfg: MerClawConfig;
...
function resolveSnapshotRuntimeConfig(snapshot: ConfigFileSnapshot | null): MerClawConfig | null {
...
function coerceStatusConfig(value: unknown): MerClawConfig {
...
export type DaemonStatus = {
  ...
  service: {
    ...
    staleUpdateLaunchdJobs?: StaleMerClawUpdateLaunchdJob[];
  };
  config?: {
    cli: ConfigSummary;
  ...
  const staleUpdateLaunchdJobs =
    opts.deep && process.platform === "darwin"
      ? await loadLaunchdModule()
          .then(({ findStaleMerClawUpdateLaunchdJobs }) => findStaleMerClawUpdateLaunchdJobs())
          .catch(() => [])
      : [];
