...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "merclaw", "terminal"] },
    { argv: ["node", "merclaw", "chat"] },
    { argv: ["node", "merclaw", "gateway"] },
    { argv: ["node", "merclaw", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "merclaw", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "merclaw", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "merclaw", "gateway", "status"] },
    { argv: ["node", "merclaw", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "merclaw", "--help"] },
    { argv: ["node", "merclaw", "gateway"] },
    { argv: ["node", "merclaw", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "merclaw.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("merclaw.internal");
    } finally {
      log.mockRestore();
    }
  });
