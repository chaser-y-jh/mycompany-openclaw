...
describe("plugins cli policy mutations", () => {
  ...
  function requireFirstWrittenConfig(): MerClawConfig {
  ...
  ): NonNullable<NonNullable<MerClawConfig["plugins"]>["entries"]> {
  ...
  it("refreshes the persisted plugin registry after enabling a plugin", async () => {
    const sourceConfig = {} as MerClawConfig;
    const enabledConfig = {
      plugins: {
        entries: {
          alpha: { enabled: true },
  ...
  it.each(["enable", "disable"] as const)(
    ...
    async (command) => {
      ...
      expect(runtimeErrors).toContain(
        "Plugin not found: missing-plugin. Run `merclaw plugins list` to see installed plugins, or `merclaw plugins search missing-plugin` to look for installable plugins.",
      );
      expect(enablePluginInConfig).not.toHaveBeenCalled();
      expect(writeConfigFile).not.toHaveBeenCalled();
