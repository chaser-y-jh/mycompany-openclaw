...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-bundled-skills-test-"));
    process.env.MERCLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
