...
vi.mock("../infra/path-env.js", () => ({
  ensureMerClawCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  beforeEach(() => {
    ...
    delete process.env.MERCLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH;
    delete process.env.MERCLAW_HIDE_BANNER;
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "merclaw", "status"]);
  ...
  it("disposes registered harnesses after full CLI command completion", async () => {
    ...
    await runCli(["node", "merclaw", "agent", "--local"]);

    expect(parseAsync).toHaveBeenCalledWith(["node", "merclaw", "agent", "--local"]);
  ...
  it("shows the standard spinner while loading the full CLI", async () => {
    ...
    await runCli(["node", "merclaw", "config"]);
  ...
  it("pauses non-tty stdin after full CLI command completion", async () => {
    ...
    try {
      await runCli(["node", "merclaw", "channels"]);

      expect(parseAsync).toHaveBeenCalledWith(["node", "merclaw", "channels"]);
  ...
  it("emits the startup banner before gateway foreground fast-path startup", async () => {
    await runCli(["node", "merclaw", "gateway", "--force"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).toHaveBeenCalledWith("9.9.9-test", {
      argv: ["node", "merclaw", "gateway", "--force"],
    });
  ...
  it("honors banner suppression on the gateway foreground fast path", async () => {
    process.env.MERCLAW_HIDE_BANNER = "1";

    await runCli(["node", "merclaw", "gateway"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).not.toHaveBeenCalled();
    expect(commanderParseAsyncMock).toHaveBeenCalledWith(["node", "merclaw", "gateway"]);
  });
  ...
  it("renders browser help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "merclaw", "browser", "--help"]);
  ...
  it("renders secrets help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "merclaw", "secrets", "--help"]);
  ...
  it("renders nodes help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "merclaw", "nodes", "--help"]);
  ...
  it("defers nodes help startup metadata when plugin config can change command metadata", async () => {
    const argv = ["node", "merclaw", "nodes", "--help"];
    const parseAsync = vi.fn().mockResolvedValueOnce(undefined);
    const program = {
  ...
  it("renders selected subcommand help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "merclaw", "doctor", "--help"]);
  ...
  it("keeps root help on the precomputed path without proxy bootstrap", async () => {
    ...
    await runCli(["node", "merclaw", "--help"]);
  ...
  it("renders setup/onboard/configure help without building the full program", async () => {
    await runCli(["node", "merclaw", "setup", "--help"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "merclaw", "--help"]);
    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(loadRootHelpRenderOptionsForConfigSensitivePluginsMock).toHaveBeenCalledTimes(1);
    expect(outputPrecomputedRootHelpTextMock).toHaveBeenCalledTimes(1);
  ...
  it.each([
    ["gateway runtime", ["node", "merclaw", "gateway", "run"]],
    ["bare gateway runtime", ["node", "merclaw", "gateway"]],
    ["node runtime", ["node", "merclaw", "node", "run"]],
    ["local agent runtime", ["node", "merclaw", "agent", "--local"]],
    ["provider inference", ["node", "merclaw", "infer", "web", "fetch", "https://example.com"]],
    ["model command", ["node", "merclaw", "models", "auth", "login", "openai"]],
    ["plugin command", ["node", "merclaw", "plugins", "marketplace", "list"]],
    ["skill command", ["node", "merclaw", "skills", "search", "browser"]],
    ["update command", ["node", "merclaw", "update", "check"]],
    ["channel probe", ["node", "merclaw", "channels", "status", "--probe"]],
    ["channel capabilities probe", ["node", "merclaw", "channels", "capabilities"]],
    ["directory plugin command", ["node", "merclaw", "directory", "peers", "list"]],
    ["message plugin command", ["node", "merclaw", "message", "send", "--to", "demo"]],
    ["metadata-owned plugin command", ["node", "merclaw", "googlemeet", "login"]],
  ])("starts managed proxy routing for %s", (_name, argv) => {
    expect(shouldStartProxyForCli(argv)).toBe(true);
  });

  it.each([
    ["root help", ["node", "merclaw", "--help"]],
    ["root version", ["node", "merclaw", "--version"]],
    ["gateway help", ["node", "merclaw", "gateway", "--help"]],
    ["gateway run help", ["node", "merclaw", "gateway", "run", "--help"]],
    ["status", ["node", "merclaw", "status"]],
    ["health", ["node", "merclaw", "health"]],
    ["gateway status", ["node", "merclaw", "gateway", "status"]],
    ["gateway health", ["node", "merclaw", "gateway", "health"]],
    ["remote agent control-plane", ["node", "merclaw", "agent", "run"]],
    ["chat control-plane", ["node", "merclaw", "chat"]],
    ["terminal control-plane", ["node", "merclaw", "terminal"]],
    ["config", ["node", "merclaw", "config", "get", "proxy.enabled"]],
    ["channels parent help", ["node", "merclaw", "channels"]],
    ["completion", ["node", "merclaw", "completion", "zsh"]],
    ["debug proxy cli", ["node", "merclaw", "proxy", "start"]],
    ["agents list", ["node", "merclaw", "agents", "list"]],
    ["models list", ["node", "merclaw", "models", "list"]],
    ["models status without live probe", ["node", "merclaw", "models", "status"]],
    ["skills check", ["node", "merclaw", "skills", "check"]],
    ["skills info", ["node", "merclaw", "skills", "info", "weather"]],
    ["skills list", ["node", "merclaw", "skills", "list"]],
    ["tasks list", ["node", "merclaw", "tasks", "list"]],
    ["legacy singular tool namespace", ["node", "merclaw", "tool", "image_generate"]],
    ["gateway tools namespace typo", ["node", "merclaw", "tools", "effective"]],
    ["migrate", ["node", "merclaw", "migrate"]],
  ...
  it("starts the managed proxy for network-capable commands by default", async () => {
    ...
    await runCli(["node", "merclaw", "plugins", "marketplace", "list"]);
  ...
  it("starts the managed proxy for metadata-owned plugin commands by default", async () => {
    ...
    await runCli(["node", "merclaw", "googlemeet", "login"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it("rejects unowned command roots before proxy and plugin runtime registration", async () => {
    await expect(runCli(["node", "merclaw", "foo"])).rejects.toThrow(
  ...
  it("rejects unowned command roots even when --help is appended (regression for #81077)", async () => {
    await expect(runCli(["node", "merclaw", "foo", "--help"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("rejects unowned command roots even when --version is appended", async () => {
    await expect(runCli(["node", "merclaw", "foo", "--version"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("does not suggest plugins.allow for unknown command roots before proxy startup", async () => {
    ...
    let error: unknown;
    try {
      await runCli(["node", "merclaw", "totally-unknown"]);
    } catch (caught) {
      error = caught;
    }
  ...
  it("preserves plugins.allow diagnostics for roots owned only by CLI metadata", async () => {
    ...
    await expect(runCli(["node", "merclaw", "qa"])).rejects.toThrow(
  ...
  it("reports plugin tool command mistakes before proxy startup", async () => {
    ...
    await expect(runCli(["node", "merclaw", "lcm_recent"])).rejects.toThrow(
  ...
  it("does not install the env proxy dispatcher for bypassed skills inspection commands", async () => {
    ...
    await runCli(["node", "merclaw", "skills", "check"]);
  ...
  it.each([
    ["auth", ["node", "merclaw", "auth", "--help"]],
    ["tool", ["node", "merclaw", "tool", "image_generate"]],
    ["tools", ["node", "merclaw", "tools", "effective"]],
  ...
  it("routes lazy plugin registration logs to stderr only during --json registration", async () => {
    ...
    await runCli(["node", "merclaw", "memory", "search", "query", "--json"]);
  ...
  it("does not route lazy plugin registration logs for pass-through --json after terminator", async () => {
    ...
    await runCli(["node", "merclaw", "memory", "--", "--json"]);
  ...
  it("fails protected commands when managed proxy activation fails", async () => {
    ...
    await expect(runCli(["node", "merclaw", "gateway", "run"])).rejects.toThrow(
  ...
  it("stops the managed proxy after normal gateway runtime completion", async () => {
    ...
    await runCli(["node", "merclaw", "gateway", "run"]);
  ...
  it("stops the managed proxy and exits after SIGINT", async () => {
    ...
    try {
      const runPromise = runCli(["node", "merclaw", "plugins", "marketplace", "list"]);
      await vi.waitFor(() => {
        expect(
  ...
  it("starts onboarding for bare root invocations when config only has metadata", async () => {
    readConfigFileSnapshotMock.mockResolvedValueOnce({
      ...
      valid: true,
      sourceConfig: {
        $schema: "https://merclaw.ai/config.json",
  ...
  it("bootstraps env proxy before modern onboard Crestodian startup", async () => {
    ...
    await runCli(["node", "merclaw", "onboard", "--modern", "--json"]);
  ...
  it("does not fail the command when memory cleanup is unavailable", async () => {
    ...
    await expect(runCli(["node", "merclaw", "status"])).resolves.toBeUndefined();
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "merclaw", "--container", "demo", "status"]);
  ...
  it("swallows Commander parse exits after recording the exit code", async () => {
    ...
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "status", ["node", "merclaw", "status"]],
    ]);
    expect(process.exitCode).toBe(1);
    process.exitCode = exitCode;
  });
  ...
  it("loads the real primary command before rendering command help", async () => {
    ...
    expect(registerCoreCliByNameMock.mock.calls).toEqual([
      [program, ctx, "doctor", ["node", "merclaw", "doctor", "--help"]],
    ]);
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "doctor", ["node", "merclaw", "doctor", "--help"]],
    ]);
  });
  ...
  it("restores terminal state before uncaught CLI exits", async () => {
    ...
    try {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "[merclaw] MerClaw hit an unexpected runtime error.",
      );
      expect(consoleErrorSpy).toHaveBeenCalledWith("[merclaw] Reason: boom");
  ...
  it("does not exit for transient uncaught CLI exceptions", async () => {
    ...
    try {
      ...
      expect(consoleWarnSpy.mock.calls).toEqual([
        ["[merclaw] Non-fatal uncaught exception (continuing):", hostUnreachable.stack],
      ]);
