...
describe("ensureConfigReady", () => {
  ...
  const originalMerClawStateDir = process.env.MERCLAW_STATE_DIR;
  ...
  function useTempMerClawHome(): string {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-config-guard-"));
    tempRoots.push(root);
    process.env.MERCLAW_HOME = root;
    delete process.env.MERCLAW_STATE_DIR;
    return root;
  }

  function writeLegacyTaskSidecarMarker(root: string): void {
    const markerPath = path.join(root, ".merclaw", "tasks", "runs.sqlite");
    fs.mkdirSync(path.dirname(markerPath), { recursive: true });
    fs.writeFileSync(markerPath, "");
  }

  function writeStateMarker(root: string, relativePath: string): void {
    const markerPath = path.join(root, ".merclaw", relativePath);
    fs.mkdirSync(path.dirname(markerPath), { recursive: true });
    fs.writeFileSync(markerPath, "{}");
  }
  ...
  afterEach(() => {
    ...
    if (originalMerClawStateDir === undefined) {
      delete process.env.MERCLAW_STATE_DIR;
    } else {
      process.env.MERCLAW_STATE_DIR = originalMerClawStateDir;
    }
    for (const root of tempRoots.splice(0)) {
      fs.rmSync(root, { recursive: true, force: true });
    }
  ...
  it("runs doctor flow for legacy sessions without task sidecars", async () => {
    const root = useTempMerClawHome();
    fs.mkdirSync(path.join(root, ".merclaw", "sessions"), { recursive: true });
  ...
  it("uses shared tilde expansion for MERCLAW_HOME in the startup detector", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-config-guard-home-"));
    tempRoots.push(root);
    process.env.HOME = root;
    process.env.MERCLAW_HOME = "~/svc";
    delete process.env.MERCLAW_STATE_DIR;
  ...
  it("exits for invalid config on non-allowlisted commands", async () => {
    ...
    expect(plainErrorCalls(runtime)).toEqual([
      "MerClaw config is invalid",
      "File: /tmp/merclaw.json",
      "Problem:",
      "  - channels.quietchat: invalid",
      "",
      `Fix: ${formatCliCommand("merclaw doctor --fix")}`,
      `Inspect: ${formatCliCommand("merclaw config validate")}`,
  ...
  it("replaces doctor fix advice for plugin packaging-only invalid config", async () => {
    ...
    expect(calls).not.toContain(`Fix: ${formatCliCommand("merclaw doctor --fix")}`);
    expect(runtime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it("runs doctor migration flow only once per module instance", async () => {
    writeLegacyTaskSidecarMarker(useTempMerClawHome());
