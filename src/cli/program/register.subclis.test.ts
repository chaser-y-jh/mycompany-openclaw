...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.MERCLAW_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.MERCLAW_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.MERCLAW_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.MERCLAW_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.MERCLAW_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      ...
      process.env.MERCLAW_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "merclaw", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "merclaw"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "merclaw", "nodes", "list"], "merclaw");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "merclaw", "infer"], "merclaw");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "merclaw", "acp", "--help"], "merclaw");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "merclaw", "gateway", "--force"];
    process.argv = argv;
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "merclaw", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("merclaw");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "merclaw", "completion", "--write-state"];
    const program = new Command().name("merclaw");
  ...
  it.each([
    ...
    process.argv = ["node", "merclaw", ...args];
    const program = new Command().name("merclaw");
  ...
  it("does not preload plugin CLI registrations for bare plugin parent help", async () => {
    process.argv = ["node", "merclaw", "plugins"];
    const program = new Command().name("merclaw");
