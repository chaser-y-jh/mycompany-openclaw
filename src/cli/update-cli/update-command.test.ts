...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/merclaw-root")).toEqual([
      path.join("/tmp/merclaw-root", "dist", "index.js"),
      path.join("/tmp/merclaw-root", "dist", "index.mjs"),
      path.join("/tmp/merclaw-root", "dist", "entry.js"),
      path.join("/tmp/merclaw-root", "dist", "entry.mjs"),
    ]);
  });
});
  ...
  it("uses the managed service port ahead of the caller environment", () => {
    expect(
      resolveUpdatedGatewayRestartPort({
        ...
        processEnv: { MERCLAW_GATEWAY_PORT: "19001" },
        serviceEnv: { MERCLAW_GATEWAY_PORT: "19002" },
      }),
    ).toBe(19002);
  });
...
describe("resolvePostUpdateServiceStateReadEnv", () => {
  it("keeps package restart preparation anchored to the pre-update service env", () => {
    const processEnv = {
      MERCLAW_STATE_DIR: "/source/state",
      MERCLAW_CONFIG_PATH: "/source/merclaw.json",
    } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = {
      MERCLAW_STATE_DIR: "/managed/state",
      MERCLAW_CONFIG_PATH: "/managed/merclaw.json",
    } as NodeJS.ProcessEnv;
  ...
  it("keeps git updates tied to the caller environment", () => {
    const processEnv = { MERCLAW_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = { MERCLAW_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
...
describe("resolvePostInstallDoctorEnv", () => {
  it("uses the managed service profile paths for post-install doctor", () => {
    const env = resolvePostInstallDoctorEnv({
      invocationCwd: "/srv/merclaw",
      baseEnv: {
        PATH: "/bin",
        MERCLAW_STATE_DIR: "/wrong/state",
        MERCLAW_CONFIG_PATH: "/wrong/merclaw.json",
        MERCLAW_PROFILE: "wrong",
      },
      serviceEnv: {
        MERCLAW_STATE_DIR: "daemon-state",
        MERCLAW_CONFIG_PATH: "daemon-state/merclaw.json",
    ...
    expect(env.MERCLAW_STATE_DIR).toBe(path.join("/srv/merclaw", "daemon-state"));
    expect(env.MERCLAW_CONFIG_PATH).toBe(
      path.join("/srv/merclaw", "daemon-state", "merclaw.json"),
    );
    expect(env.MERCLAW_PROFILE).toBe("work");
  });
  ...
  it("keeps the caller env when no managed service env is available", () => {
    const env = resolvePostInstallDoctorEnv({
      baseEnv: {
        PATH: "/bin",
        MERCLAW_STATE_DIR: "/caller/state",
        MERCLAW_PROFILE: "caller",
      },
    });

    expect(env.PATH).toBe("/bin");
    expect(env.NODE_DISABLE_COMPILE_CACHE).toBe("1");
    expect(env.MERCLAW_STATE_DIR).toBe("/caller/state");
    expect(env.MERCLAW_PROFILE).toBe("caller");
  });
});

describe("collectMissingPluginInstallPayloads", () => {
  it("reports tracked npm install records whose package payload is absent", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-update-plugin-payload-"));
    const presentDir = path.join(tmpDir, "state", "npm", "node_modules", "@merclaw", "present");
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@merclaw", "missing");
    ...
    try {
      ...
      await fs.writeFile(path.join(presentDir, "package.json"), '{"name":"@merclaw/present"}\n');
      ...
      await expect(
        collectMissingPluginInstallPayloads({
          ...
          records: {
            present: {
              source: "npm",
              spec: "@merclaw/present@beta",
              installPath: presentDir,
            },
            missing: {
              source: "npm",
              spec: "@merclaw/missing@beta",
              installPath: missingDir,
            },
            "no-package-json": {
              source: "npm",
              spec: "@merclaw/no-package-json@beta",
              installPath: noPackageJsonDir,
            },
            "missing-install-path": {
              source: "npm",
              spec: "@merclaw/missing-install-path@beta",
            },
            local: {
              source: "path",
  ...
  it("keeps disabled trusted official npm records eligible for payload repair when requested", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-update-plugin-payload-"));
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@merclaw", "codex");
    try {
      await expect(
        collectMissingPluginInstallPayloads({
          env: { HOME: tmpDir } as NodeJS.ProcessEnv,
...
describe("formatPostUpdateGatewayRecoveryInstructions", () => {
  ...
  it("uses systemd wording on Linux instead of macOS LaunchAgent instructions", () => {
    ...
    expect(line).toContain("merclaw gateway restart");
    expect(line).toContain("merclaw gateway install --force");
    expect(line).toContain("merclaw gateway status --deep");
...
describe("recoverInstalledLaunchAgentAfterUpdate", () => {
  it("re-bootstraps an installed-but-not-loaded macOS LaunchAgent after update", async () => {
    ...
    const serviceEnv = { MERCLAW_PROFILE: "stomme" } as NodeJS.ProcessEnv;
    const recoveredEnv = { ...serviceEnv, MERCLAW_PORT: "18790" } as NodeJS.ProcessEnv;
  ...
  it("does not recover a loaded LaunchAgent", async () => {
    const readState = vi.fn(async () => ({
      ...
      loaded: true,
      running: true,
      env: { MERCLAW_PROFILE: "stomme" } as NodeJS.ProcessEnv,
      command: null,
  ...
  it("does not report recovered update health until the gateway passes the post-recovery wait", async () => {
    ...
    await expect(
      recoverLaunchAgentAndRecheckGatewayHealth({
        ...
        env: { MERCLAW_PROFILE: "stomme", MERCLAW_PORT: "18790" },
...
describe("resolvePostCoreUpdateChildStdio", () => {
  it('returns "pipe" on Windows so the child never inherits the parent console handles', () => {
    ...
    // causing the terminal to hang after `merclaw update` completes (#78445).
