    ...
    plugins: [
      {
        ...
        startup: {},
        packageInstall: {
          clawhubSpec: `clawhub:@merclaw/${pluginId}`,
...
describe("listPersistedBundledPluginLocationBridges", () => {
  ...
  it("keeps persisted bundled relocations npm-first for launch", async () => {
    readPersistedInstalledPluginIndexMock.mockResolvedValue(
      makeIndex({
        ...
        manifestPath: "/app/dist/extensions/diagnostics-otel/merclaw.plugin.json",
        manifestHash: "hash",
  ...
  it("does not create a relocation bridge without persisted or official install metadata", async () => {
    readPersistedInstalledPluginIndexMock.mockResolvedValue(
      makeIndex({
        pluginId: "local-only",
        manifestPath: "/app/dist/extensions/local-only/merclaw.plugin.json",
        manifestHash: "hash",
