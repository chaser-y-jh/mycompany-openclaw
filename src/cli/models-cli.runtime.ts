...
export function rejectAgentScopedModelWrite(
  ...
  throw new Error(
    `merclaw models ${commandName} does not support --agent; it only updates global model defaults. Remove --agent, or run ${formatCliCommand("merclaw agents list")} and set the per-agent model in agent config.`,
  );
}
