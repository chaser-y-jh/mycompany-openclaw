...
export function formatCliBannerLine(version: string, options: BannerOptions = {}): string {
  ...
  const title = decorativePrefix("🦞", "MerClaw", emojiOptions);
  const prefix = decorativeEmoji("🦞", emojiOptions);
  const indent = prefix ? `${prefix} ` : "";
  const columns = options.columns ?? process.stdout.columns ?? 120;
  const companySuffix = " | Powered by MyCompany";
  const plainBaseLine = `${title} ${version} (${commitLabel})${companySuffix}`;
  const plainFullLine = tagline ? `${plainBaseLine} — ${tagline}` : plainBaseLine;
  const fitsOnOneLine = visibleWidth(plainFullLine) <= columns;
  const companyText = theme.muted(companySuffix);
  if (rich) {
    if (fitsOnOneLine) {
      if (!tagline) {
        return `${theme.heading(title)} ${theme.info(version)} ${theme.muted(`(${commitLabel})`)} ${companyText}`;
      }
      return `${theme.heading(title)} ${theme.info(version)} ${theme.muted(
        `(${commitLabel})`,
      )} ${companyText} ${theme.muted("—")} ${theme.accentDim(tagline)}`;
    }
...
function formatCliBannerArtLines(options: BannerOptions): string[] {
  ...
  const emojiOptions = resolveEmojiOptions(options);
  const title = supportsDecorativeEmoji(emojiOptions) ? "🦞 MERCLAW 🦞" : "MERCLAW";
...
export function formatCliBannerArt(options: BannerOptions = {}): string {
  ...
  const colored = lines.map((line) => {
    if (line.includes("MERCLAW")) {
      if (!icon) {
        return theme.info(centerText("MERCLAW", visibleWidth(line)));
      }
      return (
        theme.muted("              ") +
        theme.accent(icon) +
