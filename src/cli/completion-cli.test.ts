...
describe("completion-cli", () => {
  it("generates zsh functions for nested subcommands", () => {
    ...
    expect(script).toContain("_merclaw_gateway()");
    expect(script).toContain("(status) _merclaw_gateway_status ;;");
    expect(script).toContain("(restart) _merclaw_gateway_restart ;;");
  ...
  it("defers zsh registration until compinit is available", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-zsh-completion-"));
    try {
      const scriptPath = path.join(tempDir, "merclaw.zsh");
      ...
      const result = spawnSync(
        ...
        [
          ...
          `
            ...
            [[ -z "\${_comps[merclaw]-}" ]] || exit 10
            [[ "\${precmd_functions[(r)_merclaw_register_completion]}" = "_merclaw_register_completion" ]] || exit 11
            autoload -Uz compinit
            compinit -C
            _merclaw_register_completion
            [[ -z "\${precmd_functions[(r)_merclaw_register_completion]}" ]] || exit 12
            [[ "\${_comps[merclaw]-}" = "_merclaw_root_completion" ]]
          `,
        ],
        {
          encoding: "utf8",
  ...
  it("generates PowerShell command paths without the executable prefix", () => {
    ...
    expect(script).not.toContain("if ($commandPath -eq 'merclaw gateway') {");
  ...
  it("generates valid PowerShell root arrays when commands or options are empty", () => {
    const commandsOnly = new Command().name("merclaw");
    commandsOnly.command("status");
    const optionsOnly = new Command().name("merclaw").option("--json", "JSON output");
    const empty = new Command().name("merclaw");
  ...
  it("generates fish completions for root and nested command contexts", () => {
    ...
    expect(script).toContain(
      'complete -c merclaw -n "__fish_use_subcommand" -a "gateway" -d \'Gateway commands\'',
    );
    expect(script).toContain(
      'complete -c merclaw -n "__merclaw_command_path_matches gateway -- -t --token" -a "status" -d \'Show gateway status\'',
    );
    expect(script).toContain(
      "complete -c merclaw -n \"__merclaw_command_path_matches gateway -- -t --token\" -l force -d 'Force the action'",
    );
    expect(script).toContain(
      "complete -c merclaw -n \"__merclaw_command_path_matches gateway status -- -t --token\" -l json -d 'JSON output'",
    );
    expect(script).toContain("__merclaw_command_path_matches gateway -- -t --token");
  ...
  it("scopes fish value-taking option skips to the active command path", () => {
    ...
    expect(script).toContain("__merclaw_command_path_matches agent -- --verbose");
    expect(script).toContain("__merclaw_command_path_matches sessions cleanup --");
    expect(script).not.toContain("__merclaw_command_path_matches sessions cleanup -- --verbose");
    expect(script).toContain(
      "complete -c merclaw -n \"__merclaw_command_path_matches sessions cleanup --\" -l dry-run -d 'Preview cleanup'",
    );
  });
