...
export const loadConfig: Mock<LoadConfigFn> = vi.fn<LoadConfigFn>(() => ({}) as MerClawConfig);
...
export const replaceConfigFile: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: MerClawConfig }) => await writeConfigFile(params.nextConfig),
) as AsyncUnknownMock;
const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/merclaw-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: ((config: MerClawConfig) =>
    invokeMock<
      [MerClawConfig],
...
export function resetPluginsCliTestState() {
  ...
  applyExclusiveSlotSelection.mockImplementation((({ config }: { config: MerClawConfig }) => ({
    config,
    warnings: [],
  })) as (...args: unknown[]) => unknown);
