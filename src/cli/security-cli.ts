...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["merclaw security audit", "Run a local security audit."],
          [
            "merclaw security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["merclaw security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "merclaw security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          [
            "merclaw security audit --auth password --password <password>",
            "Audit a runtime-only password-mode Gateway secret.",
          ],
          ["merclaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["merclaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.merclaw.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("MerClaw security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("merclaw security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("merclaw security audit --fix")}`));
        if (!fixResult) {
