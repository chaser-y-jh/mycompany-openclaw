...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: MerClawConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: MerClawConfig }>
    >(async () => ({
      path: "/tmp/merclaw.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: MerClawConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/merclaw.json");
    ...
    expect(output).not.toContain("/tmp/merclaw.json\nforged");
