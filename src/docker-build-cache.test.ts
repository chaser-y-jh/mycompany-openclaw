...
describe("docker build cache layout", () => {
  ...
  it("uses pnpm cache mounts in Dockerfiles that install repo dependencies", async () => {
    for (const path of [
      ...
      expect(
        ...
        /--mount=type=cache,id=merclaw-pnpm-store,target=\/(?:root|home\/appuser)\/\.local\/share\/pnpm\/store,sharing=locked/,
      );
    }
  });
  ...
  it("keeps the shared e2e image on the packaged tarball install path", async () => {
    ...
    expect(dockerfile).toMatch(
      /^COPY --from=merclaw_package --chown=appuser:appuser merclaw-current\.tgz \/tmp\/merclaw-current\.tgz$/m,
    );
    expect(dockerfile).toContain(
      "npm install -g --prefix /tmp/merclaw-prefix /tmp/merclaw-current.tgz --no-fund --no-audit",
    );
    expect(dockerfile).not.toContain(
      "cp -a /tmp/merclaw-prefix/lib/node_modules/. /app/node_modules/",
    );
    expect(dockerfile).toContain("cp -a /tmp/merclaw-prefix/lib/node_modules/merclaw/. /app/");
    expect(dockerfile).toContain("rm -rf /app/node_modules/merclaw");
    expect(dockerfile).toContain("ln -sf /app /app/node_modules/merclaw");
  });
