...
type CollectDangerousConfigFlags = (cfg: MerClawConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.MERCLAW_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.MERCLAW_GATEWAY_PASSWORD);
