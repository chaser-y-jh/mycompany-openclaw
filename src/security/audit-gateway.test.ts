...
describe("security audit gateway config findings", () => {
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    await Promise.all([
      ...
      (async () => {
        ...
        const resolvedConfig: MerClawConfig = {
          gateway: {
            bind: "lan",
            auth: {},
          },
  ...
  it("warns when MERCLAW_GATEWAY_TOKEN shadows a different configured token source", () => {
    const cfg: MerClawConfig = {
  ...
  it("does not warn when gateway.auth.token resolves from MERCLAW_GATEWAY_TOKEN", () => {
    const cfg: MerClawConfig = {
