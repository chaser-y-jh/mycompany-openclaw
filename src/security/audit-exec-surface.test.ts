...
describe("security audit exec surface findings", () => {
  // Redirect the MerClaw home (MERCLAW_HOME wins over HOME/USERPROFILE in
  // `resolveRawHomeDir`) to a per-test tempdir so `saveExecApprovals` never
  // touches the real `~/.merclaw/exec-approvals.json` on the host running
  // the suite.
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".merclaw"), { recursive: true });
    // MERCLAW_HOME takes precedence over HOME/USERPROFILE in resolveRawHomeDir,
    // so all three must point at the tempdir to neutralize whichever the host
    // happens to have set.
    process.env.MERCLAW_HOME = tempDir;
  ...
  it("warns when YOLO exec overrides restrictive Claude permission mode", () => {
    ...
    expect(finding?.detail).toContain("MerClaw exec is YOLO");
  });
  ...
  it("does not warn for restrictive Claude permission mode when MerClaw exec is restrictive", () => {
    const findings = collectExecRuntimeFindings({
