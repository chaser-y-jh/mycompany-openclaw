...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("merclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("merclaw security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set MERCLAW_GATEWAY_TOKEN/MERCLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("merclaw security audit --deep")}".`,
    });
  }
  return findings;
}
