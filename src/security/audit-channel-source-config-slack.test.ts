...
function stubSlackPlugin(params: {
  resolveAccount: (cfg: MerClawConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: MerClawConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: MerClawConfig) => boolean;
}) {
...
function makeSlackHttpConfig(): MerClawConfig {
  return {
    channels: {
      slack: {
        enabled: true,
    ...
    const cases = [
      {
        ...
        plugin: (sourceConfig: MerClawConfig) =>
          stubSlackPlugin({
            inspectAccount: (cfg) => {
