...
describe("appendAssistantMessageToSessionTranscript", () => {
  ...
  it("emits transcript update events for delivery mirrors", async () => {
    ...
    expect(message?.provider).toBe("merclaw");
  ...
  it("skips transcript-only MerClaw assistant entries when reading latest assistant text", async () => {
    writeTranscriptStore();
  ...
  it("does not report transcript-only MerClaw assistant entries as latest assistant text", async () => {
    writeTranscriptStore();
  ...
  it("keeps transcript-only MerClaw assistant entries available to the tail reader", async () => {
    writeTranscriptStore();
  ...
  it("scans past trailing non-assistant entries (e.g. merclaw.cache-ttl) to find the latest assistant text", async () => {
    // Regression for merclaw/merclaw#83427: the cache-ttl custom entry was
  ...
  it("does not reuse an older matching assistant message across turns", async () => {
    ...
    if (olderResult.ok && latestResult.ok && mirrorResult.ok) {
      ...
      expect(messageLine.message.provider).toBe("merclaw");
