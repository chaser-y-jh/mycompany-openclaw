...
// Keep integration tests deterministic: never read a real merclaw.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "merclaw-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.MERCLAW_SESSION_CACHE_TTL_MS;
    process.env.MERCLAW_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  });
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.MERCLAW_SESSION_CACHE_TTL_MS;
    } else {
      process.env.MERCLAW_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
  ...
  it("removes trajectory sidecars for stale sessions pruned on write", async () => {
    ...
    await fs.writeFile(staleTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(freshTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(staleRuntime, '{"traceSchema":"merclaw-trajectory"}\n', "utf-8");
    await fs.writeFile(freshRuntime, '{"traceSchema":"merclaw-trajectory"}\n', "utf-8");
    await fs.writeFile(
      stalePointer,
      JSON.stringify({
        traceSchema: "merclaw-trajectory-pointer",
        schemaVersion: 1,
        sessionId: staleSessionId,
