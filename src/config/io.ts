...
import { collectManifestModelIdNormalizationPolicies } from "@merclaw/model-catalog-core/provider-model-id-normalization";
...
import type { MerClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
export type ConfigWriteResult = { persistedHash: string; persistedConfig: MerClawConfig };
...
export type ConfigWriteOptions = {
   ...
   */
  explicitSetValueSource?: MerClawConfig;
  /**
   ...
   */
  preCommitRuntimePreflight?: (sourceConfig: MerClawConfig) => Promise<unknown>;
};
...
function coerceConfig(value: unknown): MerClawConfig {
...
function stampConfigVersion(cfg: MerClawConfig, version?: string): MerClawConfig {
  return stampConfigWriteMetadata(cfg, new Date().toISOString(), version);
}

function warnIfConfigFromFuture(cfg: MerClawConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    ...
    logger.warn(
      [
        `Your MerClaw config was written by version ${touched}, but this command is running ${VERSION}.`,
        "Check: `merclaw --version`, `which merclaw`, and `merclaw gateway status --deep`.",
        "If unexpected, update PATH so `merclaw` points to the version you want, or reinstall the Gateway service from that same MerClaw install.",
      ].join("\n"),
    );
  }
}
...
function shouldSuppressFutureVersionWarningForEnv(env: NodeJS.ProcessEnv): boolean {
  return (
    isTruthyEnvValue(env.MERCLAW_UPDATE_IN_PROGRESS) ||
    isTruthyEnvValue(env.MERCLAW_UPDATE_POST_CORE)
  );
}
...
export function parseConfigJson5(
  ...
  try {
    ...
    // on the JSON files MerClaw writes itself.
  }
  try {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as MerClawConfig, env);
  }
...
export function createConfigIO(
  ...
  function finalizeLoadedRuntimeConfig(cfg: MerClawConfig): MerClawConfig {
  ...
  function resolveRuntimePreflightSourceConfig(candidate: MerClawConfig): MerClawConfig {
  ...
  function ensureShippedPluginInstallConfigRecordsMigratedForWrite(
    ...
    try {
      ...
      throw new Error(
        `Config write blocked: shipped plugins.installs records in ${configPath} could not be migrated into the plugin index. Fix state directory permissions or run merclaw plugins registry --refresh, then retry. ${formatErrorMessage(
          err,
        )}`,
        { cause: err },
      );
  ...
  function loadConfig(): MerClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(
        validationConfigRaw as MerClawConfig,
        {
          env: deps.env,
          homedir: deps.homedir,
        },
      ...
      const loadValidationPluginMetadataSnapshot = (config: MerClawConfig) => {
  ...
  async function readConfigFileSnapshotInternal(): Promise<ReadConfigFileSnapshotInternalResult> {
    ...
    let fallbackSourceConfig: MerClawConfig = {};
  ...
  async function readBestEffortConfig(): Promise<MerClawConfig> {
    const result = await readConfigFileSnapshotInternal();
  ...
  async function readSourceConfigBestEffort(): Promise<MerClawConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFile(
    ...
    persistCandidate = applyUnsetPathsForWrite(persistCandidate as MerClawConfig, unsetPaths);
    ...
    let cfgToWrite = persistCandidate as MerClawConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as MerClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.MERCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      if (!isVerbose() && deps.env.MERCLAW_CONFIG_OVERWRITE_LOG !== "1" && !shouldLogInVitest) {
        return;
      }
      deps.logger.warn(
        formatConfigOverwriteLogMessage({
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.MERCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      const shouldLogBenignMissingMeta =
        isVerbose() || deps.env.MERCLAW_CONFIG_WRITE_ANOMALY_LOG === "1" || shouldLogInVitest;
    ...
    const preCommitRuntimePreflight =
      ...
      (async (sourceConfig: MerClawConfig) => {
...
// module scope. `MERCLAW_CONFIG_PATH` (and friends) are expected to work even
...
export function projectConfigOntoRuntimeSourceSnapshot(config: MerClawConfig): MerClawConfig {
...
export function loadConfig(options?: {
  ...
  // reparsing merclaw.json on hot code paths.
...
export async function readSourceConfigBestEffort(): Promise<MerClawConfig> {
...
export async function writeConfigFile(
  ...
  let canonicalSourceConfig: MerClawConfig = nextCfg;
