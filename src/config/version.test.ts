...
import {
  compareMerClawVersions,
  isSameMerClawStableFamily,
  parseMerClawVersion,
  shouldWarnOnTouchedVersion,
} from "./version.js";

describe("parseMerClawVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseMerClawVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseMerClawVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseMerClawVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseMerClawVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseMerClawVersion("2026.3")).toBeNull();
    expect(parseMerClawVersion("latest")).toBeNull();
  });
});

describe("compareMerClawVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareMerClawVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareMerClawVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareMerClawVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareMerClawVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareMerClawVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareMerClawVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameMerClawStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameMerClawStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameMerClawStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameMerClawStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameMerClawStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
