...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ MERCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-merclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-merclaw#general");
  });
  ...
  it("derives session transcripts dir from MERCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { MERCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when MERCLAW_STATE_DIR differs from stored paths", () => {
