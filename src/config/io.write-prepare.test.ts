...
describe("config io write prepare", () => {
        ...
        sourceConfig: {
          plugins: {
            ...
            installs: {
              "merclaw-web-search": {
                source: "npm",
                spec: "@ollama/merclaw-web-search",
                installPath: "/tmp/merclaw-web-search",
                resolvedName: "@ollama/merclaw-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "merclaw-web-search": {
                source: "npm",
                spec: "@ollama/merclaw-web-search@0.2.2",
                installPath: "/tmp/merclaw-web-search",
                resolvedName: "@ollama/merclaw-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("merclaw config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('merclaw config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/merclaw.json",
        expectedConfigPath: "/tmp/merclaw.json",
  ...
  it("keeps runtime-only channel defaults out of the persisted candidate", () => {
    ...
    const nextConfig: MerClawConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://merclaw.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
