...
const ENV_CATALOG_PATHS = ["MERCLAW_PLUGIN_CATALOG_PATHS", "MERCLAW_MPM_CATALOG_PATHS"];
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.merclaw) || !isRecord(entry.merclaw.channel)) {
      continue;
    }
    const channel = entry.merclaw.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: MerClawConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: MerClawConfig, pluginId: string) => boolean;
