...
function collectConfigEnvVarsByTarget(cfg?: MerClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: MerClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: MerClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.MERCLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
