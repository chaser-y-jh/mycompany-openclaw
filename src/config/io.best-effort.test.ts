...
import { withTempHome, writeMerClawConfig } from "./test-helpers.js";
...
describe("readBestEffortConfig", () => {
  it("can read snapshots without updating config observation state", async () => {
    await withTempHome(async (home) => {
      ...
      const healthPath = `${home}/.merclaw/logs/config-health.json`;
  ...
  it("does not restore suspicious direct edits from .bak during ordinary reads", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeMerClawConfig(home, {
      ...
      const entries = await fs.readdir(`${home}/.merclaw`);
      expect(entries.some((entry) => entry.startsWith("merclaw.json.clobbered."))).toBe(false);
    });
  });
