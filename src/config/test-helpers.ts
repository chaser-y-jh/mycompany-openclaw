...
import { resetConfigRuntimeState, type MerClawConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        MERCLAW_MPM_CATALOG_PATHS: undefined,
        MERCLAW_LOAD_SHELL_ENV: undefined,
        MERCLAW_DEFER_SHELL_ENV_FALLBACK: undefined,
        MERCLAW_SHELL_ENV_TIMEOUT_MS: undefined,
...
export async function writeMerClawConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.MERCLAW_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected MERCLAW_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeMerClawConfig(home, config);
