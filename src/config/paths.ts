...
/**
 * Nix mode detection: When MERCLAW_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
const NEW_STATE_DIRNAME = ".merclaw";
const CONFIG_FILENAME = "merclaw.json";
...
/** Build a homedir thunk that respects MERCLAW_HOME for the given env. */
 ...
 * Can be overridden via MERCLAW_STATE_DIR.
 * Default: ~/.merclaw
 */
...
export function normalizeStateDirEnv(env: NodeJS.ProcessEnv = process.env): void {
  ...
  const merclawOverride = env.MERCLAW_STATE_DIR?.trim();
  if (merclawOverride) {
    env.MERCLAW_STATE_DIR = resolveUserPath(merclawOverride, env, effectiveHomedir);
  }
}
 ...
 * outside the config directory. Set via `MERCLAW_INCLUDE_ROOTS` as a
 ...
 * directory containing `merclaw.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.MERCLAW_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via MERCLAW_CONFIG_PATH.
 * Default: ~/.merclaw/merclaw.json (or $MERCLAW_STATE_DIR/merclaw.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.MERCLAW_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const stateOverride = env.MERCLAW_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.MERCLAW_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const merclawStateDir = env.MERCLAW_STATE_DIR?.trim();
  if (merclawStateDir) {
    const resolved = resolveUserPath(merclawStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/merclaw-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `merclaw-${uid}` : "merclaw";
 ...
 *
 * Precedence:
 * - `MERCLAW_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.MERCLAW_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.MERCLAW_GATEWAY_PORT?.trim();
