...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: MerClawConfig): boolean {
...
function isProviderConfigured(cfg: MerClawConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: MerClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: MerClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: MerClawConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: MerClawConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: MerClawConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: MerClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: MerClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: MerClawConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: MerClawConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: MerClawConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: MerClawConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: MerClawConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: MerClawConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: MerClawConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: MerClawConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: MerClawConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: MerClawConfig): boolean {
  return (
...
function hasPluginEntries(cfg: MerClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: MerClawConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: MerClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: MerClawConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: MerClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: MerClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: MerClawConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: MerClawConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: MerClawConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
