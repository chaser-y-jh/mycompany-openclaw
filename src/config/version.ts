...
export function parseMerClawVersion(raw: string | null | undefined): MerClawVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeMerClawVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseMerClawVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameMerClawStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseMerClawVersion(a);
  const parsedB = parseMerClawVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareMerClawVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseMerClawVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseMerClawVersion(current);
  const parsedTouched = parseMerClawVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameMerClawStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareMerClawVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: MerClawVersion): number {
