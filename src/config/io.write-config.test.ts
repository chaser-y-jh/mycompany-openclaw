...
import type { ConfigFileSnapshot, MerClawConfig } from "./types.merclaw.js";
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "merclaw-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { MERCLAW_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("loads shipped plugin install config records without mutating config or plugin index", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".merclaw", "merclaw.json");
      const pluginDir = path.join(home, ".merclaw", "plugins", "demo");
      const manifestPath = path.join(pluginDir, "merclaw.plugin.json");
      ...
      try {
        ...
        await expect(
          readPersistedInstalledPluginIndex({
            stateDir: path.join(home, ".merclaw"),
          }),
        ).resolves.toBeNull();
        await expect(fs.readFile(configPath, "utf-8")).resolves.toBe(initialRaw);
      } finally {
  ...
  it("migrates shipped plugin install config records during explicit writes even when the manifest is missing", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".merclaw", "merclaw.json");
      const pluginDir = path.join(home, ".merclaw", "plugins", "missing");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps shipped plugin install config records when index migration fails", async () => {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".merclaw", "merclaw.json");
      const unwritableStatePath = path.join(home, ".merclaw");
  ...
  it("keeps writes inside an MERCLAW_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".merclaw", "merclaw.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "merclaw.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "merclaw.json"), "utf-8"),
      ) as {
  ...
  it("prints missing-meta write anomalies when anomaly logging is requested", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          MERCLAW_CONFIG_WRITE_ANOMALY_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("suppresses overwrite audit output when skipOutputLogs is set", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          VITEST: "true",
          MERCLAW_TEST_CONFIG_OVERWRITE_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("preserves root $schema during partial writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      expect(persisted.$schema).toBe("https://merclaw.ai/config.json");
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/merclaw-main" }] },
  ...
  it("keeps authored agent provider params during narrowed internal agent writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as MerClawConfig;
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".merclaw", "merclaw.json");
      const includePath = path.join(home, ".merclaw", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://merclaw.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/merclaw-test-required-plugin",
          source: "/tmp/merclaw-test-required-plugin/index.ts",
          manifestPath: "/tmp/merclaw-test-required-plugin/merclaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
    ...
    await withSuiteHome(async (home) => {
      ...
      try {
        ...
        if (previousConfigPath === undefined) {
          ...
          process.env.MERCLAW_CONFIG_PATH = previousConfigPath;
        }
      }
    });
  });
  ...
  it("notifies in-process reloaders with resolved source config when persisted env refs are restored", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".merclaw", "merclaw.json");
      const previousConfigPath = process.env.MERCLAW_CONFIG_PATH;
      const previousGatewayToken = process.env.MERCLAW_GATEWAY_TOKEN;
      process.env.MERCLAW_CONFIG_PATH = configPath;
      process.env.MERCLAW_GATEWAY_TOKEN = "gateway-token-runtime";
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
          {
            gateway: {
              mode: "local",
              auth: { mode: "token", token: "${MERCLAW_GATEWAY_TOKEN}" },
            },
      ...
      try {
        ...
        expect(persisted.gateway?.auth?.token).toBe("${MERCLAW_GATEWAY_TOKEN}");
        ...
        if (previousGatewayToken === undefined) {
          ...
          process.env.MERCLAW_GATEWAY_TOKEN = previousGatewayToken;
        }
      }
    });
  });
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/merclaw-test-demo",
          source: "/tmp/merclaw-test-demo/index.ts",
          manifestPath: "/tmp/merclaw-test-demo/merclaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
    ...
    await withSuiteHome(async (home) => {
      ...
      try {
        ...
        const postWriteSnapshot = await createConfigIO({
          env: { MERCLAW_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
          homedir: () => home,
          logger: silentLogger,
  ...
  it("rolls back the root config when post-write runtime refresh fails", async () => {
    await withSuiteHome(async (home) => {
      ...
      process.env.MERCLAW_CONFIG_PATH = configPath;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies MerClawConfig;
  ...
  it("blocks runtime preflight failures before committing root writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      const previousConfigPath = process.env.MERCLAW_CONFIG_PATH;
      const initialRaw = `${JSON.stringify({ gateway: { mode: "local" } }, null, 2)}\n`;
      let observedSource: MerClawConfig | undefined;
  ...
  it("restores config env vars when post-write runtime refresh rollback succeeds", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".merclaw", "merclaw.json");
      const previousConfigPath = process.env.MERCLAW_CONFIG_PATH;
      const envKey = "MERCLAW_TEST_RUNTIME_ROLLBACK_ENV";
  ...
  it("skipPluginValidation bypasses plugin schema rejection on writeConfigFile (#76800)", async () => {
    await withSuiteHome(async (home) => {
        ...
        plugins: [
          {
            ...
            skills: [],
            hooks: [],
            rootDir: "/tmp/merclaw-test-strict-plugin",
            source: "/tmp/merclaw-test-strict-plugin/index.ts",
            manifestPath: "/tmp/merclaw-test-strict-plugin/merclaw.plugin.json",
            configSchema: {
              type: "object",
      ...
      try {
        ...
        await expect(
          writeConfigFile({ agents: { list: "not-array" } } as unknown as MerClawConfig, {
  ...
  it("preserves authored tilde paths when runtime-shaped writes hand back absolute paths", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            logging: { file: "~/merclaw-upgrade-survivor/gateway.jsonl" },
          },
          null,
          2,
        )}\n`,
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "merclaw-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as MerClawConfig;
      expect(persisted.logging?.file).toBe("~/merclaw-upgrade-survivor/gateway.jsonl");
