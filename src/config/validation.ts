...
import { isCanonicalDottedDecimalIPv4, isLoopbackIpAddress } from "@merclaw/net-policy/ip";
...
import type { MerClawConfig, ConfigValidationIssue } from "./types.js";
...
function materializeBundledModelProviderOverlays(config: MerClawConfig): MerClawConfig {
...
const SECRETREF_POLICY_DOC_URL = "https://docs.merclaw.ai/reference/secretref-credential-surface";
...
function formatMissingOfficialExternalPluginWarning(
  ...
  if (pluginId === "memory-lancedb" && opts?.selectedMissingMemorySlot) {
    return `plugin not installed: ${pluginId} — gateway will run without persistent memory until installed; install the official external plugin with: merclaw plugins install ${installSpec}`;
  }
  return `plugin not installed: ${pluginId} — install the official external plugin with: merclaw plugins install ${installSpec}`;
}
...
function collectRawBundledChannelConfigIssues(config: MerClawConfig): ConfigValidationIssue[] {
...
function validateIdentityAvatar(config: MerClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: MerClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleAuth(config: MerClawConfig): ConfigValidationIssue[] {
  const tailscaleMode = config.gateway?.tailscale?.mode ?? "off";
...
export function validateConfigObjectRaw(
...
): { ok: true; config: MerClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = MerClawSchema.safeParse(normalizedRaw);
  ...
  const validatedConfig = materializeBundledModelProviderOverlays(validated.data as MerClawConfig);
  const channelIssues =
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: MerClawConfig | null | undefined;
  ...
  const ensureCompatConfig = (): MerClawConfig => {
  ...
  const validateWebSearchProvider = () => {
    ...
    if (installCatalogEntry) {
      const issue = {
        path,
        message: `web_search provider is not available: ${trimmed} (install or enable plugin "${installCatalogEntry.pluginId}", then run merclaw doctor --fix)`,
      ...
      if (hasPluginEvidenceForWebSearchProvider(trimmed, installCatalogEntry.pluginId)) {
        warnings.push({
          ...issue,
          message: `web_search provider is not available: ${trimmed} (configured plugin "${installCatalogEntry.pluginId}" is unavailable; Gateway will ignore this optional provider until the plugin is installed/enabled or merclaw doctor --fix repairs the config)`,
        });
        return;
      }
      issues.push(issue);
    ...
    if (hasStalePluginEvidenceForUnknownWebSearchProvider(trimmed)) {
      warnings.push({
        ...issue,
        message: `${issue.message} (stale web search plugin config ignored; run merclaw doctor --fix to remove stale config, or install the plugin)`,
      });
      return;
    }
    issues.push(issue);
    ...
    for (const key of Object.keys(config.channels)) {
      ...
      if (!allowedChannels.has(trimmed)) {
        ...
        if (hasStalePluginEvidenceForUnknownChannel(trimmed)) {
          warnings.push({
            ...issue,
            message: `${issue.message} (stale channel plugin config ignored; run merclaw doctor --fix to remove stale config, or install the plugin)`,
          });
        } else {
          issues.push(issue);
        }
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native MerClaw config schema.
