...
vi.mock("./loader.js", () => ({
  ...
  loadMerClawPlugins: mocks.loadMerClawPlugins,
...
describe("web-provider-runtime-shared", () => {
  ...
  it("caches setup web provider plugin loads by default", () => {
    ...
    const mapRegistryProviders = vi.fn(() => ["provider"]);
    mocks.loadMerClawPlugins.mockReturnValue(loadedRegistry as never);
    ...
    expect(providers).toEqual(["provider"]);
    expect(mockArg(mocks.loadMerClawPlugins).cache).toBe(true);
    expect(mockArg(mocks.loadMerClawPlugins).onlyPluginIds).toEqual(["brave"]);
  });
  ...
  it("falls back to a scoped provider load when the active runtime registry has no web providers", () => {
    ...
    mocks.loadMerClawPlugins.mockReturnValue(fallbackRegistry as never);
  ...
  it("keeps explicit setup web provider cache opt-outs", () => {
    ...
    expect(mockArg(mocks.loadMerClawPlugins).cache).toBe(false);
    expect(mockArg(mocks.loadMerClawPlugins).onlyPluginIds).toEqual(["brave"]);
  });
});
