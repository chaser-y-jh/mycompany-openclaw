...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/merclaw-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "merclaw.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): MerClawConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    MERCLAW_HOME: path.join(homeDir, "home"),
    MERCLAW_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
