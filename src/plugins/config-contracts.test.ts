...
const mocks = vi.hoisted(() => {
  ...
  return {
    discoverMerClawPlugins: vi.fn(() => ({ candidates: [], diagnostics: [] })),
...
vi.mock("./discovery.js", () => ({
  discoverMerClawPlugins: mocks.discoverMerClawPlugins,
}));
...
function createPluginRecord(
  ...
  return {
    ...
    hooks: [],
    source: `/tmp/${overrides.id}/merclaw.plugin.json`,
...
describe("resolvePluginConfigContractsById", () => {
  beforeEach(() => {
    mocks.discoverMerClawPlugins.mockReset();
    mocks.discoverMerClawPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
