...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".merclaw", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "MERCLAW_HOME", "MERCLAW_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("merclaw-bundle-commands-home-");
      const workspaceDir = await createTempDir("merclaw-bundle-commands-workspace-");
