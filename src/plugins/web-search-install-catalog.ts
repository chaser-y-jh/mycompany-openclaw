...
function getConfigPath(config: MerClawConfig | undefined, path: string): unknown {
...
function setConfigPath(target: MerClawConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: MerClawConfig) =>
      getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: MerClawConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: MerClawConfig) =>
