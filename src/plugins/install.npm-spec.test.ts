...
const resolveMerClawPackageRootSyncMock = vi.fn();
...
vi.mock("../infra/merclaw-root.js", () => ({
  resolveMerClawPackageRootSync: (...args: unknown[]) =>
    resolveMerClawPackageRootSyncMock(...args),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("merclaw-plugin-install-npm-spec");
...
function writeInstalledNpmPlugin(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      merclaw: params.merclaw ?? { extensions: ["./dist/index.js"] },
      ...(params.dependency
...
type MockNpmPackage = {
  ...
  materializesRootMerClaw?: boolean;
...
function writeNpmRootPackageLock(params: {
  ...
  for (const pkg of params.packages) {
    ...
    if (pkg.materializesRootMerClaw) {
      lockPackages["node_modules/merclaw"] = {
        peer: true,
        version: "2026.5.3",
      };
    }
...
function prunePluginLocalMerClawPeerLinks(npmRoot: string) {
  ...
  for (const entry of fs.readdirSync(nodeModulesDir, { withFileTypes: true })) {
    ...
    for (const packageDir of packageDirs) {
      ...
      fs.rmSync(path.join(packageNodeModulesDir, "merclaw"), {
        recursive: true,
        force: true,
      });
    }
  ...
  runCommandWithTimeoutMock.mockImplementation(
    async (argv: string[], options?: { cwd?: string }) => {
      ...
      if (viewPackage) {
        return successfulSpawn(
          JSON.stringify({
            ...
            ...(viewPackage.merclaw ? { merclaw: viewPackage.merclaw } : {}),
          }),
        );
      }
      ...
      if (isManagedNpmInstallCommand(argv)) {
        ...
        prunePluginLocalMerClawPeerLinks(npmRoot);
        for (const packageName of Object.keys(manifest.dependencies ?? {})) {
          if (packageName === "merclaw") {
            const merclawRoot = path.join(npmRoot, "node_modules", "merclaw");
            fs.mkdirSync(merclawRoot, { recursive: true });
            fs.writeFileSync(
              path.join(merclawRoot, "package.json"),
              JSON.stringify({ name: "merclaw", version: "0.0.0-test" }),
              "utf8",
            );
            continue;
          }
          ...
          if (pkg.materializesRootMerClaw) {
            ...
            fs.writeFileSync(
              path.join(merclawRoot, "package.json"),
              JSON.stringify({ name: "merclaw", version: "2026.5.3" }),
              "utf8",
            );
          }
      ...
      if (argv[0] === "npm" && argv[1] === "uninstall") {
        ...
        if (packageName === "merclaw") {
          ...
          fs.rmSync(path.join(npmRoot, "node_modules", "merclaw"), {
            recursive: true,
            force: true,
          });
          return successfulSpawn();
...
beforeEach(() => {
  ...
  resolveMerClawPackageRootSyncMock.mockReset();
  const hostRoot = suiteTempRootTracker.makeTempDir();
  fs.writeFileSync(
    path.join(hostRoot, "package.json"),
    `${JSON.stringify({ name: "merclaw", version: "0.0.0-test" }, null, 2)}\n`,
    "utf8",
  );
  resolveMerClawPackageRootSyncMock.mockReturnValue(hostRoot);
  vi.unstubAllEnvs();
  process.env.NPM_CONFIG_GLOBALCONFIG = npmGlobalConfigPath;
});
...
beforeAll(async () => {
  ...
  const archivePath = path.join(stateDir, "merclaw-pack-demo-1.2.3.tgz");
  fs.writeFileSync(archivePath, "fixture pack contents", "utf8");

  mockNpmViewAndInstallMany([
    {
      packageName: "@merclaw/pack-demo",
      version: "1.2.3",
      pluginId: "pack-demo",
      npmRoot,
  ...
  const dependencySpec = managedManifest.dependencies?.["@merclaw/pack-demo"];
...
describe("installPluginFromNpmSpec", () => {
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@merclaw/pack-demo"));
    expect(result.npmResolution?.resolvedSpec).toBe("@merclaw/pack-demo@1.2.3");
    expect(result.npmResolution?.integrity).toBe("sha512-pack-demo");
    expect(result.npmTarballName).toBe("merclaw-pack-demo-1.2.3.tgz");
    expectNpmInstallIntoProject({
      calls,
      npmRoot,
      packageName: "@merclaw/pack-demo",
    });
    expect(dependencySpec).toMatch(/^file:\.\/_merclaw-pack-archives\/.+\.tgz$/);
  ...
  it("rejects npm pack archive metadata with traversal package names", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "_merclaw-pack-archives"))).toBe(false);
    expect(runCommandWithTimeoutMock.mock.calls).toHaveLength(1);
  });

  it("installs npm plugins into .merclaw/npm", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@merclaw/voice-call"));
    expect(result.npmResolution?.resolvedSpec).toBe("@merclaw/voice-call@0.0.1");
  ...
  it.runIf(process.platform !== "win32")(
    "does not let managed merclaw peer links poison later npm installs",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      mockNpmViewAndInstallMany([
        {
          ...
          npmRoot,
          peerDependencies: { merclaw: "^2026.0.0" },
        },
        {
      ...
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "merclaw")).isSymbolicLink(),
      ).toBe(true);
      ...
      if (!second.ok) {
        expect(second.error).not.toContain("peer-plugin/node_modules/merclaw");
      }
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "merclaw")).isSymbolicLink(),
      ).toBe(true);
  ...
  it("rejects managed npm plugins when their merclaw peer link cannot be repaired", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const warnings: string[] = [];

    resolveMerClawPackageRootSyncMock.mockReturnValue(null);
    mockNpmViewAndInstall({
      spec: "@merclaw/codex@2026.5.7",
      packageName: "@merclaw/codex",
      version: "2026.5.7",
      pluginId: "@merclaw/codex",
      npmRoot,
      peerDependencies: { merclaw: ">=2026.5.7" },
    });
    ...
    expect(result.error).toContain("@merclaw/codex");
    expect(result.error).toContain("plugin-local node_modules/merclaw link");
    expect(
      warnings.some((warning) => warning.includes("Could not locate merclaw package root")),
    ).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@merclaw/codex"))).toBe(false);
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@merclaw/codex",
    });
    ...
    expect(managedManifest.dependencies?.["@merclaw/codex"]).toBeUndefined();
  });

  it("rejects exact npm plugins whose package compatibility requires a newer host", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    vi.stubEnv("MERCLAW_COMPATIBILITY_HOST_VERSION", "2026.5.10-beta.1");

    mockNpmViewAndInstall({
      spec: "@merclaw/whatsapp@2026.5.27",
      packageName: "@merclaw/whatsapp",
      version: "2026.5.27",
      pluginId: "whatsapp",
      npmRoot,
      peerDependencies: { merclaw: ">=2026.5.27" },
      merclaw: {
    ...
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@merclaw", "whatsapp"))).toBe(false);
    expect(fs.existsSync(path.join(npmRoot, "package.json"))).toBe(false);
    expect(
  ...
  it("installs the newest compatible npm version for unpinned plugins", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@merclaw/whatsapp@2026.5.26",
        packageName: "@merclaw/whatsapp",
        version: "2026.5.26",
        pluginId: "whatsapp",
        npmRoot,
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@merclaw/whatsapp@2026.5.26");
    expect(result.npmResolution?.version).toBe("2026.5.26");
    expect(warnings.join("\n")).toContain("using newest compatible @merclaw/whatsapp@2026.5.26");
    expect(
      JSON.parse(
        fs.readFileSync(
          path.join(resolveTestPluginPackageDir(npmRoot, "@merclaw/whatsapp"), "package.json"),
          "utf8",
        ),
      ).version,
    ).toBe("2026.5.26");
  ...
  it("preserves an existing npm plugin by resolving update metadata to a compatible version", async () => {
    ...
    expect(managedManifest.dependencies?.["@merclaw/whatsapp"]).toBe("2026.5.26");
    expect(
      runCommandWithTimeoutMock.mock.calls.some(([argv]) => isManagedNpmInstallCommand(argv)),
    ).toBe(true);
  });
  ...
  it.runIf(process.platform !== "win32")(
    "repairs root merclaw materialized by npm peer handling",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(fs.existsSync(path.join(npmProjectRoot, "node_modules", "merclaw"))).toBe(false);
      const lockfile = JSON.parse(
        fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
      ) as {
        packages?: Record<string, unknown>;
      };
      expect(lockfile.packages?.["node_modules/merclaw"]).toBeUndefined();
      expect(
        fs.lstatSync(path.join(requiredPeerPluginDir, "node_modules", "merclaw")).isSymbolicLink(),
      ).toBe(true);
    },
  );

  it("repairs stale managed merclaw root packages before npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
    ...
    fs.mkdirSync(path.join(npmProjectRoot, "node_modules", "merclaw"), { recursive: true });
    fs.writeFileSync(
      path.join(npmProjectRoot, "package.json"),
      JSON.stringify(
        {
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/merclaw": {
              version: "2026.5.4",
              resolved: "https://registry.npmjs.org/merclaw/-/merclaw-2026.5.4.tgz",
            },
          },
          dependencies: {
            merclaw: {
    ...
    fs.writeFileSync(
      path.join(npmProjectRoot, "node_modules", "merclaw", "package.json"),
      JSON.stringify({
        name: "merclaw",
        version: "2026.5.4",
      }),
    ...
    mockNpmViewAndInstall({
      ...
      pluginId: "discord",
      npmRoot,
      peerDependencies: { merclaw: ">=2026.5.5-beta.1" },
    ...
    expect(manifest.dependencies).not.toHaveProperty("merclaw");
    expect(manifest.dependencies?.["@merclaw/discord"]).toBe("2026.5.5-beta.1");
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
    ) as {
      packages?: Record<string, unknown>;
    ...
    expect(lockfile.packages?.["node_modules/merclaw"]).toBeUndefined();
    expect(lockfile.dependencies?.merclaw).toBeUndefined();
  });

  it("preserves the active host merclaw runtime package during npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const hostPackageRoot = path.join(npmRoot, "node_modules", "merclaw");
    ...
    resolveMerClawPackageRootSyncMock.mockReturnValue(hostPackageRoot);
    mockNpmViewAndInstall({
      spec: "@xdarkicex/merclaw-memory-libravdb@1.4.69",
      packageName: "@xdarkicex/merclaw-memory-libravdb",
      version: "1.4.69",
    ...
    expect(baseManifest.dependencies?.merclaw).toBe("2026.5.12-beta.6");
    expect(baseManifest.dependencies?.["@xdarkicex/merclaw-memory-libravdb"]).toBeUndefined();
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@xdarkicex/merclaw-memory-libravdb",
    });
    ...
    expect(projectManifest.dependencies?.["@xdarkicex/merclaw-memory-libravdb"]).toBe("1.4.69");
    expect(fs.existsSync(hostPackageRoot)).toBe(true);
    expect(result.targetDir).toBe(
      resolveTestPluginPackageDir(npmRoot, "@xdarkicex/merclaw-memory-libravdb"),
    );
    expect(
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@merclaw/voice-call@0.0.1"))) {
          return successfulSpawn(
            JSON.stringify({
              name: "@merclaw/voice-call",
              version: "0.0.1",
              dist: {
        ...
        if (argv[0] === "npm" && argv[1] === "uninstall") {
          if (!(argv as string[]).includes("--legacy-peer-deps")) {
            fs.mkdirSync(path.join(options?.cwd ?? npmRoot, "node_modules", "merclaw"), {
              recursive: true,
            });
          }
          return successfulSpawn("");
    ...
    await expect(
      fs.promises.access(path.join(npmProjectRoot, "node_modules", "merclaw")),
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (isManagedNpmInstallCommand(argv)) {
          ...
          const manifest = JSON.parse(
            fs.readFileSync(path.join(npmProjectRoot, "package.json"), "utf8"),
          ) as { overrides?: Record<string, unknown>; merclaw?: { managedOverrides?: string[] } };
          if (installAttempts === 1) {
            expect(manifest.overrides?.["node-domexception"]).toBe(
              "npm:@nolyfill/domexception@1.0.28",
            );
            expect(manifest.merclaw?.managedOverrides).toEqual([
              "axios",
              "nested",
              "node-domexception",
            ]);
          ...
          expect(manifest.merclaw?.managedOverrides).toEqual(["axios", "nested"]);
        }
  ...
  it.each(officialLaunchPluginCases)(
    ...
    async ({ spec, pluginId, indexJs }) => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes("allowed because it is an official MerClaw package"),
        ),
      ).toBe(false);
    },
  );
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = resolveTestPluginPackageDir(npmRoot, "@merclaw/voice-call");
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@merclaw/voice-call"))).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@merclaw/whatsapp"))).toBe(true);
  });
  ...
  it("rejects implicit prerelease npm specs with beta guidance", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@merclaw/voice-call@beta"');
    }
  });
  ...
  it("falls back to the latest stable version for official prerelease packages", async () => {
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@merclaw/voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @merclaw/voice-call@0.0.1");
  });
  ...
  it("keeps stable correction versions when resolving official npm packages", async () => {
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe("@merclaw/voice-call@2026.5.3-1");
  ...
  it("uses the newest prerelease when an official package has no stable versions", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@merclaw/voice-call@0.0.2-beta.1",
        packageName: "@merclaw/voice-call",
        version: "0.0.2-beta.1",
        pluginId: "voice-call",
        npmRoot: prereleaseOnlyNpmRoot,
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe("@merclaw/voice-call@0.0.2-beta.1");
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @merclaw/voice-call@0.0.2-beta.1",
    );
  });
  ...
  it("accepts explicit prerelease npm dist-tags", async () => {
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@merclaw/voice-call@0.0.2-beta.1");
    expectNpmInstallIntoProject({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
      packageName: "@merclaw/voice-call",
