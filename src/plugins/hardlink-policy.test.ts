...
const nixEnv: NodeJS.ProcessEnv = { MERCLAW_NIX_MODE: "1" };
...
describe("plugin hardlink policy", () => {
  ...
  it("does not treat MERCLAW_NIX_MODE as enough by itself", () => {
    expect(
      shouldRejectHardlinkedPluginFiles({
        origin: "config",
        rootDir: "/tmp/plugin",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      expect(isNixStorePluginRoot("/nix/store/abc-merclaw-plugin")).toBe(true);
      expect(isNixStorePluginRoot("/tmp/nix/store/abc-merclaw-plugin")).toBe(false);
      expect(
        shouldRejectHardlinkedPluginFiles({
          origin: "config",
          rootDir: "/nix/store/abc-merclaw-plugin",
          env: nixEnv,
        }),
      ).toBe(false);
    },
