...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as MerClawConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as MerClawConfig;
    const load = vi.fn(async (config?: MerClawConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: MerClawConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
