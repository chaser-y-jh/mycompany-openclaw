...
import { listMerClawPluginManifestMetadata } from "./manifest-metadata-scan.js";
...
const ORIGINAL_ENV = {
  ...
  MERCLAW_BUNDLED_PLUGINS_DIR: process.env.MERCLAW_BUNDLED_PLUGINS_DIR,
} as const;

const tempDirs: string[] = [];

function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-model-id-normalization-"));
  tempDirs.push(dir);
  return dir;
}
...
function createCurrentSnapshot(params: {
  ...
  const index: InstalledPluginIndex = {
    ...
    plugins: [
      {
        pluginId: "normalizer",
        manifestPath: `/tmp/normalizer-${params.manifestHash}/merclaw.plugin.json`,
...
describe("manifest model id normalization", () => {
  ...
  it("reuses current metadata when callers omit config", () => {
    const config: MerClawConfig = { plugins: { allow: ["normalizer"] } };
    setCurrentPluginMetadataSnapshot(
      createCurrentSnapshot({
        manifestHash: "alpha",
        prefix: "alpha",
  ...
  it("keeps process metadata stable across manifest edits and reflects state-dir changes", () => {
    ...
    process.env.MERCLAW_STATE_DIR = stateDirA;
    process.env.MERCLAW_HOME = undefined;
    process.env.MERCLAW_DISABLE_BUNDLED_PLUGINS = "1";
    process.env.MERCLAW_BUNDLED_PLUGINS_DIR = undefined;
    ...
    process.env.MERCLAW_STATE_DIR = stateDirB;
    clearPluginMetadataLifecycleCaches();
  ...
  it("reuses manifest metadata while file fingerprints are unchanged", () => {
    ...
    expect(listMerClawPluginManifestMetadata(process.env)).toHaveLength(1);
    expect(listMerClawPluginManifestMetadata(process.env)).toHaveLength(1);
