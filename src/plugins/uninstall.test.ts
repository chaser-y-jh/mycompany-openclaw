...
type PluginConfig = NonNullable<MerClawConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: MerClawConfig["channels"];
}): MerClawConfig {
...
function createSinglePluginWithEmptySlotsConfig(): MerClawConfig {
...
function createSingleNpmInstallConfig(installPath: string): MerClawConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("removePluginFromConfig", () => {
  ...
  it("removes absolute load path for a workspace-relative install source path", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tempRoot, "merclaw-uninstall-portable-source-"));
    try {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as MerClawConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
...
describe("uninstallPlugin", () => {
      ...
      config: createPluginConfig({
        installs: {
          "missing-linked-plugin": createPathInstallRecord(
            "/missing/merclaw/plugin",
            "/missing/merclaw/plugin",
          ),
        },
        loadPaths: ["/missing/merclaw/plugin", "/keep/this/plugin"],
      }),
      expectedActions: {
        entry: false,
        install: true,
  ...
  it("uninstalls npm-managed packages through npm before deleting the package directory", async () => {
    ...
    const pluginDir = path.join(npmRoot, "node_modules", "@merclaw", "kitchen-sink");
    ...
    const plan = planPluginUninstall({
      config: createPluginConfig({
        entries: createSinglePluginEntries("merclaw-kitchen-sink-fixture"),
        installs: {
          "merclaw-kitchen-sink-fixture": {
            source: "npm",
            spec: "@merclaw/kitchen-sink@1.0.0",
            installPath: pluginDir,
          },
        },
      }),
      pluginId: "merclaw-kitchen-sink-fixture",
      deleteFiles: true,
      extensionsDir,
    });
    ...
    expect(plan.directoryRemoval).toEqual({
      ...
      cleanup: {
        kind: "npm",
        npmRoot,
        packageName: "@merclaw/kitchen-sink",
      },
    });
    ...
    expect(applied).toEqual({ directoryRemoved: true, warnings: [] });
    expectNpmUninstallCommand({ packageName: "@merclaw/kitchen-sink", npmRoot });
    await expectPathAccessState(pluginDir, "missing");
  });
  ...
  it("repairs remaining npm plugin merclaw peer links after npm uninstall prunes them", async () => {
    const stateDir = path.join(tempDir, "state");
    const npmRoot = path.join(stateDir, "npm");
    ...
    const peerLink = path.join(peerPluginDir, "node_modules", "merclaw");
    ...
    await expectPathAccessState(path.join(npmRoot, "node_modules", "merclaw"), "missing");
  ...
  it("prunes managed peer dependencies after their owning npm plugin is uninstalled", async () => {
    ...
    expect(rootManifest.merclaw?.managedPeerDependencies ?? []).not.toContain("runtime-peer");
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("returns managed install path for copied path installs", () => {
    const extensionsDir = path.join(os.tmpdir(), "merclaw-uninstall-safe");
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    ...
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-merclaw-plugin-install/my-plugin",
      },
      extensionsDir,
    });
  ...
  it("uses configured installPath when npm installed it under the managed npm root", () => {
    const stateDir = path.join(os.tmpdir(), "merclaw-uninstall-safe");
    const extensionsDir = path.join(stateDir, "extensions");
    const installPath = path.join(stateDir, "npm", "node_modules", "@merclaw", "kitchen-sink");
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@merclaw/kitchen-sink@latest",
          installPath,
        },
        extensionsDir,
      }),
  ...
  it("uses configured installPath when npm installed it under a managed npm project", () => {
    ...
    const installPath = path.join(npmRoot, "node_modules", "@merclaw", "kitchen-sink");
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@merclaw/kitchen-sink@latest",
          installPath: path.join(os.tmpdir(), "npm", "node_modules", "@merclaw", "kitchen-sink"),
        },
        extensionsDir,
      }),
    ).toBe(resolvePluginInstallDir("merclaw-kitchen-sink-fixture", extensionsDir));
  });

  it("uses configured installPath when it is under the recorded managed extensions root", () => {
    const currentExtensionsDir = path.join(os.tmpdir(), "merclaw-uninstall-current", "extensions");
    const recordedExtensionsDir = path.join(
      os.tmpdir(),
      "merclaw-uninstall-recorded",
      "extensions",
    );
    const installPath = resolvePluginInstallDir("my-plugin", recordedExtensionsDir);
