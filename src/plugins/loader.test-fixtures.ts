...
import { clearPluginLoaderCache, loadMerClawPlugins } from "./loader.js";
import { resetPluginRuntimeStateForTest } from "./runtime.js";

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadMerClawPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadMerClawPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "merclaw-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.MERCLAW_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.MERCLAW_DISABLE_BUNDLED_PLUGINS;
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".merclaw", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ MERCLAW_STATE_DIR: stateDir, ...params.env }, () =>
    loadMerClawPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.MERCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.MERCLAW_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.MERCLAW_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
