...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before MerClaw consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether MerClaw should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume MerClaw's bundle
   ...
   *
   * Use this when the backend should consume a canonical MerClaw auth profile
   ...
   * `profile-only` treats the selected MerClaw auth profile as the sole auth
   ...
   * as static config, such as mapping MerClaw thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside MerClaw's tool
