...
function createTempRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-manifest-metadata-"));
  tempRoots.push(root);
  return root;
}
...
describe("listMerClawPluginManifestMetadata", () => {
  afterEach(() => {
  ...
  it("prefers the active bundled manifest over stale persisted bundled installs", () => {
    ...
    writeJson(path.join(bundledRoot, "openai", "merclaw.plugin.json"), {
      id: "openai",
      providerEndpoints: [{ endpointClass: "openai-public", hosts: ["api.openai.com"] }],
    });
    writeJson(path.join(staleBundledRoot, "openai", "merclaw.plugin.json"), {
      id: "openai",
      providers: ["openai"],
    });
    ...
    const records = listMerClawPluginManifestMetadata({
      MERCLAW_HOME: home,
