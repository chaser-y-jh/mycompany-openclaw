...
function collectConfiguredPluginEntryIds(config: MerClawConfig): string[] {
...
function resolveRegister(mod: MerClawPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: MerClawPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as MerClawPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: MerClawPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so MerClaw ignored the plugin setup runtime entry. Remove setup-api/merclaw.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
