...
export type ConfigScopedRuntimeCache<T> = WeakMap<MerClawConfig, Map<string, T>>;

export type ConfigScopedPromiseLoader<T> = {
  load(config?: MerClawConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: MerClawConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<MerClawConfig, Promise<T>>();

  const createPromise = (config?: MerClawConfig): Promise<T> => {
  ...
  return {
    async load(config?: MerClawConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<MerClawConfig, Promise<T>>();
    },
  };
}
