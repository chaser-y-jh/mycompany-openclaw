...
vi.mock("../infra/merclaw-root.js", () => ({
  resolveMerClawPackageRootSync: vi.fn(),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("merclaw-plugin-install");
...
function setPluginMinHostVersion(pluginDir: string, minHostVersion: string) {
  const packageJsonPath = path.join(pluginDir, "package.json");
  const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
    merclaw?: { install?: Record<string, unknown> };
  };
  manifest.merclaw = {
    ...manifest.merclaw,
    install: {
      ...manifest.merclaw?.install,
      minHostVersion,
    },
  };
  fs.writeFileSync(packageJsonPath, JSON.stringify(manifest), "utf-8");
...
function setPluginPackageCompatibility(pluginDir: string, pluginApiRange: unknown) {
  const packageJsonPath = path.join(pluginDir, "package.json");
  const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
    merclaw?: { compat?: Record<string, unknown> };
  };
  manifest.merclaw = {
    ...manifest.merclaw,
    compat: {
      ...manifest.merclaw?.compat,
...
async function ensureDynamicArchiveTemplate(params: {
  ...
  if (params.writePluginManifest !== false) {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "merclaw.plugin.json"),
      JSON.stringify({
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "merclaw.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs package archive runtime dependencies", async () => {
    ...
    expect(commandOptions.cwd).toContain(".merclaw-install-stage-");
  });
  ...
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@merclaw/voice-call" });
  ...
  it("rejects native plugin zip archives without merclaw.plugin.json", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package missing valid merclaw.plugin.json");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_PLUGIN_MANIFEST);
    }
    expect(fs.existsSync(resolvePluginInstallDir("@merclaw/zipper", extensionsDir))).toBe(false);
  });
  ...
  it("rejects packages without merclaw.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@merclaw/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("merclaw.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_MERCLAW_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when merclaw.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@merclaw/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing merclaw.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_MERCLAW_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without merclaw.extensions");
  });

  it("rejects package installs when merclaw.extensions entries escape the package", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_MERCLAW_EXTENSIONS);
  ...
  it("allows missing TypeScript source entries when an inferred built runtime entry exists", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        merclaw: { extensions: ["./src/index.ts"] },
      }),
    );
    fs.writeFileSync(path.join(pluginDir, "dist", "index.js"), "export {};\n");
  ...
  it("rejects package installs when merclaw.extensions contains a blank entry", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_MERCLAW_EXTENSIONS);
      expect(result.error).toContain("merclaw.extensions[1]");
  ...
  it("rejects package installs when runtimeExtensions contains a blank entry", async () => {
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_MERCLAW_EXTENSIONS);
      expect(result.error).toContain("merclaw.runtimeExtensions[0]");
      expect(result.error).toContain("non-empty string");
    }
  });
  ...
  it("rejects package installs when an extension entry is a symlink escape", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        merclaw: { extensions: ["./linked/escape.js"] },
      }),
    );
  ...
  it("still scans declared package entrypoints when they live under test-looking paths", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        merclaw: { extensions: ["tests/runtime.test.js"] },
      }),
    );
    fs.mkdirSync(path.join(pluginDir, "tests"), { recursive: true });
    fs.writeFileSync(
  ...
  it.runIf(process.platform !== "win32")(
    "allows package installs when node_modules/merclaw points at the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-merclaw");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"merclaw"}\n');
      vi.mocked(resolveMerClawPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "merclaw-peer-plugin");

      const nodeModulesDir = path.join(pluginDir, "node_modules");
      fs.mkdirSync(nodeModulesDir, { recursive: true });
      fs.symlinkSync(hostRoot, path.join(nodeModulesDir, "merclaw"), "junction");
  ...
  it.runIf(process.platform !== "win32")(
    "allows package installs when node_modules/.bin/merclaw points inside the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-merclaw");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"merclaw"}\n');
      const hostBin = path.join(hostRoot, "merclaw.mjs");
      fs.writeFileSync(hostBin, "#!/usr/bin/env node\n");
      vi.mocked(resolveMerClawPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "merclaw-bin-peer-plugin");

      const binDir = path.join(pluginDir, "node_modules", ".bin");
      fs.mkdirSync(binDir, { recursive: true });
      fs.symlinkSync(hostBin, path.join(binDir, "merclaw"), "file");
  ...
  it.runIf(process.platform !== "win32")(
    "fails package installs when node_modules/merclaw points outside the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-merclaw");
      const spoofedRoot = path.join(tmpDir, "spoofed-merclaw");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.mkdirSync(spoofedRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"merclaw"}\n');
      fs.writeFileSync(path.join(spoofedRoot, "package.json"), '{"name":"merclaw"}\n');
      vi.mocked(resolveMerClawPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "spoofed-merclaw-peer-plugin");

      const nodeModulesDir = path.join(pluginDir, "node_modules");
      fs.mkdirSync(nodeModulesDir, { recursive: true });
      fs.symlinkSync(spoofedRoot, path.join(nodeModulesDir, "merclaw"), "junction");
      ...
      if (!result.ok) {
        expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED);
        expect(result.error).toContain("node_modules/merclaw");
      }
    },
  );

  it.runIf(process.platform !== "win32")(
    "fails package installs for nested or non-exact merclaw node_modules symlinks",
    async () => {
      const cases = [
        {
          pluginName: "nested-merclaw-peer-plugin",
          relativePath: path.join("node_modules", "vendor", "node_modules", "merclaw"),
        },
        {
          pluginName: "uppercase-merclaw-peer-plugin",
          relativePath: path.join("node_modules", "MerClaw"),
        },
        {
          pluginName: "trailing-space-merclaw-peer-plugin",
          relativePath: path.join("node_modules", "merclaw "),
        },
      ] as const;
  ...
  it("fails package installs when manifest traversal exceeds the directory cap", async () => {
    vi.stubEnv("MERCLAW_INSTALL_SCAN_MAX_DIRECTORIES", "4");
  ...
  it("fails package installs when manifest traversal exceeds the depth cap", async () => {
    vi.stubEnv("MERCLAW_INSTALL_SCAN_MAX_DEPTH", "2");
  ...
  it("does not flag the real qa-matrix plugin as dangerous install code", async () => {
    ...
    vi.mocked(resolveMerClawPackageRootSync).mockReturnValue(process.cwd());
  ...
  it("scans extension entry files in hidden directories", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        merclaw: { extensions: [".hidden/index.js"] },
      }),
    );
    fs.writeFileSync(
...
describe("installPluginFromDir", () => {
  ...
  it("preserves local package manifests without dependency surgery", async () => {
    ...
    expect(manifest.devDependencies?.merclaw).toBe("workspace:*");
  ...
  it.each([
    {
      ...
      expectedMessageIncludes: ["requires MerClaw >=2026.3.22, but this host is 2026.3.21"],
    },
    {
      name: "rejects plugins with invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedCode: PLUGIN_INSTALL_ERROR_CODE.INVALID_MIN_HOST_VERSION,
      expectedMessageIncludes: ["invalid package.json merclaw.install.minHostVersion"],
    },
    {
  ...
  it("rejects plugins whose package plugin API metadata is malformed", async () => {
    ...
    expectFailedInstallResult({
      ...
      messageIncludes: ["merclaw.compat.pluginApi", "must be a string"],
    });
    expect(vi.mocked(runCommandWithTimeout)).not.toHaveBeenCalled();
  });
  ...
  it("checks package plugin API before current-host extension shape validation", async () => {
    ...
    if (!result.ok) {
      expect(result.error).not.toContain("merclaw.extensions");
    }
    expect(vi.mocked(runCommandWithTimeout)).not.toHaveBeenCalled();
  });
  ...
  it("rejects bundle package installs whose package plugin API range is newer than the current host", async () => {
    ...
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@merclaw/future-bundle",
        version: "2026.5.27",
        merclaw: { compat: { pluginApi: ">=2026.5.27" } },
      }),
      "utf-8",
    );
  ...
  it("allows plugins when a beta host is on the package plugin API floor", async () => {
    ...
    expect(result.pluginId).toBe("@merclaw/test-plugin");
  });

  it("uses merclaw.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@merclaw/cognee-merclaw"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedPluginId: "@merclaw/test-plugin",
      install: (pluginDir: string, extensionsDir: string) =>
        installPluginFromDir({
          dirPath: pluginDir,
          extensionsDir,
...
describe("linkMerClawPeerDependencies (via installPluginFromDir)", () => {
  const resolveRootMock = vi.mocked(resolveMerClawPackageRootSync);
  ...
  it("creates a node_modules/merclaw symlink when peerDependencies declares merclaw", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    const run = vi.mocked(runCommandWithTimeout);
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { merclaw: "*" });
    ...
    const symlinkPath = path.join(result.targetDir, "node_modules", "merclaw");
  ...
  it("keeps the merclaw peer symlink when a local plugin already has dependencies", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { merclaw: "*" }, { "is-number": "7.0.0" });
  ...
  it("does not create a symlink when peerDependencies is empty", async () => {
    ...
    const symlinkPath = path.join(nodeModulesDir, "merclaw");
  ...
  it("is idempotent - re-installing replaces an existing symlink without error", async () => {
    ...
    const symlinkPath = path.join(second.targetDir, "node_modules", "merclaw");
    expect(fs.lstatSync(symlinkPath).isSymbolicLink()).toBe(true);
  });

  it("rejects when resolveMerClawPackageRootSync returns null", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    ...
    expectWarningIncludes(warnings, "Could not locate merclaw package root");
  });
});
