...
type LoadMerClawPlugins = typeof import("./loader.js").loadMerClawPlugins;
...
const loadMerClawPluginsMock = vi.fn<LoadMerClawPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/merclaw.plugin.json`,
  };
}
...
function expectLastSetupRegistryCall(params: {
  ...
  const call = getLastMockCallArg(loadMerClawPluginsMock, "MerClaw plugin setup loader");
...
function getLastSetupLoadedPluginConfig() {
  const call = expectRecordFields(
    getLastMockCallArg(loadMerClawPluginsMock, "MerClaw plugin setup loader"),
    {},
  );
...
function createAutoEnabledProviderConfig() {
  const rawConfig: MerClawConfig = {
    plugins: {},
  };
  const autoEnabledConfig: MerClawConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadMerClawPlugins: (...args: Parameters<LoadMerClawPlugins>) =>
        loadMerClawPluginsMock(...args),
