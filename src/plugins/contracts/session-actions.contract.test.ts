...
describe("plugin session actions", () => {
  ...
  it("emits plugin-attributed agent events through the plugin API", () => {
    ...
    const { config, registry } = createPluginRegistryFixture();
    let bundledApi: MerClawPluginApi | undefined;
    let workspaceApi: MerClawPluginApi | undefined;
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("blocks agent events from stale and non-activating plugin API closures", () => {
    ...
    try {
      ...
      let neverActiveApi: MerClawPluginApi | undefined;
      registerTestPlugin({
      ...
      let inactiveApi: MerClawPluginApi | undefined;
      registerTestPlugin({
