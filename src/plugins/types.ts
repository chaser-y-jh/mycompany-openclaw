 ...
 */
export type MerClawPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<MerClawConfig>;
  defaultModel?: string;
  notes?: string[];
  /**
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). MerClaw uses this to expose
 ...
 *
 * Runs after MerClaw resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common MerClaw auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and MerClaw wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before MerClaw builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. MerClaw no
...
/**
 * Provider-owned stream wrapper hook after MerClaw applies its generic
 ...
 *
 * Return `true` or `false` to override MerClaw's built-in provider cache TTL
 ...
 *
 * Runs only after MerClaw exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that MerClaw should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after MerClaw loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, MerClaw falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * MerClaw calls this only from async model resolution paths. After it
   ...
   *
   * Runs after MerClaw performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before MerClaw registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic MerClaw wrappers.
   *
   ...
   *
   * Called after MerClaw resolves the raw configured credential but before the
   ...
   * hint than MerClaw's generic auth-store guidance.
   */
   ...
   * MerClaw's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when MerClaw should surface a provider-specific hint for
   ...
   * merging. MerClaw deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. MerClaw stores the
   ...
   * without replacing the full MerClaw-owned system prompt.
   */
   ...
   *
   * Runs after MerClaw's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep MerClaw's default fallback policy.
   */
   ...
   * rewrites after MerClaw has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before MerClaw handles
   ...
   *
   * MerClaw uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `merclaw doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `merclaw doctor` should offer.
   *
   ...
   *
   * MerClaw calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep MerClaw's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * MerClaw falls back to that stored profile.
   */
...
export type MerClawPluginGatewayMethod = {
  method: string;
...
export type PluginCommandDiagnosticsSession = {
  ...
  sessionKey?: string;
  /** Ephemeral MerClaw session id when available. */
  sessionId?: string;
  /** Transcript file for this MerClaw session when available. */
  sessionFile?: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active MerClaw session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current MerClaw configuration */
  config: MerClawConfig;
...
export type MerClawPluginCommandDefinition = {
...
export type MerClawPluginHttpRouteAuth = "gateway" | "plugin";
export type MerClawPluginHttpRouteMatch = "exact" | "prefix";
export type MerClawPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type MerClawPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type MerClawPluginHttpRouteUpgradeHandler = (
  req: IncomingMessage,
  socket: Duplex,
  head: Buffer,
) => Promise<boolean | void> | boolean | void;

export type MerClawPluginHttpRouteParams = {
  path: string;
...
export type MerClawPluginHostedMediaResolver = (
  mediaUrl: string,
) => string | null | undefined | Promise<string | null | undefined>;

export type MerClawPluginCliContext = {
  /**
   * Command object where this plugin should register its commands.
   *
   * For root CLI registrations this is the root `merclaw` program. For nested
...
export type MerClawPluginCliRegistrar = (ctx: MerClawPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want MerClaw to keep a plugin command lazy-loaded while still
 ...
 */
export type MerClawPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type MerClawPluginNodeCliFeatureOptions = {
  /** Explicit node feature command names owned under `merclaw nodes`. */
  commands?: string[];
  /**
   * Parse-time command descriptors for lazy node feature CLI registration.
   *
   * Descriptors are registered under `merclaw nodes`, so a descriptor named
   * `"camera"` exposes `merclaw nodes camera`.
   */
  descriptors?: MerClawPluginCliCommandDescriptor[];
};

export type MerClawPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type MerClawPluginNodeHostCommand = {
  command: string;
  cap?: string;
  dangerous?: boolean;
...
export type MerClawPluginNodeInvokeTransportResult =
  | {
      ok: true;
      payload?: unknown;
...
export type MerClawPluginNodeInvokeApprovalDecision = "allow-once" | "allow-always" | "deny";

export type MerClawPluginNodeInvokePolicyApprovalRuntime = {
  request: (input: {
    title: string;
    description: string;
    ...
    id?: string;
    decision?: MerClawPluginNodeInvokeApprovalDecision | null;
  }>;
};

export type MerClawPluginNodeInvokePolicyContext = {
  nodeId: string;
  command: string;
  params: unknown;
  timeoutMs?: number;
  ...
  approvals?: MerClawPluginNodeInvokePolicyApprovalRuntime;
  invokeNode: (input?: {
    params?: unknown;
    timeoutMs?: number;
    idempotencyKey?: string;
  }) => Promise<MerClawPluginNodeInvokeTransportResult>;
};

export type MerClawPluginNodeInvokePolicyResult =
  | {
      ok: true;
      payload?: unknown;
      payloadJSON?: string | null;
...
export type MerClawPluginNodeInvokePolicy = {
  commands: string[];
  /**
  ...
  handle: (
    ctx: MerClawPluginNodeInvokePolicyContext,
  ) => Promise<MerClawPluginNodeInvokePolicyResult> | MerClawPluginNodeInvokePolicyResult;
};

export type MerClawPluginSecurityAuditContext = {
  config: MerClawConfig;
...
export type MerClawPluginSecurityAuditCollector = (
  ctx: MerClawPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

export type MerClawGatewayDiscoveryAdvertiseContext = {
...
export type MerClawGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: MerClawGatewayDiscoveryAdvertiseContext,
  ) => void | Promise<void | { stop?: () => void | Promise<void> }>;
};

/** Context passed to long-lived plugin services. */
export type MerClawPluginServiceContext = {
  config: MerClawConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type MerClawPluginService = {
  id: string;
  start: (ctx: MerClawPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: MerClawPluginServiceContext) => void | Promise<void>;
};

export type MerClawPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type MerClawPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  reload?: MerClawPluginReloadRegistration;
  nodeHostCommands?: MerClawPluginNodeHostCommand[];
  securityAuditCollectors?: MerClawPluginSecurityAuditCollector[];
  register?: (api: MerClawPluginApi) => void;
  activate?: (api: MerClawPluginApi) => void;
};

export type MerClawPluginModule = MerClawPluginDefinition | ((api: MerClawPluginApi) => void);
...
export type PluginConfigMigration = (config: MerClawConfig) =>
  | {
      config: MerClawConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `merclaw migrate`. */
...
export type MerClawPluginSessionStateApi = {
...
export type MerClawPluginSessionWorkflowApi = {
...
export type MerClawPluginSessionControlsApi = {
...
export type MerClawPluginSessionApi = {
  state: MerClawPluginSessionStateApi;
  workflow: MerClawPluginSessionWorkflowApi;
  controls: MerClawPluginSessionControlsApi;
};

export type MerClawPluginAgentEventsApi = {
...
export type MerClawPluginAgentApi = {
  events: MerClawPluginAgentEventsApi;
};

export type MerClawPluginRunContextApi = {
...
export type MerClawPluginLifecycleApi = {
...
export type MerClawPluginApi = {
   ...
   */
  session: MerClawPluginSessionApi;
  /** Grouped facade for agent-event workflow seams. */
  agent: MerClawPluginAgentApi;
  /** Grouped facade for run-scoped plugin scratch state. */
  runContext: MerClawPluginRunContextApi;
  /** Grouped facade for plugin-owned lifecycle cleanup hooks. */
  lifecycle: MerClawPluginLifecycleApi;
  registerTool: (
    tool: AnyAgentTool | MerClawPluginToolFactory,
    opts?: MerClawPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: MerClawPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: MerClawPluginHttpRouteParams) => void;
  /** Register a plugin-owned resolver for browser-style hosted media URLs. */
  registerHostedMediaResolver: (resolver: MerClawPluginHostedMediaResolver) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: MerClawPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    ...
    opts?: {
       ...
       *
       * When descriptors cover every command exposed at `parentPath`, MerClaw
  ...
  /**
   * Register a plugin-owned node feature command group under `merclaw nodes`.
   *
  ...
  registerNodeCliFeature: (
    registrar: MerClawPluginCliRegistrar,
    opts?: MerClawPluginNodeCliFeatureOptions,
  ) => void;
  registerReload: (registration: MerClawPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: MerClawPluginNodeHostCommand) => void;
  registerNodeInvokePolicy: (policy: MerClawPluginNodeInvokePolicy) => void;
  registerSecurityAuditCollector: (collector: MerClawPluginSecurityAuditCollector) => void;
  registerService: (service: MerClawPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: MerClawGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `merclaw migrate` (migration capability). */
