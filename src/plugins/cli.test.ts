...
const mocks = vi.hoisted(() => ({
  ...
  loadMerClawPluginCliRegistry: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadMerClawPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadMerClawPluginCliRegistry(...args),
  loadMerClawPlugins: (...args: unknown[]) => mocks.loadMerClawPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  ...
  autoEnabledConfig: MerClawConfig;
  ...
  const loadOptions = getMockCallObject(mocks.loadMerClawPlugins);
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadMerClawPluginCliRegistry.mockReset();
    mocks.loadMerClawPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadMerClawPlugins.mockReset();
    mocks.loadMerClawPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as MerClawConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as MerClawConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as MerClawConfig, env);
  ...
  it("injects gateway-backed node runtime into plugin CLI commands", async () => {
    await registerPluginCliCommands(createProgram(), {} as MerClawConfig);

    const loadOptions = getMockCallObject(mocks.loadMerClawPlugins) as {
  ...
  it("reuses loaded plugin CLI entries on repeat calls for the same program", async () => {
    ...
    expect(mocks.loadMerClawPlugins).toHaveBeenCalledTimes(1);
  });

  it("reloads plugin CLI entries when the requested primary command changes", async () => {
    const program = createProgram();

    await registerPluginCliCommands(program, {} as MerClawConfig, undefined, undefined, {
      primary: "memory",
    });
    await registerPluginCliCommands(program, {} as MerClawConfig);

    expect(mocks.loadMerClawPlugins).toHaveBeenCalledTimes(2);
  });

  it("reloads plugin CLI entries when config or environment identity changes", async () => {
    const program = createProgram();
    const configA = {} as MerClawConfig;
    const configB = { plugins: {} } as MerClawConfig;
    const envA = { MERCLAW_HOME: "/tmp/a" } as NodeJS.ProcessEnv;
    const envB = { MERCLAW_HOME: "/tmp/b" } as NodeJS.ProcessEnv;
    ...
    expect(mocks.loadMerClawPlugins).toHaveBeenCalledTimes(3);
  });
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadMerClawPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    const registryOptions = getMockCallObject(mocks.loadMerClawPluginCliRegistry);
  ...
  it("keeps root-help descriptor load failures quiet", async () => {
    ...
    mocks.loadMerClawPluginCliRegistry.mockImplementationOnce((options: { logger?: unknown }) => {
    ...
    await expect(
      getPluginCliCommandDescriptors({ plugins: { entries: { stale: {} } } } as MerClawConfig),
    ).resolves.toEqual([]);
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadMerClawPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as MerClawConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("scopes full CLI loading through CLI metadata when manifest planning finds no plugin match", async () => {
    ...
    expect(mocks.loadMerClawPluginCliRegistry).toHaveBeenCalled();
    const loadOptions = getMockCallObject(mocks.loadMerClawPlugins);
    expect(loadOptions.onlyPluginIds).toEqual(["memory-core"]);
  });
  ...
  it("scopes nested CLI loading through CLI metadata parent paths", async () => {
    ...
    mocks.loadMerClawPluginCliRegistry.mockResolvedValue(nestedRegistry);
    mocks.loadMerClawPlugins.mockReturnValue(nestedRegistry);
    const program = createProgram("nodes");
    program.exitOverride();
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as MerClawConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
