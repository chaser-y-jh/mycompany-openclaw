...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "merclaw", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".merclaw", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".merclaw", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "merclaw-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".merclaw", "extensions"),
      },
    });
  });
});
