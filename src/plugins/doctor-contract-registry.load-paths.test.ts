...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "merclaw-doctor-contract-load-paths-"),
  );
  tempDirs.push(dir);
  return dir;
}
...
function createDoctorPluginConfig(pluginRoot: string, pluginId: string): MerClawConfig {
  return {
    plugins: {
...
function readPluginLlmPolicy(config: MerClawConfig, pluginId: string): Record<string, unknown> {
