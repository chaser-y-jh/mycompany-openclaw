...
import { bundledPluginRootAt } from "merclaw/plugin-sdk/test-fixtures";
...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-merclaw",
    targetDir: params?.targetDir ?? "/tmp/opik-merclaw",
...
function createSuccessfulClawHubUpdateResult(params?: {
  ...
  return {
    ...
    targetDir: params?.targetDir ?? "/tmp/merclaw-plugins/legacy-chat",
...
function createInstalledPackageDir(params: {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-plugin-update-test-"));
  tempDirs.push(dir);
  fs.writeFileSync(
...
function createMerClawPeerLinkFixtures(plugins: Array<{ pluginId: string; packageName: string }>) {
  const peerTarget = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-peer-target-"));
  ...
  const installPaths = Object.fromEntries(
    plugins.map(({ pluginId, packageName }) => [
      ...
      createInstalledPackageDir({
        name: packageName,
        version: "2026.5.4",
        peerDependencies: { merclaw: ">=2026.5.4" },
      }),
    ]),
  );
  const peerLinkPath = (pluginId: string) =>
    path.join(installPaths[pluginId], "node_modules", "merclaw");
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@merclaw/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  const install = params.result.config.plugins?.installs?.["merclaw-codex-app-server"];
  expect(install?.source).toBe("npm");
  expect(install?.spec).toBe(params.spec);
  expect(install?.installPath).toBe("/tmp/merclaw-codex-app-server");
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-merclaw",
        spec: "@opik/opik-merclaw",
        integrity: "sha512-old",
        installPath: "/tmp/opik-merclaw",
      }),
      pluginIds: ["opik-merclaw"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-merclaw",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        ...
        spec: "merclaw-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/merclaw-codex-app-server",
      }),
      ...
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "merclaw-codex-app-server",
        targetDir: "/tmp/merclaw-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "merclaw-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it("trusts official catalog npm updates when the installed package matches the catalog", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        spec: "@merclaw/acpx",
        installPath,
        resolvedName: "@merclaw/acpx",
        resolvedSpec: "@merclaw/acpx@2026.5.2-beta.1",
        resolvedVersion: "2026.5.2-beta.1",
      }),
      pluginIds: ["acpx"],
    });

    expect(npmInstallCall()?.spec).toBe("@merclaw/acpx");
  ...
  it("does not skip trusted official default updates when latest resolves to the installed prerelease", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@merclaw/acpx",
          version: "2026.5.2",
          resolvedSpec: "@merclaw/acpx@2026.5.2",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        shasum: "beta",
        resolvedName: "@merclaw/acpx",
        resolvedSpec: "@merclaw/acpx@2026.5.2-beta.2",
  ...
  it("updates trusted official npm plugins when latest resolves to a stable correction release", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@merclaw/acpx",
          version: "2026.5.3-1",
          resolvedSpec: "@merclaw/acpx@2026.5.3-1",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        installPath,
        resolvedName: "@merclaw/acpx",
        resolvedSpec: "@merclaw/acpx@2026.5.3",
  ...
  it("repairs missing merclaw peer links before skipping unchanged npm plugins", async () => {
    const installPath = createInstalledPackageDir({
      name: "@merclaw/codex",
      version: "2026.5.3",
      peerDependencies: { merclaw: ">=2026.5.3" },
    });
    mockNpmViewMetadata({
      name: "@merclaw/codex",
      version: "2026.5.3",
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@merclaw/codex",
          version: "2026.5.3",
          resolvedSpec: "@merclaw/codex@2026.5.3",
        },
      }),
    );
    const config: MerClawConfig = {
    ...
    expect(npmInstallCall()?.spec).toBe("@merclaw/codex");
  ...
  it("skips unchanged npm plugins when the merclaw peer link already resolves", async () => {
    const installPath = createInstalledPackageDir({
      name: "@merclaw/codex",
      version: "2026.5.3",
      peerDependencies: { merclaw: ">=2026.5.3" },
    });
    fs.mkdirSync(path.join(installPath, "node_modules", "merclaw"), { recursive: true });
    mockNpmViewMetadata({
      name: "@merclaw/codex",
      version: "2026.5.3",
      integrity: "sha512-same",
  ...
  it("repairs merclaw peer links after batch npm updates prune earlier plugin links", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@merclaw/brave-plugin" },
      { pluginId: "codex", packageName: "@merclaw/codex" },
      { pluginId: "discord", packageName: "@merclaw/discord" },
    ];
    const { installPaths, peerLinkPath, linkPeer } = createMerClawPeerLinkFixtures(plugins);
  ...
  it("repairs sibling merclaw peer links after a targeted npm update prunes the shared install tree", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@merclaw/brave-plugin" },
      { pluginId: "codex", packageName: "@merclaw/codex" },
      { pluginId: "discord", packageName: "@merclaw/discord" },
      ...
      return Promise.resolve(
        createSuccessfulNpmUpdateResult({
          ...
          npmResolution: {
            name: "@merclaw/codex",
            version: "2026.5.4",
            resolvedSpec: "@merclaw/codex@2026.5.4",
          },
        }),
      );
    });
  ...
  it("continues repairing sibling merclaw peer links after one recorded npm install cannot be relinked", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@merclaw/brave-plugin" },
      { pluginId: "codex", packageName: "@merclaw/codex" },
    ];
      ...
      config: {
        plugins: {
          installs: {
            broken: {
              source: "npm",
              spec: "@merclaw/broken-plugin",
              installPath: brokenInstallPath,
              resolvedName: "@merclaw/broken-plugin",
              resolvedVersion: "2026.5.4",
              resolvedSpec: "@merclaw/broken-plugin@2026.5.4",
            },
            ...Object.fromEntries(
              plugins.map(({ pluginId, packageName }) => [
                pluginId,
    ...
    expect(warnMessages).toEqual([
      `Could not repair merclaw peer link for "broken" at ${brokenInstallPath}: Skipping merclaw peerDependency link because ${path.join(brokenInstallPath, "node_modules")} is not a real directory.`,
    ]);
  });
  ...
  it("expands home-relative install paths before checking installed npm versions", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-plugin-update-home-"));
    tempDirs.push(home);
    const installPath = path.join(home, ".merclaw", "extensions", "lossless-claw");
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "lossless-claw",
        spec: "@martian-engineering/lossless-claw",
        installPath: "~/.merclaw/extensions/lossless-claw",
        resolvedName: "@martian-engineering/lossless-claw",
        resolvedVersion: "0.9.0",
        resolvedSpec: "@martian-engineering/lossless-claw@0.9.0",
        integrity: "sha512-same",
  ...
  it("updates disabled trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@merclaw/diagnostics-otel",
      }),
    );
    ...
    const config = createClawHubInstallConfig({
      ...
      clawhubFamily: "code-plugin",
      clawhubChannel: "official",
      spec: "clawhub:@merclaw/diagnostics-otel@2026.5.3",
    });
    const result = await updateNpmInstalledPlugins({
      config: {
        ...config,
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@merclaw/diagnostics-otel");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-otel"], {
      source: "clawhub",
      spec: "clawhub:@merclaw/diagnostics-otel",
      version: "2026.5.4",
      clawhubPackage: "@merclaw/diagnostics-otel",
      clawhubChannel: "official",
    });
  ...
  it("updates bare trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@merclaw/diagnostics-prometheus",
      }),
    );
        ...
        plugins: {
          installs: {
            "diagnostics-prometheus": {
              source: "clawhub",
              spec: "clawhub:@merclaw/diagnostics-prometheus@2026.5.3",
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@merclaw/diagnostics-prometheus");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-prometheus"], {
      source: "clawhub",
      spec: "clawhub:@merclaw/diagnostics-prometheus",
      version: "2026.5.4",
      clawhubPackage: "@merclaw/diagnostics-prometheus",
      clawhubChannel: "official",
    });
  ...
  it("clears stale plugin policy and slot references when disabling failed updates", async () => {
    ...
    const message =
      'Disabled "demo" after plugin update failure; MerClaw will continue without it. Failed to update demo: security scan blocked install';
    expect(warn).toHaveBeenCalledWith(message);
    expect(result.changed).toBe(true);
    expect(result.config.plugins?.entries?.demo).toEqual({
      enabled: false,
  ...
  it("aborts exact pinned npm plugin updates on integrity drift by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockImplementation(
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            integrity: "sha512-new",
            resolvedSpec: "@opik/opik-merclaw@0.2.5",
            version: "0.2.5",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @opik/opik-merclaw@0.2.5",
          };
        }
    ...
    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for "opik-merclaw" (@opik/opik-merclaw@0.2.5): expected sha512-old, got sha512-new',
    );
    expect(result.changed).toBe(false);
    expect(result.config).toBe(config);
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update opik-merclaw: aborted: npm package integrity drift detected for @opik/opik-merclaw@0.2.5",
      },
    ]);
  });
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @merclaw/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@merclaw/missing",
        installPath: "/tmp/missing",
      }),
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @merclaw/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        ...
        resolvedSpec: "merclaw-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "merclaw-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "merclaw-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "merclaw-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
      ...
      expectedSpec: "merclaw-codex-app-server@beta",
      expectedRecordSpec: "merclaw-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "merclaw-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "merclaw-codex-app-server",
      });
  ...
  it("falls back to the default npm spec when a beta tag is unavailable", async () => {
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error:
          "npm ERR! code ETARGET\nnpm ERR! No matching version found for merclaw-codex-app-server@beta.",
      })
      .mockResolvedValueOnce(
        createSuccessfulNpmUpdateResult({
          pluginId: "merclaw-codex-app-server",
          ...
          npmResolution: {
            name: "merclaw-codex-app-server",
            version: "0.2.6",
            resolvedSpec: "merclaw-codex-app-server@0.2.6",
          },
        }),
      );
    ...
    expect(npmInstallCall(0)?.spec).toBe("merclaw-codex-app-server@beta");
    expect(npmInstallCall(1)?.spec).toBe("merclaw-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "merclaw-codex-app-server" has no beta npm release for merclaw-codex-app-server@beta; using merclaw-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "merclaw-codex-app-server",
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Updated merclaw-codex-app-server: unknown -> 0.2.6. (warning: beta channel fallback used merclaw-codex-app-server because merclaw-codex-app-server@beta could not be used).",
    );
  });
  ...
  it("falls back to the default npm spec when the beta package exists but is invalid", async () => {
    ...
    expect(npmInstallCall(1)?.spec).toBe("merclaw-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "merclaw-codex-app-server" failed beta npm update for merclaw-codex-app-server@beta; using merclaw-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "merclaw-codex-app-server",
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update merclaw-codex-app-server: npm package not found for merclaw-codex-app-server.",
      },
    ]);
  });
  ...
  it("preserves explicit npm tags when updating on the beta channel", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createCodexAppServerInstallConfig({
        spec: "merclaw-codex-app-server@rc",
      }),
      pluginIds: ["merclaw-codex-app-server"],
      updateChannel: "beta",
      dryRun: true,
  ...
  it("falls back to npm for trusted official ClawHub artifact blocks", async () => {
    ...
    installPluginFromClawHubMock.mockResolvedValueOnce({
      ...
      error:
        'ClawHub artifact download for "@merclaw/discord@2026.5.16-beta.5" is not available yet (ClawHub /api/v1/packages/%40merclaw%2Fdiscord/versions/2026.5.16-beta.5/artifact/download failed (403): Blocked: this package release has been flagged as malicious and cannot be downloaded.). Use "npm:@merclaw/discord@2026.5.16-beta.5" for launch installs while ClawHub artifact routing is being rolled out.',
    });
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        pluginId: "discord",
        targetDir: "/tmp/merclaw-plugins/discord",
        version: "2026.5.16-beta.5",
        npmResolution: {
          name: "@merclaw/discord",
          version: "2026.5.16-beta.5",
          resolvedSpec: "@merclaw/discord@2026.5.16-beta.5",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createClawHubInstallConfig({
        ...
        installPath,
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@merclaw/discord",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        spec: "clawhub:@merclaw/discord",
      }),
      pluginIds: ["discord"],
      updateChannel: "beta",
      disableOnFailure: true,
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@merclaw/discord@beta");
    expect(npmInstallCall()?.spec).toBe("@merclaw/discord@beta");
    ...
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
      spec: "@merclaw/discord",
      installPath: "/tmp/merclaw-plugins/discord",
      version: "2026.5.16-beta.5",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        message:
          "Updated discord: 2026.5.12 -> 2026.5.16-beta.5. (warning: official ClawHub artifact fallback used @merclaw/discord@beta).",
      },
    ]);
    expect(warnMessages).toEqual([
      'Plugin "discord" could not download official ClawHub artifact for clawhub:@merclaw/discord@beta; using npm @merclaw/discord@beta instead. Core update can still complete.',
    ]);
  });
  ...
  it("uses the default npm spec when beta ClawHub falls back before an artifact block", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@merclaw/discord",
          version: "2026.5.16",
          resolvedSpec: "@merclaw/discord@2026.5.16",
        },
      }),
    );
    ...
    expect(clawHubInstallCall(0)?.spec).toBe("clawhub:@merclaw/discord@beta");
    expect(clawHubInstallCall(1)?.spec).toBe("clawhub:@merclaw/discord");
    expect(npmInstallCall()?.spec).toBe("@merclaw/discord");
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
      spec: "@merclaw/discord",
      installPath: "/tmp/merclaw-plugins/discord",
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Updated discord: 2026.5.12 -> 2026.5.16. (warning: official ClawHub artifact fallback used @merclaw/discord).",
    );
    expect(warnMessages).toEqual([
      'Plugin "discord" has no beta ClawHub release for clawhub:@merclaw/discord@beta; using clawhub:@merclaw/discord instead. Core update can still complete.',
      'Plugin "discord" could not download official ClawHub artifact for clawhub:@merclaw/discord; using npm @merclaw/discord instead. Core update can still complete.',
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        nextVersion: "2026.5.16-beta.5",
        message:
          "discord is up to date (2026.5.16-beta.5). (warning: official ClawHub artifact fallback would use @merclaw/discord@beta).",
      },
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update discord: artifact unavailable (ClawHub clawhub:@merclaw/discord@beta).",
      },
    ]);
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@merclaw/voice-call",
      targetDir: "/tmp/merclaw-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(npmInstallCall()?.spec).toBe("@merclaw/voice-call");
    expect(npmInstallCall()?.expectedPluginId).toBe("voice-call");
    expect(result.config.plugins?.allow).toEqual(["@merclaw/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@merclaw/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@merclaw/voice-call");
    expect(result.config.plugins?.entries?.["@merclaw/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expectRecordFields(result.config.plugins?.installs?.["@merclaw/voice-call"], {
      source: "npm",
      spec: "@merclaw/voice-call",
      installPath: "/tmp/merclaw-voice-call",
      version: "0.0.2",
    });
  ...
  it("migrates context engine slot when a plugin id changes during update", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@merclaw/context-engine",
      targetDir: "/tmp/merclaw-context-engine",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.slots?.contextEngine).toBe("@merclaw/context-engine");
    expectRecordFields(result.config.plugins?.installs?.["@merclaw/context-engine"], {
      source: "npm",
      spec: "@merclaw/context-engine",
      installPath: "/tmp/merclaw-context-engine",
      version: "0.0.2",
    });
  ...
  it("forwards dangerous force unsafe install to plugin update installers", async () => {
    ...
    expect(npmInstallCall()?.spec).toBe("merclaw-codex-app-server@beta");
    expect(npmInstallCall()?.dangerouslyForceUnsafeInstall).toBe(true);
    expect(npmInstallCall()?.expectedPluginId).toBe("merclaw-codex-app-server");
  });

  it("reuses the recorded managed extensions root when updating external plugins", async () => {
    const installPath = "/var/merclaw/extensions/demo";
    const extensionsDir = "/var/merclaw/extensions";
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "demo",
        targetDir: installPath,
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    resolveBundledPluginSourcesMock.mockReturnValue(new Map());
    const env = { MERCLAW_HOME: "/srv/merclaw-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/merclaw-home";
    mockBundledSources(
      createBundledSource({
  ...
  it("installs an externalized bundled plugin and rewrites its old bundled path plugin index", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/merclaw-plugins/legacy-chat",
        version: "2.0.0",
        npmResolution: {
          name: "@merclaw/legacy-chat",
          version: "2.0.0",
          resolvedSpec: "@merclaw/legacy-chat@2.0.0",
        },
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@merclaw/legacy-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
      spec: "@merclaw/legacy-chat",
      installPath: "/tmp/merclaw-plugins/legacy-chat",
      version: "2.0.0",
      resolvedName: "@merclaw/legacy-chat",
  ...
  it("marks official externalized bundled npm installs as trusted", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "voice-call",
        targetDir: "/tmp/merclaw-plugins/voice-call",
  ...
  it("falls back from ClawHub to npm only when the ClawHub package is absent", async () => {
    ...
    expect(result.summary.warnings).toEqual([
      "ClawHub clawhub:legacy-chat@2026.5.1-beta.2 unavailable for legacy-chat; falling back to npm @merclaw/legacy-chat.",
    ]);
    expect(result.summary.errors).toStrictEqual([]);
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
  ...
  it("does not fall back from ClawHub to non-MerClaw npm packages", async () => {
    resolveBundledPluginSourcesMock.mockReturnValue(new Map());
    installPluginFromClawHubMock.mockResolvedValue({
      ok: false,
      code: "package_not_found",
      ...
      externalizedBundledPluginBridges: [
        {
          bundledPluginId: "voice-call",
          preferredSource: "clawhub",
          clawhubSpec: "clawhub:@merclaw/voice-call",
          npmSpec: "@merclaw/voice-call",
          channelIds: ["voice-call"],
        },
      ],
  ...
  it("externalizes bundled plugins that were enabled by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/merclaw-plugins/default-chat",
        version: "2.0.0",
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@merclaw/default-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["default-chat"], {
      source: "npm",
      spec: "@merclaw/default-chat",
      installPath: "/tmp/merclaw-plugins/default-chat",
      version: "2.0.0",
    });
  });
      ...
      config: {
        ...
        plugins: {
          ...
          installs: {
            "legacy-chat": {
              source: "npm",
              spec: "@merclaw/legacy-chat@1.2.3",
              resolvedSpec: "@merclaw/legacy-chat@1.2.3",
              installPath: "/tmp/merclaw-plugins/legacy-chat",
            },
          },
        },
