...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[merclaw proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[merclaw proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`merclaw proxy coverage\` for details.`,
  );
}
