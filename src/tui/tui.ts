...
const MERCLAW_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../merclaw.mjs", import.meta.url));
const MERCLAW_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const MERCLAW_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const MERCLAW_DIST_ENTRY_MJS_PATH = fileURLToPath(
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(MERCLAW_DIST_ENTRY_JS_PATH) || existsSync(MERCLAW_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(MERCLAW_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? MERCLAW_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? MERCLAW_RUN_NODE_SCRIPT_PATH;
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run merclaw devices list",
      pairingHint:
        "Pairing required. Run `merclaw devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export function scheduleProcessExitAfterTuiReturn(
  ...
  const timer = setTimeoutFn(() => {
    try {
      writeStderr("merclaw tui forcing process exit after return\n");
    } catch {
...
function resolveEmptySessionInfoDefaults(config: MerClawConfig): SessionInfo {
  return {
...
export async function runTui(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const previousConsoleSubsystemFilter = isLocalMode
    ...
    : null;
  if (isLocalMode) {
    setConsoleSubsystemFilter(["__merclaw_tui_quiet__"]);
  }
  ...
  const updateHeader = () => {
    ...
    const title = opts.title ?? "merclaw tui";
    header.setText(
      theme.header(
  ...
  const forceExit = () => {
    try {
      process.stderr.write("merclaw tui forcing exit\n");
    } catch {
    ...
    void Promise.resolve()
      ...
      .catch((err) => {
        if (!isTuiTerminalLossError(err)) {
          try {
            process.stderr.write(`merclaw tui shutdown failed: ${String(err)}\n`);
          } catch {
