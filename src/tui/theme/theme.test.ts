...
describe("light background detection", () => {
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      MERCLAW_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when MERCLAW_THEME=light", async () => {
    const mod = await importThemeWithEnv({ MERCLAW_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when MERCLAW_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ MERCLAW_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats MERCLAW_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ MERCLAW_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("MERCLAW_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      MERCLAW_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ MERCLAW_THEME: "light" });
    const darkMod = await importThemeWithEnv({ MERCLAW_THEME: "dark" });
