...
export async function controlRealtimeVoiceAgentRun(
  ...
  if (mode === "cancel") {
    if (!sessionId) {
      return {
        ...
        message: "There is no active MerClaw run to cancel.",
        speak: true,
        show: true,
        suppress: false,
      };
    ...
    const message = aborted
      ? "Cancelled the active MerClaw run."
      : "MerClaw could not cancel the active run.";
    return {
      ok: aborted,
      mode,
      sessionKey,
  ...
  return {
    ...
    message:
      mode === "followup"
        ? "Queued that follow-up for the active MerClaw run."
