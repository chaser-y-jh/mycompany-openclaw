...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME = "merclaw_agent_consult";
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Delegate the caller's request to the configured MerClaw agent for normal tool-backed work, actions, context, memory, or reasoning before speaking.",
  parameters: {
    type: "object",
    properties: {
      question: {
...
export function buildRealtimeVoiceAgentConsultWorkingResponse(
  ...
  return {
    ...
    message: `Tell the ${audienceLabel} briefly that you are checking, then wait for the final MerClaw result before answering with the actual result.`,
  };
}
...
export function buildRealtimeVoiceAgentConsultPolicyInstructions(config: {
  ...
  if (config.consultPolicy === "always") {
    return [
      "Consult behavior:",
      "- Call merclaw_agent_consult before every substantive answer.",
  ...
  return [
    ...
    "- Call merclaw_agent_consult before answering requests that need facts, memory, current information, tools, workspace state, or the user's MerClaw-specific context.",
...
export function buildRealtimeVoiceAgentConsultPrompt(params: {
  ...
  return [
    ...
    "Act as the configured MerClaw agent on behalf of this user. Use available tools when the request asks you to do work.",
