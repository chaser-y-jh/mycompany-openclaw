...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        MERCLAW_BASE_ENV: "base",
        MERCLAW_TO_REMOVE: undefined,
      },
      env: {
        MERCLAW_TEST_ENV: "ok",
      },
    ...
    expect(resolved.MERCLAW_BASE_ENV).toBe("base");
    expect(resolved.MERCLAW_TEST_ENV).toBe("ok");
    expect(resolved.MERCLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.MERCLAW_CLI).toBe(MERCLAW_CLI_ENV_VALUE);
  });
