...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: vi.fn(() => "/tmp/merclaw-agent"),
  resolveAgentDir: vi.fn(() => "/tmp/merclaw-agent/.merclaw-agent"),
  resolveAgentEffectiveModelPrimary: vi.fn((cfg: MerClawConfig) => {
...
function requireFirstRunOptions(): Record<string, unknown> {
  ...
  if (!call) {
    throw new Error("expected embedded MerClaw agent run");
  }
  const [options] = call;
  if (!options || typeof options !== "object") {
    throw new Error("expected embedded MerClaw agent run options");
  }
