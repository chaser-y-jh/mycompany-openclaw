...
const fixtureRoot = path.join(process.cwd(), ".tmp", `merclaw-hook-install-${randomUUID()}`);
...
const npmPackHooksBuffer = await createTarGzHookPackBuffer({
  packageName: "@merclaw/test-hooks",
  hookName: "one-hook",
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@merclaw/test-hooks",
      version: "0.0.1",
      merclaw: { hooks: params.hooks },
...
async function createZipHookPackBuffer(params: {
  ...
  const packageJson = JSON.stringify({
    ...
    version: "0.0.1",
    merclaw: { hooks: [`./hooks/${params.hookName}`] },
  });
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "merclaw.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "merclaw.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.npmResolution?.resolvedSpec).toBe("@merclaw/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@merclaw/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = runCommandWithTimeoutMock;
    mockNpmPackMetadataResult(run, {
      id: "@merclaw/test-hooks@0.0.2-beta.1",
      name: "@merclaw/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@merclaw/test-hooks@beta"');
    }
  });
});
