...
homepage: https://docs.merclaw.ai/automation/hooks#command-logger
metadata:
  {
    "merclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.merclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.merclaw/logs/commands.log ~/.merclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/merclaw`:
   ```
   /home/username/.merclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
