...
This directory contains hooks that ship with MerClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.merclaw/workspace`)
...
homepage: https://docs.merclaw.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.merclaw/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.merclaw/merclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'merclaw.*gateway' && pnpm merclaw gateway`
3. Enable the hook: `merclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.merclaw.ai/automation/hooks
