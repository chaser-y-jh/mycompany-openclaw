...
export function hasConfiguredInternalHooks(config: MerClawConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: MerClawConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
