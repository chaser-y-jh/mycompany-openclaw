...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "merclaw-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.MERCLAW_BUNDLED_HOOKS_DIR;
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.MERCLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.MERCLAW_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".merclaw", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): MerClawConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entry.hook.source).toBe("merclaw-plugin");
  ...
  it("does not treat Claude hooks.json bundles as MerClaw hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".merclaw", "extensions", "claude-bundle");
