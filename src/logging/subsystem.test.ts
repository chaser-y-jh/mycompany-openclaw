...
const logPathTracker = createSuiteLogPathTracker("merclaw-subsystem-log-");
...
describe("createSubsystemLogger().isEnabled", () => {
  ...
  it("keeps long-lived subsystem loggers on the current-day rolling file", () => {
    ...
    const firstDay = path.join(logDir, "merclaw-2026-01-01.log");
    const secondDay = path.join(logDir, "merclaw-2026-01-02.log");
    vi.useFakeTimers();
