...
const logPathTracker = createSuiteLogPathTracker("merclaw-log-redaction-");
const originalConfigPath = process.env.MERCLAW_CONFIG_PATH;
const originalHome = process.env.HOME;
const originalTestFileLog = process.env.MERCLAW_TEST_FILE_LOG;
...
describe("file log redaction", () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    expect(loggerTest.resolveActiveLogFile("~/custom-merclaw.log")).toBe(
      path.join(home, "custom-merclaw.log"),
    );
  });
