...
function resolveDiagnosticSessionStorePaths(config?: MerClawConfig): string[] | undefined {
  if (!config) {
    return undefined;
  }
  try {
...
function shouldWriteCriticalMemoryPressureBundle(config?: MerClawConfig): boolean {
...
export function resolveStuckSessionWarnMs(config?: MerClawConfig): number {
