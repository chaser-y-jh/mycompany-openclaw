...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-log-tail-redaction-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("readConfiguredLogTail redaction", () => {
  it("redacts raw auth headers before returning log lines", async () => {
    ...
    const logFile = path.join(dir, "merclaw.log");
    const configFile = path.join(dir, "merclaw.json");
    ...
    process.env.MERCLAW_CONFIG_PATH = configFile;
    ...
    expect(text).toContain("X-MerClaw-Token: supers…7890");
    ...
    expect(text).not.toContain(merClawToken);
