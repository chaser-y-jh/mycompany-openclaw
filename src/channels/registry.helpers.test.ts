...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://merclaw.ai");
  });

  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    const startupRegistry = createRegistryWithRegisteredChannel("merclaw-weixin", ["weixin"]);
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["merclaw-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("merclaw-weixin");
    expect(getRegisteredChannelPluginMeta("MERCLAW-WEIXIN")?.aliases).toEqual(["weixin"]);
  });
