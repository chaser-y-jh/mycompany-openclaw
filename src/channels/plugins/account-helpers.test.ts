...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): MerClawConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as MerClawConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: MerClawConfig) => string[];
  input: MerClawConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: MerClawConfig, expected: string) {
