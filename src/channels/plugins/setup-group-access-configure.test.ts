...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: MerClawConfig, policy: ChannelAccessPolicy) => MerClawConfig;
  resolveAllowlist: (params: { cfg: MerClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: MerClawConfig; resolved: TResolved }) => MerClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: MerClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: MerClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: MerClawConfig, policy: ChannelAccessPolicy): MerClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: MerClawConfig, policy: ChannelAccessPolicy): MerClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: MerClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: MerClawConfig; resolved: string[] }) => {
      calls.push("apply");
