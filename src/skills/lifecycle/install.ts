...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "merclaw");
  }
  return path.join(homedir(), ".merclaw");
}
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const trustedInstallSources = new Set(["merclaw-bundled", "merclaw-managed", "merclaw-extra"]);
