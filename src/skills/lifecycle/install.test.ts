...
import { resolveMerClawMetadata, resolveSkillInvocationPolicy } from "../loading/frontmatter.js";
...
metadata: {"merclaw":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
function loadTestWorkspaceSkillEntries(workspaceDir: string): SkillEntry[] {
  ...
  return skills.map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveMerClawMetadata(frontmatter),
      invocation,
      exposure: {
...
const workspaceSuite = createFixtureSuite("merclaw-skills-install-");
...
async function withWorkspaceCase(
  ...
  const envSnapshot = captureEnv(["MERCLAW_STATE_DIR"]);
  try {
...
describe("installSkill code safety scanning", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("MERCLAW_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an MerClaw-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["MERCLAW_STATE_DIR", "MERCLAW_CONFIG_PATH"]);
    try {
      process.env.MERCLAW_STATE_DIR = "/tmp/untrusted-state";
      process.env.MERCLAW_CONFIG_PATH = "/tmp/untrusted-config/merclaw.json";
  ...
  it("surfaces plugin scanner findings from before_install", async () => {
    ...
    await withWorkspaceCase(async ({ workspaceDir }) => {
      ...
      expect(payload?.origin).toBe("merclaw-workspace");
