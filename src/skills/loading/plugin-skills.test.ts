    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "merclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "merclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "merclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("merclaw-");
  const acpxRoot = await tempDirs.make("merclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("merclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("merclaw-");
  const pluginRoot = await tempDirs.make("merclaw-plugin-");
  const outsideDir = await tempDirs.make("merclaw-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("merclaw-");
    const pluginRoot = await tempDirs.make("merclaw-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("merclaw-");
    const pluginRoot = await tempDirs.make("merclaw-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
