...
const TASK_REGISTRY_DELIVERY_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "merclaw.taskRegistry.deliveryRuntimeOverride",
);
const TASK_REGISTRY_CONTROL_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "merclaw.taskRegistry.controlRuntimeOverride",
);
  ...
  try {
    if (task.runtime !== "cli") {
      ...
      if (!childSessionKey) {
        ...
        // not have MerClaw child sessions to terminate. Cancellation clears
