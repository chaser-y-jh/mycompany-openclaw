...
function configureTaskRegistryMaintenanceRuntimeForTest(params: {
  ...
  setTaskRegistryMaintenanceRuntimeForTests({
    ...
    resolveCronJobsStorePath: () => "/tmp/merclaw-test-cron/jobs.json",
...
function createAcpSessionStoreEntry(params: {
  ...
  return {
    cfg: {} as never,
    storePath: "/tmp/merclaw-test-sessions.json",
...
async function withTaskRegistryTempDir<T>(
  ...
  return await withTempDir({ prefix: "merclaw-task-registry-" }, async (root) => {
...
describe("task-registry", () => {
  ...
  it("cancels childless codex-native tasks without routing through MerClaw subagent sessions", async () => {
    await withTaskRegistryTempDir(async (root) => {
      process.env.MERCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      const task = createTaskRecord({
