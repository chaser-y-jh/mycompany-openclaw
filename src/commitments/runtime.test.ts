...
function requireFirstEmbeddedAgentRequest(): {
  ...
  if (!call) {
    throw new Error("expected embedded MerClaw agent extraction request");
  }
  const [request] = call;
...
describe("commitment extraction runtime", () => {
  ...
  async function createConfig(): Promise<MerClawConfig> {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-commitment-runtime-"));
    tmpDirs.push(tmpDir);
    vi.stubEnv("MERCLAW_STATE_DIR", tmpDir);
    return {
      commitments: {
        enabled: true,
      },
