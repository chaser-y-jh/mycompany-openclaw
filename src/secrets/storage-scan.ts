...
import { isRecord as isJsonObject } from "@merclaw/normalization-core/record-coerce";
...
export function listAuthProfileStorePaths(config: MerClawConfig, stateDir: string): string[] {
...
function resolveActiveAgentDir(stateDir: string, env: NodeJS.ProcessEnv = process.env): string {
  const override = env.MERCLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
  if (override) {
