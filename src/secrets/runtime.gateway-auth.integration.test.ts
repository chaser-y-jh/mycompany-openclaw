...
describe("secrets runtime snapshot gateway-auth integration", () => {
  ...
  it(
    ...
    async () => {
      await withTempHome("merclaw-secrets-runtime-gateway-auth-reload-lkg-", async (home) => {
        ...
        await fs.writeFile(
          path.join(home, ".merclaw", "merclaw.json"),
          `${JSON.stringify(
            {
              gateway: {
                auth: {
        ...
        const persistedConfig = JSON.parse(
          await fs.readFile(path.join(home, ".merclaw", "merclaw.json"), "utf8"),
        ) as MerClawConfig;
