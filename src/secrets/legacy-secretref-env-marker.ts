...
function toCandidate(
  ...
  defaults: NonNullable<MerClawConfig["secrets"]>["defaults"] | undefined,
...
export function migrateLegacySecretRefEnvMarkers(config: MerClawConfig): {
  config: MerClawConfig;
  changes: string[];
} {
  ...
  const next = structuredClone(config) as MerClawConfig & Record<string, unknown>;
