...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: MerClawConfig): unknown =>
...
type SecretRegistryEntry = {
  id: string;
  configFile: "merclaw.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.MERCLAW_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_MERCLAW_COVERAGE_EXCLUSIONS = new Set([
...
function collectMerClawCoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "merclaw.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_MERCLAW_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForMerClawTarget(
...
async function expectMerClawCoverageBatchResolved(
  label: string,
  ...
  for (const [index, entry] of batch.entries()) {
    const envId = `MERCLAW_SECRET_TARGET_${entry.id}`;
    ...
    applyConfigForMerClawTarget(config, entry, envId, wildcardToken);
  }
...
const MERCLAW_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectMerClawCoverageEntries({ includePluginEntries: false }),
);
const MERCLAW_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
  collectMerClawCoverageEntries({ includePluginEntries: true }),
);
...
describe("secrets runtime target coverage", () => {
  beforeAll(async () => {
    ...
    const googleChatBatch = MERCLAW_CORE_COVERAGE_BATCHES.find((batch) =>
      batch.some((entry) => entry.id === "channels.googlechat.serviceAccount"),
    );
    if (googleChatBatch) {
      await expectMerClawCoverageBatchResolved("merclaw.json core", googleChatBatch);
    }
  });

  describe("merclaw.json core and channel registry targets", () => {
    test.each(MERCLAW_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectMerClawCoverageBatchResolved("merclaw.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("merclaw.json plugin registry targets", () => {
    test.each(MERCLAW_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectMerClawCoverageBatchResolved("merclaw.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = `MERCLAW_AUTH_SECRET_TARGET_${entry.id}`;
        ...
        const snapshot = await prepareAuthCoverageSnapshot({
          ...
          env,
          agentDirs: ["/tmp/merclaw-agent-main"],
