...
import { mergeInboundPathRoots } from "@merclaw/media-core/inbound-path-policy";
...
async function probeAntigravityCliCandidate(command: string): Promise<string | null> {
  ...
  const probeDir = await fs.mkdtemp(
    path.join(resolvePreferredMerClawTmpDir(), "merclaw-antigravity-probe-"),
  );
  try {
...
async function resolveAntigravityCliBinary(): Promise<string | null> {
  ...
  const resolved = (async () => {
    const configured = process.env.MERCLAW_ANTIGRAVITY_CLI?.trim();
