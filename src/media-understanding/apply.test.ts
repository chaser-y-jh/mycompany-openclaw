...
const TEMP_MEDIA_PREFIX = "merclaw-media-";
...
function createGroqAudioConfig(): MerClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): MerClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): MerClawConfig {
  return {
  ...
  return await withEnvAsync(
    {
      ...
      MERCLAW_ANTIGRAVITY_CLI: undefined,
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: MerClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredMerClawTmpDir();
  ...
  it("uses Antigravity CLI as the last auto image fallback", async () => {
    ...
    const cfg: MerClawConfig = { tools: { media: { image: {} } } };
    mockedResolveApiKey.mockResolvedValue({
      source: "none",
      mode: "api-key",
    });
    ...
    expect(probeOptions).toEqual({
      timeoutMs: 3000,
      cwd: expect.stringContaining("merclaw-antigravity-probe-"),
    });
