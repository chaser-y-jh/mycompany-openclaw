...
export const MERCLAW_WRAPPER_ENV_KEY = "MERCLAW_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/merclaw -> .pnpm/merclaw@X.Y.Z/...
...
export async function resolveMerClawWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${MERCLAW_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveMerClawWrapperPath(params.wrapperPath);
  if (wrapperPath) {
