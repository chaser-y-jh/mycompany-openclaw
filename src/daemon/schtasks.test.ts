...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when MERCLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".merclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when MERCLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", MERCLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".merclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers MERCLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        MERCLAW_PROFILE: "rescue",
        MERCLAW_STATE_DIR: "C:\\State\\merclaw",
      },
      expected: path.join("C:\\State\\merclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", MERCLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".merclaw", "gateway.cmd"),
    },
    {
      name: "uses a custom task script file name inside the state directory",
      env: {
        USERPROFILE: "C:\\Users\\test",
        MERCLAW_TASK_SCRIPT_NAME: "gateway-node.cmd",
      },
      expected: path.join("C:\\Users\\test", ".merclaw", "gateway-node.cmd"),
    },
  ])("$name", ({ env, expected }) => {
  ...
  it.each([
    ...
    expect(() =>
      resolveTaskScriptPath({
        USERPROFILE: "C:\\Users\\test",
        MERCLAW_TASK_SCRIPT_NAME: scriptName,
      }),
    ).toThrow("MERCLAW_TASK_SCRIPT_NAME must be a file name only");
  });
});
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem MerClaw Gateway",
          "cd /d C:\\Projects\\merclaw",
          "set NODE_ENV=production",
          "set MERCLAW_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\merclaw",
          environment: {
            NODE_ENV: "production",
            MERCLAW_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\merclaw\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\merclaw\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\MerClaw Share\\node.exe" "\\\\fileserver\\MerClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\MerClaw Share\\node.exe",
            "\\\\fileserver\\MerClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from MERCLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ MERCLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
