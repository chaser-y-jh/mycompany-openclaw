...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `merclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      MERCLAW_LAUNCHD_LABEL: `ai.merclaw.launchd-int-${testId}`,
      MERCLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
