...
const MERCLAW_UPDATE_LAUNCHD_LABEL_PREFIX = "ai.merclaw.update.";

export type StaleMerClawUpdateLaunchdJob = {
  label: string;
  pid?: number;
  lastExitStatus?: number;
};

function normalizeMerClawUpdateLaunchdLabel(label: unknown): string | null {
  if (typeof label !== "string") {
    return null;
  }
  const trimmed = label.trim();
  return trimmed.startsWith(MERCLAW_UPDATE_LAUNCHD_LABEL_PREFIX) ? trimmed : null;
}

function isCurrentGatewayLaunchdLabel(label: string, env: NodeJS.ProcessEnv): boolean {
  const gatewayProfileLabel = resolveGatewayLaunchAgentLabel(env.MERCLAW_PROFILE);
  if (label === gatewayProfileLabel) {
    return true;
  }
  if (
    env.MERCLAW_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER ||
    env.MERCLAW_SERVICE_KIND?.trim() !== GATEWAY_SERVICE_KIND
  ) {
    return false;
  }
  const configuredLabel = env.MERCLAW_LAUNCHD_LABEL?.trim();
  return Boolean(configuredLabel && label === configuredLabel);
}

export function isMerClawUpdateLaunchdLabel(label: unknown): label is string {
  return normalizeMerClawUpdateLaunchdLabel(label) !== null;
}

function resolveCurrentMerClawUpdateLaunchdJobLabel(
  ...
  for (const label of [
    ...
    env.MERCLAW_LAUNCHD_LABEL,
  ]) {
    const normalized = normalizeMerClawUpdateLaunchdLabel(label);
    if (normalized) {
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.MERCLAW_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.MERCLAW_PROFILE));
}
...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by MerClaw. Do not edit while the gateway service is installed.",
...
export function parseLaunchctlListMerClawUpdateJobs(
  output: string,
): StaleMerClawUpdateLaunchdJob[] {
  const jobs: StaleMerClawUpdateLaunchdJob[] = [];
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    if (!label.startsWith(MERCLAW_UPDATE_LAUNCHD_LABEL_PREFIX)) {
      continue;
    }
...
export async function findStaleMerClawUpdateLaunchdJobs(): Promise<
  StaleMerClawUpdateLaunchdJob[]
> {
  ...
  return parseLaunchctlListMerClawUpdateJobs(result.stdout);
}

export async function removeMerClawUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeMerClawUpdateLaunchdLabel(label);
...
export async function disableMerClawUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeMerClawUpdateLaunchdLabel(label);
  if (process.platform !== "darwin" || !normalizedLabel) {
    return false;
  }
...
export async function disableCurrentMerClawUpdateLaunchdJob(
  env: NodeJS.ProcessEnv = process.env,
): Promise<boolean> {
  const label = resolveCurrentMerClawUpdateLaunchdJobLabel(env);
  if (!label) {
    return false;
  }
  return await disableMerClawUpdateLaunchdJob(label);
}
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromServiceEnv = parseTcpPort(command?.environment?.MERCLAW_GATEWAY_PORT ?? "");
  if (fromServiceEnv !== null) {
    return fromServiceEnv;
  }
  return parseTcpPort(env.MERCLAW_GATEWAY_PORT ?? "");
}
...
function throwBootstrapGuiSessionError(params: {
  ...
  throw new Error(
    [
      ...
      "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.merclaw.ai/gateway",
    ].join("\n"),
  );
}
...
export async function stopLaunchAgent({
  ...
  if (!persistDisable) {
    ...
    // `merclaw gateway start` re-enables cleanly without a manual `launchctl enable`.
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.MERCLAW_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "merclaw gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "merclaw gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `merclaw gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    await bootstrapLaunchAgentOrThrow({
      ...
      serviceTarget,
      plistPath,
      actionHint: "merclaw gateway restart",
    });
