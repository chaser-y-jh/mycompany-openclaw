...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevMerClawHome = process.env.MERCLAW_HOME;
    const prevHome = process.env.HOME;
    process.env.MERCLAW_HOME = "/srv/merclaw-home";
    process.env.HOME = "/home/other";
    const merClawHome = path.join(path.resolve("/srv/merclaw-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", merClawHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevMerClawHome === undefined) {
        delete process.env.MERCLAW_HOME;
      } else {
        process.env.MERCLAW_HOME = prevMerClawHome;
      }
