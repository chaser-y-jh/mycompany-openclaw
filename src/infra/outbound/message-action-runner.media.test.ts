...
describe("runMessageAction media behavior", () => {
  ...
  describe("reply hydration", () => {
    ...
    // params as send. Before merclaw#79864 the runner only hydrated
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred MerClaw tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideMerClaw = path.join(os.tmpdir(), "outside-merclaw", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
