...
function candidateBinDirs(
  ...
  // subprocesses keep using the same Node/Bun the current MerClaw process is on.
  try {
  ...
  // Bundled macOS app: `merclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "merclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.MERCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "merclaw"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `merclaw`
...
/**
 * Best-effort PATH bootstrap so skills that require the `merclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureMerClawCliOnPath(opts: EnsureMerClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.MERCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.MERCLAW_PATH_BOOTSTRAPPED = "1";
