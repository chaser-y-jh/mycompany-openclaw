...
async function withRestartSentinelStateDir(run: () => Promise<void>): Promise<void> {
  ...
  try {
    await withTempDir({ prefix: "merclaw-sentinel-" }, async (tempDir) => {
...
describe("restart sentinel message dedup", () => {
  ...
  it("keeps profile-aware doctor guidance actionable outside constrained delivery surfaces", () => {
    expect(
      formatDoctorNonInteractiveHint({
        MERCLAW_PROFILE: "isolated",
        PATH: "/usr/bin:/bin",
      }),
    ).toBe(
      "Recommended follow-up: run merclaw --profile isolated doctor --non-interactive in a terminal or approvals-capable MerClaw surface.",
    );
  });
});
