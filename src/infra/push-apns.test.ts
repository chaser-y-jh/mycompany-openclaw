...
function createRelayApnsSendFixture(params: {
  ...
  return {
    ...
    relayConfig: {
      baseUrl: "https://relay.merclaw.test",
      timeoutMs: 2_500,
    },
    gatewayIdentity: {
...
describe("push APNs send semantics", () => {
  it("sends alert pushes with alert headers and payload", async () => {
    ...
    const merclawPayload = requireRecord(payload.merclaw, "merclaw payload");
    expectRecordFields(merclawPayload, {
      kind: "push.test",
      nodeId: "ios-node-alert",
    });
    expect(typeof merclawPayload.ts).toBe("number");
  ...
  it("routes direct APNs HTTP/2 requests through the active managed proxy", async () => {
    ...
    try {
      ...
      expect(request?.headers["apns-topic"]).toBe("ai.merclaw.ios");
  ...
  it("sends exec approval alert pushes with generic modal-only metadata", async () => {
    ...
    expect(payload.aps).toEqual({
      alert: {
        ...
        body: "Open MerClaw to review this request.",
      },
      sound: "default",
      category: "merclaw.exec-approval",
      "content-available": 1,
    });
    const merclawPayload = requireRecord(payload.merclaw, "merclaw payload");
    expectRecordFields(merclawPayload, {
    ...
    expect(typeof merclawPayload.ts).toBe("number");
    expectNoProperties(merclawPayload, [
      "host",
      "nodeId",
      "agentId",
      "commandText",
  ...
  it("defaults background wake reason when not provided", async () => {
    ...
    expectRecordFields(requireRecord(payload.merclaw, "merclaw payload"), {
      kind: "node.wake",
      reason: "node.invoke",
      nodeId: "ios-node-wake-default-reason",
    });
