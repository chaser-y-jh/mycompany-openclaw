...
const fixtureRootTracker = createSuiteTempRootTracker({
  prefix: "merclaw-npm-managed-root-",
});
...
describe("managed npm root", () => {
  ...
  it("syncs MerClaw-owned overrides without dropping unrelated local overrides", async () => {
    const npmRoot = await makeTempRoot();
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
  ...
  it("reads package-level npm overrides for managed plugin installs", async () => {
    await expect(readMerClawManagedNpmRootOverrides()).resolves.toEqual({
      axios: "1.16.0",
      "fast-uri": "3.1.2",
  ...
  it("resolves npm override dependency references from the host package manifest", async () => {
    ...
    await expect(readMerClawManagedNpmRootOverrides({ packageRoot })).resolves.toEqual({
      "managed-runtime": "3.1024.0",
      nested: {
        "optional-runtime": "2.0.0",
      ...
      `${JSON.stringify(
        {
          lockfileVersion: 3,
          packages: {
            "node_modules/@merclaw/discord": {
              version: "2026.5.2",
              resolved: "https://registry.npmjs.org/@merclaw/discord/-/discord-2026.5.2.tgz",
    ...
    const runCommand = vi.fn(async (_args: string[], optionsOrTimeout: number | CommandOptions) => {
      ...
      if (runCommand.mock.calls.length === 1) {
        return {
          code: 1,
          stdout: "",
          stderr: "npm ERR! notarget No matching version found for merclaw@2026.5.99-beta.1",
          signal: null,
          killed: false,
          termination: "exit" as const,
        };
  ...
  it("repairs stale managed merclaw peer state without dropping plugin packages", async () => {
    const npmRoot = await makeTempRoot();
    await fs.mkdir(path.join(npmRoot, "node_modules", "merclaw"), { recursive: true });
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
        {
    ...
    await fs.writeFile(
      path.join(npmRoot, "node_modules", "merclaw", "package.json"),
      `${JSON.stringify({ name: "merclaw", version: "2026.5.4" })}\n`,
    );
    await fs.mkdir(path.join(npmRoot, "node_modules", ".bin"), { recursive: true });
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "merclaw"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "merclaw.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "merclaw.ps1"), "ps1 shim");
    await fs.writeFile(
    ...
    await expect(repairManagedNpmRootMerClawPeer({ npmRoot, runCommand })).resolves.toBe(true);
    expect(runCommand).toHaveBeenCalledTimes(1);
    ...
    expect(lockfile.packages?.["node_modules/@merclaw/discord"]?.version).toBe("2026.5.4");
    expect(lockfile.dependencies?.merclaw).toBeUndefined();
    await expectPathMissing(path.join(npmRoot, "node_modules", "merclaw"));
    for (const binName of ["merclaw", "merclaw.cmd", "merclaw.ps1"]) {
  ...
  it("does not repair the active MerClaw host package in a root-managed install", async () => {
    const npmRoot = await makeTempRoot();
      ...
      `${JSON.stringify(
        {
          ...
          dependencies: {
            ...
            "@xdarkicex/merclaw-memory-libravdb": "1.4.69",
          },
        },
        null,
        2,
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({ name: "merclaw", version: "2026.5.12-beta.6" })}\n`,
    );
  ...
  it("scrubs managed ownership metadata without deleting a linked active host package", async () => {
    const npmRoot = await makeTempRoot();
    const hostPackageRoot = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-host-package-"));
    ...
    await fs.symlink(hostPackageRoot, path.join(npmRoot, "node_modules", "merclaw"), "dir");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "merclaw"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "merclaw.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "merclaw.ps1"), "ps1 shim");
    await fs.writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/@xdarkicex/merclaw-memory-libravdb": {
              version: "1.4.69",
            },
          },
          dependencies: {
    ...
    expect(runCommand).not.toHaveBeenCalled();
    await expect(fs.realpath(path.join(npmRoot, "node_modules", "merclaw"))).resolves.toBe(
    ...
    expect(lockfile.packages?.["node_modules/merclaw"]).toBeUndefined();
    expect(lockfile.packages?.["node_modules/@xdarkicex/merclaw-memory-libravdb"]?.version).toBe(
      "1.4.69",
    );
    expect(lockfile.dependencies?.merclaw).toBeUndefined();
    for (const binName of ["merclaw", "merclaw.cmd", "merclaw.ps1"]) {
