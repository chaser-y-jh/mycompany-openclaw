...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".merclaw");
...
function sharedMainOpsConfig(sharedStorePath: string): MerClawConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: MerClawConfig = OPS_WORK_CONFIG) {
