...
const stateDir = "/opt/merclaw/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/merclaw/state/sessions/../config.jsonl` resolves to
    // `/opt/merclaw/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("normalizes Windows-style separators before anchor checks", () => {
    const winStateDir = "C:\\merclaw\\state";
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/merclaw/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
