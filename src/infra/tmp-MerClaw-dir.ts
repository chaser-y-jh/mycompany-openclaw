...
export const POSIX_MERCLAW_TMP_DIR = "/tmp/merclaw";
...
export type ResolvePreferredMerClawTmpDirOptions = {
...
export function resolvePreferredMerClawTmpDir(
  options: ResolvePreferredMerClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "merclaw" : `merclaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[merclaw] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback MerClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback MerClaw temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_MERCLAW_TMP_DIR;
