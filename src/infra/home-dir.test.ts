...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers MERCLAW_HOME over HOME and USERPROFILE",
      env: {
  ...
  it("expands MERCLAW_HOME when set to ~", () => {
    const env = {
      MERCLAW_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for MERCLAW_HOME",
      env: { MERCLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when MERCLAW_HOME is tilde-only and no fallback home exists",
      env: { MERCLAW_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores MERCLAW_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          MERCLAW_HOME: "/srv/merclaw-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/merclaw-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/merclaw-home " },
      expected: "/srv/merclaw-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/merclaw-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of MERCLAW_HOME", () => {
    expect(
