...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/merclaw-state", "devices")).toEqual({
      dir: path.join("/tmp/merclaw-state", "devices"),
      pendingPath: path.join("/tmp/merclaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/merclaw-state", "devices", "paired.json"),
    });
  });
