...
import { readRootJsonObjectSync } from "@merclaw/fs-safe/json";
...
export function installedPackageNeedsMerClawPeerLinkRepair(dir: string): boolean {
  ...
  try {
    fsSync.statSync(path.join(dir, "node_modules", "merclaw"));
    return false;
  } catch (error) {
