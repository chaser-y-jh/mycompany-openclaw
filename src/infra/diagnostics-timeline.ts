...
const MERCLAW_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "merclaw.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.MERCLAW_DIAGNOSTICS)) &&
    typeof env.MERCLAW_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.MERCLAW_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: MERCLAW_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.MERCLAW_DIAGNOSTICS_RUN_ID ? { runId: env.MERCLAW_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.MERCLAW_DIAGNOSTICS_ENV ? { envName: env.MERCLAW_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.MERCLAW_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
