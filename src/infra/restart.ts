...
export function emitGatewayRestart(
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerMerClawRestart() uses schtasks to restart the gateway.
...
export function triggerMerClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.MERCLAW_SYSTEMD_UNIT,
      process.env.MERCLAW_PROFILE,
    );
  ...
  const label =
    process.env.MERCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.MERCLAW_PROFILE);
