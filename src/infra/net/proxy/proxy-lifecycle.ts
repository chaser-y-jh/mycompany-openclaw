...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    MERCLAW_PROXY_ACTIVE: process.env["MERCLAW_PROXY_ACTIVE"],
    MERCLAW_PROXY_LOOPBACK_MODE: process.env["MERCLAW_PROXY_LOOPBACK_MODE"],
    MERCLAW_PROXY_CA_FILE: process.env["MERCLAW_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["MERCLAW_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["MERCLAW_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["MERCLAW_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["MERCLAW_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or MERCLAW_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["MERCLAW_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["MERCLAW_PROXY_CA_FILE"],
  });
 ...
 * loopback CDP probe to a Chromium instance MerClaw spawned itself.
 *
