...
const SUPERVISOR_HINTS = {
  launchd: ["MERCLAW_LAUNCHD_LABEL"],
  systemd: ["MERCLAW_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["MERCLAW_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "MERCLAW_SERVICE_MARKER",
  "MERCLAW_SERVICE_KIND",
] as const;
...
function isCurrentGatewayLaunchdJob(env: NodeJS.ProcessEnv): boolean {
  const expectedLabel = resolveGatewayLaunchAgentLabel(env.MERCLAW_PROFILE);
  if (
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.MERCLAW_SERVICE_MARKER?.trim();
