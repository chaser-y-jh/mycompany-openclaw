...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__merclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "merclaw-root");
...
function setPackageRoot(root: string, name = "merclaw") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./merclaw-root.js").resolveMerClawPackageRootSync,
  asyncResolver: typeof import("./merclaw-root.js").resolveMerClawPackageRoot,
  opts: Parameters<typeof import("./merclaw-root.js").resolveMerClawPackageRootSync>[0],
...
vi.mock("./merclaw-root.fs.runtime.js", () => ({
  merClawRootFsSync: mockFsModule(),
  merClawRootFs: mockFsPromisesModule(),
}));

describe("resolveMerClawPackageRoot", () => {
  let resolveMerClawPackageRoot: typeof import("./merclaw-root.js").resolveMerClawPackageRoot;
  let resolveMerClawPackageRootSync: typeof import("./merclaw-root.js").resolveMerClawPackageRootSync;
  let clearMerClawPackageRootCaches: typeof import("./merclaw-root.js").testing.clearMerClawPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveMerClawPackageRoot,
      resolveMerClawPackageRootSync,
      testing: { clearMerClawPackageRootCaches },
    } = await import("./merclaw-root.js"));
  });

  beforeEach(() => {
    clearMerClawPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "merclaw");
        const pkgRoot = path.join(project, "node_modules", "merclaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "merclaw");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "merclaw.mjs")));
    ...
    {
      name: "falls through from a non-merclaw moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-merclaw");
    ...
    {
      name: "returns null for non-merclaw package roots",
      setup: () => {
        const pkgRoot = fx("not-merclaw");
        setPackageRoot(pkgRoot, "not-merclaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "merclaw.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "merclaw");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
