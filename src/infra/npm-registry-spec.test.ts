...
import {
  ...
  isMerClawOrgNpmSpec,
  isMerClawStableCorrectionVersion,
...
describe("npm registry spec validation", () => {
  it.each([
    "@merclaw/voice-call",
    "@merclaw/voice-call@1.2.3",
    "@merclaw/voice-call@1.2.3-beta.4",
    "@merclaw/voice-call@latest",
    "@merclaw/voice-call@beta",
  ])("accepts %s", (spec) => {
  ...
  it.each([
    ...
    {
      spec: "git+ssh://github.com/merclaw/merclaw",
      expected: "URLs are not allowed",
    },
    {
...
describe("npm registry spec parsing helpers", () => {
  it.each([
    ...
    {
      spec: "@merclaw/voice-call@2026.5.3-1",
      expected: {
        name: "@merclaw/voice-call",
        raw: "@merclaw/voice-call@2026.5.3-1",
        selector: "2026.5.3-1",
    ...
    {
      spec: "@merclaw/voice-call@1.2.3-beta.1",
      expected: {
        name: "@merclaw/voice-call",
        raw: "@merclaw/voice-call@1.2.3-beta.1",
  ...
  it.each([
    { spec: "@merclaw/voice-call", expected: true },
    { spec: "@merclaw/voice-call@1.2.3", expected: true },
    { spec: "@other/voice-call", expected: false },
    { spec: "voice-call", expected: false },
    { spec: "npm:@merclaw/voice-call", expected: false },
    { spec: undefined, expected: false },
  ])("detects MerClaw-org npm specs for %s", ({ spec, expected }) => {
    expect(isMerClawOrgNpmSpec(spec)).toBe(expected);
  });
  ...
  ])("detects MerClaw stable correction versions for %s", ({ value, expected }) => {
    expect(isMerClawStableCorrectionVersion(value)).toBe(expected);
  });
  ...
  ])("compares MerClaw release versions for %s and %s", ({ left, right, expected }) => {
    expect(compareMerClawReleaseVersions(left, right)).toBe(expected);
  });
});
...
describe("npm prerelease resolution policy", () => {
  it.each([
    ...
    {
      spec: "@merclaw/voice-call@latest",
  ...
  it.each([
    {
      spec: "@merclaw/voice-call",
      resolvedVersion: "1.2.3-beta.1",
      expected: `Use "@merclaw/voice-call@beta"`,
    },
    {
      spec: "@merclaw/voice-call@beta",
      resolvedVersion: "1.2.3-rc.1",
