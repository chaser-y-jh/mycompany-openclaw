...
vi.mock("@merclaw/fs-safe/config", () => ({
...
describe("fs-safe defaults", () => {
  afterEach(() => {
    ...
    delete process.env.MERCLAW_FS_SAFE_PYTHON_MODE;
  });

  it("disables the Python helper by default in MerClaw", async () => {
  ...
  it("honors the MerClaw-specific env mode override", async () => {
    process.env.MERCLAW_FS_SAFE_PYTHON_MODE = "auto";
