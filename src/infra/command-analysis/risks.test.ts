...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "MERCLAW_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "merclaw", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("merclaw channels login --channel whatsapp");
  });
