...
function listAgents(cfg: MerClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: MerClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<MerClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: MerClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: MerClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: MerClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<MerClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  MerClawConfig,
  {
    bindingsRef: MerClawConfig["bindings"];
    agentsRef: MerClawConfig["agents"];
    sessionRef: MerClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: MerClawConfig): Map<string, ResolvedAgentRoute> {
