...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createMerClawCodingToolsMock: UnknownMock;
...
vi.mock("../../docs-path.js", () => ({
  resolveMerClawReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../agent-tools.js", () => ({
  createMerClawCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createMerClawCodingToolsMock(options),
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("merclaw.cache-ttl", data),
  ...
  readLastCacheTtlTimestamp: (
    ...
    for (let index = calls.length - 1; index >= 0; index -= 1) {
      ...
      if (customType !== "merclaw.cache-ttl") {
        continue;
      }
      const entry = data as
        | {
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createMerClawCodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.MERCLAW_TRAJECTORY;
  if (params.trajectory !== true) {
    process.env.MERCLAW_TRAJECTORY = "0";
  }
  try {
    return await (
      await loadRunEmbeddedAttempt()
    ...
    if (previousTrajectoryEnv === undefined) {
      delete process.env.MERCLAW_TRAJECTORY;
    } else {
      process.env.MERCLAW_TRAJECTORY = previousTrajectoryEnv;
    }
  }
}
