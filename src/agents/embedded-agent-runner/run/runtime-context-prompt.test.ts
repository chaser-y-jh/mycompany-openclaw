...
describe("runtime context prompt submission", () => {
  ...
  it("ignores repeated inline marker mentions without recursive stack growth", () => {
    const inlineMarkers = Array.from(
      { length: 250 },
      () => "inline <<<BEGIN_MERCLAW_INTERNAL_CONTEXT>>> marker",
    ).join("\n");
    const effectivePrompt = [
      inlineMarkers,
      "",
    ...
    expect(parts.modelPrompt).toContain("inline <<<BEGIN_MERCLAW_INTERNAL_CONTEXT>>> marker");
  ...
  it("uses a marker prompt for runtime-only events", () => {
    ...
    expect(parts).toEqual({
      prompt: "Continue the MerClaw runtime event.",
      runtimeContext: "internal event",
      runtimeOnly: true,
      runtimeSystemContext: [
        "MerClaw runtime event.",
  ...
  it("submits empty-transcript model prompts when persistence is suppressed separately", () => {
    expect(
      resolveRuntimeContextPromptParts({
        effectivePrompt: "[MerClaw room event]",
        transcriptPrompt: "",
  ...
  it("labels runtime-only events as system context", async () => {
    ...
    expect(buildRuntimeEventSystemContext("internal event")).toContain("MerClaw runtime event.");
