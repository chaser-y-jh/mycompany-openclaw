...
function stripUnsafeBlockedRunMetadata(messages: AgentMessage[]): AgentMessage[] {
  ...
  const nextMessages = messages.map((message) => {
    const merclaw = (message as unknown as Record<string, unknown>)["__merclaw"];
    if (!merclaw || typeof merclaw !== "object") {
      return message;
    }
    const beforeAgentRunBlocked = (merclaw as { beforeAgentRunBlocked?: unknown })
    ...
    const nextMerClaw = {
      ...(merclaw as Record<string, unknown>),
      beforeAgentRunBlocked: safeBlocked,
    };
    changed = true;
    return {
      ...(message as unknown as Record<string, unknown>),
      __merclaw: nextMerClaw,
