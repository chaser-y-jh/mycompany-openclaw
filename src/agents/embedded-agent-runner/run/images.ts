...
function isMerClawCliImageCachePath(filePath: string): boolean {
  const parts = filePath.replaceAll("\\", "/").split("/");
  return parts.some((part, index) => {
    if (part === ".merclaw-cli-images") {
      return true;
    }
    const parent = parts[index - 1] ?? "";
    return part === "merclaw-cli-images" && /^merclaw(?:-\d+)?$/.test(parent);
  });
}
...
export function detectImageReferences(prompt: string): DetectedImageRef[] {
  ...
  const addPathRef = (raw: string) => {
    ...
    if (isMerClawCliImageCachePath(resolved)) {
      return;
    }
    seen.add(dedupeKey);
