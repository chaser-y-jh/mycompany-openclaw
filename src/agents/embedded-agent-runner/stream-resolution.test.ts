...
afterEach(() => {
  testing.resetMerClawNativeCodexResponsesStreamFnForTest();
});
...
describe("describeEmbeddedAgentStreamStrategy", () => {
  ...
  it("describes default Codex fallback as MerClaw native", () => {
    expect(
      describeEmbeddedAgentStreamStrategy({
        currentStreamFn: undefined,
        model: {
    ...
    ).toBe("merclaw-native-codex-responses");
  });
...
describe("resolveEmbeddedAgentStreamFn", () => {
  ...
  it("routes Codex responses fallbacks through MerClaw native transport", async () => {
    const nativeStreamFn = vi.fn(async (_model, context, options) => ({ context, options }));
    testing.setMerClawNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
      currentStreamFn: undefined,
      sessionId: "session-1",
      model: {
  ...
  it("routes MerClaw native OpenAI-compatible provider streams through boundary-aware transports", async () => {
  ...
  it("injects the resolved run api key into the MerClaw native Codex Responses fallback", async () => {
  ...
  it("falls back to authStorage when no resolved api key is available for MerClaw native fallback", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const authStorage = {
  ...
  it("forwards the run abort signal into the MerClaw native fallback when callers omit one", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    testing.setMerClawNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
  ...
  it("does not overwrite an explicit signal on the MerClaw native fallback path", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    const explicitSignal = new AbortController().signal;
    testing.setMerClawNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
  ...
  it("forwards the run signal on the sync MerClaw native fallback path without auth credentials", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    testing.setMerClawNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
  ...
  it("strips cache boundary markers on the MerClaw native fallback path", async () => {
