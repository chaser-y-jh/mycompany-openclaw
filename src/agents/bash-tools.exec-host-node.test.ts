...
describe("executeNodeHostCommand", () => {
  ...
  it("requires human approval when prepared shell payload has multiple commands", async () => {
    const chainPlan = {
      argv: ["/bin/sh", "-lc", "merclaw status; id"],
      cwd: "/tmp/work",
      commandText: `/bin/sh -lc "merclaw status; id"`,
      commandPreview: "merclaw status; id",
      agentId: "prepared-agent",
      sessionKey: "prepared-session",
    };
    parsePreparedSystemRunPayloadMock.mockReturnValue({
          ...
          ? [
              {
                resolution: null,
                argv: ["/bin/sh", "-lc", "merclaw status; id"],
                raw: `/bin/sh -lc "merclaw status; id"`,
              },
            ]
          : [
              {
  ...
  it("does not treat read-only suppression inspections as wrapper writes", async () => {
    const wrapperPlan = {
      argv: ["/bin/sh", "-lc", "merclaw config get security.audit.suppressions"],
      cwd: "/tmp/work",
      commandText: `/bin/sh -lc "merclaw config get security.audit.suppressions"`,
      commandPreview: "merclaw config get security.audit.suppressions",
      agentId: "prepared-agent",
      sessionKey: "prepared-session",
    };
    parsePreparedSystemRunPayloadMock.mockReturnValue({
        ...
        segments: [
          command.startsWith("/bin/sh")
            ? {
                resolution: null,
                argv: ["/bin/sh", "-lc", "merclaw config get security.audit.suppressions"],
                raw: `/bin/sh -lc "merclaw config get security.audit.suppressions"`,
              }
            : {
                resolution: null,
