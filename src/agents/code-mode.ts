...
function readCodeModeRawConfig(config?: MerClawConfig, agentId?: string): Record<string, unknown> {
...
export function resolveCodeModeConfig(config?: MerClawConfig, agentId?: string): CodeModeConfig {
...
export function createCodeModeTools(ctx: CodeModeToolContext): AnyAgentTool[] {
  ...
  const waitTool = markCodeModeControlTool({
    ...
    label: "wait",
    description: "Resume a suspended MerClaw code mode run returned by exec.",
    parameters: Type.Object({
