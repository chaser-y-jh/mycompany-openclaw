...
describe("provider transport stream contracts", () => {
    ...
    const cases = [
      {
        ...
        alias: "merclaw-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "merclaw-openai-completions-transport",
      },
      {
        ...
        alias: "merclaw-azure-openai-responses-transport",
      },
      {
        ...
        alias: "merclaw-anthropic-messages-transport",
      },
      {
        ...
        alias: "merclaw-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on MerClaw transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createMerClawTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the MerClaw simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("merclaw-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the MerClaw transport until MerClaw preserves attribution", () => {
