...
import { resolveOptionalMediaToolFactoryPlan } from "./merclaw-tools.media-factory-plan.js";
...
type CreateMerClawToolsOptions = Parameters<
  typeof import("./merclaw-tools.js").createMerClawTools
>[0];
let createMerClawToolsForTestModule: typeof import("./merclaw-tools.js").createMerClawTools;
let legacyComfyToolNames: string[];

async function createMerClawToolsForTest(options?: CreateMerClawToolsOptions) {
  return createMerClawToolsForTestModule(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/merclaw.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): MerClawConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  beforeAll(async () => {
    ({ createMerClawTools: createMerClawToolsForTestModule } =
      await import("./merclaw-tools.js"));
    ...
    legacyComfyToolNames = (
      await createMerClawToolsForTest({
        config,
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: MerClawConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createMerClawToolsForTest({
      config,
      agentDir: "/tmp/merclaw-agent-main",
  ...
  it("does not register the image tool without cheap vision availability evidence", async () => {
    const config: MerClawConfig = {};
    const workspaceDir = "/tmp/merclaw-workspace";
