...
const LIVE = isLiveTestEnabled(["MERCLAW_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "merclaw-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("merclaw_live_probe", "MerClaw Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an MerClaw-configured streamable-http server", async () => {
    const geminiCommand = process.env.MERCLAW_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("merclawLiveProbe");
