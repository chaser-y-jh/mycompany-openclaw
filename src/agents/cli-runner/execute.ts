...
const CLI_BACKEND_PRESERVE_ENV = "MERCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.MERCLAW_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.MERCLAW_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.MERCLAW_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.MERCLAW_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.MERCLAW_MCP_MESSAGE_CHANNEL || "<empty>"}`,
  ].join(" ");
}
...
export async function executePreparedCliRun(
  ...
  const basePrompt = cliSessionIdToUse
    ? params.prompt
    : (context.merClawHistoryPrompt ?? params.prompt);
      ...
      try {
        cliBackendLog.info(
          buildCliExecLogLine({
            ...
            hasHistoryPrompt: Boolean(context.merClawHistoryPrompt),
          }),
        );
        const logOutputText =
        ...
        if (logOutputText) {
          ...
          if (env.MERCLAW_MCP_TOKEN || env.MERCLAW_MCP_SESSION_KEY || env.MERCLAW_MCP_AGENT_ID) {
        ...
        if (result.exitCode !== 0 || result.reason !== "exit") {
          if (result.reason === "no-output-timeout" || result.noOutputTimedOut) {
            ...
            const deferWatchdogNoticeForFreshRetry =
              ...
              Boolean(context.merClawHistoryPrompt) &&
