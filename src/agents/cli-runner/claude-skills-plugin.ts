...
const MERCLAW_CLAUDE_PLUGIN_NAME = "merclaw-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredMerClawTmpDir(), "merclaw-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, MERCLAW_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: MERCLAW_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped MerClaw skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
