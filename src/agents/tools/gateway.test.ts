...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    merclaw: process.env.MERCLAW_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.merclaw === undefined) {
      delete process.env.MERCLAW_GATEWAY_TOKEN;
    } else {
      process.env.MERCLAW_GATEWAY_TOKEN = envSnapshot.merclaw;
    }
  });
  ...
  it("uses MERCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.MERCLAW_GATEWAY_TOKEN = "local-env-token";
    mocks.configState.value = {
      gateway: {
