...
async function createStoreConfig(): Promise<{ config: MerClawConfig; template: string }> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-goal-tools-"));
  const template = path.join(dir, "{agentId}", "sessions.json");
  return {
    config: { session: { store: template } } as MerClawConfig,
    template,
  };
}
