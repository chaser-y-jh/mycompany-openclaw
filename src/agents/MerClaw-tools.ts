...
} from "./merclaw-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./merclaw-tools.nodes-workspace-guard.js";
import {
  collectPresentMerClawTools,
...
type MerClawToolsDeps = {
  callGateway: typeof callGateway;
  config?: MerClawConfig;
};

const defaultMerClawToolsDeps: MerClawToolsDeps = {
  callGateway,
};

let merClawToolsDeps: MerClawToolsDeps = defaultMerClawToolsDeps;

export function createMerClawTools(
  options?: {
  ...
  const resolvedConfig = options?.config ?? merClawToolsDeps.config;
  ...
  options?.recordToolPrepStage?.("merclaw-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("merclaw-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("merclaw-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("merclaw-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("merclaw-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("merclaw-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("merclaw-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("merclaw-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("merclaw-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("merclaw-tools:nodes-tool");
  ...
  const effectiveCallGateway = embedded
    ...
    : merClawToolsDeps.callGateway;
  const includeUpdatePlanTool = shouldIncludeUpdatePlanToolForMerClawTools({
    config: resolvedConfig,
    agentSessionKey: options?.agentSessionKey,
    agentId: options?.requesterAgentIdOverride,
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentMerClawTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentMerClawTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...(embedded
      ...
      : [
          createSessionsSendTool({
            ...
            sandboxed: options?.sandboxed,
            config: resolvedConfig,
            callGateway: merClawToolsDeps.callGateway,
          }),
        ]),
    ...
    ...collectPresentMerClawTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("merclaw-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    options?.recordToolPrepStage?.("merclaw-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("merclaw-tools:tool-hooks");
...
export const testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<MerClawToolsDeps>) {
    merClawToolsDeps = overrides
      ? {
          ...defaultMerClawToolsDeps,
          ...overrides,
        }
      : defaultMerClawToolsDeps;
  },
