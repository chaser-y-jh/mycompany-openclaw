import { resolveIntegerOption } from "@merclaw/normalization-core/number-coercion";
...
const PERSISTED_DETAIL_REDACTION_BOUNDARY = "\u0000MERCLAW_PERSISTED_DETAIL_BOUNDARY\u0000";
...
function redactPersistedDetailString(
  ...
  const redactedPrefix =
    boundaryIndex >= 0
      ...
      : "[MerClaw persisted detail redacted: boundary marker removed]";
  ...
  const persistedPrefix =
    ...
    PARTIAL_PRIVATE_KEY_BLOCK_RE.test(initialPersistedPrefix)
      ? "[MerClaw persisted detail redacted: partial secret span omitted]"
      : initialPersistedPrefix;
  const boundaryNotice = "[MerClaw persisted detail redacted: boundary overlap omitted]";
  return `${persistedPrefix}${persistedPrefix ? "\n" : ""}${boundaryNotice}\n\n[MerClaw persisted detail truncated: ${Math.max(
    0,
...
function redactPersistedDetailValue(
  ...
  if (depth >= 8) {
    return "[MerClaw persisted detail redacted: max depth exceeded]";
  }
...
function isTranscriptOnlyMerClawAssistantMessage(message: AgentMessage): boolean {
  ...
  return provider === "merclaw" && (model === "delivery-mirror" || model === "gateway-injected");
}
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage) => {
    ...
    const transcriptOnlyAssistant =
      nextRole === "assistant" &&
      toolCalls.length === 0 &&
      isTranscriptOnlyMerClawAssistantMessage(nextMessage);
    if (
