...
let configOverride: MerClawConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: MerClawConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/merclaw/merclaw/issues/31298
    previousFastTestEnv = process.env.MERCLAW_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.MERCLAW_TEST_FAST = previousFastTestEnv;
  });
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // MERCLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("MerClaw runtime context (internal):");
