...
import { __setFsSafeTestHooksForTest } from "@merclaw/fs-safe/test-hooks";
...
const parseMerClawChannelsLoginShellCommand = testing.parseMerClawChannelsLoginShellCommand;
...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("merclaw-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive MerClaw channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", () => {
    expect(
      parseMerClawChannelsLoginShellCommand("merclaw channels login --channel whatsapp"),
    ).toBe(true);
    expect(
      parseMerClawChannelsLoginShellCommand(
        "pnpm exec merclaw channels login --channel whatsapp --verbose",
      ),
    ).toBe(true);
    expect(parseMerClawChannelsLoginShellCommand("merclaw channels status --deep")).toBe(false);
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-merclaw-channel-login", {
        command: "merclaw channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive MerClaw channel login commands/);
    await expect(
      tool.execute("call-wrapped-merclaw-channel-login", {
        command: "sudo -u merclaw bash -lc 'merclaw channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive MerClaw channel login commands/);
    await expect(
      tool.execute("call-clustered-sudo-channel-login", {
        command: "sudo -EH bash -lc 'merclaw channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive MerClaw channel login commands/);
    await expect(
      tool.execute("call-deep-env-channel-login", {
        command: "env env env env env env merclaw channels login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive MerClaw channel login commands/);
    await expect(
      tool.execute("call-env-s-trailing-channel-login", {
        command: "env -S 'merclaw channels' login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive MerClaw channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("merclaw-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("merclaw-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("merclaw-exec-preflight-nonblock-", async (tmp) => {
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("merclaw-exec-preflight-win-", async (tmp) => {
      await fs.writeFile(path.join(tmp, "bad.py"), "payload = $DM_JSON", "utf-8");
...
describe("exec interpreter heuristics ReDoS guard", () => {
  it("does not hang on long commands with VAR=value assignments and whitespace-heavy text", async () => {
    ...
    const command = `ACCESS_TOKEN=$(__merclaw_missing_redos_guard__)\ncat > /tmp/out.html << 'EOF'\n${htmlBlock}\nEOF`;
