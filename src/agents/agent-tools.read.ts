...
function resolveAdaptiveReadMaxBytes(options?: MerClawReadToolOptions): number {
...
function withWorkspaceSafeTempHint(error: unknown): unknown {
  ...
  const message = error.message.includes(".merclaw/tmp/")
    ? error.message
    : `${error.message}. Use a relative path under \`.merclaw/tmp/\` inside the workspace for scratch/temp/meta files that file tools need to read or write later.`;
...
export function createSandboxedReadTool(params: SandboxToolParams) {
  ...
  return createMerClawReadTool(base, {
...
export function createMerClawReadTool(
  base: AnyAgentTool,
  options?: MerClawReadToolOptions,
): AnyAgentTool {
  return {
    ...base,
