...
describe("isSilentOverflowProneModel", () => {
  ...
  // merclaw#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable MerClaw runtime's secondary compaction path;
  // MerClaw's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of merclaw#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering MerClaw runtime's
  ...
  // state.messages intact; MerClaw's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep MerClaw runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove MerClaw runtime's
