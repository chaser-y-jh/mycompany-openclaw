...
function resolveConfiguredAgentIds(cfg: MerClawConfig): string[] {
...
async function prepareSubagentSessionContext(params: {
  ...
  try {
    const forked = (await updateSubagentSessionStore(childTarget.storePath, async (store) => {
      ...
      if (!fork) {
        throw new Error(
          'context="fork" requested but MerClaw could not fork the requester transcript.',
        );
      }
      pruneLegacyStoreKeys({
        store,
    ...
    if (params.contextMode === "fork") {
      if (!parentEntry || !forked) {
        ...
        return {
          status: "error",
          error: 'context="fork" requested but MerClaw could not prepare forked context.',
        };
      }
      return {
        status: "ok",
