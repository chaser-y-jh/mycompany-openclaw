...
export async function markRestartAbortedMainSessions(params: {
  cfg?: MerClawConfig;
  additionalCfgs?: Iterable<MerClawConfig | undefined>;
  stateDir?: string;
  ...
  const env =
    params.stateDir === undefined
      ? process.env
      : { ...process.env, MERCLAW_STATE_DIR: params.stateDir };
  const stateDir = resolveStateDir(env);
  const configs = [params.cfg, ...(params.additionalCfgs ?? [])].filter(
    (cfg): cfg is MerClawConfig => Boolean(cfg),
  );
...
function buildResumeMessage(pendingFinalDeliveryText?: string | null): string {
  const base =
    ...
    "MerClaw was waiting on tool/model work. Continue from the existing " +
