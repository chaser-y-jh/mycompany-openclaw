...
import "./test-helpers/fast-merclaw-tools.js";
import { createMerClawCodingTools } from "./agent-tools.js";
import type { AuthProfileStore } from "./auth-profiles/types.js";
import * as merClawPluginTools from "./merclaw-plugin-tools.js";
...
function createToolsForStoredSession(storeTemplate: string, sessionKey: string) {
  return createMerClawCodingTools({
    sessionKey,
    config: {
      session: {
        store: storeTemplate,
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createMerClawCodingTools>) {
...
type MerClawCodingTool = ReturnType<typeof createMerClawCodingTools>[number];
type MerClawToolsOptions = NonNullable<Parameters<typeof createMerClawTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: MerClawCodingTool[], name: string): MerClawCodingTool {
...
function requireToolExecute(tool: MerClawCodingTool): NonNullable<MerClawCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateMerClawToolsOptions(): MerClawToolsOptions {
  const calls = vi.mocked(createMerClawTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createMerClawTools call");
  }
  return options;
}
...
describe("createMerClawCodingTools", () => {
  ...
  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createMerClawCodingTools({ config: testConfig });
  ...
  it("passes explicit hook channel ids to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-hook-channel-"));
  ...
  it("keeps Tool Search controls when core MerClaw tools are not materialized", () => {
    const createMerClawToolsMock = vi.mocked(createMerClawTools);
    createMerClawToolsMock.mockClear();
    ...
    expect(createMerClawToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createMerClawToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateMerClawToolsOptions();
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:merclaw"], ["*"]]) {
      const tools = createMerClawCodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("passes source reply delivery mode to MerClaw tool construction", () => {
    const createMerClawToolsMock = vi.mocked(createMerClawTools);
    createMerClawToolsMock.mockClear();
    ...
    expect(createMerClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateMerClawToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it("passes plugin suppression into MerClaw tool construction plans", () => {
    const createMerClawToolsMock = vi.mocked(createMerClawTools);
    createMerClawToolsMock.mockClear();
    ...
    expect(createMerClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateMerClawToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the MerClaw core factory", () => {
    const createMerClawToolsMock = vi.mocked(createMerClawTools);
    createMerClawToolsMock.mockClear();
  ...
  it("forwards active model metadata to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(merClawPluginTools, "resolveMerClawPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createMerClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateMerClawToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
      DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY,
    ]);
  });

  it("passes explicit denylist entries to MerClaw tool factory planning", () => {
    const createMerClawToolsMock = vi.mocked(createMerClawTools);
    createMerClawToolsMock.mockClear();
    ...
    expect(createMerClawToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateMerClawToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to MerClaw plugin discovery", async () => {
    const createMerClawToolsMock = vi.mocked(createMerClawTools);
    createMerClawToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `merclaw-session-store-${agentId}-{agentId}.json`,
    );
    ...
    expect(createMerClawToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateMerClawToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes effective allow-list-restricted tool surface to spawned sessions", () => {
    ...
    expect(createMerClawToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateMerClawToolsOptions().inheritedToolAllowlist;
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("merclaw-tools:test-helper")).toBeLessThan(
      stages.indexOf("merclaw-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createMerClawCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createMerClawCodingTools({
      config: testConfig,
    ...
    const codexTools = createMerClawCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: MerClawConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createMerClawCodingTools({
    ...
    const anthropicTools = createMerClawCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: MerClawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createMerClawCodingTools({
    ...
    const denied = createMerClawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createMerClawCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createMerClawCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createMerClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createMerClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createMerClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createMerClawCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createMerClawCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createMerClawCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createMerClawCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createMerClawCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createMerClawCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-read-"));
    try {
      ...
      const contents = "Hello from merclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "merclaw-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "merclaw-workspace"),
    ...
    const tools = createMerClawCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-canonical-"));
    try {
      const tools = createMerClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-memory-cwd-"));
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-legacy-alias-"));
    try {
      const tools = createMerClawCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-structured-write-"));
    try {
      const tools = createMerClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-structured-edit-"));
    try {
