...
function createClaudeCliOverrideConfig(config: CliBackendConfig): MerClawConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from MerClaw's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__merclaw__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__merclaw__*");
  ...
  it("keeps Claude permission mode unset when MerClaw exec policy is not YOLO", () => {
    const resolved = requireCliBackendConfig("claude-cli", {
