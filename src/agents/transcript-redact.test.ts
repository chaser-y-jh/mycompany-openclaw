...
function cfg(mode: "tools" | "off", patterns?: string[]): MerClawConfig {
  return {
    logging: {
...
describe("redactTranscriptMessage", () => {
  ...
  it("redacts nested strings in assistant tool-call arguments", () => {
    ...
    expect(argumentsValue.command).toBe("OPENAI_API_KEY=sk-abc…0xyz merclaw health");
    expect(argumentsValue.env.nested[0]).toBe("token sk-abc…0xyz");
    expect(argumentsValue.count).toBe(1);
    expect(serializedArguments).toContain("merclaw health");
  ...
  it("redacts structured tool-use input payloads", () => {
    ...
    expect(inputValue.command).toBe("OPENAI_API_KEY=sk-abc…0xyz merclaw health");
