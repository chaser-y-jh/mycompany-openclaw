...
const GOOGLE_LIVE_SINGLE_KEY = "MERCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "MERCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "MERCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "MERCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `MERCLAW_LIVE_${base}_KEY`;
