...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses MERCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MERCLAW_HOME: "/srv/merclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/merclaw-home"), ".merclaw", "workspace"));
  });

  it("prefers MERCLAW_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MERCLAW_WORKSPACE_DIR: "/srv/merclaw-workspace",
      MERCLAW_HOME: "/srv/merclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/merclaw-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".merclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("merclaw-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-workspace-hardlink-"));
    try {
