...
type SkillDiscoveryMode = "merclaw" | "agents";
...
function readResourceManifestFile(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    const pkg = JSON.parse(content) as { merclaw?: ResourceManifest };
    return pkg.merclaw ?? null;
  } catch {
    return null;
  }
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
  if (resourceType === "skills") {
    return collectSkillEntries(dir, "merclaw");
  }
...
export class DefaultPackageManager implements PackageManager {
  ...
  private getTemporaryDir(prefix: string, suffix?: string): string {
    const hash = createHash("sha256")
      ...
      .digest("hex")
      .slice(0, 8);
    return join(tmpdir(), "merclaw-resources", prefix, hash, suffix ?? "");
  }
  ...
  private addAutoDiscoveredResources(
    ...
    addResources(
      "skills",
      collectAutoSkillEntries(projectDirs.skills, "merclaw"),
      projectMetadata,
    ...
    // User extensions from ~/.merclaw/agent/
    addResources(
      "extensions",
    ...
    // User skills from ~/.merclaw/agent/
    addResources(
      "skills",
      collectAutoSkillEntries(userDirs.skills, "merclaw"),
      userMetadata,
