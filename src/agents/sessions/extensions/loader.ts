...
import * as bundledLlm from "merclaw/plugin-sdk/llm";
...
import { installMerClawInternalCorePackageNativeResolver } from "../../../plugins/plugin-sdk-native-resolver.js";
import {
...
const VIRTUAL_MODULES: Record<string, unknown> = {
  ...
  "merclaw/plugin-sdk/agent-core": bundledAgentCore,
  "@merclaw/plugin-sdk/agent-core": bundledAgentCore,
  "merclaw/plugin-sdk/llm": bundledLlm,
  "@merclaw/plugin-sdk/llm": bundledLlm,
  "merclaw/plugin-sdk/agent-sessions": bundledAgentSessions,
  "@merclaw/plugin-sdk/agent-sessions": bundledAgentSessions,
};
...
const EXTENSION_LOADER_ALIAS_IMPORT_PATTERN =
  /(?:@merclaw\/plugin-sdk|merclaw\/plugin-sdk|@sinclair\/typebox|typebox)(?:\/[A-Za-z0-9_-]+)?/u;
...
function getExtensionLoaderAliases(): Record<string, string> {
  ...
  aliases = {
    ...
    "merclaw/plugin-sdk/agent-sessions": agentSessionsEntry,
    "@merclaw/plugin-sdk/agent-sessions": agentSessionsEntry,
...
async function loadExtensionSourceTransformModule(
  ...
  if (!extensionSourceTransformLoader) {
    installMerClawInternalCorePackageNativeResolver({ moduleUrl: import.meta.url });
...
function readResourceManifest(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    if (pkg.merclaw && typeof pkg.merclaw === "object") {
      return pkg.merclaw as ResourceManifest;
    }
    return null;
  } catch {
    return null;
 ...
 *
 * Checks for:
 * 1. package.json with "merclaw.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
  // Check for package.json with "merclaw" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "merclaw" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
  ...
  for (const p of configuredPaths) {
    ...
    if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
      // Check for package.json with MerClaw manifest or index.ts
