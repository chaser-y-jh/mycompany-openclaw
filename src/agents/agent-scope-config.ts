...
export function listAgentEntries(cfg: MerClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: MerClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: MerClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: MerClawConfig, agentId: string): AgentEntry | undefined {
