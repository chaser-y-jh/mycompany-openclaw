...
const envKeys = ["MERCLAW_DISABLE_PERSISTED_PLUGIN_REGISTRY", "MERCLAW_STATE_DIR"] as const;
...
describe("provider auth warm worker", () => {
  ...
  it("preserves runtime-only auth profile snapshots in the worker warm input", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "merclaw-provider-auth-worker-"));
    tempDirs.push(root);
    ...
    process.env.MERCLAW_STATE_DIR = path.join(root, "state");
