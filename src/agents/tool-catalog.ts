...
function buildCoreToolGroupMap() {
  ...
  const merclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInMerClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:merclaw": merclawTools,
