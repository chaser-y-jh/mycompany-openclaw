...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-managed-media-"));
...
async function withOutsideHardlinkInMerClawTmp<T>(
  params: {
    merClawTmpDir: string;
  ...
  const hardlinkPath = path.join(params.merClawTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.merClawTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.merClawTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.merClawTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const merClawTmpDir = resolvePreferredMerClawTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred MerClaw tmp root",
      media: path.join(merClawTmpDir, "image.png"),
      expected: path.join(merClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred MerClaw tmp root",
      media: pathToFileURL(path.join(merClawTmpDir, "photo.png")).href,
      expected: path.join(merClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred MerClaw tmp root",
      media: path.join(merClawTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(merClawTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(merClawTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside merclaw tmp root",
      media: path.join(os.tmpdir(), "outside-merclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked MerClaw tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(merClawTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(merClawTmpDir, { recursive: true });
      const symlinkPath = path.join(merClawTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked MerClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInMerClawTmp(
      {
        merClawTmpDir,
  ...
  it("rejects symlinked MerClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInMerClawTmp(
