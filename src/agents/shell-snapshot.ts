...
const CAPTURE_MARKER = "__MERCLAW_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__MERCLAW_SHELL_SNAPSHOT_ENV__";
...
async function captureShellSnapshot(opts: ShellSnapshotWrapOptions): Promise<string | null> {
  ...
  const captureOutputDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-shell-snapshot-"));
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // MERCLAW_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.MERCLAW_SHELL === "exec") {
    env.MERCLAW_SHELL = "exec";
  }
  return env;
}
...
function buildSnapshotFile(stdout: string): string | null {
  ...
  return [
    "# MerClaw exec shell snapshot. Generated; do not edit.",
