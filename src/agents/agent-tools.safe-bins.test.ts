...
let createMerClawCodingTools: typeof import("./agent-tools.js").createMerClawCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      MERCLAW_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "merclaw-test-no-bundled-extensions"),
    },
    async () => {
      ({ createMerClawCodingTools } = await import("./agent-tools.js"));
    },
  );
});
...
vi.mock("merclaw/plugin-sdk/agent-sessions", () => ({
  AuthStorage: vi.fn(),
...
async function withSafeBinsExecTool(
  ...
  try {
    await withEnvAsync(
      {
        MERCLAW_SHELL_ENV_TIMEOUT_MS: "1",
        PATH: "/usr/bin:/bin",
        SHELL: "/bin/sh",
      },
      async () => {
...
describe("createMerClawCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "merclaw-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "merclaw-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "merclaw-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "merclaw-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "merclaw-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "merclaw-safe-bins-grep-",
        safeBins: ["grep"],
