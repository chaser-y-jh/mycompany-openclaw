...
describe("path-resolve helpers (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-path-direct-"));
  ...
  it("resolveAuthStorePath falls back to the default agent dir when agentDir is omitted", () => {
    ...
    // MERCLAW_STATE_DIR set to our tempdir, the resolved path must live under it.
  ...
  it("resolveAuthStorePathForDisplay preserves a tilde-rooted path unchanged", () => {
    ...
    const tildeAgentDir = "~fake-merclaw-no-expand";
...
describe("ensureAuthStoreFile (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-path-ensure-"));
    process.env.MERCLAW_STATE_DIR = stateDir;
  });
