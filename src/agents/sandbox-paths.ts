...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const merClawTmpDir = path.resolve(resolvePreferredMerClawTmpDir());
  if (!isPathInside(merClawTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: merClawTmpDir });
  return resolved;
}
