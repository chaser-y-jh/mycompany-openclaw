...
export function resolveCompactionReserveTokensFloor(cfg?: MerClawConfig): number {
...
export function resolveEffectiveCompactionMode(cfg?: MerClawConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers MerClaw runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (merclaw#75799).
 *
...
/**
 * Disable MerClaw runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when MerClaw or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (merclaw#75799).
 * Default-mode runs against ordinary providers keep MerClaw runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores MerClaw runtime's
