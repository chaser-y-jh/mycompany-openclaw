...
import type { AssistantMessage, Message, Tool } from "merclaw/plugin-sdk/llm";
...
describeCacheLive("embedded agent runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.MERCLAW_CACHE_TRACE,
      file: process.env.MERCLAW_CACHE_TRACE_FILE,
      messages: process.env.MERCLAW_CACHE_TRACE_MESSAGES,
      prompt: process.env.MERCLAW_CACHE_TRACE_PROMPT,
      system: process.env.MERCLAW_CACHE_TRACE_SYSTEM,
    };
    process.env.MERCLAW_CACHE_TRACE = "1";
    process.env.MERCLAW_CACHE_TRACE_FILE = liveCacheTraceFile;
    process.env.MERCLAW_CACHE_TRACE_MESSAGES = "0";
    process.env.MERCLAW_CACHE_TRACE_PROMPT = "0";
    process.env.MERCLAW_CACHE_TRACE_SYSTEM = "0";
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("MERCLAW_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("MERCLAW_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("MERCLAW_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("MERCLAW_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("MERCLAW_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "MERCLAW_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "MERCLAW_LIVE_ANTHROPIC_CACHE_MODEL",
