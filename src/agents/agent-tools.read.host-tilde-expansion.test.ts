...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("merclaw-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("merclaw-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("merclaw-tilde-test-mkdir-");
  ...
  it("ignores MERCLAW_HOME for write operations", async () => {
    const merclawHome = await createTempDir("merclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("merclaw-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("MERCLAW_HOME", merclawHome);

    createHostWorkspaceWriteTool(merclawHome, { workspaceOnly: false });
    ...
    await expectMissingPath(fs.access(path.join(merclawHome, path.basename(testFile))));
  });

  it("ignores MERCLAW_HOME for mkdir operations", async () => {
    const merclawHome = await createTempDir("merclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("merclaw-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expectMissingPath(fs.access(path.join(merclawHome, path.basename(newDir))));
  });

  it("ignores MERCLAW_HOME for readFile operations", async () => {
    const merclawHome = await createTempDir("merclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("merclaw-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(merclawHome, { workspaceOnly: false });
    const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores MERCLAW_HOME for access operations", async () => {
    const merclawHome = await createTempDir("merclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("merclaw-tilde-test-edit-");
