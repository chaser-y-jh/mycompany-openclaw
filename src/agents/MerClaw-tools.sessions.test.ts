...
import "./test-helpers/fast-merclaw-tools-sessions.js";
...
function createMerClawTools(options?: {
...
describe("sessions tools", () => {
  ...
  it.each([
    ...
    const tool = createMerClawTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
  ...
  it("sessions_list forwards mailbox filters and includes messages", async () => {
    ...
    const tool = createMerClawTools().find((candidate) => candidate.name === "sessions_list");
    if (!tool) {
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-sessions-list-preview-"));
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createMerClawTools().find((candidate) => candidate.name === "sessions_history");
    if (!tool) {
  ...
  it("sessions_history caps oversized payloads and strips heavy fields", async () => {
    ...
    expect(thinkingBlock?.merclawReasoningReplay).toBeUndefined();
  });
