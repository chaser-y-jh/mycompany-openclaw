...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-agent-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: MerClawConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<MerClawConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<MerClawConfig["agents"]>["list"]>[number]["tools"];
  }): MerClawConfig {
    return {
      tools: params.tools,
      agents: {
