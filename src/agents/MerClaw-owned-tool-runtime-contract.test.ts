...
import type { ExtensionContext } from "merclaw/plugin-sdk/agent-sessions";
...
describe("MerClaw-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetMerClawOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installMerClawOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installMerClawOwnedToolHooks();
    ...
    const originalParams = {
      ...
      mediaUrl: "/tmp/merclaw-reply.png",
      provider: "telegram",
      to: "chat-1",
    };
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/merclaw-reply.png"]);
    expect(
    ...
    ).toEqual([
      {
        ...
        mediaUrls: ["/tmp/merclaw-reply.png"],
      },
    ]);
    const [afterPayload, afterContext] = await waitForAfterToolCall(hooks);
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installMerClawOwnedToolHooks({ blockReason: "blocked by policy" });
