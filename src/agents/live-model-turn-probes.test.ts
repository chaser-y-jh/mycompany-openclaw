...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "MERCLAW_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { MERCLAW_LIVE_MODEL_IMAGE_PROBE: "false" },
        "MERCLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { MERCLAW_LIVE_MODEL_IMAGE_PROBE: "1" },
        "MERCLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
