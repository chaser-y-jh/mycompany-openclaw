...
describe("ensureAuthProfileStore", () => {
  ...
  function restoreAgentDirEnv(params: {
    ...
    if ("previousStateDir" in params) {
      restoreEnvValue("MERCLAW_STATE_DIR", params.previousStateDir);
    }
    restoreEnvValue("MERCLAW_AGENT_DIR", params.previousAgentDir);
  }
  ...
  function configureMainAuthTestDirs(root: string): {
    ...
    process.env.MERCLAW_AGENT_DIR = mainDir;
    clearRuntimeAuthProfileStoreSnapshots();
  ...
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-auth-profiles-"));
    try {
  ...
  it("ignores array-shaped auth profile stores instead of loading numeric profile ids", () => {
    withTempAgentDir("merclaw-auth-profiles-array-", (agentDir) => {
  ...
  it("ignores top-level array auth stores instead of treating entries as profiles", () => {
    withTempAgentDir("merclaw-auth-top-array-", (agentDir) => {
      writeRawAuthProfileStore(agentDir, [
        {
          type: "api_key",
          provider: "openai",
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-auth-merge-"));
  ...
  it("uses the main agent's newer OAuth profile when an agent still has a stale default profile", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-auth-drift-"));
    const { mainDir, agentDir, previousStateDir, previousAgentDir } =
      configureMainAuthTestDirs(root);
    try {
  ...
  it("keeps a newer agent replacement credential while repairing stale default references", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-auth-drift-newer-agent-"));
    const { mainDir, agentDir, previousStateDir, previousAgentDir } =
      configureMainAuthTestDirs(root);
    try {
      const freshProfileId = "openai:user@example.com";
  ...
  it("preserves a valid main default OAuth profile while replacing a stale agent override", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-auth-drift-base-default-"));
    const { mainDir, agentDir, previousStateDir, previousAgentDir } =
      configureMainAuthTestDirs(root);
    try {
      const freshProfileId = "openai:user@example.com";
  ...
  it("keeps a stale default OAuth profile when the main profile belongs to a different identity", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-auth-drift-mismatch-"));
    const { mainDir, agentDir, previousStateDir, previousAgentDir } =
      configureMainAuthTestDirs(root);
    try {
      const freshProfileId = "openai:user@example.com";
  ...
  it("keeps an invalidated identity-specific agent profile when the main agent has a different identity", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-auth-codex-relogin-"));
    const { mainDir, agentDir, previousStateDir, previousAgentDir } =
      configureMainAuthTestDirs(root);
    try {
      const now = Date.now();
  ...
  it.each([
    ...
    ({ name, profile, expected }) => {
      withTempAgentDir("merclaw-auth-alias-", (agentDir) => {
        const storeData = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("normalizes mode/apiKey aliases while migrating legacy auth.json", () => {
    withTempAgentDir("merclaw-auth-legacy-alias-", (agentDir) => {
      fs.writeFileSync(
  ...
  it("does not load legacy flat auth-profiles.json entries at runtime", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-auth-flat-profiles-"));
    try {
  ...
  it("merges legacy oauth.json into auth-profiles.json", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-oauth-migrate-"));
    const previousStateDir = process.env.MERCLAW_STATE_DIR;
    const previousAgentDir = process.env.MERCLAW_AGENT_DIR;
    try {
      ...
      process.env.MERCLAW_STATE_DIR = root;
      process.env.MERCLAW_AGENT_DIR = agentDir;
      clearRuntimeAuthProfileStoreSnapshots();
      ...
      clearRuntimeAuthProfileStoreSnapshots();
      restoreEnvValue("MERCLAW_STATE_DIR", previousStateDir);
  ...
  it("exposes provider-managed runtime auth without persisting copied tokens", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-external-auth-"));
    const previousAgentDir = process.env.MERCLAW_AGENT_DIR;
    try {
      const agentDir = path.join(root, "agent");
      fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("does not write inherited auth stores during secrets runtime reads", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-secrets-runtime-"));
    const previousStateDir = process.env.MERCLAW_STATE_DIR;
    try {
  ...
  it("does not clone inherited auth stores during normal agent reads", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "merclaw-auth-read-through-"));
    const previousStateDir = process.env.MERCLAW_STATE_DIR;
    try {
      const stateDir = path.join(root, ".merclaw");
      const mainAgentDir = path.join(stateDir, "agents", "main", "agent");
  ...
  it("logs one warning with aggregated reasons for rejected auth-profiles entries", () => {
    ...
    try {
      withTempAgentDir("merclaw-auth-invalid-", (agentDir) => {
        const invalidStore = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it.each([
    ...
    },
    {
      ...
      prefix: "merclaw-nonstr-key-malformed-ref-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
    ...
    },
    {
      ...
      prefix: "merclaw-nonstr-token-ref-",
    ...
    },
    {
      ...
      prefix: "merclaw-nonstr-token-num-",
      profileId: "anthropic:default",
      profile: {
        type: "token",
        provider: "anthropic",
