...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "merclaw-test-"));

    try {
      await withEnvAsync(
        { MERCLAW_STATE_DIR: stateDir, MERCLAW_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          config: {
            tools: {
              exec: {
                ...
                safeBinTrustedDirs: ["/tmp/merclaw-bin"],
  ...
  it("rejects config.apply when it changes safe bin trusted directories", async () => {
    ...
    await expect(
      tool.execute("call-protected-safe-bin-trust-apply", {
        action: "config.apply",
        raw: '{ tools: { exec: { ask: "on-miss", security: "allowlist", safeBinTrustedDirs: ["/tmp/merclaw-bin"] } } }',
      }),
    ).rejects.toThrow(
