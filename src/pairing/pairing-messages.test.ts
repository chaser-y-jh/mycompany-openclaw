...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["MERCLAW_CONTAINER_HINT", "MERCLAW_PROFILE"]);
    delete process.env.MERCLAW_CONTAINER_HINT;
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:merclaw|merclaw) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
