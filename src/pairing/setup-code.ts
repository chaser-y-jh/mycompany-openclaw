...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.MERCLAW_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.MERCLAW_GATEWAY_PASSWORD);
  const token =
...
export async function resolvePairingSetupFromConfig(
  ...
  const cfgForAuth = await materializeGatewayAuthSecretRefs({
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(env.MERCLAW_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(env.MERCLAW_GATEWAY_PASSWORD)),
  });
