...
describe("media store", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("merclaw-test-home-");
    home = tempHome.home;
  ...
  it.each([
    {
      ...
      run: async () => {
        await withTempStore(async (store, home) => {
          ...
          expect(path.normalize(dir)).toContain(`${path.sep}.merclaw${path.sep}media`);
    ...
    },
    {
      ...
      run: async () => {
        await withTempStore(async (store, home) => {
          ...
          const targetDir = path.join(home, ".merclaw", "media", "oversized-stream");
  ...
  it("prefers header mime extension when sniffed mime lacks mapping", async () => {
    await withTempStore(async (_store, home) => {
      vi.doMock("@merclaw/media-core/mime", async () => {
        const actual = await vi.importActual<typeof import("@merclaw/media-core/mime")>(
          "@merclaw/media-core/mime",
        );
        return {
          ...actual,
      ...
      try {
        ...
        vi.doUnmock("@merclaw/media-core/mime");
      }
    });
  });
