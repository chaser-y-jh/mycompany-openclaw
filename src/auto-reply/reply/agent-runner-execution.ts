...
// See: https://github.com/merclaw/merclaw/issues/58348
...
function buildCodexAppServerFailureText(message: string): string | null {
  ...
  if (CODEX_APP_SERVER_CLIENT_CLOSED_BEFORE_REPLY_RE.test(normalizedMessage)) {
    return "⚠️ Codex app-server connection closed before this turn finished. MerClaw retried once when the stdio turn was still replay-safe; please try again if this keeps happening.";
  }
  if (CODEX_APP_SERVER_TURN_COMPLETION_IDLE_TIMEOUT_RE.test(normalizedMessage)) {
    return "⚠️ Codex app-server stopped before confirming turn completion. MerClaw did not replay the turn automatically because it may still be active; try again, or use /new if the session stays stuck.";
  }
  return null;
}
...
function buildMissingApiKeyFailureText(message: string): string | null {
  ...
  if (provider === "openai") {
    return '⚠️ Missing API key for provider "openai". Run `merclaw doctor --fix` to repair stale OpenAI model/session routes, restart the gateway if doctor asks, then try again. If doctor has nothing to repair or the error persists, re-auth with `merclaw models auth login --provider openai` or run `merclaw configure`.';
  }
        ...
        runWithModelFallback<EmbeddedAgentRunResult>({
          ...
          run: async (provider, model, runOptions) => {
            ...
            const embeddedRunHarnessOverride =
              ...
              (agentHarnessPolicy.runtime === "merclaw" && embeddedRunProvider !== provider
                ? "merclaw"
                : undefined);
            return (async () => {
                  ...
                  runEmbeddedAgent({
                    ...
                    onToolResult: onToolResult
                      ? (() => {
                          ...
                          // See: https://github.com/merclaw/merclaw/issues/11044
