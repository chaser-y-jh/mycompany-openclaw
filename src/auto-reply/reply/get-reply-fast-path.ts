...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("merclaw.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("merclaw.reply.full-runtime");

type ReplyConfigWithMarker = MerClawConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    env.MERCLAW_ALLOW_SLOW_REPLY_TESTS === "1" || env.MERCLAW_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function markCompleteReplyConfig<T extends MerClawConfig>(
  config: T,
...
export function withFastReplyConfig<T extends MerClawConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends MerClawConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

function isCompleteReplyConfig(config: unknown): config is MerClawConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set MERCLAW_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  ...
  return applyMergePatch(params.getRuntimeConfig(), configOverride) as MerClawConfig;
}
