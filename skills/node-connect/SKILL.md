---
name: node-connect
description: "Diagnose MerClaw Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify MerClaw is advertising that route, then fix pairing/auth.
...
- whether `merclaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `merclaw qr --json`. It uses the same setup-code payload Android scans.

```bash
merclaw config get gateway.mode
merclaw config get gateway.bind
merclaw config get gateway.tailscale.mode
merclaw config get gateway.remote.url
merclaw config get gateway.auth.mode
merclaw config get gateway.auth.allowTailscale
merclaw config get plugins.entries.device-pair.config.publicUrl
merclaw qr --json
merclaw devices list
merclaw nodes status
```

If this MerClaw instance is pointed at a remote gateway, also run:

```bash
merclaw qr --remote --json
```
...
If `merclaw qr --json` says `Gateway is only bound to loopback`:
...
```bash
merclaw devices list
merclaw devices approve --latest   # preview only; copy the requestId from output
...
- `merclaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run merclaw qr again, rescan, then approve the pending device pairing.`
