...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.merclaw`, `$MERCLAW_STATE_DIR`, or active MerClaw state dirs.
...
- Worker must send completion/failure via `merclaw message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/merclaw`; use an isolated checkout.
...
merclaw message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use merclaw system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t merclaw-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
<notification block>
EOF
