---
name: canvas
description: "Present HTML on connected MerClaw node canvases, navigate/eval/snapshot, and debug canvas host URLs."
metadata: { "merclaw": { "emoji": "🖼️" } }
---
...
Active config: `$MERCLAW_CONFIG_PATH` or `~/.merclaw/merclaw.json`.
...
4. Present the hosted URL: `/__merclaw__/canvas/<file>.html`.
5. Use `snapshot` when the user needs proof.

## URL shape

```text
http://<gateway-host>:<gateway.port>/__merclaw__/canvas/index.html
http://<gateway-host>:<gateway.port>/__merclaw__/canvas/games/snake.html
```

Path mapping:

- `/__merclaw__/canvas/index.html` -> `<canvas host root>/index.html`
- `/__merclaw__/canvas/games/snake.html` -> `<canvas host root>/games/snake.html`
