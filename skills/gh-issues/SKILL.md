...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${MERCLAW_CONFIG_PATH:-${MERCLAW_STATE_DIR:-$HOME/.merclaw}/merclaw.json}"
...
```text
${MERCLAW_STATE_DIR:-$HOME/.merclaw}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with merclaw message send if route provided.
```
