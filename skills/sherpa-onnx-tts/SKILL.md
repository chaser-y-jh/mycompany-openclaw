...
1. Download the runtime for your OS (extracts into `$MERCLAW_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.merclaw/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$MERCLAW_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.merclaw/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active MerClaw config file (`$MERCLAW_CONFIG_PATH`, default `~/.merclaw/merclaw.json`):
